...
public:
	...
	ColorRgb(T red, T green, T blue) : b(blue), g(green), r(red) {
	}

	ColorRgb(const ColorRgb<T> &x) : b(x.b), g(x.g), r(x.r) {
	}
...
public:
	...
	ColorRgba(T red, T green, T blue, T alpha) : ColorRgb<T>(red, green, blue), a(alpha) {
	}
