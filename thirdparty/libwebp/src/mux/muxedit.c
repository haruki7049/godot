...
WebPMuxError WebPMuxPushFrame(WebPMux *mux, const WebPMuxFrameInfo *info,
	...
	if (mux->images_ != NULL) {
		...
		const uint32_t image_id = (image->header_ != NULL) ?
										  ChunkGetIdFromTag(image->header_->tag_) :
										  WEBP_CHUNK_IMAGE;
		if (image_id != info->id) {
...
WebPMuxError WebPMuxAssemble(WebPMux *mux, WebPData *assembled_data) {
	...
	// Allocate data.
	size = ChunkListDiskSize(mux->vp8x_) + ChunkListDiskSize(mux->iccp_) + ChunkListDiskSize(mux->anim_) + ImageListDiskSize(mux->images_) + ChunkListDiskSize(mux->exif_) + ChunkListDiskSize(mux->xmp_) + ChunkListDiskSize(mux->unknown_) + RIFF_HEADER_SIZE;
