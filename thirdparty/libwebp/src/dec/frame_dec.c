		...
		for (s = 0; s < NUM_MB_SEGMENTS; ++s) {
			...
			for (i4x4 = 0; i4x4 <= 1; ++i4x4) {
				...
				if (level > 0) {
					...
					info->hev_thresh_ = (level >= 40) ? 2 : (level >= 15) ? 1 :
																			0;
				} else {
...
void VP8InitDithering(const WebPDecoderOptions *const options,
	...
	if (options != NULL) {
		...
		const int f = (d < 0) ? 0 : (d > 100) ? max_amp :
												(d * max_amp / 100);
		if (f > 0) {
			int s;
			int all_amp = 0;
...
static int AllocateMemory(VP8Decoder *const dec) {
	...
	const uint64_t alpha_size = (dec->alpha_data_ != NULL) ?
										(uint64_t)dec->pic_hdr_.width_ * dec->pic_hdr_.height_ :
										0ULL;
	const uint64_t needed = (uint64_t)intra_pred_mode_size + top_size + mb_info_size + f_info_size + yuv_size + mb_data_size + cache_size + alpha_size + WEBP_ALIGN_CST;
	uint8_t *mem;
