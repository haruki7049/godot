...
static WEBP_UBSAN_IGNORE_UNDEF WEBP_INLINE void VP8LoadNewBytes(VP8BitReader *const br) {
...
static WEBP_UBSAN_IGNORE_UNSIGNED_OVERFLOW WEBP_INLINE int VP8GetSigned(VP8BitReader *const br, int v, const char label[]) {
	if (br->bits_ < 0) {
		VP8LoadNewBytes(br);
	}
	{
