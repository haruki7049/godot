...
AutoencoderFilter::AutoencoderFilter(const Ref<Device> &device) : Filter(device) {
}
...
std::shared_ptr<Executable> AutoencoderFilter::buildNet() {
	...
	if ((color.format != Format::Float3) || (albedo && albedo.format != Format::Float3) || (normal && normal.format != Format::Float3) || (output.format != Format::Float3))
		throw Exception(Error::InvalidOperation, "unsupported image format");

	if ((albedo && (albedo.width != W || albedo.height != H)) || (normal && (normal.width != W || normal.height != H)) || (output.width != W || output.height != H))
	...
	const auto temp0Dims = getMaxTensorDims({ conv1Dims,
			conv2Dims,
			conv3Dims,
			conv4Dims,
			conv5Dims,
...
RTLightmapFilter::RTLightmapFilter(const Ref<Device> &device) : AutoencoderFilter(device) {
