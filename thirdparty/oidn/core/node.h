...
public:
	MklNode(const primitive &prim, const std::unordered_map<int, memory> &args) : prim(prim),
																				  args(args) {}
...
public:
	ConvNode(const convolution_forward::primitive_desc &desc,
			...
			const std::shared_ptr<memory> &dst) : MklNode(convolution_forward(desc),
														  { { MKLDNN_ARG_SRC, *src },
...
public:
	PoolNode(const pooling_forward::primitive_desc &desc,
			const std::shared_ptr<memory> &src,
			const std::shared_ptr<memory> &dst) : MklNode(pooling_forward(desc),
														  { { MKLDNN_ARG_SRC, *src },
																  { MKLDNN_ARG_DST, *dst } }),
												  src(src),
												  dst(dst) {}
...
public:
	ReorderNode(const std::shared_ptr<memory> &src,
			const std::shared_ptr<memory> &dst) : MklNode(reorder(reorder::primitive_desc(*src, *dst)),
														  { { MKLDNN_ARG_SRC, *src },
																  { MKLDNN_ARG_DST, *dst } }),
												  src(src),
												  dst(dst) {}
