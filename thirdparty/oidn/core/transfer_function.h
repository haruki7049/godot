...
public:
	LogTransferFunction(float exposure = 1.f) : HDRTransferFunction(exposure) {
	}
...
public:
	PQXTransferFunction(float exposure = 1.f) : HDRTransferFunction(exposure) {
	}
...
public:
	AutoexposureNode(const Image &color,
			const std::shared_ptr<HDRTransferFunction> &transferFunc) : color(color),
