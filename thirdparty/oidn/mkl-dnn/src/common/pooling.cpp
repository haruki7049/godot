...
status_t pooling_desc_init(pooling_desc_t *pool_desc,
	...
	bool args_ok = true && !any_null(pool_desc, src_desc, dst_desc, strides, kernel, padding_l) && one_of(alg_kind, pooling_max, pooling_avg_include_padding, pooling_avg_exclude_padding) && one_of(padding_kind, padding_kind::padding_zero);
	if (!args_ok)
	...
	bool consistency = true && utils::one_of(src_desc->ndims, 4, 5) && utils::one_of(dst_desc->ndims, 4, 5) && src_desc->dims[0] == dst_desc->dims[0] && src_desc->dims[1] == dst_desc->dims[1];
	for (int i = 2; i < src_desc->ndims; ++i)
		consistency = consistency && ((src_desc->dims[i] - kernel[i - 2] + padding_l[i - 2] + padding_r[i - 2]) / strides[i - 2] + 1 == dst_desc->dims[i]);
	if (!consistency)
		return invalid_arguments;
