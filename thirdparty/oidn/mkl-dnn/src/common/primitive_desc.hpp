...
struct mkldnn_primitive_desc : public mkldnn::impl::c_compatible {
	...
	mkldnn_primitive_desc(mkldnn::impl::engine_t *engine,
			...
			mkldnn::impl::primitive_kind_t kind) : engine_(engine),
												   attr_(*attr),
												   kind_(kind) { info_[0] = '\0'; }
	...
	mkldnn::impl::memory_tracking::registry_t &scratchpad_registry() { return scratchpad_registry_; }
	const mkldnn::impl::memory_tracking::registry_t &scratchpad_registry() const { return scratchpad_registry_; }
	virtual mkldnn::impl::engine_t *scratchpad_engine() const { return engine_; }

	virtual const mkldnn::impl::op_desc_t *op_desc() const { return nullptr; }

	enum class arg_usage_t { unused,
		input,
		output };
...
#define DECLARE_MD_STUB(stub) \
	virtual const mkldnn::impl::memory_desc_t *stub(int idx = 0) const { return nullptr; }
...
#undef DECLARE_MD_STUB
	...
	mkldnn::impl::dim_t scratchpad_size(
		...
		if (mode != attr_.scratchpad_mode_)
			return 0;
...
protected:
	...
	bool compare_ws(const mkldnn_primitive_desc *fwd_pd) const {
		using namespace mkldnn::impl;
		if (!workspace_md())
			return true; // the impl lives fine w/o workspace
		return fwd_pd && fwd_pd->workspace_md() && *fwd_pd->workspace_md() == *workspace_md();
	}
};

#define DECLARE_COMMON_PD_t(impl_name, ...)                                   \
	virtual pd_t *clone() const override {                                    \
		return new pd_t(*this);                                               \
	}                                                                         \
