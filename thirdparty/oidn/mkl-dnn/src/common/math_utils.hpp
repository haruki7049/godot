...
inline U sqrt_bwd(T dd, T s) {
	return s > 0 ? (U)(dd / (2 * ::sqrtf((float)(s)))) : 0;
}
...
inline bool eltwise_fwd_preserves_zero(alg_kind_t alg, bool jit_impl = false) {
	...
	const bool preserves_zero = true && !one_of(alg, eltwise_linear, eltwise_soft_relu, eltwise_logistic) && IMPLICATION(jit_impl, !one_of(alg, eltwise_elu, eltwise_tanh));
...
#define CASE(dt) \
	case dt:     \
		return (float)((const prec_traits<dt>::type *)bias)[offset]
