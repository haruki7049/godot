...
struct deconvolution_pd_t : public primitive_desc_t {
	...
	deconvolution_pd_t(engine_t *engine,
			...
			const deconvolution_fwd_pd_t *hint_fwd_pd) : primitive_desc_t(engine, attr, base_pkind),
														 desc_(*adesc),
	...
	dim_t G() const { return with_groups() ? conv_prop_invariant_wei_d(&desc_)->dims[0] : 1; }

	dim_t ID() const {
		return ndims() >= 5 ? conv_prop_invariant_src_d(&desc_)->dims[ndims() - 3] : 1;
	}
	dim_t IH() const {
		return ndims() >= 4 ? conv_prop_invariant_src_d(&desc_)->dims[ndims() - 2] : 1;
	}
	dim_t IW() const {
	...
	dim_t OD() const {
		return ndims() >= 5 ? conv_prop_invariant_dst_d(&desc_)->dims[ndims() - 3] : 1;
	}
	dim_t OH() const {
		return ndims() >= 4 ? conv_prop_invariant_dst_d(&desc_)->dims[ndims() - 2] : 1;
	}
	dim_t OW() const {
	...
	dim_t KD() const {
		...
		return ndims() >= 5 ? conv_prop_invariant_wei_d(&desc_)->dims[w_ndims - 3] : 1;
	}
	dim_t KH() const {
		const int w_ndims = ndims() + with_groups();
		return ndims() >= 4 ? conv_prop_invariant_wei_d(&desc_)->dims[w_ndims - 2] : 1;
	}
	dim_t KW() const {
		const int w_ndims = ndims() + with_groups();
	...
	bool with_bias() const {
		return !memory_desc_wrapper(*conv_prop_invariant_bia_d(&desc_)).is_zero();
	}

	bool with_groups() const { return conv_prop_invariant_wei_d(&desc_)->ndims == ndims() + 1; }
...
struct deconvolution_fwd_pd_t : public deconvolution_pd_t {
	...
	deconvolution_fwd_pd_t(engine_t *engine,
			const deconvolution_desc_t *adesc,
			const primitive_attr_t *attr,
			const deconvolution_fwd_pd_t *hint_fwd_pd) : deconvolution_pd_t(engine, adesc, attr, hint_fwd_pd),
