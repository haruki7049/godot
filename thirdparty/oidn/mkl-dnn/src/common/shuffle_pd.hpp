...
struct shuffle_pd_t : public primitive_desc_t {
	...
	shuffle_pd_t(engine_t *engine,
			...
			const shuffle_pd_t *hint_fwd_pd) : primitive_desc_t(engine, attr, base_pkind),
											   desc_(*adesc),
	...
	virtual const memory_desc_t *src_md(int index = 0) const override { return index == 0 && is_fwd() ? &data_md_ : nullptr; }
	virtual const memory_desc_t *dst_md(int index = 0) const override { return index == 0 && is_fwd() ? &data_md_ : nullptr; }

	virtual const memory_desc_t *diff_src_md(int index = 0) const override { return index == 0 && !is_fwd() ? &data_md_ : nullptr; }
	virtual const memory_desc_t *diff_dst_md(int index = 0) const override { return index == 0 && !is_fwd() ? &data_md_ : nullptr; }
