...
bool simple_get_params_check(const post_ops_t *post_ops, int index,
	...
	bool ok = true && post_ops != nullptr && 0 <= index && index < post_ops->len_ && post_ops->entry_[index].kind == kind;
	return ok;
}
} // namespace
...
status_t mkldnn_post_ops_get_params_sum(const post_ops_t *post_ops, int index,
		float *scale) {
	bool ok = true && simple_get_params_check(post_ops, index, primitive_kind::sum) && !any_null(scale);
	if (!ok)
		return invalid_arguments;
...
status_t mkldnn_post_ops_get_params_eltwise(const post_ops_t *post_ops,
	...
	bool ok = true && simple_get_params_check(post_ops, index, primitive_kind::eltwise) && !any_null(scale, alpha, beta);
	if (!ok)
		return invalid_arguments;
