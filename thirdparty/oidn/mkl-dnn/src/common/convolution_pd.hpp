...
struct convolution_pd_t : public primitive_desc_t {
	...
	convolution_pd_t(engine_t *engine,
			...
			const convolution_fwd_pd_t *hint_fwd_pd) : primitive_desc_t(engine, attr, base_pkind),
													   desc_(*adesc),
...
#define IS_OK(f)                    \
	do {                            \
		...
		if (src_md.format_kind == format_kind::any && !utils::one_of(src_tag, any, undef))
			IS_OK(memory_desc_init_by_tag(src_md, src_tag));
		if (dst_md.format_kind == format_kind::any && !utils::one_of(dst_tag, any, undef))
			IS_OK(memory_desc_init_by_tag(dst_md, dst_tag));
		if (wei_md.format_kind == format_kind::any && !utils::one_of(wei_tag, any, undef))
...
#undef IS_OK
	...
	bool expect_data_types(data_type_t src_dt, data_type_t wei_dt,
		...
		bool ok = true && (src_dt == data_type::undef || _src_md()->data_type == src_dt) && (wei_dt == data_type::undef || _wei_md()->data_type == wei_dt) && (dst_dt == data_type::undef || _dst_md()->data_type == dst_dt) && (acc_dt == data_type::undef || desc_.accum_data_type == acc_dt);
...
struct convolution_fwd_pd_t : public convolution_pd_t {
	...
	convolution_fwd_pd_t(engine_t *engine,
			const convolution_desc_t *adesc,
			const primitive_attr_t *attr,
			const convolution_fwd_pd_t *hint_fwd_pd) : convolution_pd_t(engine, adesc, attr, hint_fwd_pd),
