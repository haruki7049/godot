...
struct scales_t : public c_compatible {
	...
	scales_t(const scales_t &rhs) : scales_t() { set(rhs.count_, rhs.mask_, rhs.scales_); }
...
struct mkldnn_post_ops : public mkldnn::impl::c_compatible {
	struct entry_t {
		...
		bool is_eltwise(bool require_scale_one = true) const {
			...
			return kind == primitive_kind::eltwise && IMPLICATION(require_scale_one, eltwise.scale == 1.f);
		}

		bool is_relu(bool require_scale_one = true,
				bool require_nslope_zero = true) const {
			using namespace mkldnn::impl;
			return is_eltwise(require_scale_one) && eltwise.alg == alg_kind::eltwise_relu && IMPLICATION(require_nslope_zero, eltwise.alpha == 0.f);
		}

		bool is_sum(bool require_scale_one = true) const {
			using namespace mkldnn::impl;
			return kind == primitive_kind::sum && IMPLICATION(require_scale_one, sum.scale == 1.f);
		}
	};
	...
	bool contain(mkldnn::impl::primitive_kind_t kind, int index) const { return find(kind, index, index + 1) == index; }
...
struct mkldnn_primitive_attr : public mkldnn::impl::c_compatible {
	mkldnn_primitive_attr() : scratchpad_mode_(mkldnn::impl::scratchpad_mode::library) {}

	mkldnn_primitive_attr *clone() const { return new mkldnn_primitive_attr(*this); }

	/** Returns true if the attributes have default values.
	 *
	 * @note The scratchpad_mode_ is not take into account */
	bool has_default_values() const {
		return true && output_scales_.has_default_values() && post_ops_.has_default_values() && rnn_data_qparams_.has_default_values() && rnn_weights_qparams_.has_default_values();
	}
