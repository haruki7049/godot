...
status_t eltwise_desc_init(eltwise_desc_t *eltwise_desc, prop_kind_t prop_kind,
	...
	bool args_ok = true && !any_null(eltwise_desc, data_desc) && one_of(prop_kind, forward_training, forward_inference, backward_data) && one_of(alg_kind, eltwise_relu, eltwise_tanh, eltwise_elu, eltwise_square, eltwise_abs, eltwise_sqrt, eltwise_linear, eltwise_bounded_relu, eltwise_soft_relu, eltwise_logistic) && IMPLICATION(prop_kind == backward_data, diff_data_desc != nullptr);
	if (!args_ok)
	...
	bool consistency = true && IMPLICATION(ed.prop_kind == backward_data,
