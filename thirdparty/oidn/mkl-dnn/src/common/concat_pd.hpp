...
struct concat_pd_t : public primitive_desc_t {
	concat_pd_t(engine_t *engine, const primitive_attr_t *attr,
			...
			const memory_desc_t *src_mds) : primitive_desc_t(engine, attr, primitive_kind::concat),
											n_(n),
	...
	virtual arg_usage_t arg_usage(primitive_arg_index_t arg) const override {
		if (arg >= MKLDNN_ARG_MULTIPLE_SRC && arg < MKLDNN_ARG_MULTIPLE_SRC + n_inputs())
	...
	virtual const memory_desc_t *src_md(int index = 0) const override { return index < n_inputs() ? &src_mds_[index] : nullptr; }
	...
	const memory_desc_t *src_image_md(int index = 0) const { return index < n_inputs() ? &src_image_mds_[index] : nullptr; }
...
protected:
	...
	status_t init() {
		bool ok = true && set_default_params() == status::success && attr()->has_default_values();
		if (!ok)
