...
status_t deconv_desc_init(deconvolution_desc_t *deconv_desc,
	...
	bool args_ok = true && !any_null(deconv_desc, src_desc, weights_desc, dst_desc, strides, padding_l) && one_of(alg_kind, deconvolution_direct, deconvolution_winograd) && one_of(padding_kind, padding_kind::padding_zero);
	if (!args_ok)
	...
	const bool with_bias = bias_desc && bias_desc->format_kind != format_kind::undef;
	...
	(prop_kind == backward_weights ? dd.diff_weights_desc : dd.weights_desc) = *weights_desc;
	if (with_bias)
		(prop_kind == backward_weights ? dd.diff_bias_desc : dd.bias_desc) = *bias_desc;
	...
	bool consistency = true && src_desc->ndims == dst_desc->ndims && utils::one_of(src_desc->ndims, 3, 4, 5) && utils::one_of(weights_desc->ndims, src_desc->ndims, src_desc->ndims + 1) && (with_bias ? bias_desc->ndims == 1 : true) && (with_bias ? bias_desc->dims[0] == dst_desc->dims[1] : true) && src_desc->dims[0] == dst_desc->dims[0] && src_desc->dims[1] == g * weights_desc->dims[with_groups + 1] && dst_desc->dims[1] == g * weights_desc->dims[with_groups + 0];
