...
struct pooling_pd_t : public primitive_desc_t {
	...
	pooling_pd_t(engine_t *engine,
			...
			const pooling_fwd_pd_t *hint_fwd_pd) : primitive_desc_t(engine, attr, base_pkind),
												   desc_(*adesc),
	...
	bool has_zero_dim_memory() const { return memory_desc_wrapper(src_desc()).has_zero_dim(); }
...
protected:
	...
	data_type_t indices_data_type() const {
		...
		return utils::array_product(desc()->kernel, ndims()) <= u8_max ? data_type::u8 : data_type::s32;
	}

private:
	const memory_desc_t &src_desc() const { return is_fwd() ? desc_.src_desc : desc_.diff_src_desc; }
	const memory_desc_t &dst_desc() const { return is_fwd() ? desc_.dst_desc : desc_.diff_dst_desc; }
};
...
struct pooling_fwd_pd_t : public pooling_pd_t {
	...
	pooling_fwd_pd_t(engine_t *engine,
			const pooling_desc_t *adesc,
			const primitive_attr_t *attr,
			const pooling_fwd_pd_t *hint_fwd_pd) : pooling_pd_t(engine, adesc, attr, hint_fwd_pd),
	...
	virtual int n_outputs() const override { return 1 + (workspace_md() != nullptr); }
...
struct pooling_bwd_pd_t : public pooling_pd_t {
	...
	virtual int n_inputs() const override { return 1 + (workspace_md() != nullptr); }
