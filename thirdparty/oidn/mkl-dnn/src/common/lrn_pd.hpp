...
struct lrn_pd_t : public primitive_desc_t {
	...
	lrn_pd_t(engine_t *engine,
			...
			const lrn_fwd_pd_t *hint_fwd_pd) : primitive_desc_t(engine, attr, base_pkind),
											   desc_(*adesc),
...
struct lrn_fwd_pd_t : public lrn_pd_t {
	...
	lrn_fwd_pd_t(engine_t *engine,
			const lrn_desc_t *adesc,
			const primitive_attr_t *attr,
			const lrn_fwd_pd_t *hint_fwd_pd) : lrn_pd_t(engine, adesc, attr, hint_fwd_pd) {}
	...
	virtual const memory_desc_t *src_md(int index = 0) const override { return index == 0 ? &data_md_ : nullptr; }
	virtual const memory_desc_t *dst_md(int index = 0) const override { return index == 0 ? &data_md_ : nullptr; }
...
struct lrn_bwd_pd_t : public lrn_pd_t {
	...
	virtual int n_inputs() const override { return 2 + (workspace_md() != nullptr); }
