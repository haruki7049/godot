...
bool memory_desc_sanity_check(int ndims, const dims_t dims,
	...
	bool ok = true && dims != nullptr && 0 < ndims && ndims <= MKLDNN_MAX_NDIMS && one_of(data_type, f32, s32, s8, u8) && format_kind != format_kind::undef;
	if (!ok)
		return false;
...
status_t mkldnn_memory_desc_init_by_tag(memory_desc_t *memory_desc, int ndims,
	...
	/* memory_desc != 0 */
	bool args_ok = !any_null(memory_desc) && memory_desc_sanity_check(ndims, dims, data_type, format_kind);
	if (!args_ok)
		return invalid_arguments;
...
status_t mkldnn_memory_desc_init_by_strides(memory_desc_t *memory_desc,
	...
	/* memory_desc != 0 */
	bool args_ok = !any_null(memory_desc) && memory_desc_sanity_check(ndims, dims, data_type, format_kind::any);
	if (!args_ok)
		return invalid_arguments;
...
status_t mkldnn_memory_desc_init_submemory(memory_desc_t *md,
	...
	for (int d = 0; d < src_d.ndims(); ++d) {
		if (dims[d] < 0 || offsets[d] < 0 || (offsets[d] + dims[d] > src_d.dims()[d]))
			return invalid_arguments;
	}
	...
	for (int d = 0; d < src_d.ndims(); ++d) {
		...
		const bool ok = true && offsets[d] % blocks[d] == 0 /* [r1] */
						&& src_d.padded_offsets()[d] == 0 && (false || dims[d] % blocks[d] == 0 || dims[d] < blocks[d]);
		if (!ok)
			return unimplemented;
