...
status_t shuffle_desc_init(shuffle_desc_t *shuffle_desc, prop_kind_t prop_kind,
	...
	bool args_ok = true && !any_null(shuffle_desc, data_desc) && one_of(prop_kind, forward_training, forward_inference, backward, backward_data) && axis >= 0 && axis < data_desc->ndims && group_size > 0 && group_size <= data_desc->dims[axis];
	if (!args_ok)
