...
inline void init_scratchpad(memory_tracking::registrar_t &scratchpad,
	...
	size_t V_sz = (size_t)alpha * alpha * jcp.mb * jcp.ic * (jcp.itiles * jcp.jtiles + jcp.tile_4fma_padding);
	size_t M_sz = (size_t)alpha * alpha * jcp.mb * jcp.oc * (jcp.itiles * jcp.jtiles + jcp.tile_4fma_padding);
	...
	if (jcp.sched_policy == WSCHED_WEI_S_D_G_W) {
		...
		size_t tr_src_sz = jcp.ver != ver_4fma ? 0 : (size_t)nthr * alpha * alpha * jcp.tile_4fma * jcp.ic_simd_block;
...
struct jit_avx512_common_convolution_winograd_fwd_t
	...
	struct pd_t : public cpu_convolution_fwd_pd_t {
		...
		status_t init() {
			bool ok = true && is_fwd() && utils::one_of(desc()->alg_kind, alg_kind::convolution_auto, alg_kind::convolution_winograd) && expect_data_types(data_type::f32, data_type::f32, data_type::f32, data_type::f32, data_type::f32) && !has_zero_dim_memory() && set_default_formats();
			if (!ok)
	...
	jit_avx512_common_convolution_winograd_fwd_t(const pd_t *apd) : _jit_avx512_common_convolution_winograd_t<true>(apd->jcp_, apd->attr()), cpu_primitive_t(apd, true) {}
...
struct jit_avx512_common_convolution_winograd_bwd_data_t
	...
	struct pd_t : public cpu_convolution_bwd_data_pd_t {
		...
		status_t init() {
			bool ok = true && desc()->prop_kind == prop_kind::backward_data && expect_data_types(data_type::f32, data_type::f32, data_type::undef, data_type::f32, data_type::f32) && utils::one_of(desc()->alg_kind, alg_kind::convolution_auto, alg_kind::convolution_winograd) && !has_zero_dim_memory() && set_default_formats() && mkldnn_thr_syncable();
			if (!ok)
				return status::unimplemented;
	...
	jit_avx512_common_convolution_winograd_bwd_data_t(const pd_t *apd) : _jit_avx512_common_convolution_winograd_t<false>(apd->jcp_, apd->attr()), cpu_primitive_t(apd, true) {}
...
struct jit_avx512_common_convolution_winograd_bwd_weights_t
	...
	struct pd_t : public cpu_convolution_bwd_weights_pd_t {
		pd_t(engine_t *engine, const convolution_desc_t *adesc,
				const primitive_attr_t *attr,
				const convolution_fwd_pd_t *hint_fwd_pd) : cpu_convolution_bwd_weights_pd_t(engine, adesc, attr,
																   hint_fwd_pd),
														   jcp_() {}
		...
		status_t init() {
			bool ok = true && desc()->prop_kind == prop_kind::backward_weights && utils::one_of(desc()->alg_kind, alg_kind::convolution_auto, alg_kind::convolution_winograd) && expect_data_types(data_type::f32, data_type::f32, data_type::f32, data_type::f32, data_type::f32) && !has_zero_dim_memory() && set_default_formats() && mkldnn_thr_syncable();
			if (!ok)
				return status::unimplemented;
	...
	jit_avx512_common_convolution_winograd_bwd_weights_t(const pd_t *apd) : cpu_primitive_t(apd, true), kernel_(nullptr) {
