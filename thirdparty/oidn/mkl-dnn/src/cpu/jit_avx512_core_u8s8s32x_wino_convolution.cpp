...
struct jit_avx512_core_u8s8s32x_wino_conv_src_trans_t : public jit_generator {
	...
	jit_avx512_core_u8s8s32x_wino_conv_src_trans_t(
			jit_conv_conf_2x3_wino_t ajcp, const primitive_attr_t &attr) : jcp(ajcp),
																		   attr_(attr),
	...
	auto load_src = [=](bool mask) {
		for (int y = 0; y < jcp.alpha; y++) {
			...
			for (int x = 0; x < jcp.alpha; x++) {
				...
				int inp_offset = sizeof(uint8_t) * ((-jcp.t_pad + y) * jcp.iw * jcp.ic + (-jcp.l_pad + x) * jcp.ic);
				if (mask) {
...
bool jit_avx512_core_u8s8s32x_wino_conv_dst_trans_t::maybe_relu(int position) {
	...
	if (position == 0) {
		/* relu before sum */
		return false || p.contain(eltwise, 0) || (jcp.dst_dt == data_type::u8 && !p.contain(sum, 0));
	} else if (position == 1) {
		/* relu after sum */
		const int sum_idx = p.contain(sum, 0) ? 0 : (p.contain(sum, 1) ? 1 : -1);
		if (sum_idx == -1)
			return false;

		return false || p.contain(eltwise, sum_idx + 1) || jcp.dst_dt == data_type::u8;
	}
		...
		if (jcp.with_bias) {
			...
			switch (jcp.bia_dt) {
				...
				case data_type::s32:
					...
					break;
				case data_type::s8:
					vpmovsxbd(vreg_bias, bias_addr);
					break;
				case data_type::u8:
					vpmovzxbd(vreg_bias, bias_addr);
					break;
				default:
...
struct jit_avx512_core_u8s8s32x_wino_conv_fwd_ker_t : public jit_generator {
	...
	Zmm vreg_wei(int i) {
		assert(31 - jcp.n2_block * jcp.m_block - i > (jcp.ver == ver_vnni ? 0 : 2));
...
bool jit_avx512_core_u8s8s32x_wino_conv_fwd_ker_t::post_ops_ok(
	...
	switch (p.len_) {
		case 0:
			return true;
		case 1:
			return is_relu(0) || p.contain(sum, 0);
		case 2:
			return (p.contain(sum, 0) && is_relu(1)) ||
				   (p.contain(sum, 1) && is_relu(0));
		case 3:
			return is_relu(0) && p.contain(sum, 1) && is_relu(2);
		default:
			return false;
	}
	...
	L(mb_loop_label);
	{
		...
		L(K_loop_label);
		{
			for (int k = 0; k < jcp.k2_block; k += 4) {
				for (int nb2 = 0; nb2 < jcp.n2_block; nb2++) {
					int wei_offset = jcp.typesize_in * (nb2 * jcp.n_block * jcp.K);
...
bool is_winograd_faster_than_direct(const jit_conv_conf_2x3_wino_t &jcp) {
	if (jcp.ver == ver_vnni) {
		return (jcp.mb <= mkldnn_get_max_threads() && (jcp.mb > 4 && jcp.ic > 64 && !(jcp.oc > 128 && jcp.ih < 14))) || jcp.mb > mkldnn_get_max_threads();
	}
	return true;
}
} // namespace

status_t jit_avx512_core_u8s8s32x_wino_conv_fwd_ker_t ::init_conf(jit_conv_conf_2x3_wino_t &jcp,
	...
	if (!(mayiuse(avx512_core) &&
				src_d.data_type() == data_type::u8 && wei_d.data_type() == data_type::s8 && one_of(dst_d.data_type(), data_type::f32, data_type::s32, data_type::s8, data_type::u8)))
	...
	bool ok = true && jcp.ngroups == 1 && jcp.oc % load_block == 0 && jcp.ic % load_block == 0 && jcp.oc % jcp.oc_block == 0 && jcp.ic % jcp.ic_block == 0 && everyone_is(3, jcp.kh, jcp.kw) && everyone_is(1, jcp.stride_h, jcp.stride_w) && everyone_is(0, jcp.dilate_h, jcp.dilate_w) && jcp.t_pad == jcp.b_pad && jcp.l_pad == jcp.r_pad && one_of(jcp.t_pad, 0, 1) && one_of(jcp.l_pad, 0, 1);
	if (!ok)
		return status::unimplemented;
	...
	auto get_mem_eff = [&](int small_mb, int ix, int iy, int n2_b) {
		...
		if (small_mb == 0) { // outer parallelization strategy
			...
			mem_eff = req_mem < L1_cap ? 1.f : req_mem < L2_cap ? 0.5f :
																  0.f;
			...
			req_mem = (float)aa * M_per_thr * (jcp.ic + jcp.typesize_acc * jcp.oc);
			if (req_mem > L2_cap)
				mem_eff = 0.1f;
		}
		return mem_eff;
	...
	auto find_m_n2_blocks = [&](bool small_mb, int ix, int iy, float work_eff,
		...
		for (int im = max_m_block; im > 0; im--) {
			...
			for (int in2 = max_n2_block; in2 > 0; in2--) {
				...
				if (jcp.nb_oc % in2 || used_regs > free_regs || cur_tot_eff <= tot_eff)
					continue;
				tot_eff = cur_tot_eff;
				m_block = im;
				n2_block = in2;
	...
	jcp.size_wino_src = utils::rnd_up(jcp.typesize_in * alltiles * jcp.ic, PAGE_4K) / jcp.typesize_in;
...
jit_avx512_core_u8s8s32x_wino_convolution_fwd_t<dst_data_type>::
		jit_avx512_core_u8s8s32x_wino_convolution_fwd_t(const pd_t *apd) : cpu_primitive_t(apd) {
...
#pragma unroll(4)
						...
						auto local_s = src + mb * jcp.ih * jcp.iw * jcp.ic + y * jcp.iw * jcp.ic + x * jcp.ic;
...
#pragma unroll(2)
						...
						auto local_d = dst + mb * jcp.oh * jcp.ow * jcp.oc + y * jcp.ow * jcp.oc + x * jcp.oc;
			...
			for (int tile_x = 0; tile_x < jcp.ow; tile_x += jcp.xb) {
				...
				parallel_nd(div_up(jcp.yb, 2), div_up(jcp.xb, 2), jcp.mb_block,
						[&](int y_in_block_b, int x_in_block_b, int mb) {
							...
							int y = y_in_block + tile_y;
							int x = x_in_block + tile_x;
							int m = (mb * (jcp.yb / 2) + (y_in_block / 2)) * (jcp.xb / 2) + (x_in_block / 2);
...
#pragma unroll(4)
							...
							auto local_s = src + (mbb * jcp.mb_block + mb) * jcp.ih * jcp.iw * jcp.ic + y * jcp.iw * jcp.ic + x * jcp.ic;
							auto local_w = wino_src + m * jcp.ic;
				...
				parallel_nd(16, jcp.n_chunks, [&](int tile_ij, int nnb) {
					...
					gemm_p.dst = wino_dst + jcp.out_stride * tile_ij + nnb * jcp.n2_block * jcp.n_block;
					gemm_p.wei = wei + jcp.wei_stride * tile_ij + nnb * jcp.n2_block * jcp.n_block * jcp.K;
					gemm_p.dst_b = dst_bias + jcp.bia_stride * tile_ij + nnb * jcp.n2_block * jcp.n_block;
...
#pragma unroll(2)
							...
							auto local_d = dst + (mbb * jcp.mb_block + mb) * jcp.oh * jcp.ow * jcp.oc + y * jcp.ow * jcp.oc + x * jcp.oc;
							auto local_w = wino_dst + m * jcp.oc;
