	...
	L(ic_block_label);
	{
		for (int y = 0; y < jcp.alpha; y++) {
			...
			for (int x = 0; x < jcp.alpha; x++) {
				...
				inp_offset = sizeof(float) * ((-jcp.t_pad + y) * jcp.iw * load_block + (-jcp.l_pad + x) * load_block);
				vmovups(zmm | r_mask,
...
#undef READ_PARAM
		...
		L(K_loop_label);
		{
			...
			for (int _i = 0; _i < jcp.k2_block; _i++) {
				for (int nb2 = 0; nb2 < jcp.n2_block; nb2++) {
					if (jcp.small_mb) {
						int wei_offset = sizeof(float) * ((nb2 * jcp.nb_ic * jcp.ic_block * jcp.oc_block) + _i * jcp.oc_block);
			...
			if (jcp.small_mb)
				...
				add(reg_aux_wei2,
						sizeof(float) * jcp.k2_block * jcp.n2_block * jcp.oc_block);
		}
		dec(reg_K);
		cmp(reg_K, 0);
	...
	if (!jcp.small_mb) {
		...
		add(reg_aux_wei,
				sizeof(float) * jcp.k_chunks * jcp.ic_block * jcp.n2_block * jcp.oc_block);
...
status_t jit_avx512_core_fp32_wino_conv_2x3_fwd_ker_t ::init_conf(
	...
	bool ok = true && jcp.kh == 3 && jcp.kw == 3 && jcp.ngroups == 1 && jcp.oc % jcp.oc_block == 0 && jcp.ic % jcp.ic_block == 0 && jcp.stride_h == 1 && jcp.stride_w == 1 && jcp.dilate_h == 0 && jcp.dilate_w == 0 && jcp.t_pad == jcp.b_pad && jcp.l_pad == jcp.r_pad && jcp.t_pad < 2 && jcp.t_pad >= 0 && jcp.l_pad < 2 && jcp.l_pad >= 0;
	if (!ok)
		return status::unimplemented;
	...
	if (mb > nstl::min(jcp.nthr, 28) || (!jcp.small_mb && (wei_sz >= 0.9f * L2_cap || inp_sz > L2_cap * jcp.nthr + L3_capacity)) || (jcp.small_mb && sp_sz > 196))
		return status::unimplemented;
	...
	auto find_m_n2_blocks = [=](int xb, int yb, int &M, int &m_block,
		...
		for (int im = max_m_block; im > 0; im--) {
			for (int in2 = max_n2_block; in2 > 0; in2--) {
				...
				if (M % im || jcp.nb_oc % in2 || used_regs > skx_free_regs || cur_reg_eff <= reg_eff)
					continue;
				reg_eff = cur_reg_eff;
				m_block = im;
				n2_block = in2;
	...
	wd.wino_format = jcp.small_mb ? mkldnn_wino_wei_aaOio : mkldnn_wino_wei_aaOBiOo;
	wd.r = jcp.r;
	wd.alpha = jcp.alpha;
	wd.ic = jcp.ic;
	wd.oc = jcp.oc;
...
status_t jit_avx512_core_fp32_wino_conv_2x3_fwd_t ::pd_t::jit_conf(memory_desc_t &expect_wei_md) {
...
jit_avx512_core_fp32_wino_conv_2x3_fwd_t::
		jit_avx512_core_fp32_wino_conv_2x3_fwd_t(const pd_t *apd) : cpu_primitive_t(apd) {
...
jit_avx512_core_fp32_wino_conv_2x3_fwd_t ::~jit_avx512_core_fp32_wino_conv_2x3_fwd_t() {
	delete kernel_;
	delete src_trans_;
	delete dst_trans_;
}
...
void jit_avx512_core_fp32_wino_conv_2x3_fwd_t::execute_forward_mbN(
	...
	parallel_nd(jcp.mb, div_up(jcp.oh, jcp.yb), div_up(jcp.ow, jcp.xb),
			[&](int mb, int tile_y_b, int tile_x_b) {
				...
				auto src_trans_p =
						jit_avx512_core_fp32_wino_conv_2x3_src_trans_t ::call_params_t();
				auto dst_trans_p =
						jit_avx512_core_fp32_wino_conv_2x3_dst_trans_t ::call_params_t();
...
#pragma unroll(4)
						...
						auto local_s = src + mb * jcp.nb_ic * jcp.ih * jcp.iw * jcp.ic_block + y * jcp.iw * jcp.ic_block + x * jcp.ic_block;
...
#pragma unroll(2)
						...
						auto local_d = dst + mb * jcp.nb_oc * jcp.oh * jcp.ow * jcp.oc_block + y * jcp.ow * jcp.oc_block + x * jcp.oc_block;
...
#pragma unroll(4)
				...
				parallel_nd(16, jcp.n_chunks, [&](int tile_ij, int nnb) {
					...
					gemm_p.dst = ptr_M + jcp.out_stride * tile_ij + nnb * jcp.n2_block * jcp.n_block;
