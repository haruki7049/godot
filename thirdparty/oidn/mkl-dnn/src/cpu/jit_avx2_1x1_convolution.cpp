		...
		while (iwork < end) {
			...
			while (ocb < jcp.nb_load) {
				...
				for (int icb = 0; icb < nb_ic; icb += nb_ic_blocking) {
					p.first_last_flag = 0 | (icb == 0 ? FLAG_REDUCE_FIRST : 0) | (icb + nb_ic_blocking >= nb_ic ? FLAG_REDUCE_LAST : 0);
	...
	auto oc_ic_sp_loop = [=](int sp_start, int sp_end, bool first_image,
		...
		for (int oc_b = 0; oc_b < nb_oc_blocking; oc_b += oc_b_step) {
			...
			for (int ic_b = 0; ic_b < nb_ic_blocking; ic_b += ic_b_step) {
				...
				p.output_data = store_to + oc_b * store_to_ld + ic_b * jcp.ic_block * jcp.oc_block;
				...
				for (int sp = sp_start; sp < sp_end; sp += sp_step) {
					...
					p.first_last_flag = sp == sp_start && first_image ? FLAG_REDUCE_FIRST : 0;

					p.load_data = diff_dst + (oc_b * jcp.reduce_dim + sp) * jcp.oc_block;
					...
					if (pd()->rtus_.reduce_src_) {
						...
						rp.ws = rtus_space + ithr * pd()->rtus_.space_per_thread_ + (ic_b * jcp.is + sp) * jcp.ic_block;
						if (ndims == 3)
							rp.src = src + iw * src_d.blocking_desc().strides[2];
						else
							rp.src = src + ih * src_d.blocking_desc().strides[2] + iw * src_d.blocking_desc().strides[3];
						...
						p.bcast_data = src + (ic_b * jcp.reduce_dim + sp) * jcp.ic_block;
	...
	auto ker = [&](const int ithr, const int nthr) {
		...
		for (int iwork = 0; iwork < w_njobs; ++iwork) {
			...
			if (rw->balancer().nthr_per_group_ == 1) {
				const size_t off = pd()->with_groups() ? diff_weights_d.blk_off(g, oc_b, ic_b) : diff_weights_d.blk_off(oc_b, ic_b);
	...
	auto ker_bias = [&](int ithr, int nthr) {
		...
		for (int img = img_start; img < img_end; ++img) {
			...
			for (int b_job_loc = 0; b_job_loc < b_njobs; ++b_job_loc) {
				...
				data_t *d_bias =
						rb->get_local_ptr(ithr, diff_bias, reducer_bia_scratchpad) + b_job_loc * rb->balancer().job_size_;
