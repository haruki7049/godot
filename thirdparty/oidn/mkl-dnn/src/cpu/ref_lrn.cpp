...
void ref_lrn_fwd_t<data_type>::execute_forward(const exec_ctx_t &ctx) const {
	...
	auto data_off = [&](int mb, int c, int h, int w) -> size_t {
		switch (tag) {
			case nChw16c:
			case nChw8c:
				return mb * stride_mb + c / blksize * H * W * blksize + h * W * blksize + w * blksize + c % blksize;
			case nchw:
				return mb * stride_mb + c * H * W + h * W + w;
			case nhwc:
				return mb * stride_mb + h * W * C + w * C + c;
			default:
				return data_d.off(mb, c, h, w);
		}
	};
	...
	if (tag == nChw16c || tag == nChw8c) {
		parallel_nd(MB, utils::div_up(C, blksize), H, W,
				[&](int mb, int c_blk, int h, int w) {
					int c = c_blk * blksize;
					const size_t off = mb * stride_mb + c * H * W + (h * W + w) * blksize;
					PRAGMA_OMP_SIMD()
...
void ref_lrn_bwd_t<data_type>::execute_backward(const exec_ctx_t &ctx) const {
	...
	auto ker = [=](data_t *d, int mb, int oc, int oh, int ow) {
		...
		for (int c = c_st; c < c_en; c++) {
			...
			if (c == oc)
				omega_mid = omega;
			float t = src[data_off(mb, c, oh, ow)] * fast_negative_powf(omega, beta);
