...
#define src_blk_off(f, n, c, d, h, w)                                                              \
	(pd()->ndims() == 3) ? (f).blk_off(n, c, w) : (pd()->ndims() == 4) ? (f).blk_off(n, c, h, w) : \
																		 (f).blk_off(n, c, d, h, w)

#define wht_blk_off_(f, g, ...) \
	pd()->with_groups() ? (f).blk_off(g, __VA_ARGS__) : (f).blk_off(__VA_ARGS__)
#define wht_blk_off(f, g, oc, ic, kd, kh, kw)                                                                           \
	(pd()->ndims() == 3) ? wht_blk_off_(f, g, oc, ic, kw) : (pd()->ndims() == 4) ? wht_blk_off_(f, g, oc, ic, kh, kw) : \
		...
		while (icbb < jcp.nb_ic) {
			...
			for (size_t iwork = start; iwork < end; ++iwork) {
				...
				for (int icb = icbb; icb < icbb + icb_step; ++icb) {
					...
					const int i_b_overflow = nstl::max(jcp.ih, ij + (jcp.kh - 1) * (jcp.dilate_h + 1) - jcp.t_pad + 1) - jcp.ih;
			...
			for (int oc = 0; oc < jcp.nb_oc; oc += jcp.nb_oc_blocking)
				for (int id = 0; id < jcp.id; ++id) {
					...
					for (int ih = ih_start; ih < ih_end; ++ih) {
						const int i_t_overflow = nstl::max(0, (jcp.kh - 1 - ih - jcp.t_pad) / jcp.stride_h);
						const int i_b_overflow = nstl::max(0, (jcp.kh - jcp.ih + ih - jcp.b_pad) / jcp.stride_h);
						int overflow_kh_hi = jcp.kh - 1 - abs((jcp.ih - 1 + jcp.b_pad - ih) % jcp.stride_h);
						int overflow_kh_lo = (ih + jcp.t_pad) % jcp.stride_h;

						par_conv.kd_padding = jcp.kd - d_t_overflow - d_b_overflow;
						par_conv.kh_padding = (overflow_kh_hi - overflow_kh_lo) / jcp.stride_h + 1 - i_t_overflow - i_b_overflow;
						par_conv.kw_padding = 0;

						const int k_lo = overflow_kh_lo + i_b_overflow * jcp.stride_h;
			...
			if (id_s < idp - jcp.back_pad - jcp.kd + 1)
				for (int w_job_loc = 0; w_job_loc < w_njobs; ++w_job_loc) {
					...
					for (int od = od_s; od < od_e; ++od) {
						...
						if (id >= jcp.id - jcp.back_pad - jcp.kd + 1)
							break;
