...
int get_divisor_satisfying_cond(jit_conv_winograd_conf_t &jcp, int number,
	...
	auto test_num = [&best_divisor, test](jit_conv_winograd_conf_t &jcp, int num) {
...
bool is_winograd_faster_than_direct(const jit_conv_winograd_conf_t &jcp) {
	...
	if (jcp.prop_kind == prop_kind::forward_inference) {
		...
		double src_dst_transforms_per_core = alpha * alpha * (jcp.ic + jcp.oc) * jcp.mb * ((jcp.oh + tile_size - 1) / tile_size) * ((jcp.ow + tile_size - 1) / tile_size) * sizeof(float) / 1024. / 1024. / nthreads;
		double wei_transform = alpha * alpha * jcp.ic * jcp.oc * sizeof(float) / 1024. / 1024.;

		if (jcp.prop_kind == prop_kind::backward_weights) {
			if (src_dst_transforms_per_core < 0.3 || (src_dst_transforms_per_core <= 28 && wei_transform < 4))
				return false;
			else
				return true;
		} else {
...
struct prefetcher_t {
	prefetcher_t(jit_generator *generator, Xbyak::Reg64 reg_base_addr,
			...
			int nb_instructions_in_block, int fma_ipc) : cg_(generator),
		...
		prefetch_spread_ = div_up(nb_instructions_in_block, nb_cache_lines_to_prefetch_);
		prefetch_blk_ = div_up(nb_cache_lines_to_prefetch_, nb_instructions_in_block);
	...
	void prefetch(int instruction_number) {
		if (instruction_number % prefetch_spread_ == 0) {
			for (int i = 0; (i < prefetch_blk_) && (prefetches_issued_ < nb_cache_lines_to_prefetch_);
					i++, prefetches_issued_++) {
				prefetch_inst_(cg_->EVEX_compress_addr(
						reg_base_addr_, (cache_block_size_ * prefetch_distance_) * sizeof(data_t) + (prefetches_issued_ * 64)));
			}
		}
	}
...
bool check_L2_block_per_thread(jit_conv_winograd_conf_t &jcp,
	...
	float block_size = alpha * alpha * (2 * (jcp.oc + jcp.ic) * dimN_block * jcp.dimN_reg_block + div_up(jcp.ic * jcp.oc, mkldnn_get_max_threads())) * (float)sizeof(float);
...
bool check_L1_block_gemm(jit_conv_winograd_conf_t &jcp, int dimK_block,
	...
	float gemm_block_size = (dimM_block * jcp.dimM_simd_block * dimK_block * jcp.dimK_reg_block * jcp.dimM_reg_block + dimK_block * jcp.dimK_reg_block * jcp.dimN_reg_block + dimM_block * jcp.dimM_simd_block * jcp.dimN_reg_block) * (float)sizeof(float);
...
bool check_cond1(int dimN_reg_block, int dimK_block, int dimK_reg_block,
	...
	float lhs = (dimM_block * dimN_reg_block * dimM_simd_block * dimM_reg_block + dimM_block * dimK_block * dimK_reg_block * dimM_simd_block * dimM_reg_block + dimK_block * dimN_reg_block * dimK_reg_block) * (float)sizeof(float);
	float rhs = C * L1_cache_size;
	return (lhs < rhs);
}
bool check_cond1_bis(int dimN_reg_block, int dimK_block, int dimK_reg_block,
		int dimM_block, int dimM_reg_block, int dimM_simd_block, float C) {
	float lhs = (dimM_block * dimM_reg_block * dimK_block * dimK_reg_block * dimM_simd_block + dimK_block * dimN_reg_block * dimK_reg_block) * (float)sizeof(float);
	float rhs = C * L1_cache_size;
	return (lhs < rhs);
}
bool check_cond2(int nb_dimN_reg_block, int dimN_reg_block, int dimK_nb_block,
	...
	float lhs = (nb_dimN_reg_block * dimM_block * dimN_reg_block * dimM_simd_block * dimM_reg_block + dimK_nb_block * dimM_block * dimK_block * dimK_reg_block * dimM_simd_block * dimM_reg_block + nb_dimN_reg_block * dimK_nb_block * dimK_block * dimN_reg_block * dimK_reg_block) * (float)sizeof(float);
...
bool check_kernel_cond(int dimM_block, int dimM_reg_block, int dimM_simd_block,
	...
	float A_size = dimM_block * dimM_reg_block * dimM_simd_block * dimK * (float)sizeof(float);
...
void _jit_avx512_core_fp32_wino_conv_4x3_data_kernel::gemm_loop_generate() {
	...
	auto zmm_dstC = [=](int dimM_reg_block, int tile) {
		...
		if (jcp.kernel_kind == embd_bcast)
			...
			idx = 1 + jcp.dimN_reg_block + dimM_reg_block * jcp.dimN_reg_block + tile;
		assert(idx < 32);
		return Xbyak::Zmm(idx);
	};
	...
	auto store_output = [=](bool output_is_aligned) {
		...
		for (int dimM_reg_block = 0; dimM_reg_block < jcp.dimM_reg_block;
			...
			for (int tile = 0; tile < jcp.dimN_reg_block; tile++) {
				Zmm zmm = zmm_dstC(dimM_reg_block, tile);
				int output_offset = jcp.dimN_reg_block * dimM_reg_block * 64 + tile * 64;
		...
		for (int dimM_reg_block = 0; dimM_reg_block < jcp.dimM_reg_block;
			...
			for (int tile = 0; tile < jcp.dimN_reg_block; tile++) {
				...
				if (output_is_aligned && jcp.dimK_nb_block == 1 && jcp.sched_policy == WSCHED_DATA_W_S_G_D && (jcp.dimN * jcp.dimM * alpha * alpha * sizeof(float) > 2 * LLC_data_size))
	...
	auto inner_loops = [=]() {
		...
		for (int dimK_reg_block = 0;
			...
			for (int dimM_reg_block = 0; dimM_reg_block < jcp.dimM_reg_block;
					dimM_reg_block++) {
				vmovups(zmm_srcA(),
						zword[reg_srcA + jcp.dimK_reg_block * jcp.dimK_block * 64 * dimM_reg_block + dimK_reg_block * 64]);
		...
		if (jcp.dimM_block > 1) {
			...
			if (jcp.kernel_kind == expl_bcast) {
				add(reg_srcA,
						(jcp.dimM_reg_block - 1) * jcp.dimK_reg_block * 64 * jcp.dimK_block);
			}
...
void _jit_avx512_core_fp32_wino_conv_4x3_data_kernel ::weights_transform_data_ker_generate() {
			...
			for (int i = 0; i < kw; i++) {
				if (is_fwd) {
					for (int v1 = 0; v1 < simd_w; v1++) {
						int offset_src = (j * kw * simd_w * simd_w + i * simd_w * simd_w + v1 * simd_w) * typesize;
						int offset_F = (j * kw * simd_w * simd_w + i * simd_w * simd_w + v1 * simd_w) * typesize;
						vmovups(zmm_temp, ptr[wreg_src + offset_src]);
						vmovups(ptr[wreg_F + offset_F], zmm_temp);
					}
				} else {
					int offset_src = ((2 - j) * kw * simd_w * simd_w + (2 - i) * simd_w * simd_w) * typesize;
					int offset_F = (j * kw * simd_w * simd_w + i * simd_w * simd_w) * typesize;
	...
	auto store_dst = [=]() {
		...
		int dim5 = jcp.dimK_nb_block * (jcp.dimM_block * jcp.dimM_reg_block) * jcp.dimK_block * simd_w * simd_w;

		L(Loop_j);
		{
			for (int i = 0; i < alpha; i++) {
				// touch pages
				vmovups(zmm_load(0), ptr[wreg_Fw_aux + (i * simd_w * simd_w) * typesize]);
				mov(wreg_dst_idx, i * dim5 * typesize);
				vmovntps(ptr[wreg_dst_aux + wreg_dst_idx], zmm_load(0));
			}
			for (int i = 0; i < alpha; i++) {
				for (int v1 = 1; v1 < simd_w; v1++) {
					int offset_Fw = (i * simd_w * simd_w + v1 * simd_w) * typesize;
	...
	auto trans_W_4x4_3x3 = [=]() {
		...
		for (int i = 0; i < 3; i++) {
			for (int idx = 0; idx < 3; idx++) {
				vmovups(zmm_F(idx), ptr[wreg_F_aux + (idx * 3 * simd_w * simd_w + i * simd_w * simd_w) * typesize]);
			}
			...
			for (int idx = 0; idx < 6; idx++) {
				vmovups(ptr[wreg_T + (idx * 3 * simd_w + i * simd_w) * typesize], zmm_T(idx));
			}
		}
		for (int i = 0; i < 6; i++) {
			for (int idx = 0; idx < 3; idx++) {
				vmovups(zmm_T(idx), ptr[wreg_T + (i * 3 * simd_w + idx * simd_w) * typesize]);
			}
			...
			for (int l = 0; l < 6; l++) {
				vmovups(ptr[wreg_Fw_aux + (i * 6 * simd_w * simd_w + l * simd_w * simd_w) * typesize], zmm_F(l));
			}
		}
		add(wreg_cnt_j, 1);
		cmp(wreg_cnt_j, 16);
...
void _jit_avx512_core_fp32_wino_conv_4x3_data_kernel ::output_transform_data_ker_generate() {
	bool is_fwd = one_of(jcp.prop_kind,
			mkldnn_forward_training, mkldnn_forward_inference);
	...
	auto load_src = [=]() {
		...
		imul(oreg_nb_tile_block_ur, oreg_nb_tile_block_ur,
				(jcp.dimM_block * jcp.dimM_reg_block) * jcp.dimN_reg_block * jcp.dimM_simd_block * typesize);
		...
		if (not_tiled) {
			...
			imul(oreg_tile_block, oreg_tile_block,
					jcp.dimM_nb_block * alpha * alpha * jcp.dimN_block * (jcp.dimM_block * jcp.dimM_reg_block) * jcp.dimN_reg_block * jcp.dimM_simd_block * typesize);
			add(oreg_src, oreg_tile_block);
		}

		int last4dim = jcp.dimN_block * (jcp.dimM_block * jcp.dimM_reg_block) * jcp.dimN_reg_block * jcp.dimM_simd_block * typesize;
		for (int j = 0; j < alpha; j++) {
			for (int i = 0; i < alpha; i++) {
				...
				vmovups(ptr[oreg_Ow + (j * alpha * simd_w + i * simd_w) * typesize], zmm_temp);
			}
		}
	};
	...
	auto trans_O_4x4_3x3 = [=]() {
		...
		for (int i = 0; i < alpha; i++) {
			for (int j = 0; j < alpha; j++) {
				vmovups(zmm_O(j), ptr[oreg_Ow + (j * alpha * simd_w + i * simd_w) * typesize]);
			}
			...
			for (int j = 0; j < tile_size; j++) {
				vmovups(ptr[oreg_T + (j * alpha * simd_w + i * simd_w) * typesize], zmm_T(j));
			}
		}
		for (int j = 0; j < tile_size; j++) {
			for (int i = 0; i < alpha; i++) {
				vmovups(zmm_T(i), ptr[oreg_T + (j * alpha * simd_w + i * simd_w) * typesize]);
			}
			...
			for (int i = 0; i < tile_size; i++) {
				vmovups(ptr[oreg_O + (j * tile_size * simd_w + i * simd_w) * typesize], zmm_O(i));
			}
		}
	};
...
void _jit_avx512_core_fp32_wino_conv_4x3_data_kernel ::input_transform_data_ker_generate() {
	bool is_fwd = one_of(jcp.prop_kind,
			mkldnn_forward_training, mkldnn_forward_inference);
	...
	auto load_src = [=]() {
		...
		for (int j = 0; j < alpha; j++) {
			...
			for (int i = 0; i < alpha; i++) {
				...
				vmovups(ptr[ireg_I + (j * alpha * simd_w + i * simd_w) * typesize], zmm_temp);
			}
		}
	};
	...
	auto store_Iw = [=]() {
		...
		bool streamout = jcp.dimN * jcp.dimK * alpha * alpha * sizeof(float) > 2 * LLC_data_size ? true : false;

		if (not_tiled) {
			mov(ireg_tile_block, ptr[param1 + GET_OFF(tile_block)]);
			imul(ireg_tile_block, ireg_tile_block,
					alpha * alpha * jcp.dimN_block * jcp.dimK_nb_block * jcp.dimK_block * jcp.dimN_reg_block * jcp.dimK_reg_block * typesize);
		}

		mov(ireg_nb_tile_block_ur, ptr[param1 + GET_OFF(nb_tile_block_ur)]);
		imul(ireg_nb_tile_block_ur, ireg_nb_tile_block_ur,
				jcp.dimK_nb_block * jcp.dimK_block * jcp.dimN_reg_block * jcp.dimK_reg_block * typesize);
		...
		for (int j = 0; j < alpha; j++) {
			for (int i = 0; i < alpha; i++) {
				vmovups(zmm_temp, ptr[ireg_Iw + (j * alpha * simd_w + i * simd_w) * typesize]);

				int j_base_offset =
						j * alpha * jcp.dimN_block * jcp.dimK_nb_block * jcp.dimK_block * jcp.dimN_reg_block * jcp.dimK_reg_block * typesize;
				int i_base_offset =
						i * jcp.dimN_block * jcp.dimK_nb_block * jcp.dimK_block * jcp.dimN_reg_block * jcp.dimK_reg_block * typesize;
	...
	auto trans_I_4x4_3x3 = [=]() {
		...
		for (int i = 0; i < alpha; i++) {
			for (int idx = 0; idx < alpha; idx++) {
				vmovups(zmm_I(idx), ptr[ireg_I + (idx * alpha * simd_w + i * simd_w) * typesize]);
				int j_base_offset =
						i * alpha * jcp.dimN_block * jcp.dimK_nb_block * jcp.dimK_block * jcp.dimN_reg_block * jcp.dimK_reg_block * typesize;
				int idx_base_offset =
						idx * jcp.dimN_block * jcp.dimK_nb_block * jcp.dimK_block * jcp.dimN_reg_block * jcp.dimK_reg_block * typesize;
			...
			for (int idx = 0; idx < alpha; idx++) {
				vmovups(ptr[ireg_T + (idx * alpha * simd_w + i * simd_w) * typesize], zmm_T(idx));
			}
		}
		for (int i = 0; i < alpha; i++) {
			for (int idx = 0; idx < alpha; idx++) {
				vmovups(zmm_T(idx), ptr[ireg_T + (i * alpha * simd_w + idx * simd_w) * typesize]);
			}
			...
			for (int idx = 0; idx < alpha; idx++) {
				vmovups(ptr[ireg_Iw + (i * alpha * simd_w + idx * simd_w) * typesize], zmm_I(idx));
			}
		}
	};
...
status_t _jit_avx512_core_fp32_wino_conv_4x3_data_kernel::init_conf_common(
	...
	bool layout_consistency = true && jcp.ic <= src_d.padded_dims()[1] && jcp.oc <= dst_d.padded_dims()[1] && (one_of(weights_d.format_kind(), format_kind::any, format_kind::wino) || (jcp.ic <= weights_d.padded_dims()[with_groups + 1] && jcp.oc <= weights_d.padded_dims()[with_groups + 0]));
...
void set_kernel_dims_reg_block(jit_conv_winograd_conf_t &jcp) {
	...
	auto test_cond_dimM_reg_block = [](jit_conv_winograd_conf_t &jcp,
		...
		return (dimM_reg_block >= 1) && (dimM_reg_block <= max_dimM_reg_block) && (dimM_reg_block > current_best);
	};
	...
	auto test_cond_dimN_reg_block = [](jit_conv_winograd_conf_t &jcp,
		...
		return jcp.kernel_kind == embd_bcast ? dimN_reg_block < jcp.nb_reg && dimN_reg_block > current_best : dimN_reg_block >= 1 && (dimN_reg_block * jcp.dimM_reg_block + dimN_reg_block) < jcp.nb_reg && dimN_reg_block > current_best;
	};
...
status_t set_wsched_DATA_W_SGD_avx512_core(jit_conv_winograd_conf_t &jcp) {
	...
	auto test_cond_dimN_block = [](jit_conv_winograd_conf_t &jcp,
		...
		return check_L2_block_per_thread(jcp, dimN_block, 0.1, 2.0) && (dimN_block > current_best) && ((jcp.dimN / dimN_block / jcp.dimN_reg_block) >= 1.5 * mkldnn_get_max_threads());
	};
	...
	if (check_L2_block_per_thread(jcp, jcp.dimN_block, 0.1, 3.2) && (jcp.dimN_nb_block >= 1.5 * mkldnn_get_max_threads())) {
		/* ------------------- L1 blocking for GEMM --------------*/
		/* -------------------- Choose dimK block ----------------*/

		auto test_cond_dimK_block = [](jit_conv_winograd_conf_t &jcp,
											int dimK_block, int current_best) {
			return check_L1_block_gemm(jcp, dimK_block, 1, 0.1, 0.5) && (dimK_block > current_best);
		};
		...
		if (check_L1_block_gemm(jcp, jcp.dimK_block, 1, 0.1, 1.0)) {
			...
			jcp.dimM_nb_block = jcp.dimM / jcp.dimM_block / jcp.dimM_reg_block / jcp.dimM_simd_block;
...
void set_kernel_blocking_DATA_W_S_G_D(jit_conv_winograd_conf_t &jcp) {
	...
	jcp.dimM_nb_block = jcp.dimM / (jcp.dimM_simd_block * jcp.dimM_block * jcp.dimM_reg_block);
...
bool jit_avx512_core_fp32_wino_conv_4x3_fwd_kernel::post_ops_ok(
	...
	switch (p.len_) {
		...
		case 1:
			return is_relu(0) || is_sum(0); // relu or sum
		case 2:
			return (is_sum(0) && is_relu(1)) || (is_relu(0) && is_sum(1)); // sum->relu or relu->sum
		case 3:
			return is_relu(0) && is_sum(1) && is_relu(2); // relu->sum->relu
		default:
			return false;
	}
...
void jit_avx512_core_fp32_wino_conv_4x3_bwd_weights_kernel::
	...
	auto trans_I_3x3_4x4 = [=]() {
		...
		for (int j = 0; j < alpha; j++) {
			...
			for (int i = 0; i < alpha; i++) {
				size_t dst_off = (j * alpha * jcp.ic_block * jcp.nb_tile_block_ur * jcp.tile_block_ur + i * jcp.ic_block * jcp.nb_tile_block_ur * jcp.tile_block_ur) * simd_w * typesize;
	...
	auto compute_transform = [=]() {
...

		{
			L(loop_itiles);
			{
				...
				L(next_tile_block);
				sub(reg_dst, (jcp.nb_tile_block_ur * jcp.tile_block_ur - 1) * simd_w * typesize);
				size_t tblk_off = alpha * alpha * jcp.ic_block * jcp.nb_tile_block_ur * jcp.tile_block_ur * simd_w * typesize;
...
void jit_avx512_core_fp32_wino_conv_4x3_bwd_weights_kernel::
	...
	auto trans_W_3x3_4x4 = [=]() {
		...
		for (int j = 0; j < alpha; j++) {
			...
			int alpha_offset = (jcp.oc / jcp.nb_oc) * (jcp.ntiles / jcp.tile_block) * typesize;
	...
	auto compute_transform_SDGtWo = [=]() {
		...
		mov(reg_dst, ptr[reg_transp + GET_OFF(dst)]);
		if (with_bias)
			mov(reg_bias, ptr[reg_transp + GET_OFF(bias)]);
		...
		L(loop_oc_ur);
		{
			...
			L(loop_jtiles);
			{
				L(loop_itiles);
				{
					...
					L(next_tile_block);
					sub(reg_dst, (jcp.nb_tile_block_ur * jcp.tile_block_ur - 1) * jcp.oc_reg_block * simd_w * typesize);
					int tblk_off = alpha * alpha * (jcp.oc / jcp.nb_oc) * (jcp.ntiles / jcp.tile_block) * typesize;
					add(reg_dst, tblk_off);
					xor_(reg_tile_count, reg_tile_count);
...
void jit_avx512_core_fp32_wino_conv_4x3_bwd_weights_kernel::
	...
	auto load_src = [=](int i) {
		for (int j = 0; j < alpha; j++) {
			size_t alpha_offset = jcp.oc_block * jcp.oc_reg_block * jcp.ic_block * simd_w * simd_w * typesize;
...
void jit_avx512_core_fp32_wino_conv_4x3_bwd_weights_kernel::gemm_loop_generate(
	...
	auto broadcastB = [=](size_t K_ur) {
		for (int N_bcast = 0; N_bcast < jcp.dimN_bcast_ur; N_bcast++) {
			size_t srcB_off = (K_ur * jcp.dimN_reg_block + N_bcast) * sizeof(float);
			vbroadcastss(zmm_srcB(N_bcast), EVEX_compress_addr(reg_srcB, srcB_off));
		}
	};

	auto load_srcA = [=](size_t K_ur, int M_ur) {
		size_t srcA_off = (K_ur * jcp.dimM_reg_block * jcp.dimM_simd_block + M_ur * jcp.dimM_simd_block) * sizeof(float);
	...
	auto store_dstC = [=]() {
		...
		for (int M_reg = 0; M_reg < jcp.dimM_reg_block; M_reg++) {
			for (int N_ur = 0; N_ur < jcp.dimN_bcast_ur; ++N_ur) {
				...
				size_t C_off = (N_ur * jcp.dimM_reg_block * jcp.dimM_simd_block + M_reg * jcp.dimM_simd_block) * sizeof(float);
				if (!is_first_tile) {
			...
			{ /*************** IC_block (N) loop *********/
				...
				L(dimN_bcast_ur);
				{
					...
					L(dimK_block_loop);
					{
						...
						add(reg_srcA, jcp.dimK_reg_block * jcp.dimM_reg_block * jcp.dimM_simd_block * sizeof(float));
						add(reg_srcB, jcp.dimK_reg_block * jcp.dimN_reg_block * sizeof(float));
						sub(reg_dimK_block_loop_cnt, 1);
						jnz(dimK_block_loop);
					}
					...
					sub(reg_srcA, jcp.dimK_block * jcp.dimK_reg_block * jcp.dimM_reg_block * jcp.dimM_simd_block * sizeof(float));
					sub(reg_srcB, jcp.dimK_block * jcp.dimK_reg_block * jcp.dimN_reg_block * sizeof(float));
					add(reg_srcB, jcp.dimN_bcast_ur * sizeof(float));
					add(reg_dstC, jcp.dimN_bcast_ur * jcp.dimM_reg_block * jcp.dimM_simd_block * sizeof(float));
					sub(reg_nb_dimN_bcast_ur, 1);
					jnz(dimN_bcast_ur);
				}

				sub(reg_srcB, jcp.dimN_reg_block * sizeof(float));
				add(reg_srcB, jcp.dimK_block * jcp.dimK_reg_block * jcp.dimN_reg_block * sizeof(float));
				sub(reg_dimN_block_loop_cnt, 1);
				jnz(dimN_block_loop);
			}

			sub(reg_srcB, jcp.dimN_block * jcp.dimK_block * jcp.dimK_reg_block * jcp.dimN_reg_block * sizeof(float));
			add(reg_srcA, jcp.dimK_block * jcp.dimK_reg_block * jcp.dimM_reg_block * jcp.dimM_simd_block * sizeof(float));
			sub(reg_dimM_block_loop_cnt, 1);
			jnz(dimM_block_loop);
		}
	};
...
status_t set_wsched_WEI_SDGtWo(jit_conv_winograd_conf_t &jcp) {
	...
	auto test_MV_large_enough = [](jit_conv_winograd_conf_t &jcp) {
		...
		return (((V_sz + M_sz) / nthreads) >= 2 * L2_cache_size) && (jcp.dimK / nthreads >= 1.0);
	};
	...
	auto test_min_dimK_L1 = [](jit_conv_winograd_conf_t &jcp, int dimK_block_ur,
		...
		return (L1_block_M + L1_block_N >= 0.1 * L1_cache_size) && (L1_block_M + L1_block_N <= 0.5 * L1_cache_size) && load_balance && (M_L2_block < L2_cache_size);
	};
	...
	auto blocking_ok = [&]() {
		size_t M_L2_block = alpha * alpha * M_blk * jcp.dimM_reg_block * jcp.dimM_simd_block * K_blk_ur * sizeof(float);
		size_t V_L2_block = alpha * alpha * N_blk * jcp.dimN_reg_block * K_blk_ur * sizeof(float);
		size_t U_L2_block = alpha * alpha * M_blk * jcp.dimM_reg_block * jcp.dimM_simd_block * N_blk * jcp.dimN_reg_block * sizeof(float);
...
status_t set_wsched_WEI_S_D_Giot_W(jit_conv_winograd_conf_t &jcp) {
	...
	auto blocking_ok = [&]() -> bool {
		...
		bool L1_cond = ((L1_block_N + L1_block_M) >= C1 * L1_cache_size) && ((L1_block_N + L1_block_M) <= C1_max * L1_cache_size);
...
status_t jit_avx512_core_fp32_wino_conv_4x3_bwd_weights_kernel::init_conf(
	...
	bool layout_consistency = true && jcp.ic <= src_d.padded_dims()[1] && jcp.oc <= diff_dst_d.padded_dims()[1] && jcp.ic <= diff_weights_d.padded_dims()[with_groups + 1] && jcp.oc <= diff_weights_d.padded_dims()[with_groups + 0];
	if (!layout_consistency)
		return status::unimplemented;
