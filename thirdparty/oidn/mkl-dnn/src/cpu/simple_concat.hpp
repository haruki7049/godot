...
struct simple_concat_t : public cpu_primitive_t {
	struct pd_t : public cpu_concat_pd_t {
		...
		status_t init() {
			...
			bool ok = true && cpu_concat_pd_t::init() == status::success && dst_d.ndims() <= 6;
			if (!ok)
			...
			for (size_t i = 0; i < src_mds_.size(); ++i) {
				...
				ok = ok && utils::everyone_is(data_type, i_d.data_type(), o_d.data_type()) && utils::everyone_is(format_kind::blocked, i_d.format_kind(), o_d.format_kind()) && types::blocking_desc_is_equal(i_d.blocking_desc(), o_d.blocking_desc(), ignore_strides) && types::blocking_desc_is_equal(i_d.blocking_desc(), dst_d.blocking_desc(), ignore_strides) && !i_d.is_additional_buffer();
				if (!ok)
					return status::unimplemented;
			}
			...
			if (nelems_to_concat(dst_d) !=
					dst_d.padded_dims()[concat_dim()] / blocks_[concat_dim()] * dst_d.blocking_desc().strides[concat_dim()])
				return status::unimplemented;
			...
			for (size_t i = 0; i < src_mds_.size(); ++i) {
				...
				for (int d = start_dim; d < dst_d.ndims(); ++d) {
					if (dst_d.blocking_desc().strides[iperm_[d]] != i_d.blocking_desc().strides[iperm_[d]])
						return status::unimplemented;
				}
			}
