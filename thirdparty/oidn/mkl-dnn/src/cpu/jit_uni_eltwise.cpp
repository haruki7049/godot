...
int jit_uni_eltwise_injector_f32<isa>::aux_vecs_count(alg_kind_t alg_) {
	switch (alg_) {
		...
		case alg_kind::eltwise_logistic:
			return 4;
		default:
			assert(!"unsupported eltwise algorithm");
	}
	...
	for (size_t idx = start_idx; idx < end_idx; idx++) {
		switch (alg_) {
			case eltwise_relu:
				...
				break;
			case eltwise_elu:
				elu_compute_vector(Vmm(idx));
				break;
			case eltwise_tanh:
				tanh_compute_vector(Vmm(idx));
				break;
			case eltwise_square:
				square_compute_vector(Vmm(idx));
				break;
			case eltwise_abs:
				abs_compute_vector(Vmm(idx));
				break;
			case eltwise_sqrt:
				sqrt_compute_vector(Vmm(idx));
				break;
			case eltwise_linear:
				linear_compute_vector(Vmm(idx));
				break;
			case eltwise_bounded_relu:
				bounded_relu_compute_vector(Vmm(idx));
				break;
			case eltwise_soft_relu:
				soft_relu_compute_vector(Vmm(idx));
				break;
			case eltwise_logistic:
				logistic_compute_vector(Vmm(idx));
				break;
			default:
				assert(!"unsupported eltwise algorithm");
		}
	...
	if (gen_table) {
		switch (alg_) {
			...
			case eltwise_sqrt:
				...
				break;
			case eltwise_linear:
				linear_prepare_table();
				break;
			case eltwise_bounded_relu:
				bounded_relu_prepare_table();
				break;
			case eltwise_square:
				break;
			default:
...
struct jit_uni_eltwise_kernel_f32 : public c_compatible {
	...
	jit_uni_eltwise_kernel_f32(const eltwise_desc_t &desc) : desc_(desc), ker_(nullptr) {}
...
struct jit_uni_relu_kernel_f32 : public jit_uni_eltwise_kernel_f32,
	...
	jit_uni_relu_kernel_f32(const eltwise_desc_t &desc) : jit_uni_eltwise_kernel_f32(desc), jit_generator() {
...
struct jit_uni_kernel_fwd_f32 : public jit_uni_eltwise_kernel_f32,
	...
	jit_uni_kernel_fwd_f32(const eltwise_desc_t &desc) : jit_uni_eltwise_kernel_f32(desc), jit_generator() {
...
status_t jit_uni_eltwise_fwd_t<isa>::pd_t::init() {
	...
	bool ok = true && mayiuse(isa) && is_fwd() && utils::everyone_is(data_type::f32, desc()->data_desc.data_type) && !has_zero_dim_memory() && utils::one_of(desc()->alg_kind, eltwise_relu, eltwise_tanh, eltwise_elu, eltwise_square, eltwise_abs, eltwise_sqrt, eltwise_linear, eltwise_bounded_relu, eltwise_soft_relu, eltwise_logistic) && memory_desc_wrapper(src_md()).is_dense(true) && IMPLICATION(!memory_desc_wrapper(src_md()).is_dense(false), math::eltwise_fwd_preserves_zero(desc()->alg_kind, true)) && attr()->has_default_values();

	return ok ? status::success : status::unimplemented;
}

template <cpu_isa_t isa>
jit_uni_eltwise_fwd_t<isa>::jit_uni_eltwise_fwd_t(const pd_t *apd) : cpu_primitive_t(apd), kernel_(nullptr) {
	const auto &desc = *pd()->desc();
	switch (desc.alg_kind) {
		case alg_kind::eltwise_relu:
			kernel_ = new jit_uni_relu_kernel_f32<isa>(desc);
			break;
		default:
...
status_t jit_uni_eltwise_bwd_t<isa>::pd_t::init() {
	bool ok = true && !is_fwd() && utils::one_of(desc()->alg_kind, alg_kind::eltwise_relu) && src_md()->data_type == data_type::f32 && !has_zero_dim_memory() && mayiuse(isa) && memory_desc_wrapper(src_md()).is_dense() && memory_desc_wrapper(diff_dst_md()) == memory_desc_wrapper(src_md()) && attr()->has_default_values();

	return ok ? status::success : status::unimplemented;
}

template <cpu_isa_t isa>
jit_uni_eltwise_bwd_t<isa>::jit_uni_eltwise_bwd_t(const pd_t *apd) : cpu_primitive_t(apd), kernel_(nullptr) {
	const auto &desc = *pd()->desc();
	switch (desc.alg_kind) {
		case alg_kind::eltwise_relu:
			kernel_ = new jit_uni_relu_kernel_f32<isa>(desc);
