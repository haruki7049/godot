...
struct jit_avx512_core_x8s8s32x_deconv_fwd_kernel : public jit_generator {
	...
	jit_avx512_core_x8s8s32x_deconv_fwd_kernel(
			const jit_conv_conf_t &ajcp, const primitive_attr_t &attr) : jcp(ajcp),
																		 attr_(attr),
...
private:
	...
	int get_ow_start(int ki, int l_overflow) {
		int res = (jcp.ow - 1 + jcp.r_pad) % jcp.stride_w + l_overflow * jcp.stride_w - (jcp.kw - 1 - ki) * (jcp.dilate_w + 1);
		while (res < 0)
			res += jcp.stride_w;
		return res;
	}
...
struct _jit_avx512_core_x8s8s32x_deconvolution_fwd_t : public cpu_primitive_t {
	struct pd_t : public cpu_deconvolution_fwd_pd_t {
		...
		status_t init() {
			bool ok = true && is_fwd() && (desc()->alg_kind & alg_kind::deconvolution_direct) && desc()->src_desc.data_type == src_type && desc()->dst_desc.data_type == dst_type && IMPLICATION(with_bias(), utils::one_of(desc()->bias_desc.data_type, data_type::f32, data_type::s32, data_type::s8, data_type::u8)) && desc()->accum_data_type == data_type::s32;
			if (!ok)
	...
	_jit_avx512_core_x8s8s32x_deconvolution_fwd_t(const pd_t *apd) : cpu_primitive_t(apd) {
