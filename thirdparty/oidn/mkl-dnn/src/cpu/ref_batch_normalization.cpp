...
void ref_batch_normalization_fwd_t<data_type>::execute_forward(
	...
	auto mean = pd()->stats_is_src() ? const_cast<float *>(CTX_IN_MEM(const float *, MKLDNN_ARG_MEAN)) : CTX_OUT_MEM(float *, MKLDNN_ARG_MEAN);
	auto variance = pd()->stats_is_src() ? const_cast<float *>(CTX_IN_MEM(const float *, MKLDNN_ARG_VARIANCE)) : CTX_OUT_MEM(float *, MKLDNN_ARG_VARIANCE);
	...
	parallel_nd(C, [&](dim_t c) {
		...
		float sm = (use_scaleshift ? scaleshift[scaleshift_d.off(0, c)] : 1.0f) / sqrt_variance;
