...
void typed_zero_pad_blk(
	...
	assert((A_blocked || B_blocked || C_blocked) || (A_blocked && B_blocked) || (C_blocked && B_blocked));
	...
	auto zeroize_tail_inner = [&](data_t *d, const int tail_s) {
		for (int b1 = 0; b1 < blksize; ++b1)
			for (int b2 = tail_s; b2 < blksize; ++b2)
				d[(b1 / inner_blk) * blksize * inner_blk + inner_blk * b2 + b1 % inner_blk] = 0;
	};
...
status_t cpu_memory_t::zero_pad() const {
	...
	const bool skip_zeroing = false || data_ == nullptr || mdw.is_zero() || !mdw.is_blocking_desc();
	if (skip_zeroing)
		return success;

	switch (mdw.data_type()) {
		case f32:
			return typed_zero_pad<f32>();
		case s32:
			return typed_zero_pad<s32>();
		case s8:
			return typed_zero_pad<s8>();
		case u8:
			return typed_zero_pad<u8>();
		default:
