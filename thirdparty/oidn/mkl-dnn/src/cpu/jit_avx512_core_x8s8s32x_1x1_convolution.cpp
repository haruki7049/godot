...
template <data_type_t src_type, data_type_t dst_type>
void jit_avx512_core_x8s8s32x_1x1_convolution_fwd_t<src_type, dst_type>::execute_forward_thr(const int ithr, const int nthr, const src_data_t *src,
	...
	const size_t bia_dt_size = pd()->with_bias() ? types::data_type_size(pd()->desc()->bias_desc.data_type) : 0;
	...
	int offset = jcp.ngroups * (jcp.oc / jcp.oc_block) * (jcp.ic / jcp.ic_block) * jcp.oc_block * jcp.ic_block;
	...
	auto inner_ker = [&](int ocb, int n, int g, int oh, int ow,
		...
		p.compensation = (jcp.signed_input) ? &compensation[_ocb * jcp.oc_block] : 0;
		p.scales = (jcp.signed_input && jcp.ver != ver_vnni) ? &local_scales[jcp.is_oc_scale * _ocb * jcp.oc_block] : &oscales.scales_[jcp.is_oc_scale * _ocb * jcp.oc_block];
