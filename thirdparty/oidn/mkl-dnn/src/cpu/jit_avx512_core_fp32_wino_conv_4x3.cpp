...
void _jit_avx512_core_fp32_wino_conv_4x3_t<is_fwd>::weight_transform_data(const jit_conv_winograd_conf_t &jcp,
...
template <bool is_fwd>
void _jit_avx512_core_fp32_wino_conv_4x3_t<is_fwd>::output_transform_data(int image, const jit_conv_winograd_conf_t &jcp,
...
template <bool is_fwd>
void _jit_avx512_core_fp32_wino_conv_4x3_t<is_fwd>::output_transform_tileblock_data(int tile_block,
	...
	for (int nb_tile_block_ur = 0;
		...
		for (int tile_block_ur = 0; tile_block_ur < jcp.tile_block_ur;
			...
			p.tj = tj;
			p.ti = ti;
			p.dst = outp + img * (jcp.dimM / jcp.dimM_simd_block) * outh * outw * jcp.dimM_simd_block;
...
template <bool is_fwd>
void _jit_avx512_core_fp32_wino_conv_4x3_t<is_fwd>::input_transform_data(int image, const jit_conv_winograd_conf_t &jcp,
...
template <bool is_fwd>
void _jit_avx512_core_fp32_wino_conv_4x3_t<is_fwd>::input_transform_tileblock_data(int tile_block,
...
void _jit_avx512_core_fp32_wino_conv_4x3_t<is_fwd>::_execute_data_W_S_G_D(
	...
	auto wino_wei = (jcp.prop_kind == prop_kind::forward_inference) ? wei_ptr : scratchpad.template get<float>(key_wino_U);
...
void jit_avx512_core_fp32_wino_conv_4x3_bwd_weights_t::
	...
	const int U_sz = nthreads * alpha * alpha * jcp.oc / jcp.nb_oc * jcp.ic / jcp.nb_ic;
	...
	for (int ofm1 = 0; ofm1 < jcp.nb_oc; ++ofm1) {
		...
		for (int ofm2 = 0; ofm2 < jcp.oc_block; ++ofm2) {
			for (int ofm3 = 0; ofm3 < jcp.oc_reg_block; ++ofm3) {
				int ofm = (ofm1 * jcp.oc_block + ofm2) * jcp.oc_reg_block + ofm3;
				for (int ifm2 = 0; ifm2 < jcp.ic_block; ++ifm2) {
					int ifm = ifm1 * jcp.ic_block + ifm2;
...
void jit_avx512_core_fp32_wino_conv_4x3_bwd_weights_t::
	...
	PRAGMA_OMP(parallel firstprivate(first_tblk, trans_ker_p, I, T)) {
		...
		parallel_nd_in_omp(jcp.nb_ic, jcp.ic_block, jcp.mb,
				[&](int ifm1, int ifm2, int img) {
					...
					size_t tblk2 = (tile_base_index / jcp.tile_block_ur) % jcp.nb_tile_block_ur;
					size_t tblk1 = (tile_base_index / jcp.tile_block_ur) / jcp.nb_tile_block_ur;
					...
					if (first_tblk == 0) {
						input_starts[ithr] =
								...
								(float *)&(Us(ithr, 0, 0, 0, 0, 0, 0,
										0, 0, 0));
						input_ends[ithr] = input_starts[ithr] + jcp.oc_block * jcp.ic_block * jcp.ic_simd_block * jcp.oc_reg_block * jcp.oc_simd_block;
					} else if (tblk1 == 0) {
						input_ends[ithr] += jcp.oc_block * jcp.ic_block * jcp.ic_simd_block * jcp.oc_reg_block * jcp.oc_simd_block;
					}
	...
	if (jcp.with_bias) {
		parallel_nd(jcp.oc / simd_w, [&](int ofm1) {
			...
			const int blk_sz = ofm1 == jcp.oc / simd_w - 1 ? jcp.oc_without_padding - ofm1 * simd_w : simd_w;
