...
struct nchw_pooling_bwd_t : public cpu_primitive_t {
	struct pd_t : public cpu_pooling_bwd_pd_t {
		...
		status_t init() {
			...
			bool ok = true && set_default_params() == status::success && !is_fwd() && utils::one_of(desc()->alg_kind, alg_kind::pooling_max, alg_kind::pooling_avg_include_padding, alg_kind::pooling_avg_exclude_padding) && !has_zero_dim_memory() && utils::everyone_is(data_type, diff_dst_md()->data_type, diff_src_md()->data_type) && attr()->has_default_values() && memory_desc_matches_tag(*diff_dst_md(), desired_fmt_tag) && memory_desc_matches_tag(*diff_src_md(), desired_fmt_tag);
			if (!ok)
				return status::unimplemented;

			if (desc()->alg_kind == alg_kind::pooling_max) {
				bool ws_ok = true && hint_fwd_pd_ && hint_fwd_pd_->workspace_md();
				if (!ws_ok)
					return status::unimplemented;

				const auto &ws_blk =
						hint_fwd_pd_->workspace_md()->format_desc.blocking;
				ws_ok = ws_ok && ws_blk.inner_nblks < 1 && IMPLICATION(ws_blk.inner_nblks == 1, ws_blk.inner_idxs[0] == 1);
				if (!ws_ok)
					return status::unimplemented;
