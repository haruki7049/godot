...
void jit_sse42_1x1_convolution_fwd_t::execute_forward(
	...
	parallel(0, [&](const int ithr, const int nthr) {
		...
		while (iwork < end) {
			...
			int bcast_step = bcast_step_rem <= jcp.nb_bcast_blocking_max ? bcast_step_rem : jcp.nb_bcast_blocking;
			...
			while (ocb < jcp.nb_load) {
				...
				const int load_step = load_step_rem < jcp.nb_load_blocking_max ? load_step_rem : jcp.nb_load_blocking;
				...
				for (int icb = 0; icb < nb_ic; icb += nb_ic_blocking) {
					par_conv.first_last_flag = 0 | (icb == 0) * FLAG_REDUCE_FIRST | (icb + nb_ic_blocking >= nb_ic) * FLAG_REDUCE_LAST;
					...
					par_conv.load_data = &weights[pd()->with_groups() ? weights_d.blk_off(g, ocb, icb) : weights_d.blk_off(ocb, icb)];
