...
bool check_cond1(int dimN_reg_block, int dimK_block, int dimK_reg_block,
	...
	float lhs = (dimM_block * dimN_reg_block * dimM_simd_block + dimM_block * dimK_block * dimK_reg_block * dimM_simd_block + dimK_block * dimN_reg_block * dimK_reg_block) * (float)sizeof(float);
...
bool check_cond1_bis(int dimN_reg_block, int dimK_block, int dimK_reg_block,
		int dimM_block, int dimM_simd_block, float C) {
	float lhs = (dimM_block * dimK_block * dimK_reg_block * dimM_simd_block + dimK_block * dimN_reg_block * dimK_reg_block) * (float)sizeof(float);
	float rhs = C * L1_cache_size;
	return (lhs < rhs);
}
...
bool check_cond2(int nb_dimN_reg_block, int dimN_reg_block, int dimK_nb_block,
		...
		float C) {
	float lhs = (nb_dimN_reg_block * dimM_block * dimN_reg_block * dimM_simd_block + dimK_nb_block * dimM_block * dimK_block * dimK_reg_block * dimM_simd_block + nb_dimN_reg_block * dimK_nb_block * dimK_block * dimN_reg_block * dimK_reg_block) * (float)sizeof(float);
		...
		}
		{
			...
			}
			{
				...
				for (int dimK_reg_block = 0;
					...
					current = next;
					if (jcp.double_buffering) {
						next = (dimK_reg_block + inc_dimK_reg_block) % (2 * inc_dimK_reg_block);
			...
			auto store_output = [=](bool output_is_aligned) {
				for (int tile = 0; tile < jcp.dimN_reg_block; tile++) {
					Zmm zmm(jcp.zmm_start + tile);
					if (output_is_aligned && jcp.dimK_nb_block == 1 && (jcp.dimN * jcp.dimM * alpha * alpha * sizeof(float) > 2 * LLC_data_size))
...
status_t _jit_avx512_common_conv_winograd_data_kernel_f32::init_conf_common(
	...
	bool layout_consistency = true && jcp.ic <= src_d.padded_dims()[1] && jcp.oc <= dst_d.padded_dims()[1] && jcp.ic <= weights_d.padded_dims()[with_groups + 1] && jcp.oc <= weights_d.padded_dims()[with_groups + 0];
...
status_t set_wsched_DATA_W_S_G_D_avx512_common(jit_conv_winograd_conf_t &jcp) {
	auto test_cond_dimN_reg_block = [](jit_conv_winograd_conf_t &jcp,
		...
		return (dimN_reg_block >= MIN_REQUIRED_DIMN_REG_BLOCK) && (dimN_reg_block < jcp.nb_reg) && (dimN_reg_block < current_best);
	};
	...
	if (jcp.dimN_reg_block >= jcp.nb_reg) {
		auto test_cond_dimN_reg_block = [](jit_conv_winograd_conf_t &jcp,
												int dimN_reg_block, int current_best) {
			return (dimN_reg_block < jcp.nb_reg) && (dimN_reg_block > current_best);
		};
	...
	for (int j = 0; j < alpha; j++) {
		for (int i = 0; i < alpha; i++) {
			...
			for (int tb = 0; tb < jcp.dimK_4fma; tb += 4) {
				...
				vmovntps(zword[reg_transB + reg_transB_idx + sizeof(float) * tb * jcp.dimN_reg_block],
						Zmm(curr + 2));
				vmovntps(zword[reg_transB + reg_transB_idx + sizeof(float) * (tb + 1) * jcp.dimN_reg_block],
						Zmm(curr + 3));
				vmovntps(zword[reg_transB + reg_transB_idx + sizeof(float) * (tb + 2) * jcp.dimN_reg_block],
						Zmm(8));
				vmovntps(zword[reg_transB + reg_transB_idx + sizeof(float) * (tb + 3) * jcp.dimN_reg_block],
						Zmm(9));
				curr = next;
			}
		}
...
void jit_avx512_common_conv_winograd_bwd_weights_kernel_f32::gemm_loop_generate(
	...
	auto inner_loops = [=]() {
		...
		prefetcher_t<float> L1_pf(this, reg_srcB, L1,
				...
				jcp.dimK_reg_block * jcp.dimN_reg_block * jcp.dimK_4fma / inc_fma,
				fma_ipc);
		prefetcher_t<float> L2_pf(this, reg_srcB, L2,
				jcp.dimK_reg_block * jcp.dimN_reg_block * jcp.dimK_4fma,
				jcp.dimK_reg_block * jcp.dimN_reg_block * jcp.dimK_4fma / inc_fma,
		...
		{ /************* OC_block (M) loop ***********/
			...
			{ /*************** IC_block (N) loop *********/
				...
				{ /************* nb_tile_ur(K) loop ********/
					...
					for (int dimK_reg_block = 0;
							dimK_reg_block < jcp.dimK_reg_block;
							dimK_reg_block++) {
						int srcB_offset = dimK_reg_block * jcp.dimK_4fma * jcp.dimN_reg_block;
						for (int dimK_4fma = 0; dimK_4fma < jcp.dimK_4fma;
								dimK_4fma += inc_fma) {
							int current = next;
							if (jcp.double_buffering) {
								next = (dimK_reg_block * jcp.dimK_4fma + dimK_4fma + inc_fma) % (2 * inc_fma);
								load_A(next, dimK_reg_block * jcp.dimK_4fma + dimK_4fma + inc_fma);
							} else {
								next = 0;
								load_A(next, dimK_reg_block * jcp.dimK_4fma + dimK_4fma);
							}
							for (int dimN_reg_block = 0;
									dimN_reg_block < jcp.dimN_reg_block;
									++dimN_reg_block) {
								L1_pf.prefetch(srcB_offset / inc_fma + dimK_4fma / inc_fma * jcp.dimN_reg_block + dimN_reg_block);
								L2_pf.prefetch(srcB_offset / inc_fma + dimK_4fma / inc_fma * jcp.dimN_reg_block + dimN_reg_block);
								if (jcp.ver == ver_4fma) {
									int srcB_trans_offset = (dimK_4fma / 4) * 64 + dimK_4fma % 4;
									v4fmaddps(
											Zmm(jcp.zmm_start + dimN_reg_block),
											Zmm(current),
											EVEX_compress_addr(reg_srcB,
													sizeof(float) * (srcB_offset +
																			srcB_trans_offset +
																			(dimN_reg_block % 4) * 16 +
																			(dimN_reg_block / 4) * 4)));
								} else {
				...
				add(reg_srcA, jcp.dimK_reg_block * jcp.dimK_4fma * jcp.dimM_simd_block * sizeof(float));
				add(reg_srcB, jcp.dimK_reg_block * jcp.dimN_reg_block * jcp.dimK_4fma * sizeof(float));
				if (jcp.dimK_block > 1) {
					sub(reg_dimK_block_loop_cnt, 1);
					jnz(dimK_block_loop);
				}
				...
				add(reg_dstC, jcp.dimN_reg_block * jcp.dimM_simd_block * sizeof(float));
				if (jcp.dimN_block > 1) {
			...
			if (jcp.dimM_block > 1) {
				sub(reg_srcB, jcp.dimN_block * jcp.dimK_block * jcp.dimK_reg_block * jcp.dimN_reg_block * jcp.dimK_4fma * sizeof(float));
				add(reg_srcA, jcp.dimK_block * jcp.dimK_reg_block * jcp.dimK_4fma * jcp.dimM_simd_block * sizeof(float));
				sub(reg_dimM_block_loop_cnt, 1);
				jnz(dimM_block_loop);
			}
		}
...
bool check_cond1bis_wu(int dimM_block, int dimM_simdw, int dimK_block,
		int dimK_reg_block, int dimK_4fma, int dimN_reg_block, float C) {
	float lhs = 1.0f * dimM_block * dimK_block * dimK_reg_block * dimK_4fma * dimM_simdw;
	lhs += dimK_block * dimN_reg_block * dimK_reg_block * dimK_4fma;
	lhs *= sizeof(float);
	float rhs = C * L1_cache_size;
	return (lhs <= rhs);
...
bool check_cond2bis_wu(int dimM_block, int dimM_simdw, int dimK_block,
		...
		float C) {
	float lhs = 1.0f * dimM_block * dimM_simdw * dimK_block * dimK_reg_block * dimK_4fma;
	lhs += dimK_block * dimK_reg_block * dimK_4fma * dimN_block * dimN_reg_block;
	lhs *= sizeof(float);
	float rhs = C * L2_cache_size;
	return (lhs <= rhs);
}
...
status_t set_wsched_WEI_S_D_G_W_avx512_common(jit_conv_winograd_conf_t &jcp) {
	...
	auto test_cond1_dimM_block = [](
		...
		return check_cond1_wu(dimM_block, jcp.dimM_simd_block, 1,
					   ...
					   1.0f) &&
			   (dimM_block > current_best) && (jcp.dimM / jcp.dimM_simd_block / dimM_block) >= 2;
	};
...
#define MAX_4FMA_UR 8
	if (jcp.ver == ver_4fma) {
		auto test_cond_4fma = [](jit_conv_winograd_conf_t &jcp, int dimK_4fma,
									  int current_best) {
			return (dimK_4fma % 4 == 0) && (dimK_4fma <= MAX_4FMA_UR) && (dimK_4fma > current_best);
		};
		...
		if ((jcp.itiles * jcp.jtiles) % jcp.dimK_4fma != 0)
			jcp.tile_4fma_padding = jcp.dimK_4fma - ((jcp.itiles * jcp.jtiles) % jcp.dimK_4fma);
	}
