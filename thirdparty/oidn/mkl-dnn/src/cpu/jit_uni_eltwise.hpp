...
struct jit_uni_eltwise_injector_f32 {
	...
	jit_uni_eltwise_injector_f32(jit_generator *host, alg_kind_t alg,
			...
			Xbyak::Opmask k_mask = Xbyak::Opmask(1)) : alg_(alg),
													   alpha_(alpha),
													   beta_(beta),
													   h(host),
	...
	jit_uni_eltwise_injector_f32(jit_generator *host,
			...
			Xbyak::Opmask k_mask = Xbyak::Opmask(1)) : jit_uni_eltwise_injector_f32(host, eltwise.alg, eltwise.alpha,
...
private:
	...
	Xbyak::Address table_val(int index) { return h->ptr[p_table + index * vlen]; }
