...
inline void init_scratchpad(memory_tracking::registrar_t &scratchpad,
	...
	size_t V_sz = (size_t)alpha * alpha * jcp.mb * jcp.ic * jcp.itiles * jcp.jtiles;
	size_t M_sz = (size_t)alpha * alpha * jcp.mb * jcp.oc * jcp.itiles * jcp.jtiles;

	switch (jcp.sched_policy) {
		case WSCHED_DATA_W_SGD:
			V_sz = (size_t)jcp.nthr * alpha * alpha * jcp.nb_tile_block_ur * jcp.tile_block_ur * jcp.ic;
			M_sz = (size_t)jcp.nthr * alpha * alpha * jcp.nb_tile_block_ur * jcp.tile_block_ur * jcp.oc;
			break;
		case WSCHED_WEI_SDGtWo:
			U_sz = (size_t)jcp.nthr * (alpha * alpha * jcp.oc * (jcp.ic / jcp.nb_ic) + jcp.ic * jcp.oc * jcp.kh * jcp.kw);
			M_sz = (size_t)jcp.nthr * alpha * alpha * (jcp.ntiles / jcp.tile_block) * (jcp.oc / jcp.nb_oc);
			V_sz = (size_t)jcp.nthr * alpha * alpha * (jcp.ntiles / jcp.tile_block) * (jcp.ic / jcp.nb_ic);
			break;
...
struct _jit_avx512_core_fp32_wino_conv_4x3_t {
	_jit_avx512_core_fp32_wino_conv_4x3_t(
			const jit_conv_winograd_conf_t &jcp, const primitive_attr_t *attr) : kernel_(nullptr),
																				 attr_(attr) {
...
struct jit_avx512_core_fp32_wino_conv_4x3_fwd_t
	...
	struct pd_t : public cpu_convolution_fwd_pd_t {
		pd_t(engine_t *engine, const convolution_desc_t *adesc,
				...
				const typename pd_t::base_class *hint_fwd_pd) : cpu_convolution_fwd_pd_t(engine, adesc, attr, hint_fwd_pd),
																jcp_() {}
		...
		status_t init() {
			bool ok = true && is_fwd() && utils::one_of(desc()->alg_kind, alg_kind::convolution_auto, alg_kind::convolution_winograd) && expect_data_types(data_type::f32, data_type::f32, data_type::f32, data_type::f32, data_type::f32) && set_default_formats();
			if (!ok)
	...
	protected:
		bool set_default_formats() {
			...
			auto wei_fmt = desc()->prop_kind == prop_kind::forward_training ? (with_groups() ? gOIhw16i16o : OIhw16i16o) : any;
			return set_default_formats_common(nChw16c, wei_fmt, nChw16c);
		}
	};

	jit_avx512_core_fp32_wino_conv_4x3_fwd_t(const pd_t *apd) : _jit_avx512_core_fp32_wino_conv_4x3_t<true>(apd->jcp_, apd->attr()), cpu_primitive_t(apd, true) {}
...
struct jit_avx512_core_fp32_wino_conv_4x3_bwd_data_t
	...
	struct pd_t : public cpu_convolution_bwd_data_pd_t {
		pd_t(engine_t *engine, const convolution_desc_t *adesc,
				const primitive_attr_t *attr,
				const convolution_fwd_pd_t *hint_fwd_pd) : cpu_convolution_bwd_data_pd_t(engine, adesc, attr, hint_fwd_pd),
														   jcp_() {}
		...
		status_t init() {
			bool ok = true && mkldnn_thr_syncable() && desc()->prop_kind == prop_kind::backward_data && utils::one_of(desc()->alg_kind, alg_kind::convolution_auto, alg_kind::convolution_winograd) && expect_data_types(data_type::f32, data_type::f32, data_type::undef, data_type::f32, data_type::f32) && set_default_formats();
			if (!ok)
				return status::unimplemented;

			status_t status = jit_avx512_core_fp32_wino_conv_4x3_bwd_data_kernel ::init_conf(jcp_, *desc(), *diff_src_md(), *weights_md(),
					*diff_dst_md());
			if (status != status::success)
				return status;
			set_default_alg_kind(alg_kind::convolution_winograd);
	...
	jit_avx512_core_fp32_wino_conv_4x3_bwd_data_t(const pd_t *apd) : _jit_avx512_core_fp32_wino_conv_4x3_t<false>(apd->jcp_, apd->attr()), cpu_primitive_t(apd, true) {}
...
struct jit_avx512_core_fp32_wino_conv_4x3_bwd_weights_t
	...
	struct pd_t : public cpu_convolution_bwd_weights_pd_t {
		pd_t(engine_t *engine, const convolution_desc_t *adesc,
				const primitive_attr_t *attr,
				const convolution_fwd_pd_t *hint_fwd_pd) : cpu_convolution_bwd_weights_pd_t(engine, adesc, attr, hint_fwd_pd),
														   jcp_() {}
		...
		status_t init() {
			bool ok = true && mkldnn_thr_syncable() && desc()->prop_kind == prop_kind::backward_weights && utils::one_of(desc()->alg_kind, alg_kind::convolution_auto, alg_kind::convolution_winograd) && expect_data_types(data_type::f32, data_type::f32, data_type::f32, data_type::f32, data_type::f32) && set_default_formats();
			if (!ok)
				return status::unimplemented;
	...
	jit_avx512_core_fp32_wino_conv_4x3_bwd_weights_t(const pd_t *apd) : cpu_primitive_t(apd, true), kernel_(nullptr) {
