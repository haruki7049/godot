		...
		for (int ifm2 = 0; ifm2 < ic_blk; ifm2++) {
			for (int jj = jj_start; jj < jj_end; jj++) {
				...
				if (one_of(jcp.src_tag, ncw, nchw, ncdhw))
					inp_off = sizeof(float) * ((size_t)ifm2 * id * ih * iw + (ki * dilate_w + jj * stride_w - pad_l));
				else
					inp_off = sizeof(float) * ((ki * dilate_w + jj * stride_w - pad_l) * ic_blk + ifm2);
				vbroadcastss(Ymm(oc_blocks * ur_w + jj),
						make_safe_addr(aux_reg_input, inp_off, reg_long_offt));
			}

			for (int ii = 0; ii < oc_blocks; ii++) {
				int ker_off = ii * nb_ic * kd * kh * kw * ic_blk * oc_blk + ki * ic_blk * oc_blk + ifm2 * oc_blk;
	...
	L(kw_loop);
	{
		...
		for (int ifm2 = 0; ifm2 < ic_blk; ifm2++) {
			for (int jj = jj_start; jj < jj_end; jj++) {
				size_t inp_off;
				if (one_of(jcp.src_tag, ncw, nchw, ncdhw))
					inp_off = sizeof(float) * ((size_t)ifm2 * id * ih * iw + (jj * stride_w - pad_l));
				else
					inp_off = sizeof(float) * ((jj * stride_w - pad_l) * ic_blk + ifm2);
				vbroadcastss(Ymm(oc_blocks * ur_w + jj),
						make_safe_addr(aux_reg_input, inp_off, reg_long_offt));
			}
			for (int ii = 0; ii < oc_blocks; ii++) {
		...
		add(aux_reg_input, sizeof(float) * (one_of(jcp.src_tag, ncw, nchw, ncdhw) ? dilate_w : ic_blk * dilate_w));
...
void jit_avx2_conv_fwd_kernel_f32::width_blk_step(int ur_w,
	...
	const int inp_off = one_of(jcp.src_tag, ncw, nchw, ncdhw) ? dilate_w : ic_blk * dilate_w;
	...
	if (jcp.ndims == 5) {
		...
		if ((jcp.dilate_d >= jcp.id) || (jcp.kd - 1) * (jcp.dilate_d + 1) < jcp.f_pad) {
			cmp(reg_ki, 0);
	...
	if (jcp.ndims == 5) {
		...
		add(aux_reg_ker_d, sizeof(float) * jcp.kw * jcp.kh * jcp.oc_block * jcp.ic_block);
	...
	for (int ii = 0; ii < oc_blocks; ii++) {
		for (int jj = 0; jj < ur_w; jj++) {
			const size_t o_off = sizeof(float) * ((size_t)ii * od * oh * ow + jj) * oc_blk;
...
inline void jit_avx2_conv_fwd_kernel_f32::solve_common(
	...
	int l_pad = jcp.l_pad;
	int r_pad = nstl::max(0, (int(jcp.ow) - 1) * str_w + (kw - 1) * dilate_w - (iw + l_pad - 1));
...
status_t jit_avx2_conv_fwd_kernel_f32::init_conf(jit_conv_conf_t &jcp,
	...
	bool args_ok = true && IMPLICATION(flat, true && one_of(jcp.src_tag, ncw, nwc, nchw, nhwc, ncdhw, ndhwc) && one_of(jcp.wei_tag, Owi8o, gOwi8o, Ohwi8o, gOhwi8o, Odhwi8o, gOdhwi8o)) && IMPLICATION(mimo, true && one_of(jcp.src_tag, nCw8c, nChw8c, nCdhw8c) && one_of(jcp.wei_tag, OIw8i8o, gOIw8i8o, OIhw8i8o, gOIhw8i8o, OIdhw8i8o, gOIdhw8i8o)) && one_of(jcp.dst_tag, nCw8c, nChw8c, nCdhw8c);
	if (!args_ok)
		return status::unimplemented;
	...
	args_ok = true && jcp.oc % simd_w == 0 && jcp.l_pad <= jcp.ur_w && IMPLICATION(jcp.kw > 7, (jcp.t_pad == 0 && jcp.l_pad == 0) || (jcp.stride_w == 1 && jcp.stride_h == 1)) && IMPLICATION(mimo, jcp.ic % simd_w == 0);
	if (!args_ok)
		return status::unimplemented;
	...
	if (r_pad_no_tail > jcp.ur_w * jcp.stride_w && jcp.ow / jcp.ur_w > 1) {
		...
		jcp.ur_w_tail = jcp.ow % jcp.ur_w;
		/* check again ... */
		r_pad_no_tail = nstl::max(0, (jcp.ow - jcp.ur_w_tail - 1) * jcp.stride_w + (jcp.kw - 1) * (jcp.dilate_w + 1) - (jcp.iw + jcp.l_pad - 1));
...
void jit_avx2_conv_bwd_data_kernel_f32::compute_loop(int ur_w, int l_overflow,
	...
	L(kh_loop);
	{
		for (int ki = 0; ki < kw; ki++) {
			...
			for (int ofm2 = 0; ofm2 < jcp.oc_block; ofm2++) {
				for (int jj = jj_start; jj < jj_end; jj += stride_w) {
					int aux_output_offset = (jj + jcp.l_pad - ki) / stride_w * jcp.oc_block + ofm2;
					vbroadcastss(Ymm(nb_ic_block * ur_w + jj / stride_w),
							ptr[aux_reg_ddst + sizeof(float) * aux_output_offset]);
				}
		...
		add(aux_reg_kernel, sizeof(float) * stride_h * kw * oc_block * ic_block);
	...
	if (one_of(jcp.ndims, 3, 4)) {
		int ddst_oc_shift = sizeof(float) * jcp.od * jcp.oh * jcp.ow * jcp.oc_block;
		int kernel_oc_shift = sizeof(float) * jcp.kd * jcp.kh * jcp.kw * jcp.ic * jcp.oc_block;
...
void jit_avx2_conv_bwd_data_kernel_f32::generate() {
	...
	int r_overflow = nstl::max(0, (jcp.kw - 1 - nstl::max(0, jcp.r_pad)) / jcp.stride_w);
	int r_overflow1 = nstl::max(0, (jcp.kw - 1 - nstl::max(0, jcp.r_pad) - jcp.ur_w_tail) / jcp.stride_w);
...
status_t jit_avx2_conv_bwd_data_kernel_f32::init_conf(jit_conv_conf_t &jcp,
	...
	bool args_ok = true && one_of(jcp.src_tag, nCw8c, nChw8c, nCdhw8c) && one_of(jcp.wei_tag, gOIw8o8i, OIw8i8o, gOIhw8o8i, OIhw8o8i, gOIdhw8o8i, OIdhw8o8i) && one_of(jcp.dst_tag, nCw8c, nChw8c, nCdhw8c) && jcp.stride_w == jcp.stride_h && jcp.stride_d == 1 && jcp.dilate_d == 0 && jcp.dilate_h == 0 && jcp.dilate_w == 0 && jcp.ic % simd_w == 0 && jcp.oc % simd_w == 0 && jcp.od == (jcp.idp - jcp.kd) / jcp.stride_d + 1 && jcp.oh == (jcp.ihp - jcp.kh) / jcp.stride_h + 1 && jcp.ow == (jcp.iwp - jcp.kw) / jcp.stride_w + 1;
	if (!args_ok)
		return status::unimplemented;
	...
	for (int b = 1; b <= 4; b++) {
		...
		for (int u = jcp.stride_w;
			...
			if (nfmas > best_nfmas || (nfmas == best_nfmas && jcp.ur_w < ur_w)) {
				jcp.ur_w = ur_w;
	...
	int r_overflow_no_tail = nstl::max(0, (jcp.kw - 1 - jcp.ur_w_tail - nstl::max(0, jcp.r_pad) - jcp.ur_w_tail) / jcp.stride_w);
...
status_t jit_avx2_conv_bwd_weights_kernel_f32::init_conf(jit_conv_conf_t &jcp,
	...
	int back_pad = nstl::max(0, (jcp.od - 1) * jcp.stride_d + jcp.kd - jcp.id - jcp.f_pad);
	if (ndims == 5)
	...
	const bool boundaries_ok = true && jcp.t_pad < max_h_pad && jcp.b_pad < max_h_pad && jcp.l_pad < max_w_pad && jcp.r_pad < max_w_pad;
	if (!boundaries_ok)
		return status::unimplemented;
	...
	bool args_ok = true && IMPLICATION(flat, true && one_of(jcp.src_tag, ncw, nwc, nchw, nhwc, ncdhw, ndhwc) && one_of(jcp.wei_tag, Owi8o, gOwi8o, Ohwi8o, gOhwi8o, Odhwi8o, gOdhwi8o)) && IMPLICATION(mimo, true && one_of(jcp.src_tag, nCw8c, nChw8c, nCdhw8c) && one_of(jcp.wei_tag, OIw8i8o, gOIw8i8o, OIhw8i8o, gOIhw8i8o, OIdhw8i8o, gOIdhw8i8o)) && one_of(jcp.dst_tag, nCw8c, nChw8c, nCdhw8c) && IMPLICATION(mimo, jcp.ic % simd_w == 0) && jcp.oc % simd_w == 0 && jcp.kw < 14 && jcp.kh <= jcp.t_pad + jcp.ih /* [bwd_w:r1] */
				   && jcp.kh <= jcp.ih /* [bwd_w:r2] */
				   && jcp.kd <= jcp.f_pad + jcp.id && jcp.kd <= jcp.id && jcp.t_pad < jcp.kh /* XXX: must fix the kernel! */
...
inline void jit_avx2_conv_bwd_weights_kernel_f32::od_step_comeback_pointers() {
	...
	L(kd_comeback_loop);
	{
		const int inp_mult = one_of(jcp.src_tag, ncw, nchw, ncdhw) ? 1 : jcp.ic_block;
		sub(aux_reg_input, sizeof(float) * jcp.iw * jcp.ih * inp_mult);
		sub(aux_reg_kernel, sizeof(float) * jcp.kw * jcp.kh * jcp.ic_block * jcp.oc_block);
		dec(kj);
		cmp(kj, 0);
...
inline void jit_avx2_conv_bwd_weights_kernel_f32::compute_ic_block_step(
	...
	for (int i_kw = 0; i_kw < kw; i_kw++)
		for (int i_ic = 0; i_ic < ic_block_step; i_ic++) {
			size_t off = sizeof(float) * (i_kw * ic_block + i_ic) * jcp.oc_block + kernel_offset;
			vmovups(Ymm(i_kw * ic_block_step + i_ic), yword[reg_kernel + off]);
		}

	for (int i_ur = 0; i_ur < ur_w; i_ur++) {
		vmovups(Ymm(kw * ic_block_step + 0),
				yword[reg_output + sizeof(float) * i_ur * oc_block + output_offset]);

		for (int i_kw = 0; i_kw < kw; i_kw++) {
			int i_iw = i_ur * jcp.stride_w + i_kw;
			if (i_iw - pad_l < 0 || i_iw > (ur_w - 1) * jcp.stride_w + kw - 1 - pad_r)
				continue;
			for (int i_ic = 0; i_ic < ic_block_step; i_ic++) {
				size_t i_off = (size_t)input_offset + sizeof(float) * (one_of(jcp.src_tag, ncw, nchw, ncdhw) ? (i_iw - pad_l) + i_ic * ((size_t)jcp.id * jcp.ih * jcp.iw) : (i_iw - pad_l) * ic_block + i_ic);
...
inline void jit_avx2_conv_bwd_weights_kernel_f32::compute_oh_step_disp() {
	...
	if (one_of(jcp.src_tag, ncw, nchw, ncdhw)) {
		...
		ic_block_step = jcp.kw > 7 ? 1 : jcp.kw > 3 ? 2 :
								 jcp.kw > 1			? 4 :
													  8;
	}
...
inline void jit_avx2_conv_bwd_weights_kernel_f32::compute_oh_step_unroll_ow(
	...
	L(kh_loop);
	{
		...
		L(ic_block_loop);
		{
			compute_ic_block_step(jcp.ow, jcp.l_pad, r_pad, ic_block_step, 0,
					0, 0);
			size_t inp_icblk_stride = sizeof(float) * ic_block_step * (one_of(jcp.src_tag, ncw, nchw, ncdhw) ? jcp.id * jcp.ih * jcp.iw : 1);
	...
	if (jcp.ndims == 5) {
		...
		add(aux_reg_kernel, sizeof(float) * jcp.kh * jcp.kw * ic_block * oc_block);
		dec(ki);
		cmp(ki, 0);
		jg(kd_loop, T_NEAR);
	}
	...
	L(kh_loop);
	{
		...
		L(ic_block_loop);
		{
			if (jcp.l_pad != 0) {
				...
				add(reg_input, sizeof(float) * (ur_w * stride_w - jcp.l_pad) * inp_mult);
