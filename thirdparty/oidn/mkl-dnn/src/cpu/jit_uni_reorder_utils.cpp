...
status_t prb_init(prb_t &p, const memory_desc_t &imd, const memory_desc_t &omd,
	...
	bool ok = true && im_d.is_blocking_desc() && om_d.is_blocking_desc() && !im_d.has_zero_dim() && !om_d.has_zero_dim();
	if (!ok)
	...
	for (int d = 0; d < im_d.ndims(); ++d) {
		...
		bool ok = true && pdim == om_d.padded_dims()[d] && pdim % iblocks[d] == 0 && pdim % oblocks[d] == 0;
		if (!ok)
			return unimplemented;
	}
	...
	p.scale_type = attr->output_scales_.has_default_values() ? scale_type_t::NONE : (attr->output_scales_.mask_ == 0 ? scale_type_t::COMMON : scale_type_t::MANY);

	ptrdiff_t ss[max_ndims] = { 0 };
	if (p.scale_type == scale_type_t::MANY) {
		ptrdiff_t last_ss = 1;
		for (int d = old.ndims - 1; d >= 0; --d) {
			assert((d == 0 || old.id[d - 1] <= old.id[d]) && "logical dimensions should be in ascending order");
...
void prb_normalize(prb_t &p) {
	for (int d = 0; d < p.ndims; ++d) {
		...
		for (int j = d + 1; j < p.ndims; ++j) {
			bool new_min = false || p.nodes[j].os < p.nodes[min_pos].os || (true && p.nodes[j].os == p.nodes[min_pos].os && p.nodes[j].n < p.nodes[min_pos].n);
			if (new_min)
				min_pos = j;
		}
		if (min_pos != d)
...
#pragma GCC diagnostic ignored "-Warray-bounds"
	...
	for (int d = 0; d < p.ndims - 1; ++d) {
		...
		const bool fold = false || next_node.n == (size_t)1 // trivial case, just drop next node
						  || (true // or real folding if possible
									 && next_node.is == (ptrdiff_t)this_node.n * this_node.is && next_node.os == (ptrdiff_t)this_node.n * this_node.os && next_node.ss == (ptrdiff_t)this_node.n * this_node.ss);
		if (fold) {
