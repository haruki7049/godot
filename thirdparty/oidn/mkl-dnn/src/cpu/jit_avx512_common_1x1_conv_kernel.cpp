		...
		L(bcast_loop);
		{
			...
			for (int i = 0; i < num_substeps; i++) {
				...
				if (i < num_substeps - 1) {
					...
					add(aux1_reg_bcast_data, jcp.bcast_loop_bcast_step - (num_substeps - 1) * jcp.bcast_loop_bcast_substep);
					add(aux_reg_output_data, jcp.bcast_loop_output_step - (num_substeps - 1) * jcp.bcast_loop_output_substep);
				}
			}
...
void jit_avx512_common_1x1_conv_kernel::reduce_loop(int load_loop_blk,
	...
	auto vreg_load = [=](int i_load, int i_fma) {
		return Zmm(utils::rnd_up(ur * load_loop_blk, jcp.fma_step) + jcp.fma_step * i_load + i_fma);
	};
	...
	auto bcast_ptr = [=](int i_reduce, int i_ur, bool bcast) {
		...
		if (one_of(jcp.prop_kind, forward_training, forward_inference,
			...
			offt = (i_reduce == jcp.reduce_loop_unroll) ? (jcp.bcast_dim + i_ur) * jcp.reduce_loop_unroll : i_ur * jcp.reduce_loop_unroll + i_reduce;
		} else {
	...
	auto output_ptr = [=](int i_load, int i_ur) {
		if (one_of(jcp.prop_kind, forward_training, forward_inference,
			...
			return EVEX_compress_addr(aux_reg_output_data,
					(i_load * jcp.bcast_dim + i_ur) * jcp.load_block * jcp.typesize_out);
		else
			return ptr[aux_reg_output_data +
					   (i_load ? reg_output_stride * i_load : 0) // TODO: Xbyak should allow 0 scale
	...
	auto init = [=]() {
		...
		if (jcp.with_bias && one_of(jcp.prop_kind, forward_training, forward_inference)) {
	...
	auto prefetch_callback = [=](int ur, int i_reduce, int i_ur, int i_load,
		...
		int other_pf_trigger = n_other_pf ? nstl::max(1, n_other_pf_ops / n_other_pf) : 0;
		...
		if (i_op < pf_inp_ops && i_op % pf_inp_trigger == 0) {
			...
			auto pf_reg = wraparound && last_block ? reg_bcast_data : (last_block ? aux1_reg_bcast_data : aux_reg_bcast_data);
			int offt = i_pf;
			if (jcp.prop_kind == backward_weights) {
				offt += wraparound && last_block ? 0 : (last_block ? jcp.is : jcp.reduce_block);
				offt *= jcp.bcast_block;
			} else {
				offt += wraparound && last_block ? 0 : (last_block ? jcp.ur : jcp.bcast_dim);
			...
			if (i_op % other_pf_trigger == 0) {
				...
				if (i_pf < n_pf_ker_l2) {
					...
					mic_prefetcht1(ptr[aux_reg_load_data + offt * jcp.typesize_in]);
				} else if (i_pf < n_pf_ker_l2 + n_pf_ker_l1) {
					i_pf -= n_pf_ker_l2;
					auto pf_reg = last_block ? reg_load_data : aux_reg_load_data;
					int offt = (i_pf + i_load * jcp.reduce_dim + (last_block ? (wraparound ? jcp.reduce_dim : 0) : jcp.reduce_block)) * jcp.load_block;
					mic_prefetcht0(ptr[pf_reg + offt * jcp.typesize_in]);
				} else if (i_pf < n_pf_ker_l1 + n_pf_ker_l2 + n_pf_out_l1) {
					i_pf -= n_pf_ker_l1 + n_pf_ker_l2;
					int offt = i_pf * jcp.load_block;
					mic_prefetcht0(ptr[aux_reg_output_data + offt * jcp.typesize_out]);
				}
			}
		}
	};
	...
	auto fma_block = [=](bool last_block) {
		...
		for (int i_reduce = 0; i_reduce < jcp.reduce_loop_unroll;
			...
			for (int i_load = 0; i_load < load_loop_blk; ++i_load) {
				...
				// and clear remaining
				if (jcp.transpose_src && jcp.is % jcp.fma_step && last_block && i_reduce == jcp.reduce_loop_unroll - reduce_step) {
					Label load_all;
					Label load_finish;
			...
			for (int i_ur = 0; i_ur < ur; ++i_ur) {
				if (jcp.ver == ver_avx512_core && jcp.expl_bcast && load_loop_blk > 1)
				...
				for (int i_load = 0; i_load < load_loop_blk; ++i_load) {
					...
					else if (jcp.ver == ver_avx512_core && jcp.expl_bcast && load_loop_blk > 1)
...
void jit_avx512_common_1x1_conv_kernel::generate() {
	...
	const int num_ur_cases =
			(jcp.ver == ver_4fma ? size_ur_cases_4fma : size_ur_cases_fma) / sizeof(*ur_cases);
...
status_t jit_avx512_common_1x1_conv_kernel::init_conf(jit_1x1_conv_conf_t &jcp,
	...
	jcp.with_bias = pick_by_prop_kind(jcp.prop_kind, cd.bias_desc.format_kind,
							format_kind::undef, cd.diff_bias_desc.format_kind) != format_kind::undef;
	...
	bool args_ok = true && jcp.ngroups == 1 && jcp.src_tag == dat_tag && jcp.dst_tag == dat_tag;
	if (!args_ok)
		return status::unimplemented;

	args_ok = true && jcp.oc % simd_w == 0 && jcp.ic % simd_w == 0 && jcp.t_pad == 0 && jcp.l_pad == 0 && jcp.stride_w == 1 && jcp.stride_h == 1 // TODO: support some strides
			  && jcp.kh == 1 && jcp.kw == 1;
	if (!args_ok)
		return status::unimplemented;
	...
	if (everyone_is(data_type::f32, src_d.data_type(),
		...
		format_tag_t wei_tag = with_groups ? pick(2 * ndims - 6 + is_bwd_d, gOIw16i16o, gIOw16o16i,
		...
		if (jcp.prop_kind != backward_weights && mayiuse(avx512_mic_4ops) &&
				((jcp.prop_kind == backward_data) ? jcp.oc_block : jcp.ic_block) % 4 == 0) {
			jcp.ver = ver_4fma;
			jcp.fma_step = 4;
		} else if (jcp.prop_kind == backward_weights && mayiuse(avx512_mic_4ops) && !reduce_src
				   /* Heuristic condition for relation of src size to oc. Otherwise
					  the src transposition overhead exceed the benefit from 4fma
				   */
				   && ((jcp.is * jcp.ic) / jcp.oc <= 2048) && mkldnn_thr_syncable()) {
	...
	args_ok = true && jcp.ic <= src_d.padded_dims()[1] && jcp.oc <= dst_d.padded_dims()[1] && jcp.ic <= weights_d.padded_dims()[with_groups + 1] && jcp.oc <= weights_d.padded_dims()[with_groups + 0];
	if (!args_ok)
		return status::unimplemented;
	...
	if (one_of(jcp.prop_kind, forward_training, forward_inference,
		...
		const int spatial = (one_of(jcp.prop_kind, forward_training, forward_inference)) ?
									jcp.oh :
									jcp.ih;
		if (jcp.ver == ver_avx512_core && (8 * jcp.mb) / nthreads >= 1) {
			...
			if (jcp.load_dim > 128 && jcp.load_dim < BIG_LOAD_DIM && spatial > SMALL_SPATIAL && spatial < BIG_SPATIAL) {
				max_regs = 6;
				min_regs = 5;
			}
		} else {
		...
		for (int ur_w = max_regs; ur_w >= min_regs; ur_w -= ur_step) {
			if ((spatial >= size_treshold && spatial % ur_w == 0) || (spatial < size_treshold && jcp.os % ur_w == 0)) {
				jcp.ur = ur_w;
				break;
			}
		}
		...
		if (jcp.ver == ver_avx512_core && jcp.expl_bcast) {
			if (jcp.load_dim <= BIG_LOAD_DIM && spatial > SMALL_SPATIAL && spatial < BIG_SPATIAL)
			...
			if ((jcp.mb > 28 && spatial >= 28) || (jcp.mb > 112 && spatial >= 17))
				jcp.use_vmovntps = true;
			else
		...
		if (jcp.ver == ver_avx512_core && nthreads <= 28 && jcp.mb < nthreads && nb_load * nb_bcast > nthreads) {
			// Some heuristic here
			...
			auto calc_job_cost = [&](int lb, int tg, float mem_k) {
				...
				float calc_size = (float)(bb_size * jcp.ur) * (lb * jcp.load_block) * jcp.reduce_dim;
				float mem_size = (float)(bb_size * jcp.ur + lb * jcp.load_block) * jcp.reduce_dim;
		...
		if (jcp.ver == ver_avx512_core && jcp.expl_bcast && jcp.bcast_dim <= 64 && load_size >= L2_size) {
			jcp.load_grp_count = nstl::max(jcp.load_grp_count, 4);
		} else if (jcp.bcast_dim <= 49 && jcp.mb <= nthreads && jcp.load_dim > 512 && jcp.load_dim / jcp.reduce_dim >= 4) {
			jcp.load_grp_count = nstl::max(jcp.load_grp_count, 2);
			load_blocking = jcp.load_block;
		}

		if (jcp.ver == ver_4fma && jcp.bcast_dim * jcp.mb < jcp.load_dim && jcp.oh * jcp.ow > 64 && IMPLICATION(reduce_src, jcp.load_dim < 1024)) {
			...
			for (int load_chunk = 1; load_chunk <= nb_load; load_chunk++) {
				...
				int bcast_per_thr = div_up(jcp.mb * nb_bcast, thr_per_grp) * jcp.bcast_block;
				int load_per_thr = load_chunk * simd_w;
				float data_norm = (bcast_per_thr + load_per_thr) / 2.f;
				float data_eff = (bcast_per_thr * load_per_thr) / (data_norm * data_norm);
				float thr_eff_over_grp = (float)nstl::max(1, nthreads / lgc) / div_up(nthreads, lgc);
				float thr_eff_in_grp = ((float)jcp.mb * nb_bcast) / rnd_up(jcp.mb * nb_bcast, thr_per_grp);
		...
		int space_for_bcast = (L2_capacity - /* kernel_size - */
							   2 * jcp.load_block * reduce_blocking - jcp.ur * reduce_blocking - 3 * 1024);
		if (jcp.reduce_dim * jcp.bcast_dim > L2_capacity)
			space_for_bcast /= 2;

		int bcast_in_cache = nstl::max(jcp.bcast_block, space_for_bcast / reduce_blocking);
		...
		jcp.reduce_loop_unroll = jcp.reduce_block;
		jcp.reduce_loop_bcast_step = jcp.reduce_loop_unroll * jcp.ic_block * jcp.typesize_in;
		jcp.reduce_loop_load_step = jcp.reduce_loop_unroll * jcp.oc_block * jcp.typesize_in;
		...
		if (jcp.ver == ver_avx512_core) {
			int max_reduce_blocking = nstl::min(L1_capacity / jcp.ur, jcp.reduce_dim);
			...
			reduce_blocking = nstl::max(rnd_dn(reduce_blocking, jcp.reduce_block),
					jcp.reduce_block);
		} else {
			int max_reduce_blocking = L2_capacity / ((bcast_blocking + load_blocking) * jcp.reduce_block);
			max_reduce_blocking = nstl::min(max_reduce_blocking,
					(L1_capacity / (jcp.bcast_block)) / jcp.reduce_block);

			int num_jobs = div_up(jcp.load_dim, load_blocking) * div_up(jcp.bcast_dim, bcast_blocking);
...
void jit_avx512_common_1x1_conv_kernel::init_scratchpad(
	...
	if (jcp.prop_kind != backward_data && jcp.with_bias && jcp.oc != jcp.oc_without_padding)
...
void jit_avx512_common_1x1_conv_kernel::balance(jit_1x1_conv_conf_t &jcp,
	...
	auto calc_mem_cost = [=](int nthr_mb, int nthr_oc_b, int nthr_ic_b) {
		...
		return 0 + (size_t)bcast_koeff * div_up(jcp.mb * nb_reduce, nthr_mb) * div_up(jcp.ngroups, jcp.nthr_g) * div_up(nb_bcast, nthr_ic_b) * jcp.ic_block * jcp.reduce_block / jcp.stride_h / jcp.stride_w /* (n1) */
			   + (size_t)load_koeff * div_up(jcp.mb * nb_reduce, nthr_mb) * div_up(jcp.ngroups, jcp.nthr_g) * div_up(nb_load, nthr_oc_b) * jcp.oc_block * jcp.reduce_block + (size_t)output_koeff /* (n2) */
																																													 * div_up(jcp.ngroups, jcp.nthr_g) * div_up(nb_load, nthr_oc_b) * div_up(nb_bcast, nthr_ic_b) * jcp.ic_block * jcp.oc_block;
	};
