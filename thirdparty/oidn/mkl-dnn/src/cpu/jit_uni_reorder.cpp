...
struct jit_uni_reorder_kernel_f32 : public kernel_t, public jit_generator {
	...
	static bool applicable(const prb_t &p) {
		...
		bool ok = true && p.ndims > 0 && utils::one_of(p.itype, f32, s32, s8, u8) && utils::one_of(p.otype, f32, s32, s8, u8) && utils::everyone_is(0, p.ioff, p.ooff) /* do we need this? */
				  && utils::one_of(p.beta, 0.f, 1.f) /* anything else? */
				  && simple_impl_desc_init(p, nullptr) && mayiuse(sse42) && IMPLICATION(!utils::everyone_is(f32, p.itype, p.otype), mayiuse(avx));
		if (!ok)
			return false;
		...
		for (int d = 0; d < p.ndims; ++d) {
			...
			bool strides_ok = true && p.nodes[d].is < cms / (int)data_type_size(p.itype) && p.nodes[d].os < cms / (int)data_type_size(p.otype);
			if (!strides_ok)
				return false;
		}
	...
	Address i_addr(int i_off) { return ptr[reg_ptr_in + reg_off_in + i_off * itype_sz]; }

	Address o_addr(int o_off) { return ptr[reg_ptr_out + reg_off_out + o_off * otype_sz]; }

	Address s_addr(int s_off) { return ptr[reg_ptr_scale + reg_off_scale + s_off * stype_sz]; }
	...
	bool process_unroll_tr8x8(int len) {
		bool can_do = true && mayiuse(avx2) && prb_.ndims >= 2 && utils::everyone_is(4, itype_sz, otype_sz) && utils::everyone_is(8, n(0), n(1)) && utils::everyone_is(1, os(0), is(1)) && utils::everyone_is(8, os(1), is(0)) && prb_.scale_type == scale_type_t::NONE && prb_.beta == 0.f;
		if (!can_do)
			return false;
	...
	bool process_direct_copy(int len) {
		...
		bool can_do = true && mayiuse(isa) && utils::everyone_is(1, os(0), is(0)) && (false || prb_.itype == prb_.otype || (prb_.itype == s32 && prb_.otype == f32) || (prb_.itype == f32 && prb_.otype == s32)) && len % simd_w == 0 && n(0) % len == 0 && prb_.scale_type == scale_type_t::NONE && prb_.beta == 0.f;
		if (!can_do)
			return false;
	...
	void process_unroll_generic_step(int reg_unroll, const int *i_off,
		...
		const bool interim_f32 = false || utils::one_of(f32, prb_.itype, prb_.otype) || prb_.scale_type != scale_type_t::NONE || prb_.beta != 0.f;
		...
		if (can_store_xmm) {
			...
			if (prb_.scale_type == scale_type_t::COMMON) {
				...
				enum class scale_load_type_t { bcast,
					load,
					gather };
	...
	bool simple_impl() {
		...
		const bool optimized = false || process_direct_copy<avx>(d.len_unroll) || process_direct_copy<sse42>(d.len_unroll) || process_unroll_tr8x8(d.len_unroll);
		if (!optimized)
	...
	jit_uni_reorder_kernel_f32(const desc_t &desc) : kernel_t(desc), jit_generator() {
...
kernel_t *kernel_t::create(const kernel_t::desc_t &desc) {
	switch (desc.id) {
		case 0:
			return new jit_uni_reorder_kernel_f32(desc);
		default:
			assert(!"unknown kernel id");
			return nullptr;
	}
...
static void prb_thread_kernel_balance(tr::prb_t &prb, int &ndims_ker_max) {
	...
	bool want_borrow_ker_from_drv = true && kdims < prb.ndims && sz_ker_cur < tr::ker_prb_size_min && sz_drv_cur > sz_drv_min;
	 ...
	 * sz_drv_cur. */
	bool want_borrow_drv_from_ker = true && sz_ker_cur > tr::ker_prb_size_min && sz_drv_cur < sz_drv_min;
...
struct jit_uni_reorder_t : public cpu_primitive_t {
	struct pd_t : public cpu_reorder_pd_t {
		...
		static status_t create(reorder_pd_t **reorder_pd,
			...
			if (prb_init_status != status::success)
			...
			status_t ker_init_status = tr::kernel_t::desc_init(ker_desc, prb, ndims_ker_max);
	...
	void omp_driver_2d(int ithr, int nthr, int off, const char *in, char *out,
		...
		for_nd(ithr, nthr, (ptrdiff_t)ns[1].n, (ptrdiff_t)ns[0].n,
				[&](ptrdiff_t d1, ptrdiff_t d0) {
					auto c = tr::call_param_t();
					c.in = in + (d0 * ns[0].is + d1 * ns[1].is) * data_type_size(pd()->prb_.itype);
					c.out = out + (d0 * ns[0].os + d1 * ns[1].os) * data_type_size(pd()->prb_.otype);
	...
	void omp_driver_3d(int ithr, int nthr, int off, const char *in, char *out,
		...
		for_nd(ithr, nthr, (ptrdiff_t)ns[2].n, (ptrdiff_t)ns[1].n,
				...
				[&](ptrdiff_t d2, ptrdiff_t d1, ptrdiff_t d0) {
					auto c = tr::call_param_t();
					c.in = in + (d0 * ns[0].is + d1 * ns[1].is + d2 * ns[2].is) * data_type_size(pd()->prb_.itype);
					c.out = out + (d0 * ns[0].os + d1 * ns[1].os + d2 * ns[2].os) * data_type_size(pd()->prb_.otype);
	...
	void omp_driver_4d(int ithr, int nthr, int off, const char *in, char *out,
		...
		for_nd(ithr, nthr, (ptrdiff_t)ns[3].n, (ptrdiff_t)ns[2].n,
				...
				[&](ptrdiff_t d3, ptrdiff_t d2, ptrdiff_t d1, ptrdiff_t d0) {
					auto c = tr::call_param_t();
					c.in = in + (d0 * ns[0].is + d1 * ns[1].is + d2 * ns[2].is + d3 * ns[3].is) * data_type_size(pd()->prb_.itype);
					c.out = out + (d0 * ns[0].os + d1 * ns[1].os + d2 * ns[2].os + d3 * ns[3].os) * data_type_size(pd()->prb_.otype);
					c.scale = scale + d0 * ns[0].ss + d1 * ns[1].ss + d2 * ns[2].ss + d3 * ns[3].ss;
					(*kernel_)(&c);
				});
	}
