...
void jit_avx512_common_1x1_convolution_fwd_t<src_type, wei_type, dst_type>::
	...
	auto init_reduce = [&](int icb) {
		...
		p.first_last_flag = 0 | (icb == 0 ? FLAG_REDUCE_FIRST : 0) | (icb + nb_ic_blocking_step >= nb_ic ? FLAG_REDUCE_LAST : 0);
	...
	auto inner_ker = [&](int ocb, int icb, int n, int g, int oh, int ow,
		...
		p.load_data = &weights[pd()->with_groups() ? weights_d.blk_off(g, ocb, icb) : weights_d.blk_off(ocb, icb)];

		const int _icb = g * nb_ic + icb;
		if (pd()->rtus_.reduce_src_) {
			rp.ws = rtus_space + ithr * pd()->rtus_.space_per_thread_ + _icb * jcp.is * jcp.ic_block;
			if (ocb == ocb_start) {
...
void jit_avx512_common_1x1_convolution_bwd_data_t<diff_dst_type, wei_type,
	...
	parallel(0, [&](const int ithr, const int nthr) {
		...
		bool reduce_outer = (jcp.loop_order == loop_rbl || jcp.loop_order == loop_rlb);
			...
			for (int icb = icb_start; icb < icb_end; icb += load_step) {
				...
				for (int iwork = bcast_start; iwork < bcast_end;
					...
					for (int ocb_inner = 0; ocb_inner < nboc_inner;
						...
						int nb_oc_blocking_step = reduce_outer ? cur_ocb_outer : cur_ocb_inner;
						const int _ocb = g * nb_oc + ocb;
...
jit_avx512_common_1x1_convolution_bwd_weights_t ::
		jit_avx512_common_1x1_convolution_bwd_weights_t(const pd_t *apd) : cpu_primitive_t(apd),
																		   kernel_(nullptr),
																		   acc_ker_(nullptr),
...
void jit_avx512_common_1x1_convolution_bwd_weights_t::execute_backward_weights(
	...
	data_t *diff_bias = pd()->wants_padded_bias() ? scratchpad.get<data_t>(key_conv_padded_bias) : diff_bias_in;
	...
	auto ker = [&](const int ithr, const int nthr) {
		...
		const int ithr_but_oc = (ithr_mb * jcp.nthr_g + ithr_g) * jcp.nthr_ic_b + ithr_ic_b;
		...
		data_t *diff_wei = ithr_mb == 0 ? diff_weights : wei_reduction + (ithr_mb - 1) * wei_size;
			...
			for (int g = g_start; g < g_end; ++g) {
				...
				for (int ic_b = ic_b_start; ic_b < ic_b_end;
					...
					if (jcp.transpose_src) {
						...
						const int sp_size = nstl::min(sp_b_step * jcp.reduce_block,
					...
					for (int oc_b = oc_b_start; oc_b < oc_b_end;
						...
						const data_t *pdiff_dst = &diff_dst[diff_dst_d.blk_off(img, _oc_b)];
						const data_t *local_src = diff_src;

						auto p = jit_1x1_conv_call_s();
						auto rp = rtus_driver_t<avx512_common>::call_params_t();

						p.output_stride = jcp.ic * jcp.oc_block * jcp.typesize_out;
						...
						p.first_last_flag = 0 | (mb_sp_b == mb_sp_b_start ? FLAG_REDUCE_FIRST : 0) | (sp_b_end == sp_nb ? FLAG_SP_LAST : 0);
						...
						if (pd()->rtus_.reduce_src_) {
							...
							rp.ws = rtus_space + ithr * pd()->rtus_.space_per_thread_ + sp * jcp.ic_block;

							if (ndims == 3)
								rp.src = local_src + iw * src_d.blocking_desc().strides[2];
							else
								rp.src = local_src + ih * src_d.blocking_desc().strides[2] + iw * src_d.blocking_desc().strides[3];
							rtus_driver_->ker_(&rp);
		...
		if (jcp.nthr_mb > 1) {
			...
			for (int thr_mb = 1; thr_mb < jcp.nthr_mb; ++thr_mb) {
				...
				while (w < end) {
					...
					const int acc_size = nstl::min(end - w, ic_b_work - sub_ic_b_start) * jcp.ic_block * jcp.oc_block;
