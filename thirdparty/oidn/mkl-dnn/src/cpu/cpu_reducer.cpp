...
void reduce_balancer_t::balance() {
	...
	for (int c_njobs_per_group = min_njobs_per_group;
		...
		int c_nthr_per_group = syncable_ ? min(nthr_ / c_ngroups, reduction_size_) : 1;
		...
		size_t c_thread_complexity_ub = c_group_size_ub * (job_complexity * c_thread_reduction_ub + (c_nthr_per_group != 1));
	...
	assert((size_t)njobs_per_group_ub * job_size_ * nthr_ <= max_buffer_size_ || nthr_per_group == 1); /* no reduction buffer overflow */
...
struct reducer_2d_driver_t : public c_compatible {
	...
	reducer_2d_driver_t(int n_src, size_t src_ld,
			size_t src_step, size_t dst_step, bool nullify_dst) : n_src_(n_src),
																  src_ld_(src_ld),
																  src_step_(src_step),
																  dst_step_(dst_step),
...
struct reducer_2d_driver_f_s_32_t : public reducer_2d_driver_t<data_type>,
	...
	const int typesize = sizeof(typename mkldnn::impl::prec_traits<data_type>::type);
	...
	reducer_2d_driver_f_s_32_t(int n_src, size_t src_ld, size_t src_step,
			size_t dst_step, bool nullify_dst) : reducer_2d_driver_t<data_type>(n_src, src_ld, src_step,
														 dst_step, nullify_dst) { generate(); }
...
void cpu_reducer_t<data_type>::conf_t::init_scratchpad(
	...
	const size_t space_size = balancer_.ngroups_ * (balancer_.nthr_per_group_ - 1) * cpu_reducer_t<data_type>::space_per_thread(balancer_);
...
template <impl::data_type_t data_type>
cpu_reducer_t<data_type>::cpu_reducer_t(const conf_t &conf) : conf_(conf), drv_(nullptr) {
	if (balancer().nthr_per_group_ == 1)
		return;
...
cpu_reducer_t<data_type>::get_local_ptr(int ithr, data_t *dst,
	...
	const int offset_factor = grp_id * (balancer().nthr_per_group_ - 1) + (id_in_grp - 1);
...
void cpu_reducer_t<data_type>::reduce_nolock(int ithr, data_t *dst,
		const memory_tracking::grantor_t &scratchpad) const {
	bool redundant_reduction = balancer().nthr_per_group_ == 1 || balancer().idle(ithr);
	if (redundant_reduction)
		return;
	...
	const data_t *space = get_local_ptr(ithr - id_in_grp + 1, dst, scratchpad) + start * cl;
...
void cpu_reducer_2d_t<data_type>::conf_t::init_scratchpad(
	...
	const size_t space_size = balancer_.ngroups_ * balancer_.nthr_per_group_ * cpu_reducer_2d_t<data_type>::space_per_thread(balancer_);
...
template <impl::data_type_t data_type>
cpu_reducer_2d_t<data_type>::cpu_reducer_2d_t(const conf_t &conf) : conf_(conf), drv_(nullptr) {
	if (balancer().nthr_per_group_ == 1)
		return;
...
int cpu_reducer_2d_t<data_type>::choose_x_blocking(int nx, int ny,
	...
	assert(conf_.x_block_ > 0 && nx > conf_.x_block_ && nx % conf_.x_block_ == 0);
	...
	if (x_blocking >= min_x_blocking * 4)
		x_blocking = 1;
...
void cpu_reducer_2d_t<data_type>::reduce_block(const data_t *space_base,
	...
	const data_t *space = space_base + job * balancer().job_size_ + ny_start * conf_.job_size_x_ + nx_start;
#ifdef SIMPLE_IMPL
	for (int idg = 0; idg < balancer().nthr_per_group_; ++idg) {
		...
		for (int y = 0; y < ny_step; ++y)
			for (int x = 0; x < nx_step; ++x) {
				d[y * conf_.dst_x_ + x] = (idg == 0 ? 0 : d[y * conf_.dst_x_ + x]) + w[y * conf_.job_size_x_ + x];
			}
	}
#else
