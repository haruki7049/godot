...
status_t jit_avx512_core_x8s8s32x_deconv_fwd_kernel::init_conf(
	...
	jcp.is_depthwise = true && with_groups && utils::everyone_is(1, jcp.ic_without_padding, jcp.oc_without_padding);
	...
	auto set_or_check_wei_format = [&]() {
		...
		format_tag_t wei_tag = is_1d ? (jcp.is_depthwise ? Goiw16g : (with_groups ? gOIw4i16o4i : OIw4i16o4i)) : (jcp.is_depthwise ? Goihw16g : (with_groups ? gOIhw4i16o4i : OIhw4i16o4i));
	...
	if (!IMPLICATION(jcp.dilate_h, jcp.stride_h == 1) || !IMPLICATION(jcp.dilate_w, jcp.stride_w == 1))
		return status::unimplemented;

	/* padding: bottom and right */
	jcp.b_pad = (jcp.ih - 1) * jcp.stride_h + (jcp.kh - 1) * (jcp.dilate_h + 1) - (jcp.oh + jcp.t_pad - 1);
	jcp.r_pad = (jcp.iw - 1) * jcp.stride_w + (jcp.kw - 1) * (jcp.dilate_w + 1) - (jcp.ow + jcp.l_pad - 1);
	...
	for (; jcp.nb_oc_blocking > 1; jcp.nb_oc_blocking--)
		if (jcp.nb_oc % jcp.nb_oc_blocking == 0 && jcp.l_pad <= regs / (jcp.nb_oc_blocking + 1))
			break;
	...
	if (jcp.ow < jcp.ur_w) {
		...
		for (; jcp.ur_w >= 1; jcp.ur_w--) {
			...
			int r_overflow_no_tail = max(0, ((jcp.kw - 1) * (jcp.dilate_w + 1) - max(0, jcp.r_pad) - jcp.ur_w_tail) / jcp.stride_w);
...
bool jit_avx512_core_x8s8s32x_deconv_fwd_kernel::post_ops_ok(
	...
	switch (p.len_) {
		...
		case 2:
			return (p.contain(sum, 0) && is_eltwise(1)) || (p.contain(sum, 1) && is_eltwise(0));
		default:
			return false;
	}
...
void jit_avx512_core_x8s8s32x_deconv_fwd_kernel::compute_ker(int ur_w,
	...
	auto src_offset = [=](int oj, int icb, int ki) {
		return jcp.typesize_in * (((oj + jcp.l_pad - ki * (jcp.dilate_w + 1)) / jcp.stride_w) * jcp.ngroups * jcp.ic_without_padding + icb * 4);
	};

	auto kernel_offset = [=](int ocb, int icb, int ki) {
		return jcp.typesize_in * (ocb * jcp.nb_ic * jcp.kh * jcp.kw * ch_block_all + icb * jcp.oc_block * jcp.ic_block / 4 + ki * ch_block_all);
	};
		...
		for (int icb1 = 0; icb1 < n_ic_blocks; icb1++) {
			if (h_padded == true) {
				...
				for (int jj = _start; jj < _end; jj += ur_w_stride) {
					...
					if (jj >= jj_start && jj < jj_end && ((jj + jcp.l_pad - ki) % jcp.stride_w == 0)) {
						if (jcp.is_depthwise) {
							vpmovzxbd(zmm_inp(jj, jcp.nb_oc_blocking),
									EVEX_compress_addr(
											aux_reg_src, aux_src_off));
						} else if ((last_ic_block_flag & last_sp_block) && tail_size != 0 && icb1 == n_ic_blocks - 1) {
							xmm_t xmm_tmp = xmm_t(
...
void jit_avx512_core_x8s8s32x_deconv_fwd_kernel::kh_loop(int ur_w,
	...
	int shift_src_ih = jcp.typesize_in * (jcp.dilate_h + 1) * jcp.iw * jcp.ngroups * jcp.ic_without_padding;
	...
	if (jcp.signed_input || ((!jcp.signed_input) && ((min(jcp.t_pad, jcp.b_pad) < 0) || ((jcp.kh - 1) * (jcp.dilate_h + 1) < nstl::max(jcp.t_pad, jcp.b_pad))))) {
		cmp(reg_kh, 0);
...
void jit_avx512_core_x8s8s32x_deconv_fwd_kernel::store_output(
	...
	const float *p_sum_scale = (sum_idx != -1) ? &p.entry_[sum_idx].sum.scale : nullptr;
	...
	if (p_sum_scale) { // post_op: sum
		for (int k = 0; k < jcp.nb_oc_blocking; k++) {
			const bool mask_flag = last_oc_block == 1 && k == jcp.nb_oc_blocking - 1;
			for (int j = 0; j < ur_w; j++) {
				int aux_output_offset = jcp.typesize_out * (k * jcp.oc_block + j * jcp.oc_without_padding * jcp.ngroups);
	...
	for (int ocb = 0; ocb < jcp.nb_oc_blocking; ocb++) {
		...
		for (int ur = 0; ur < ur_w; ur++) {
			int aux_dst_off = jcp.typesize_out * (ur * jcp.ngroups * jcp.oc_without_padding + ocb * jcp.oc_block);
...
void jit_avx512_core_x8s8s32x_deconv_fwd_kernel::generate() {
	...
	int l_overflow = max(
			0, ((jcp.kw - 1) * (jcp.dilate_w + 1) - jcp.l_pad) / jcp.stride_w);
	int r_overflow = max(0, ((jcp.kw - 1) * (jcp.dilate_w + 1) - max(0, jcp.r_pad)) / jcp.stride_w);

	int r_overflow1 = nstl::max(0, ((jcp.kw - 1) * (jcp.dilate_w + 1) - nstl::max(0, jcp.r_pad) - jcp.ur_w_tail) / jcp.stride_w);
...
void _jit_avx512_core_x8s8s32x_deconvolution_fwd_t<src_type,
	...
	if (jcp.signed_input && jcp.ver != ver_vnni) {
		auto local_scales = scratchpad(ctx).template get<float>(key_conv_adjusted_scales);
	...
	parallel(0, [&](const int ithr, const int nthr) {
		...
		while (start < end) {
			...
			auto bias_w = jcp.with_bias ?
								  bias + (bias_d.blk_off(g_oc) * jcp.typesize_bia) :
								  0;
			int32_t *compensation_w = (jcp.signed_input) ? compensation + g_oc : 0;
			...
			for (int oj = oh_s; oj < oh_e; oj++) {
				...
				if (jcp.dilate_h != 0 && jcp.stride_h == 1) {
					...
					int o_t_overflow = div_up(
							...
							dilate_h);
					int o_b_overflow = div_up(max(0, (jcp.kh - 1) * dilate_h + 1 - jcp.oh + oj - jcp.b_pad),
							dilate_h);
					...
					int o_b_overflow = max(0, ((oj + jcp.kh) - (jcp.oh + jcp.b_pad)) / jcp.stride_h);
					int overflow_kh_hi = jcp.kh - 1 - abs(jcp.oh + jcp.b_pad - (oj + 1)) % jcp.stride_h;
					int overflow_kh_lo = (oj + jcp.t_pad) % jcp.stride_h;

					kh_len = (overflow_kh_hi - overflow_kh_lo) / jcp.stride_h + 1 - o_t_overflow - o_b_overflow;
					kh_lo = overflow_kh_lo + o_b_overflow * jcp.stride_h;
					ih_max = (oj + jcp.t_pad - kh_lo) / jcp.stride_h;
				}

				int wei_stride = (!jcp.signed_input) ? kh_lo * wht_kh_stride : 0;
				...
				p.t_overflow = max(
						0, jcp.kh - (kh_lo + max(0, kh_len - 1) * jcp.stride_h + 1));
				p.b_overflow = kh_lo;
				p.kh_padding = kh_len;
				p.scales = scales;
				p.oc_blocks = jcp.is_depthwise ? g : ocb;
