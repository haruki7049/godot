					...
					for (int oh = 0; oh < jcp.oh; ++oh) {
						...
						for (int kw = 0; kw < jcp.kw; ++kw) {
							...
							for (int ow = 0; ow < jcp.ow; ++ow) {
								...
								const size_t col_idx = kw * OHW + oh * jcp.ow + ow;
...
void im2col(const jit_gemm_conv_conf_t &jcp, const float *__restrict im,
	...
	if (jcp.stride_w == 1) {
		...
		auto ker = [&](int ic, int kh, int kw, int oh) {
			...
			float *__restrict col_ = col + ic * col_step + ((kh * jcp.kw + kw) * hb + oh) * wb;

			const int ih = (oh + hs) * jcp.stride_h - jcp.t_pad + kh * (1 + jcp.dilate_h);
			if (ih < 0 || ih >= jcp.ih) {
		...
		parallel_nd(jcp.kh, hb, [&](int kh, int oh) {
			...
			if (ih < 0 || ih >= jcp.ih)
				for (int kw = 0; kw < jcp.kw; ++kw) {
					for (int ow = 0; ow < wb; ++ow) {
						const size_t col_idx = ((kh * jcp.kw + kw) * hb + oh) * wb + ow;
						col[col_idx] = 0;
					}
				}
			else
				for (int kw = 0; kw < jcp.kw; ++kw) {
					for (int ow = 0; ow < wb; ++ow) {
						const int iw = (ow + ws) * jcp.stride_w - jcp.l_pad + kw * (1 + jcp.dilate_w);
						const size_t col_idx = ((kh * jcp.kw + kw) * hb + oh) * wb + ow;
						const size_t im_idx = ih * jcp.iw + iw;
						if (iw < 0 || iw >= jcp.iw)
							col[col_idx] = 0;
...
void im2col_u8(const jit_gemm_conv_conf_t &jcp, const T *__restrict im,
	...
	if (jcp.outer_threading && sh == 1 && sw == 1 && dh == 1 && dw == 1) {
		...
		for (int kh = 0; kh < jcp.kh; kh++) {
			...
			for (int kw = 0; kw < jcp.kw; kw++) {
				const ptrdiff_t col_idx_kw = col_idx_kh + kw * jcp.ic * col_ic_str;
			...
			for (int ow = 0; ow < jcp.ow; ++ow) {
				for (int kh = 0; kh < jcp.kh; ++kh) {
					...
					for (int kw = 0; kw < jcp.kw; ++kw) {
						...
						const size_t im_idx = (ih * jcp.iw + iw) * jcp.ic;
						PRAGMA_OMP_SIMD()
						for (int ic = 0; ic < jcp.ic; ++ic) {
...
status_t init_conf(jit_gemm_conv_conf_t &jcp,
	...
	jcp.with_bias = cd.bias_desc.format_kind != format_kind::undef || cd.diff_bias_desc.format_kind != format_kind::undef;
	...
	bool is_int8_conv = utils::one_of(src_d.data_type(), s32, s8, u8) && weights_d.data_type() == s8;

	const int vlen = mayiuse(avx512_common) ? cpu_isa_traits<avx512_common>::vlen : mayiuse(avx) ? cpu_isa_traits<avx>::vlen :
																			mayiuse(sse42)		 ? cpu_isa_traits<sse42>::vlen :
																								   4;
	...
	const int L2 = get_cache_size(2, true) / (is_int8_conv ? sizeof(int8_t) : sizeof(float));
	bool is_blocking_applicable = true && is_fwd && jcp.im2col_sz && jcp.id == 1 && jcp.od == 1 && jcp.dilate_h == 0 && jcp.dilate_w == 0 && !is_depthwise && wei_size < L2 / 2;
	if (is_blocking_applicable) {
		...
		const float inner_thr_eff = (float)inner_work / rnd_up(inner_work, max_threads);
	...
	if (is_int8_conv) {
		if (is_fwd) {
			if (!jcp.outer_threading) {
				...
				const float outer_thr_eff = (float)outer_work / rnd_up(outer_work, max_threads);
				const size_t inner_work = div_up(jcp.is, simd_w) * div_up(jcp.ic, simd_w);
				const float inner_thr_eff = (float)inner_work / rnd_up(inner_work, max_threads);
				jcp.outer_threading = (is_depthwise || (jcp.is / max_threads < 64 && jcp.mb != 1)) && (outer_thr_eff / inner_thr_eff >= 1.f || (jcp.os * jcp.ic * jcp.oc) / max_threads < gemm_thrld);
			}
			...
			const size_t inner_work = div_up(jcp.is, simd_w) * div_up(jcp.ic, simd_w);
			const float inner_thr_eff = (float)inner_work / rnd_up(inner_work, max_threads);
			jcp.outer_threading = (is_depthwise || (jcp.is / max_threads < 64 && jcp.mb != 1)) && (outer_thr_eff / inner_thr_eff >= 1.f || (jcp.is * jcp.ic * jcp.oc) / max_threads < gemm_thrld);
		...
		if (is_fwd) {
			if (!jcp.outer_threading) {
				...
				const float outer_thr_eff = (float)outer_work_amount / rnd_up(outer_work_amount, max_threads);
				const size_t inner_work_amount = div_up(jcp.os, simd_w) * div_up(jcp.oc, simd_w);
				const float inner_thr_eff = (float)inner_work_amount / rnd_up(inner_work_amount, max_threads);
				jcp.outer_threading = jcp.os / max_threads < 512 && IMPLICATION(jcp.od == 1, jcp.mb != 1 || jcp.ngroups > 2) && (outer_thr_eff / inner_thr_eff >= 1.f || (jcp.os * jcp.ic * jcp.oc) / max_threads < gemm_thrld);
			}
		} else if (is_bwd_d) {
			...
			const size_t inner_work = div_up(jcp.is, simd_w) * div_up(jcp.ic, simd_w);
			const float inner_thr_eff = (float)inner_work / rnd_up(inner_work, max_threads);
			jcp.outer_threading = (jcp.os / max_threads < 512 || jcp.ks < 64) && (jcp.mb != 1 || jcp.ngroups > 2) && (outer_thr_eff / inner_thr_eff >= 1.f || (jcp.is * jcp.ic * jcp.oc) / max_threads < gemm_thrld);
		} else if (is_bwd_w)
			jcp.outer_threading = jcp.os / max_threads < 256 && (jcp.mb != 1 || jcp.ngroups > 2);
		...
		if (is_bwd_w) {
			jcp.need_wei_reduction = mkldnn_thr_syncable() ? jcp.mb != 1 && jcp.nthr != 1 : false;
