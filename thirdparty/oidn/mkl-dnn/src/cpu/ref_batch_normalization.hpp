...
struct ref_batch_normalization_fwd_t : public cpu_primitive_t {
	struct pd_t : public cpu_batch_normalization_fwd_pd_t {
		...
		status_t init() {
			bool ok = true && is_fwd() && src_md()->data_type == data_type && IMPLICATION(use_scaleshift(), weights_md()->data_type == data_type::f32) && (attr()->has_default_values() || with_relu_post_op());
			if (!ok)
...
struct ref_batch_normalization_bwd_t : public cpu_primitive_t {
	struct pd_t : public cpu_batch_normalization_bwd_pd_t {
		...
		status_t init() {
			bool ok = true && is_bwd() && utils::everyone_is(data_type, src_md()->data_type, diff_src_md()->data_type) && IMPLICATION(use_scaleshift(), utils::everyone_is(data_type, weights_md()->data_type, diff_weights_md()->data_type)) && attr()->has_default_values();
			if (!ok)
				return status::unimplemented;
