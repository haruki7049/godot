...
struct _gemm_x8s8s32x_convolution_fwd_t : public cpu_primitive_t {
	struct pd_t : public cpu_convolution_fwd_pd_t {
		...
		status_t init() {
			...
			bool ok = true && is_fwd() && set_default_alg_kind(alg_kind::convolution_direct) && expect_data_types(src_type, s8, data_type::undef, dst_type, s32) && IMPLICATION(with_bias(), utils::one_of(desc()->bias_desc.data_type, f32, s32, s8, u8)) && !has_zero_dim_memory() && set_default_formats_common(dat_tag(), format_tag::any, dat_tag()) && post_ops_ok() && memory_desc_matches_tag(*src_md(), dat_tag()) && memory_desc_matches_tag(*dst_md(), dat_tag()) && set_or_check_wei_format();
			if (!ok)
	...
	protected:
		...
		bool set_or_check_wei_format() {
			...
			if (is_src_s8) {
				...
				want_wei_md.extra.compensation_mask = (1 << 0) + (with_groups() ? (1 << 1) : 0);
		...
		bool post_ops_ok() const {
			...
			auto is_relu = [&](int idx) { return po.entry_[idx].is_relu(true, false); };

			switch (po.len_) {
				case 0:
					return true;
				case 1:
					return is_relu(0) || po.contain(sum, 0);
				case 2:
					return po.contain(sum, 0) && is_relu(1);
				default:
					return false;
			}
			return false;
	...
	_gemm_x8s8s32x_convolution_fwd_t(const pd_t *apd) : cpu_primitive_t(apd, true), pp_ker_(nullptr) { pp_ker_ = new pp_ker_t(pd()); }
...
struct _gemm_u8s8s32x_convolution_bwd_data_t : public cpu_primitive_t {
	...
	_gemm_u8s8s32x_convolution_bwd_data_t(const pd_t *apd) : cpu_primitive_t(apd, true) {}
