	...
	for (int j = 0; j < kh; j++) {
		for (int i = 0; i < kw; i++) {
			for (int v1 = 0; v1 < simd_w; v1++) {
				float *base_inp = is_fwd ? &(input(0, 0, j, i, v1, 0)) : &(input(0, 0, 2 - j, 2 - i, v1, 0));
				PRAGMA_OMP_SIMD()
						...
						if (xdim < outw) {
							...
							if (is_fwd) {
								...
								for (int v = 0; v < simd_w; v++) {
									...
									O[j][i][v] = true && with_relu_presum && O[j][i][v] < 0.f ? O[j][i][v] * jcp.eltwise.alpha : O[j][i][v];
								}
							}
							if (with_sum)
...
void diff_src_transform_bwd_weights(int image, jit_conv_winograd_conf_t conv,
	...
	int nb_tile_block_ur =
			(tile_base_index / conv.tile_4fma / conv.tile_block_ur) % conv.nb_tile_block_ur;
...
void diff_dst_transform_bwd_weights(int image, jit_conv_winograd_conf_t conv,
	...
	int nb_tile_block_ur =
			(tile_base_index / conv.tile_block_ur / conv.tile_4fma) % conv.nb_tile_block_ur;
...
void _jit_avx512_common_convolution_winograd_t<is_fwd>::_execute_data_W_S_G_D(
	...
	auto output_transform = jcp.with_bias ? (jcp.with_eltwise ? (jcp.with_sum ? output_transform_data<is_fwd, true, true, true> : output_transform_data<is_fwd, true, true, false>) : (jcp.with_sum ? output_transform_data<is_fwd, true, false, true> : output_transform_data<is_fwd, true, false, false>)) : (jcp.with_eltwise ? (jcp.with_sum ? output_transform_data<is_fwd, false, true, true> : output_transform_data<is_fwd, false, true, false>) : (jcp.with_sum ? output_transform_data<is_fwd, false, false, true> : output_transform_data<is_fwd, false, false, false>));
	...
	array_offset_calculator<float, 8> M(is_fwd ? scratchpad.template get<float>(key_wino_M) : scratchpad.template get<float>(key_wino_V),
			jcp.dimN_nb_block, jcp.dimM_nb_block,
			alpha, alpha,
			jcp.dimN_block, jcp.dimM_block,
			jcp.dimN_reg_block, jcp.dimM_simd_block);
	...
	array_offset_calculator<float, 8> V(is_fwd ? scratchpad.template get<float>(key_wino_V) : scratchpad.template get<float>(key_wino_M),
			jcp.dimN_nb_block, alpha, alpha,
			jcp.dimN_block, jcp.dimK_nb_block,
			jcp.dimK_block, jcp.dimN_reg_block, jcp.dimK_reg_block);

	bool V_streamout = jcp.dimN * jcp.dimK * alpha * alpha * sizeof(float) > 2 * LLC_cache_size ? true : false;

	const bool output_is_aligned = ((size_t)out_ptr & (64 - 1)) == 0;

	const bool wants_padded_bias = jcp.with_bias && jcp.oc_without_padding != jcp.oc;
...

	{
		...
		parallel_nd(jcp.nb_oc, jcp.nb_ic, jcp.oc_block, jcp.ic_block,
				[&](int ofm1, int ifm1, int ofm2, int ifm2) {
					float *U_base_ptr = is_fwd ? &(U(ofm1, 0, 0, ifm1, ofm2, ifm2, 0, 0)) : &(U(ifm1, 0, 0, ofm1, ifm2, ofm2, 0, 0));
		...
		parallel_nd(jcp.mb, jcp.dimM_nb_block, jcp.dimM_block,
				[&](int img, int M_blk1, int M_blk2) {
					...
					float *bias_ptr = wants_padded_bias && M_blk == jcp.dimM / jcp.dimM_simd_block - 1 ? last_slice_bias : &bias(M_blk, 0);
...
void jit_avx512_common_convolution_winograd_bwd_weights_t::
	...
	auto diff_dst_transform_bwd_weights_ver = jcp.with_bias ? diff_dst_transform_bwd_weights<true> : diff_dst_transform_bwd_weights<false>;
	...
	array_offset_calculator<float, 2> diff_bias(pd()->wants_padded_bias() ? scratchpad.get<float>(key_conv_padded_bias) : ptr_diff_bias,
	...
	const int trans_buffer_size = alpha * alpha * jcp.tile_4fma * jcp.ic_simd_block;
	...
	for (int v = 0; v < simd_w; v++)
		...
		parallel_nd_in_omp(jcp.mb, jcp.nb_ic, jcp.ic_block,
				[&](int img, int ifm1, int ifm2) {
					float *transb = jcp.ver == ver_4fma ? &(trans_buffer(ithread, 0)) : NULL;
		...
		parallel_nd_in_omp(jcp.mb, jcp.nb_oc, jcp.oc_block,
				[&](int img, int ofm1, int ofm2) {
					float *dbias = jcp.with_bias ? &(diff_bias_prv(ithread,
