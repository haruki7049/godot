...
void ref_inner_product_fwd_t<src_type, wei_type, dst_type, acc_type>::
			...
			for (int kd = 0; kd < KD; ++kd) {
				for (int kh = 0; kh < KH; ++kh) {
					for (int kw = 0; kw < KW; ++kw) {
						switch (ndims) {
							case 3:
								d += (acc_data_t)src[src_d.off(mb, ic, kd, kh, kw)] * weights[weights_d.off(
																							  oc, ic, kd, kh, kw)];
								break;
							case 2:
								d += (acc_data_t)src[src_d.off(mb, ic, kh, kw)] * weights[weights_d.off(oc, ic, kh, kw)];
								break;
							case 1:
								d += (acc_data_t)src[src_d.off(mb, ic, kw)] * weights[weights_d.off(oc, ic, kw)];
								break;
							default:
								assert(!"unsupported ndims size");
						}
					}
				}
			}
	...
	auto ker_no_spatial = [=](int mb, int oc) {
		acc_data_t d = 0;
		for (int ic = 0; ic < IC; ++ic) {
			d += (acc_data_t)src[src_d.off(mb, ic)] * weights[weights_d.off(oc, ic)];
		}
		return d;
	};

	parallel_nd(MB, OC, [&](int mb, int oc) {
		float a = bias ? get_bias(bias, bias_d.off(oc), pd()->desc()->bias_desc.data_type) : 0;
		if (src_has_spatial)
...
void ref_inner_product_bwd_data_t<diff_src_type, wei_type, diff_dst_type,
	...
	const bool diff_src_has_spatial = utils::one_of(diff_src_d.ndims(), 3, 4, 5);
				...
				for (int kh = 0; kh < KH; ++kh)
					for (int kw = 0; kw < KW; ++kw) {
						...
						for (int oc = 0; oc < OC; ++oc) {
							switch (ndims) {
								case 3:
									ds += (acc_data_t)(diff_dst[diff_dst_d.off(mb, oc)] * weights[weights_d.off(oc, ic, kd, kh, kw)]);
									break;
								case 2:
									ds += (acc_data_t)(diff_dst[diff_dst_d.off(mb, oc)] * weights[weights_d.off(oc, ic, kh, kw)]);
									break;
								case 1:
									ds += (acc_data_t)(diff_dst[diff_dst_d.off(mb, oc)] * weights[weights_d.off(oc, ic, kw)]);
									break;
								default:
									assert(!"unsupported ndims size");
							}
						...
						switch (ndims) {
							case 3:
								diff_src[diff_src_d.off(mb, ic, kd, kh, kw)] = (diff_src_data_t)ds;
								break;
							case 2:
								diff_src[diff_src_d.off(mb, ic, kh, kw)] = (diff_src_data_t)ds;
								break;
							case 1:
					...
					for (int kw = 0; kw < KW; ++kw) {
						...
						*dw = data_t(0);
						for (int mb = 0; mb < MB; ++mb) {
							switch (ndims) {
								case 3:
									*dw += diff_dst[diff_dst_d.off(mb, oc)] * src[src_d.off(mb, ic, kd, kh, kw)];
									break;
								case 2:
									*dw += diff_dst[diff_dst_d.off(mb, oc)] * src[src_d.off(mb, ic, kh, kw)];
									break;
								case 1:
									*dw += diff_dst[diff_dst_d.off(mb, oc)] * src[src_d.off(mb, ic, kw)];
									break;
								default:
									assert(!"unsupported ndims size");
							}
