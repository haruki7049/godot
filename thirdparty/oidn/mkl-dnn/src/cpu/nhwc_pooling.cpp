...
#define DECLARE_READ_STRIDES(name)                                                                                             \
	...
	const size_t name##_d_stride = (!is_3d) ? 0 : MEM_D(name).blocking_desc().strides[2];                                      \
	const size_t name##_h_stride = (!is_3d) ? MEM_D(name).blocking_desc().strides[2] : MEM_D(name).blocking_desc().strides[3]; \
	const size_t name##_w_stride = (!is_3d) ? MEM_D(name).blocking_desc().strides[3] : MEM_D(name).blocking_desc().strides[4];
							...
							if (alg == pooling_max) {
								...
								for (int oc = 0; oc < OC; ++oc) {
									const int index_from_ws =
											(MEM_D(ws).data_type() == data_type::u8) ? (int)ws[ws_offset_init + oc] : ((int *)ws)[ws_offset_init + oc];
									...
									if (!(KD == SD && KH == SH && KW == SW))
										diff_src[src_offset_init + oc] +=
												(index_from_ws == index) ? d : data_type_t(0);
									else
										diff_src[src_offset_init + oc] =
												(index_from_ws == index) ? d : data_type_t(0);
								}
