...
_gemm_x8s8s32x_convolution_fwd_t<src_type, dst_type>::pp_ker_t::pp_ker_t(
	...
	const size_t vlen_start = cpu_isa_traits<avx512_common>::vlen / sizeof(float);
		...
		if (do_sum_) {
			...
			switch (dst_type) {
				...
				case data_type::s32:
					...
					break;
				case data_type::s8:
					vpmovsxbd(vreg_prev_dst_, dst_addr);
					break;
				case data_type::u8:
					vpmovzxbd(vreg_prev_dst_, dst_addr);
					break;
				default:
...
template <data_type_t src_type, data_type_t dst_type>
void _gemm_x8s8s32x_convolution_fwd_t<src_type, dst_type>::pp_ker_t::operator()(dst_data_t *dst, const acc_data_t *acc, const char *bias,
...
void _gemm_x8s8s32x_convolution_fwd_t<src_type, dst_type>::
	...
	auto col = scratchpad.get<uint8_t>(key_conv_gemm_col) + (ptrdiff_t)ithr * jcp.im2col_sz;
	src_data_t *__restrict imtr = scratchpad.get<src_data_t>(key_conv_gemm_imtr) + (ptrdiff_t)ithr * jcp.is * jcp.ic;
	auto acc = scratchpad.get<acc_data_t>(key_conv_int_dat_in_acc_dt) + (ptrdiff_t)ithr * jcp.oh_block * jcp.ow_block * jcp.oc;
	...
	for (size_t iwork = start; iwork < end; ++iwork) {
		...
		const src_data_t *__restrict src = src_base + n * src_mb_stride + g * src_g_stride;
		...
		auto wei_adj_scale =
				(wei_md.extra().flags | memory_extra_flags::scale_adjust) ? wei_md.extra().scale_adjust : 1.f;
...
void _gemm_u8s8s32x_convolution_bwd_data_t<dst_type>::
	...
	auto col = scratchpad.get<acc_data_t>(key_conv_gemm_col) + (ptrdiff_t)ithr * jcp.im2col_sz;
	auto acc = scratchpad.get<acc_data_t>(key_conv_int_dat_in_acc_dt) + (ptrdiff_t)ithr * jcp.is * jcp.ic;
	...
	for (size_t iwork = start; iwork < end; ++iwork) {
		const diff_dst_data_t *diff_dst = diff_dst_base + n * diff_dst_mb_stride + g * diff_dst_g_stride;
		const wei_data_t *wei = wei_base + g * wei_g_stride;
		diff_src_data_t *diff_src = diff_src_base + n * diff_src_mb_stride + g * diff_src_g_stride;
