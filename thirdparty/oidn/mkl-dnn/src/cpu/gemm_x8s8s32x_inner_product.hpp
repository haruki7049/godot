...
struct gemm_x8s8s32x_inner_product_fwd_t : public cpu_primitive_t {
	struct pd_t : public cpu_inner_product_fwd_pd_t {
		...
		DECLARE_COMMON_PD_T(src_type == data_type::u8 ? IGEMM_S8U8S32_IMPL_STR : IGEMM_S8S8S32_IMPL_STR,
				gemm_x8s8s32x_inner_product_fwd_t);

		status_t init() {
			using namespace data_type;

			bool ok = true && set_default_params() == status::success && is_fwd() && !has_zero_dim_memory() && src_md()->data_type == src_type && dst_md()->data_type == dst_type && weights_md()->data_type == s8 && IMPLICATION(with_bias(), utils::one_of(weights_md(1)->data_type, f32, s32, s8, u8)) && attr()->post_ops_.len_ <= 1 && IMPLICATION(attr()->post_ops_.len_, attr()->post_ops_.entry_[0].is_relu(true, false)) && dense_gemm_consitency_check(src_md(), weights_md(), dst_md());
			if (!ok)
	...
	gemm_x8s8s32x_inner_product_fwd_t(const pd_t *apd) : cpu_primitive_t(apd, true) { pp_kernel_ = new pp_kernel_t(apd, pd()->dst_is_acc_); }
