...
struct jit_uni_pooling_fwd_t : public cpu_primitive_t {
	struct pd_t : public cpu_pooling_fwd_pd_t {
		...
		status_t init() {
			...
			bool ok = true && set_default_params() == status::success && is_fwd() && !has_zero_dim_memory() && everyone_is(data_type::f32, src_md()->data_type, dst_md()->data_type) && attr()->has_default_values() && memory_desc_matches_tag(*src_md(), desired_fmt_tag()) && memory_desc_matches_tag(*dst_md(), desired_fmt_tag());
			if (!ok)
		...
		format_tag_t desired_fmt_tag() {
			...
			return ndims() == 4 ? isa == avx512_common ? nChw16c : nChw8c : isa == avx512_common ? nCdhw16c :
																								   nCdhw8c;
		}
	...
	jit_uni_pooling_fwd_t(const pd_t *apd) : cpu_primitive_t(apd) { kernel_ = new jit_uni_pool_kernel_f32<isa>(pd()->jpp_); }
...
struct jit_uni_pooling_bwd_t : public cpu_primitive_t {
	struct pd_t : public cpu_pooling_bwd_pd_t {
		...
		status_t init() {
			...
			bool ok = true && set_default_params() == status::success && !is_fwd() && !has_zero_dim_memory() && everyone_is(data_type::f32, diff_src_md()->data_type, diff_dst_md()->data_type) && attr()->has_default_values() && memory_desc_matches_tag(*diff_dst_md(), desired_fmt_tag()) && memory_desc_matches_tag(*diff_src_md(), desired_fmt_tag());
			if (!ok)
				return status::unimplemented;
		...
		format_tag_t desired_fmt_tag() {
			using namespace format_tag;
			return ndims() ? isa == avx512_common ? nChw16c : nChw8c : isa == avx512_common ? nCdhw16c :
																							  nCdhw8c;
		}
	...
	jit_uni_pooling_bwd_t(const pd_t *apd) : cpu_primitive_t(apd) { kernel_ = new jit_uni_pool_kernel_f32<isa>(pd()->jpp_); }
