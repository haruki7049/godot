...
struct wino_reorder_t : public cpu_primitive_t {
	struct pd_t : public cpu_reorder_pd_t {
		...
		static status_t create(reorder_pd_t **reorder_pd,
			...
			bool args_ok = true && id.data_type() == type_i && od.data_type() == type_o && id.matches_tag(utils::pick(id.ndims() - 4, format_tag::oihw, format_tag::goihw)) && od.format_kind() == format_kind::wino && utils::one_of(od.wino_desc().wino_format, mkldnn_wino_wei_aaOIoi, mkldnn_wino_wei_aaOio, mkldnn_wino_wei_aaOBiOo, mkldnn_wino_wei_OBaaIBOIio);
			if (!args_ok)
	...
	void transform(out_data_t *__restrict tmp_wei,
		...
		for (int iic = 0; iic < ic_; iic++) {
			for (int ob = 0; ob < nb_oc_; ob++) {
				const in_data_t *__restrict _inp = input + (ob * oc_block_ * or_ic_ + iic) * kh_ * kw_;
				out_data_t *__restrict _out = tmp_wei + (iic * nb_oc_ + ob) * oc_block_;
				...
				for_nd(0, 1, r_, w_alpha_, oc_block_,
						[&](int ih, int j, int ioc) {
							for (int iw = 0; iw < r_; ++iw) {
								...
								int inp_ic = iic;
								in_data_t inp_v = (inp_ic < or_ic_ && inp_oc < or_oc_) ? _inp[ioc * or_ic_ * kh_ * kw_ + ih * kw_ + iw] : 0.f;
								wspace[(ih * w_alpha_ + j) * oc_block_ + ioc] += inp_v * g[j * r_ + iw];
							}
						});
				...
				for_nd(0, 1, w_alpha_, w_alpha_, oc_block_,
						[&](int i, int j, int ioc) {
							float t = 0;
							for (int k = 0; k < r_; ++k)
								t += g[i * r_ + k] * wspace[(k * w_alpha_ + j) * oc_block_ + ioc];
							if (type_o == data_type::s8) {
								const float scale = (D_mask == 1) ? scales[0] : scales[ob * oc_block_ + ioc];
								_out[(i * w_alpha_ + j) * Z + ioc] = qz_b0<in_data_t, out_data_t>()(
					...
					for (int ib = 0; ib < nb_ic_; ib++) {
						for (int i = 0; i < ic_block_; i++) {
							for (int o = 0; o < oc_block_; o++) {
								int src_offset = u_h * w_alpha_ * ic_ * oc_ + u_w * ic_ * oc_ + (ib * ic_block_ + i) * oc_ + (ob * oc_block_ + o);

								int dst_offset = u_h * w_alpha_ * nb_oc_ * nb_ic_ * ic_block_ * oc_block_ + u_w * nb_oc_ * nb_ic_ * ic_block_ * oc_block_ + ob * nb_ic_ * ic_block_ * oc_block_ + ib * ic_block_ * oc_block_ + i * oc_block_ + o;
								output[dst_offset] = tmp_wei[src_offset];
							}
						}
					}
	...
	void reorder_to_aaOBiOo(out_data_t *__restrict output,
		...
		for_nd(0, 1, w_alpha_, w_alpha_, oc_chunks,
				[&](int u_h, int u_w, int occ) {
					for (int ib = 0; ib < nb_ic_; ib++) {
						out_data_t *__restrict wei_ptr = output + (((u_h * w_alpha_ + u_w) * oc_chunks + occ) * nb_ic_ + ib) * oc2_block_ * ic_block_ * oc_block_;
						int wei_offset = 0;
						for (int i = 0; i < ic_block_; i++) {
							for (int ob2 = 0; ob2 < oc2_block_; ob2++) {
								for (int o = 0; o < oc_block_; o++) {
									...
									int src_offset = u_h * w_alpha_ * ic_ * oc_ + u_w * ic_ * oc_ + icp * oc_ + ocp;
						...
						for (int ob = 0; ob < oc2_block_; ob++) {
							...
							for (int ib = 0; ib < ic2_block_; ib++) {
								for (int i = 0; i < ic_block_; i++) {
									...
									int src_offset = u_h * w_alpha_ * ic_ * oc_ + u_w * ic_ * oc_ + icp * oc_ + ocp;
									int wei_offset = ((((((occ * w_alpha_ + u_h) * w_alpha_ + u_w) * ic_chunks + icc) * oc2_block_ + ob) * ic2_block_ + ib) * ic_block_ + i) * oc_block_;
									for (int o = 0; o < oc_block_; o++)
