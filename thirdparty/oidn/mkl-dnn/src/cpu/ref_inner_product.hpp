...
struct ref_inner_product_fwd_t : public cpu_primitive_t {
	struct pd_t : public cpu_inner_product_fwd_pd_t {
		...
		status_t init() {
			...
			bool ok = true && set_default_params() == status::success && is_fwd() && src_md()->data_type == src_type && weights_md()->data_type == wei_type && desc()->accum_data_type == acc_type && dst_md()->data_type == dst_type && IMPLICATION(with_bias(), utils::one_of(weights_md(1)->data_type, f32, s32, s8, u8)) && attr()->output_scales_.has_default_values() && attr()->post_ops_.len_ <= 1 && IMPLICATION(attr()->post_ops_.len_ == 1, attr()->post_ops_.entry_[0].is_relu(true, false));
...
struct ref_inner_product_bwd_data_t : public cpu_primitive_t {
	struct pd_t : public cpu_inner_product_bwd_data_pd_t {
		...
		status_t init() {
			bool ok = true && set_default_params() == status::success && desc()->prop_kind == prop_kind::backward_data && diff_src_md()->data_type == diff_src_type && weights_md()->data_type == wei_type && desc()->accum_data_type == acc_type && diff_dst_md()->data_type == diff_dst_type && attr()->has_default_values();
			return ok ? status::success : status::unimplemented;
		}
	};
...
struct ref_inner_product_bwd_weights_t : public cpu_primitive_t {
	struct pd_t : public cpu_inner_product_bwd_weights_pd_t {
		...
		status_t init() {
			bool ok = true && set_default_params() == status::success && desc()->prop_kind == prop_kind::backward_weights && utils::everyone_is(data_type, src_md()->data_type, diff_dst_md()->data_type, diff_weights_md()->data_type) && IMPLICATION(with_bias(), data_type == diff_weights_md(1)->data_type) && attr()->has_default_values();
			return ok ? status::success : status::unimplemented;
		}
	};
