...
inline bool simple_fmt_check(bool order_keep, impl::format_tag_t tag_i,
	...
	return input_d.matches_tag(order_keep ? tag_i : tag_o) && output_d.matches_tag(order_keep ? tag_o : tag_i);
}
...
struct simple_reorder_impl<SIMPLE_REORDER_TEMPL_CALL,
		typename utils::enable_if<tag_i == any && (false || tag_o == hwio || tag_o == hwigo), spec::conv_s8s8>::type> {
	static bool is_applicable(const memory_desc_wrapper &input_d,
		...
		return output_d.matches_tag(tag_o) && (output_d.extra().flags & memory_extra_flags::compensation_conv_s8s8) && (input_d.data_type() == f32 || input_d.data_type() == s8) && output_d.data_type() == s8 && (D_mask == 1 || D_mask == (size_t)g * oc);
	}
	...
	static status_t execute(const cpu_reorder_pd_t *pd,
		...
		assert(output_d.extra().flags & memory_extra_flags::compensation_conv_s8s8);
		float adj_scale =
				(output_d.extra().flags & memory_extra_flags::scale_adjust) ? output_d.extra().scale_adjust : 1.f;
...
struct simple_reorder_impl<SIMPLE_REORDER_TEMPL_CALL,
		typename utils::enable_if<
				(tag_i == goiw && tag_o == gOIw4i16o4i) || (tag_i == oiw && tag_o == OIw4i16o4i) || (tag_i == goihw && tag_o == gOIhw4i16o4i) || (tag_i == oihw && tag_o == OIhw4i16o4i) || (tag_i == goihw && tag_o == gOIhw2i8o4i) || (tag_i == goihw && tag_o == gOIhw4o4i), spec::conv_s8s8>::type> {
	static bool is_applicable(const memory_desc_wrapper &input_d,
			const memory_desc_wrapper &output_d, const primitive_attr_t *attr) {
		const size_t D_mask = utils::array_product(input_d.dims(),
				math::ilog2q(attr->output_scales_.mask_ + 1));
		...
		return input_d.matches_tag(tag_i) && output_d.matches_tag(tag_o) && (output_d.extra().flags & memory_extra_flags::compensation_conv_s8s8) && (input_d.data_type() == f32 || input_d.data_type() == s8) && output_d.data_type() == s8 && (D_mask == 1 || D_mask == (size_t)g * oc);
	}
	...
	static status_t execute(const cpu_reorder_pd_t *pd,
		...
		constexpr int blksize = tag_traits<tag_o>::inner_blks == ib::_4b4c ? 4 : tag_traits<tag_o>::inner_blks == ib::_2c8b4c ? 8 :
																																16;

		const auto &_g_oihw_d = order_keep ? input_d : output_d;
		const auto &dims = input_d.dims();
		const auto &pdims = order_keep ? output_d.padded_dims() : input_d.padded_dims();
			...
			for (int ic = 0; ic < ic_block; ++ic) {
				for (int oc = 0; oc < oc_block; ++oc) {
					const auto _g_oihw_off =
							oc * _g_oihw_d.blocking_desc().strides[w_groups + 0] + ic * _g_oihw_d.blocking_desc().strides[w_groups + 1];
...
#define wei_blk_off(md, g, o, i, h, w) \
	(is_1d ? (md).blk_off<!w_groups>(g, o, i, w) : (md).blk_off<!w_groups>(g, o, i, h, w))
...
template <SIMPLE_REORDER_TEMPL_DECL>
struct simple_reorder_impl<SIMPLE_REORDER_TEMPL_CALL,
		typename utils::enable_if<false || (tag_i == goiw && tag_o == Goiw16g) || (tag_i == goihw && tag_o == Goihw16g), spec::conv_s8s8>::type> {
	static bool is_applicable(const memory_desc_wrapper &input_d,
			const memory_desc_wrapper &output_d, const primitive_attr_t *attr) {
		const size_t D_mask = utils::array_product(input_d.dims(),
				math::ilog2q(attr->output_scales_.mask_ + 1));
		...
		return true && order_keep && input_d.matches_tag(tag_i) && output_d.matches_tag(tag_o) && (output_d.extra().flags & memory_extra_flags::compensation_conv_s8s8) && (input_d.data_type() == f32 || input_d.data_type() == s8) && output_d.data_type() == s8 && (D_mask == 1 || D_mask == (size_t)g * oc);
	}
...
template <SIMPLE_REORDER_TEMPL_DECL>
struct simple_reorder_impl<SIMPLE_REORDER_TEMPL_CALL,
		typename utils::enable_if<false || (tag_i == nCdhw8c && tag_o == nCdhw16c) || (tag_i == nChw8c && tag_o == nChw16c) || (tag_i == nCw8c && tag_o == nCw16c)>::type> {
	static bool is_applicable(const memory_desc_wrapper &input_d,
			const memory_desc_wrapper &output_d, const primitive_attr_t *attr) {
		return simple_fmt_check(order_keep, tag_i, tag_o, input_d, output_d) && simple_attr_check(attr, false);
	}
...
#define data_blk_off(md, n, c, d, h, w)                                    \
	(is_1d ? (md).blk_off(n, c, w) : is_3d ? (md).blk_off(n, c, d, h, w) : \
											 (md).blk_off(n, c, h, w))
...
#define PLAIN_TO_BLOCKED_IS_APPLICABLE()                                                                                                                               \
	static bool is_applicable(const memory_desc_wrapper &input_d,                                                                                                      \
			const memory_desc_wrapper &output_d, const primitive_attr_t *attr) {                                                                                       \
		return simple_attr_check(attr, false) && (order_keep ? output_d.matches_tag(tag_o) && input_d.is_plain() : input_d.matches_tag(tag_o) && output_d.is_plain()); \
	}

template <SIMPLE_REORDER_TEMPL_DECL>
struct simple_reorder_impl<SIMPLE_REORDER_TEMPL_CALL,
		typename utils::enable_if<tag_i == any && (tag_traits<tag_o>::block_dims == bd::_A || tag_traits<tag_o>::block_dims == bd::_B) && tag_traits<tag_o>::ndims >= 3 && tag_traits<tag_o>::ndims <= 6>::type> {
	...
	static status_t execute(const cpu_reorder_pd_t *pd,
		...
		constexpr int blksize = false ? 0 : utils::one_of(tag_traits<tag_o>::inner_blks, ib::_4a, ib::_4b) ? 4 :
			...
			if (alpha == 1.0 && beta == 0.0) {
				for (int l = 0; l < L; ++l)
					for (int blk = 0; blk < block; ++blk) {
						const dim_t flat_off = 0 + blk * flat_d.blocking_desc().strides[blk_idx] + l * flat_d.blocking_desc().strides[ndims - 1];
						if (order_keep) {
...
#define off(md, h0, h1, m0, m1, m2)                                                              \
	(ndims >= 6 ? (md).blk_off(h0, h1, m0, m1, m2) : ndims >= 5 ? (md).blk_off(h0, h1, m1, m2) : \
...
template <SIMPLE_REORDER_TEMPL_DECL>
struct simple_reorder_impl<SIMPLE_REORDER_TEMPL_CALL,
		typename utils::enable_if<tag_i == any && (tag_traits<tag_o>::block_dims == bd::_AB || tag_traits<tag_o>::block_dims == bd::_BC) && IMPLICATION(tag_traits<tag_o>::block_dims == bd::_AB, tag_traits<tag_o>::ndims >= 3 && tag_traits<tag_o>::ndims <= 5) && IMPLICATION(tag_traits<tag_o>::block_dims == bd::_BC, tag_traits<tag_o>::ndims >= 4 && tag_traits<tag_o>::ndims <= 6)>::type> {
	PLAIN_TO_BLOCKED_IS_APPLICABLE();
	...
	static status_t execute(const cpu_reorder_pd_t *pd,
		...
		constexpr int blksize_0 = false ? 0 : utils::one_of(tag_traits<tag_o>::inner_blks, ib::_4b4a, ib::_4b4c, ib::_4c4b)																														   ? 4 :
									  utils::one_of(tag_traits<tag_o>::inner_blks, ib::_8a8b, ib::_8b8a, ib::_8b8c, ib::_8c8b, ib::_2c8b4c)																										   ? 8 :
									  utils::one_of(tag_traits<tag_o>::inner_blks, ib::_16a16b, ib::_16a4b, ib::_16b16a, ib::_16b4c, ib::_16b16c, ib::_16c16b, ib::_8a16b2a, ib::_4b16a4b, ib::_8b16a2b, ib::_8b16c2b, ib::_4c16b4c, ib::_8c16b2c) ? 16 :
																																																													 INT_MIN;
			...
			if (alpha == 1.0 && beta == 0.0) {
				for (int h0 = 0; h0 < block_h0; ++h0)
					for (int h1 = 0; h1 < block_h1; ++h1) {
						const dim_t flat_off = 0 + h0 * flat_d.blocking_desc().strides[with_g + 0] + h1 * flat_d.blocking_desc().strides[with_g + 1];
						if (order_keep) {
...
#define off(md, g, h0, h1, m0, m1, m2)                                                                                                     \
	(ndims >= 5 + with_g ? (md).blk_off<!with_g>(g, h0, h1, m0, m1, m2) : ndims >= 4 + with_g ? (md).blk_off<!with_g>(g, h0, h1, m1, m2) : \
...
struct simple_reorder_impl<SIMPLE_REORDER_TEMPL_CALL,
	...
	static bool is_applicable(const memory_desc_wrapper &input_d,
		...
		return input_d.similar_to(output_d, true, false, 0) && input_d.is_dense() && output_d.is_dense() && simple_attr_check(attr, false);
	}
		...
		parallel(0, [&](const int ithr, const int nthr) {
			...
			if (alpha == 1.0 && beta == 0.0) {
				...
				for (size_t e = start; e < end; ++e) {
					output[e] = qz_a1b0<data_t<type_i>, data_t<type_o>>()(input[e]);
				}
			} else if (alpha == 1.0) {
				PRAGMA_OMP_SIMD()
				for (size_t e = start; e < end; ++e) {
					output[e] = qz_a1<data_t<type_i>, data_t<type_o>>()(input[e], output[e], beta);
				}
			} else if (beta == 0.0) {
				PRAGMA_OMP_SIMD()
				for (size_t e = start; e < end; ++e) {
					output[e] = qz_b0<data_t<type_i>, data_t<type_o>>()(input[e], alpha);
				}
			} else {
				PRAGMA_OMP_SIMD()
				for (size_t e = start; e < end; ++e) {
					output[e] = qz<data_t<type_i>, data_t<type_o>>()(input[e], output[e], alpha, beta);
				}
			}
...
struct simple_reorder_impl<SIMPLE_REORDER_TEMPL_CALL,
	...
	static bool is_applicable(const memory_desc_wrapper &input_d,
		...
		/* FIXME: is the formula correct? */
		return input_d.similar_to(output_d, true, false, 1) && is_dense_no_0(input_d) && is_dense_no_0(output_d) && simple_attr_check(attr, false);
	}
		...
		if (alpha == 1.0 && beta == 0.0) {
			parallel(0, [&](const int ithr, const int nthr) {
				...
				while (start < end) {
					...
					dim_t dim1_e = dim1_s + work_rem > nelems_no_d0 ? nelems_no_d0 : dim1_s + work_rem;
					PRAGMA_OMP_SIMD()
			...
			parallel(0, [&](const int ithr, const int nthr) {
				...
				while (start < end) {
					dim_t work_rem = end - start;
					dim_t dim1_e =
							dim1_s + work_rem > nelems_no_d0 ? nelems_no_d0 : dim1_s + work_rem;
					PRAGMA_OMP_SIMD()
					for (dim_t e = dim1_s; e < dim1_e; ++e) {
...
struct simple_reorder_impl<SIMPLE_REORDER_TEMPL_CALL,
	...
	static bool is_applicable(const memory_desc_wrapper &input_d,
		...
		for (; smask > 0 && smask & 0x1; smask >>= 1)
			;
		return true && input_d.is_blocking_desc() && output_d.is_blocking_desc() && !output_d.is_additional_buffer() && !input_d.is_additional_buffer() && smask == 0;
	}
	...
	static status_t execute(const cpu_reorder_pd_t *pd,
		...
		const ptrdiff_t D_start = utils::array_product(input_d.dims(), ndims_start);
		const ptrdiff_t D_mask = utils::array_product(input_d.dims() + ndims_start, ndims_mask);
...
struct simple_reorder_t : public cpu_primitive_t {
	struct pd_t : public cpu_reorder_pd_t {
		...
		static status_t create(reorder_pd_t **reorder_pd,
			...
			bool args_ok = true && src_md->data_type == type_i && dst_md->data_type == type_o && simple_reorder_impl<SIMPLE_REORDER_TEMPL_CALL, spec>::is_applicable(src_md, dst_md, attr);
			if (!args_ok)
