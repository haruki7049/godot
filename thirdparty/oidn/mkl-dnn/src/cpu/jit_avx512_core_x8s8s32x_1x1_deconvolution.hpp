...
struct jit_avx512_core_x8s8s32x_1x1_deconvolution_fwd_t
	...
	struct pd_t : public cpu_deconvolution_fwd_pd_t {
		...
		pd_t(const pd_t &other) : cpu_deconvolution_fwd_pd_t(other), conv_pd_(other.conv_pd_->clone()) {}
		...
		status_t init() {
			bool ok = true && is_fwd() && desc()->alg_kind == alg_kind::deconvolution_direct && !has_zero_dim_memory() && desc()->src_desc.data_type == src_type && desc()->dst_desc.data_type == dst_type && desc()->weights_desc.data_type == data_type::s8 && IMPLICATION(with_bias(), utils::one_of(desc()->bias_desc.data_type, data_type::f32, data_type::s32, data_type::s8, data_type::u8)) && desc()->accum_data_type == data_type::s32;
			if (!ok)
	...
	protected:
		...
		using conv_pd_t = typename jit_avx512_core_x8s8s32x_1x1_convolution_fwd_t<src_type, dst_type>::pd_t;
		friend jit_avx512_core_x8s8s32x_1x1_deconvolution_fwd_t;
		primitive_desc_t *conv_pd_;
	};

	jit_avx512_core_x8s8s32x_1x1_deconvolution_fwd_t(const pd_t *apd) : cpu_primitive_t(apd) { pd()->conv_pd_->create_primitive((primitive_t **)&conv_p_); }
