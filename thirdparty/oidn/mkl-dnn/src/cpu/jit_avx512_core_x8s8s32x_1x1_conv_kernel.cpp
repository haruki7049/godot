	...
	L(bcast_loop);
	{
		...
		for (int i = 0; i < num_substeps; i++) {
			...
			if (i < num_substeps - 1) {
				...
				int output_offset = jcp.bcast_loop_output_step - (num_substeps - 1) * jcp.bcast_loop_output_substep;
...
void jit_avx512_core_x8s8s32x_1x1_conv_kernel::reduce_loop(int load_loop_blk,
	...
	auto fma_block = [=](bool last_block) {
		...
		int loop_unroll = last_block && jcp.ic != jcp.ic_without_padding ? rnd_up(jcp.ic_without_padding % jcp.ic_block, reduce_step) : jcp.reduce_loop_unroll;
		for (int i_reduce = 0; i_reduce < loop_unroll;
				i_reduce += reduce_step) {
			for (int i_load = 0; i_load < load_loop_blk; ++i_load)
				vmovups(vreg_load(i_load), load_ptr(i_reduce, i_load));
			for (int i_ur = 0; i_ur < ur; ++i_ur) {
				if (last_block && tail_size != 0 && i_reduce == loop_unroll - reduce_step) {
					Xmm xmm_bcast = Xmm(zmm_bcast.getIdx());
					for (int r = 0; r < tail_size; ++r)
						vpinsrb(xmm_bcast, xmm_bcast, ptr[aux_reg_bcast_data + jcp.ic_without_padding * i_ur + i_reduce + r], r);
...
status_t jit_avx512_core_x8s8s32x_1x1_conv_kernel::init_conf(
	...
	if (!one_of(src_d.data_type(), data_type::u8, data_type::s8) || weights_d.data_type() != data_type::s8 || !one_of(dst_d.data_type(), data_type::f32, data_type::s32, data_type::s8, data_type::u8))
		return status::unimplemented;
	...
	int min_regs = 6;
	if (jcp.ver == ver_vnni)
		max_regs = ((jcp.oh > size_treshold && jcp.ow > size_treshold) && (jcp.oc < 128 || jcp.ic < 128)) ? min_regs : 9;
	else
		max_regs = 8;
	jcp.expl_bcast = true;

	if (jcp.mb == 1 && jcp.ic > 128 && (jcp.oh <= size_treshold && jcp.ow <= size_treshold)) {
	...
	} else if (jcp.bcast_dim <= SMALL_SPATIAL && jcp.mb <= nthreads && jcp.load_dim > 512 && jcp.load_dim / jcp.reduce_dim >= 4) {
	...
	if (jcp.mb == 1 && jcp.nb_load % 4 == 0 && jcp.ic / jcp.oc >= 4 && jcp.ic * jcp.oc <= L2_size) {
		jcp.nb_load_chunk = 4;
