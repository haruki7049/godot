...
struct ref_shuffle_t : public cpu_primitive_t {
	...
	struct pd_t : public cpu_shuffle_pd_t {
		...
		status_t init() {
			...
			bool ok = true && data_type_size == types::data_type_size(data_md()->data_type);
			if (!ok)
	...
	ref_shuffle_t(const pd_t *apd) : cpu_primitive_t(apd) {
		...
		const int transpose_col = pd()->is_fwd() ? axis_size / group_size : group_size;
	...
	virtual status_t execute(const exec_ctx_t &ctx) const override {
		...
		switch (pd()->dat_tag_) {
			case nCdhw16c:
				execute_<nCdhw16c>(ctx);
				break;
			case nChw16c:
				execute_<nChw16c>(ctx);
				break;
			case nCdhw8c:
				execute_<nCdhw8c>(ctx);
				break;
			case nChw8c:
				execute_<nChw8c>(ctx);
				break;
