...
void ncsp_batch_normalization_fwd_t::execute_forward(
	...
	auto maybe_post_op = [&](data_t res) { return (with_relu && res < 0) ? 0 : res; };
	...
	parallel(0, [&](const int ithr, const int nthr) {
		...
		bool spatial_thr_allowed = bnorm_utils::thread_balance(do_blocking, true, ithr, nthr, N,
		...
		for (int64_t it = 0; it < iters; ++it) {
			...
			if (calculate_stats) {
				...
				for (dim_t c = C_blk_s; c < C_blk_e; c++) {
					...
					ws_reduce[ws_iter_off + SP_N_ithr * C_blks_per_iter + c] = sum;
				}
				...
				for (dim_t c = C_blk_gl_s; c < C_blk_gl_e; c++) {
					...
					for (dim_t n = 0; n < SP_N_nthr; n++)
						mean_blk[c] += ws_reduce[ws_iter_off + n * C_blks_per_iter + c];
					mean_blk[c] /= (N * SP);
				}
				...
				for (dim_t c = C_blk_s; c < C_blk_e; c++) {
					...
					for (dim_t n = N_s; n < N_e; ++n)
						PRAGMA_OMP_SIMD(reduction(+ : sum))
					for (dim_t sp = S_s; sp < S_e; ++sp) {
						data_t m = src[off * SP + n * C * SP + sp] - mean[off];
						sum += m * m;
					}
					ws_reduce[ws_iter_off + SP_N_ithr * C_blks_per_iter + c] = sum;
				}
				...
				for (dim_t c = C_blk_gl_s; c < C_blk_gl_e; c++) {
					variance_blk[c] = 0.;
					for (dim_t n = 0; n < SP_N_nthr; n++)
						variance_blk[c] += ws_reduce[ws_iter_off + n * C_blks_per_iter + c];
					variance_blk[c] /= (N * SP);
				}
			...
			for (dim_t c = C_blk_s; c < C_blk_e; c++) {
				size_t off = c + C_off;
				data_t sqrt_variance = static_cast<data_t>(sqrtf(variance[off] + eps));
				...
				for (dim_t sp = S_s; sp < S_e; ++sp) {
					...
					data_t bn_res = sm * (src[d_off] - mean[off]) + sv;
					if (fuse_bn_relu) {
						if (bn_res <= 0) {
							bn_res = 0;
							if (is_training)
	...
	parallel(0, [&](const int ithr, const int nthr) {
		...
		for (int64_t it = 0; it < iters; ++it) {
			...
			for (dim_t c = C_blk_s; c < C_blk_e; c++) {
				...
				ws_reduce[ws_iter_off + SP_N_ithr * C_blks_per_iter + c] = diff_gamma;
				ws_reduce[ws_iter_off + SP_N_nthr * C_blks_per_iter + SP_N_ithr * C_blks_per_iter + c] = diff_beta;
			}
			...
			for (dim_t c = C_blk_gl_s; c < C_blk_gl_e; c++) {
				...
				diff_beta_blk[c] = 0.;
				for (dim_t n = 0; n < SP_N_nthr; n++) {
					diff_gamma_blk[c] += ws_reduce[ws_iter_off + n * C_blks_per_iter + c];
					diff_beta_blk[c] += ws_reduce[ws_iter_off + SP_N_nthr * C_blks_per_iter + n * C_blks_per_iter + c];
				}
			...
			for (dim_t c = C_blk_s; c < C_blk_e; c++) {
				...
				data_t sqrt_variance = static_cast<data_t>(1.0f / sqrtf(variance[off] + eps));
				data_t v_mean = mean[off];
				for (dim_t n = N_s; n < N_e; ++n)
#if SAFE_TO_USE_OMP_SIMD
					PRAGMA_OMP_SIMD()
				...
				for (dim_t sp = S_s; sp < S_e; ++sp) {
					...
					if (calculate_diff_stats) {
						v_diff_src -= diff_beta_blk[c] / (SP * N) + (src[d_off] - v_mean) * diff_gamma_blk[c] * sqrt_variance / (SP * N);
					}
