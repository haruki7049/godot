...
struct reduce_balancer_t {
	...
	reduce_balancer_t(int nthr, int job_size, int njobs, int reduction_size,
			size_t max_buffer_size) { init(nthr, job_size, njobs, reduction_size, max_buffer_size); }
...
private:
	static size_t space_per_thread(const reduce_balancer_t &balancer) { return balancer.njobs_per_group_ub_ * balancer.job_size_; }
