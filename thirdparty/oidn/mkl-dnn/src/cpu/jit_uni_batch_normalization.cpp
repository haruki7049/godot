...
#undef PARAM_OFF
	...
	void prepare_relu() {
		with_relu = bdesc_->is_fwd() ? bdesc_->with_relu_post_op() || bdesc_->fuse_bn_relu() : bdesc_->fuse_bn_relu();
		with_relu_inf_only = with_relu && bdesc_->is_fwd() && !(bdesc_->fuse_bn_relu() && bdesc_->is_training());
	...
	Address diff_gamma_ptr(size_t offt = 0) {
		return vmmword[reg_diff_scale_shift + reg_coff + offt + 0 * chan_data_offt];
	}

	Address diff_beta_ptr(size_t offt = 0) {
		return vmmword[reg_diff_scale_shift + reg_coff + offt + 1 * chan_data_offt];
	}
	...
	void mean_channels() {
		...
		L(ch_label);
		{
			...
			spat_loop(spat_size, unroll_blocks, unroll_regs, [=](size_t base_reg) {
                        Vmm v = Vmm(base_reg * 2);
                        if (base_reg)
                            uni_vpxor(v, v, v); }, [=](size_t base_reg, size_t i) {
	...
	void forward_channels() {
		...
		L(ch_label);
		{
			...
			auto compute = [=](bool output_is_aligned) {
				spat_loop(spat_size, unroll_blocks, unroll_regs, [](size_t base_reg) { UNUSED(base_reg); }, [=](size_t base_reg, size_t i) {
                             Vmm v = Vmm(base_reg);
                             size_t offt = i * vlen;
                             uni_vmovups(v,
                                 vmmword[reg_src + reg_soff + offt]);
			...
			auto compute = [=](bool output_is_aligned) {
				spat_loop(spat_size, unroll_blocks, unroll_regs, [=](size_t base_reg) { UNUSED(base_reg); }, [=](size_t base_reg, size_t i) {
                            ...
                            mic_prefetcht1(ptr[reg_diff_dst + reg_soff
                                    + offt + t1_pf_offt]);
                            mic_prefetcht1(ptr[reg_src + reg_soff + offt
                                    + t1_pf_offt]); }, [=](size_t base_reg) { UNUSED(base_reg); });
			};
	...
	jit_bnorm_t(const batch_normalization_pd_t *bdesc) : bdesc_(bdesc) {
		static_assert(isa == sse42 || isa == avx2 || isa == avx512_common || isa == avx512_mic, "unsupported isa");
...
struct uni_bnorm_driver_t : public c_compatible {
	uni_bnorm_driver_t(const batch_normalization_pd_t *bdesc) : bdesc_(bdesc), ker_(bdesc_) {
		const int nthrs = mkldnn_get_max_threads();
		const dim_t C_PADDED = get_c_padded(bdesc_);

		size_t data_size = sizeof(data_t) * bdesc_->MB() * C_PADDED * bdesc_->D() * bdesc_->H() * bdesc_->W();
	...
	void exec(int ithr, int nthr, const data_t *src, data_t *diff_src,
		...
		if (do_blocking_) {
			...
			size_t working_set_size = (N * D * H * W * simd_w * sizeof(data_t)) * num_tensors;
		...
		for (int64_t it = 0; it < iters; it++) {
			...
			size_t soff_base = global_C_blk_s * p.spat_size * simd_w + N_s * img_size;
			...
			// the last iteration.
			p.rbuf1 = rbuf + ((it * C_blks_per_iter) * SP_N_nthr + C_blk_s * p.N_nthr + p.N_ithr * C_blks_thr) * simd_w;
			// rbuf1 and rbuf2 have to be disjoint
			p.rbuf2 = p.rbuf1 + C_PADDED * nthr;
			p.is_cblk_tail = (it * C_blks_per_iter + C_blk_e) * simd_w > C;

			size_t iter_bariers = do_blocking_ ? it * global_barriers_per_iter : 0;
...
private:
	...
	static bool use_tmp_stats(const batch_normalization_pd_t *bdesc) {
		return true && !bdesc->stats_is_src() && bdesc->desc()->prop_kind == prop_kind::forward_inference;
	}

	static bool use_tmp_diff_scale_shift(const batch_normalization_pd_t *bdesc) {
		return false || (bdesc->is_bwd() && !bdesc->use_scaleshift()) || bdesc->desc()->prop_kind == prop_kind::backward_data;
	}

	static dim_t get_c_padded(const batch_normalization_pd_t *bdesc) { return bdesc->src_md()->padded_dims[1]; }
...
status_t jit_uni_batch_normalization_fwd_t<isa>::pd_t::init() {
	auto desired_fmt_tag = (ndims() == 4) ? isa == avx512_common ? nChw16c : nChw8c : isa == avx512_common ? nCdhw16c :
																											 nCdhw8c;

	bool ok = true && mayiuse(isa) && is_fwd() && !has_zero_dim_memory() && one_of(ndims(), 4, 5) && src_md()->data_type == f32 && IMPLICATION(use_scaleshift(), weights_md()->data_type == f32) && memory_desc_matches_tag(*src_md(), desired_fmt_tag) && (attr()->has_default_values() || this->with_relu_post_op());
	if (!ok)
...
status_t jit_uni_batch_normalization_fwd_t<isa>::execute(
	...
	auto mean = pd()->stats_is_src() ? const_cast<data_t *>(CTX_IN_MEM(const data_t *, MKLDNN_ARG_MEAN)) : CTX_OUT_MEM(data_t *, MKLDNN_ARG_MEAN);
	auto var = pd()->stats_is_src() ? const_cast<data_t *>(CTX_IN_MEM(const data_t *, MKLDNN_ARG_VARIANCE)) : CTX_OUT_MEM(data_t *, MKLDNN_ARG_VARIANCE);
