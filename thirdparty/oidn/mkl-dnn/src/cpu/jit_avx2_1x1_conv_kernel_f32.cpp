...
void jit_avx2_1x1_conv_kernel_f32::generate_reduce_loop(
	...
	auto load_ptr = [=](int u, int i) {
		...
		return ptr[aux_reg_load_data + u1 * jcp.reduce_loop_load_step + sizeof(float) * offt];
	};
...
void jit_avx2_1x1_conv_kernel_f32::generate_diff_bias_loop(int load_loop_blk) {
	...
	auto load_ptr = [=](int u, int i) {
		return ptr[aux_reg_load_data + (i * jcp.os + u) * jcp.oc_block * sizeof(float)];
	};
...
status_t jit_avx2_1x1_conv_kernel_f32::init_conf(jit_1x1_conv_conf_t &jcp,
	...
	args_ok = true && jcp.ih == jcp.oh && jcp.iw == jcp.ow && jcp.oc % simd_w == 0 && jcp.ic % simd_w == 0 && jcp.t_pad == 0 && jcp.l_pad == 0 && jcp.stride_w == 1 && jcp.stride_h == 1 // TODO: support some strides
			  && jcp.kh == 1 && jcp.kw == 1;
	if (!args_ok)
		return status::unimplemented;
