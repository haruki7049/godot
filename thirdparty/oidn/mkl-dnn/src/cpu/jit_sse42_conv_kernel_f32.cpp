	...
	for (int ki = 0; ki < kw; ki++) {
		...
		for (int ifm2 = 0; ifm2 < ic_blk; ifm2++) {
			...
			for (int ii = 0; ii < oc_blocks; ii++) {
				int ker_off = ii * nb_ic * kh * kw * ic_blk * oc_blk + ki * ic_blk * oc_blk + ifm2 * oc_blk;
...
void jit_sse42_conv_fwd_kernel_f32::width_blk_step(int ur_w,
	...
	int ic_blk = jcp.ic_block;
	int oc_blk = jcp.oc_block;
	const int inp_mult = one_of(jcp.src_tag, ncw, nchw) ? dilate_h : ic_blk * dilate_h;
	const int inp_off = one_of(jcp.src_tag, ncw, nchw) ? dilate_w : ic_blk * dilate_w;
	...
	for (int ii = 0; ii < oc_blocks; ii++)
		for (int jj = 0; jj < ur_w; jj++)
			movups(Xmm(ur_w * ii + jj + 1), xword[reg_output + sizeof(float) * (ii * oh * ow + jj) * oc_blk]);
...
status_t jit_sse42_conv_fwd_kernel_f32::init_conf(jit_conv_conf_t &jcp,
	...
	bool args_ok = true && IMPLICATION(flat, one_of(jcp.src_tag, ncw, nwc, nchw, nhwc) && one_of(jcp.wei_tag, Owi8o, gOwi8o, Ohwi8o, gOhwi8o)) && IMPLICATION(mimo, one_of(jcp.src_tag, nCw8c, nChw8c) && one_of(jcp.wei_tag, OIw8i8o, gOIw8i8o, OIhw8i8o, gOIhw8i8o)) && one_of(jcp.dst_tag, nCw8c, nChw8c);
	if (!args_ok)
		return status::unimplemented;
