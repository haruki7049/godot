...
struct ref_softmax_fwd_t : public cpu_primitive_t {
	...
	ref_softmax_fwd_t(const pd_t *apd) : cpu_primitive_t(apd) {
		...
		use_dense_ = inner_size_ == 1 && data_d.is_dense() && no_axis_blocking && data_d.blocking_desc().strides[axis] == 1;
	}
...
struct ref_softmax_bwd_t : public cpu_primitive_t {
	struct pd_t : public cpu_softmax_bwd_pd_t {
		...
		status_t init() {
			bool ok = true && !is_fwd() && utils::everyone_is(data_type, dst_md()->data_type, diff_src_md()->data_type) && attr()->has_default_values();
			if (!ok)
				return status::unimplemented;
	...
	ref_softmax_bwd_t(const pd_t *apd) : cpu_primitive_t(apd) {
		...
		use_dense_ = true && inner_size_ == 1 && diff_d == data_d && diff_d.is_dense() && no_axis_blocking && diff_d.blocking_desc().strides[axis] == 1;
	}
