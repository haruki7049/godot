...
struct ref_pooling_fwd_t : public cpu_primitive_t {
	struct pd_t : public cpu_pooling_fwd_pd_t {
		...
		status_t init() {
			bool ok = true && set_default_params() == status::success && is_fwd() && utils::everyone_is(data_type, src_md()->data_type, dst_md()->data_type) && desc()->accum_data_type == acc_type && attr()->has_default_values();
			if (!ok)
...
struct ref_pooling_bwd_t : public cpu_primitive_t {
	struct pd_t : public cpu_pooling_bwd_pd_t {
		...
		status_t init() {
			bool ok = true && set_default_params() == status::success && !is_fwd() && utils::everyone_is(data_type, diff_dst_md()->data_type, diff_src_md()->data_type) && attr()->has_default_values();
			if (!ok)
				return status::unimplemented;
