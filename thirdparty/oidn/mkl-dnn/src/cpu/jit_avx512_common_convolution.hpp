...
struct jit_avx512_common_convolution_fwd_t : public cpu_primitive_t {
	struct pd_t : public cpu_convolution_fwd_pd_t {
		...
		status_t init() {
			bool ok = true && is_fwd() && set_default_alg_kind(alg_kind::convolution_direct) && expect_data_types(src_type, wei_type, dst_type, dst_type, data_type::undef) && !has_zero_dim_memory();
			if (!ok)
	...
	jit_avx512_common_convolution_fwd_t(const pd_t *apd) : cpu_primitive_t(apd) {
...
struct jit_avx512_common_convolution_bwd_data_t : public cpu_primitive_t {
	...
	jit_avx512_common_convolution_bwd_data_t(const pd_t *apd) : cpu_primitive_t(apd) { kernel_ = new jit_avx512_common_conv_bwd_data_kernel_f32(pd()->jcp_); }
...
struct jit_avx512_common_convolution_bwd_weights_t : public cpu_primitive_t {
	struct pd_t : public cpu_convolution_bwd_weights_pd_t {
		...
		status_t init() {
			bool ok = true && desc()->prop_kind == prop_kind::backward_weights && set_default_alg_kind(alg_kind::convolution_direct) && expect_data_types(src_type, diff_weights_type, diff_weights_type, diff_dst_type, data_type::undef) && !has_zero_dim_memory();
			if (!ok)
				return status::unimplemented;
