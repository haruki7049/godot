		...
		if (calculate_stats) {
			...
			for (dim_t n = N_s; n < N_e; n++)
				for (dim_t sp = 0; sp < SP; sp++)
					PRAGMA_OMP_SIMD()
			for (dim_t c = 0; c < C; c++)
				ws_reduce[C * ithr + c] += src[(size_t)n * SP * C + sp * C + c];
			...
			for (dim_t n = N_s; n < N_e; n++)
				for (dim_t sp = 0; sp < SP; sp++)
					PRAGMA_OMP_SIMD()
			for (dim_t c = 0; c < C; c++) {
				data_t m = src[(size_t)n * SP * C + sp * C + c] - mean_loc[c];
...
void nspc_batch_normalization_bwd_t::execute_backward(
	...
	parallel(0, [&](const int ithr, const int nthr) {
		...
		for (dim_t c = C_s; c < C_e; c++) {
			data_t sqrt_variance = static_cast<data_t>(1.0f / sqrtf(variance[c] + eps));
			diff_gamma[c] = 0;
			diff_beta[c] = 0;
			for (dim_t n = 0; n < nthr; n++) {
			...
			for (dim_t sp = 0; sp < SP; sp++) {
				...
				for (dim_t c = 0; c < C; c++) {
					...
					if (calculate_diff_stats) {
						v_diff_src -= diff_beta_loc[c] / (SP * N) + (src[d_off] - mean[c]) * diff_gamma_loc[c] * sqrt_variance / (SP * N);
					}
