	...
	parallel(jcp.nthr, [&](const int ithr, const int nthr) {
		...
		for (size_t iwork = start; iwork < end; ++iwork) {
			...
			for (int od = 0; od < jcp.od; ++od) {
				const data_t *_diff_dst = diff_dst + (n * jcp.ngroups + g) * dst_step + od * m;
...
void gemm_convolution_bwd_weights_t::execute_backward_weights(
	...
	parallel(jcp.nthr, [&](const int ithr, const int nthr) {
		...
		if (ithr_g != -1 && ithr_mb != -1) {
			...
			data_t *_col = col + (ptrdiff_t)ithr * jcp.im2col_sz;
			data_t *weights_reduce_base = wei_reduction + ithr_g * nthr_mb * weights_g_size;
			data_t *weights_reduce = weights_reduce_base + ithr_mb * weights_g_size;

			for (size_t g = g_start; g < g_end; ++g) {
				data_t *_diff_weights = need_reduction ? weights_reduce : (diff_weights + g * weights_g_size);
				for (size_t mb = mb_start; mb < mb_end; ++mb) {
					const data_t *_src = src + (mb * jcp.ngroups + g) * src_step;
					for (int od = 0; od < jcp.od; ++od) {
						const data_t *_diff_dst = diff_dst + (mb * jcp.ngroups + g) * dst_step + od * k;
