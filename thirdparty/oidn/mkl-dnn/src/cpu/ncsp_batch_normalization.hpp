...
struct ncsp_batch_normalization_fwd_t : public cpu_primitive_t {
	struct pd_t : public cpu_batch_normalization_fwd_pd_t {
		...
		status_t init() {
			...
			bool ok = true && is_fwd() && !has_zero_dim_memory() && src_md()->data_type == f32 && IMPLICATION(use_scaleshift(), weights_md()->data_type == f32) && memory_desc_matches_one_of_tag(*src_md(), ncdhw, nchw, nc) && (attr()->has_default_values() || this->with_relu_post_op());
			if (!ok)
...
struct ncsp_batch_normalization_bwd_t : public cpu_primitive_t {
	struct pd_t : public cpu_batch_normalization_bwd_pd_t {
		...
		status_t init() {
			...
			bool ok = true && is_bwd() && !has_zero_dim_memory() && utils::everyone_is(f32, src_md()->data_type, diff_src_md()->data_type) && IMPLICATION(use_scaleshift(), utils::everyone_is(f32, weights_md()->data_type, diff_weights_md()->data_type)) && memory_desc_matches_one_of_tag(*src_md(), ncdhw, nchw, nc) && memory_desc_matches_one_of_tag(*diff_src_md(), ncdhw, nchw, nc) && attr()->has_default_values();
			if (!ok)
				return status::unimplemented;
