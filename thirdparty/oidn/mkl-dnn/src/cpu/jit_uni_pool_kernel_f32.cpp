...
status_t jit_uni_pool_kernel_f32<isa>::init_conf(jit_pool_conf_t &jpp,
	...
	bool args_ok = true && mayiuse(isa) && utils::one_of(pd.alg_kind, pooling_max, pooling_avg_include_padding, pooling_avg_exclude_padding);
	if (!args_ok)
	...
	jpp.ind_dt = ppd->workspace_md() ? ppd->workspace_md()->data_type : data_type::undef;

	jpp.simple_alg = jpp.is_training || IMPLICATION(jpp.is_backward, jpp.kd <= jpp.stride_d);
...
inline void jit_uni_pool_kernel_f32<isa>::avg_step(int ur_w, int pad_l,
	...
	L(kh_label);
	{
		for (int ki = 0; ki < kw; ki++) {
			...
			int jj_end = ur_w - utils::div_up(nstl::max(0, ki + pad_r - (kw - 1)), stride_w);
...
inline void jit_uni_pool_kernel_f32<isa>::max_step_fwd(int ur_w, int pad_l,
	...
	for (int jj = 0; jj < ur_w; jj++) {
		...
		if (jpp.is_training) {
			const size_t step_index = jj * c_block * types::data_type_size(jpp.ind_dt);
...
void jit_uni_pool_kernel_f32<isa>::generate() {
	...
	if (l_pad > 0) {
		...
		if (isa == sse42) {
			...
			if (jpp.alg == pooling_max && (jpp.is_training || jpp.is_backward))
				add(reg_index, (2 * ur_w - 1) * c_block / 2 * types::data_type_size(jpp.ind_dt));
		} else {
			add(reg_input, sizeof(float) * (ur_w * stride_w - l_pad) * c_block);
			add(reg_output, sizeof(float) * ur_w * c_block);
			if (jpp.alg == pooling_max && (jpp.is_training || jpp.is_backward))
				add(reg_index, ur_w * c_block * types::data_type_size(jpp.ind_dt));
		}
	}
