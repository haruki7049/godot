...
struct jit_avx512_common_1x1_convolution_fwd_t : public cpu_primitive_t {
	struct pd_t : public cpu_convolution_fwd_pd_t {
		...
		status_t init() {
			...
			bool ok = true && is_fwd() && set_default_alg_kind(alg_kind::convolution_direct) && expect_data_types(src_type, wei_type, dst_type, dst_type, data_type::undef) && !has_zero_dim_memory() && set_default_formats();
			if (!ok)
	...
	jit_avx512_common_1x1_convolution_fwd_t(const pd_t *apd) : cpu_primitive_t(apd), kernel_(nullptr), rtus_driver_(nullptr) {
		kernel_ =
...
using jit_avx512_common_1x1_convolution_fwd_f32_t = jit_avx512_common_1x1_convolution_fwd_t<data_type::f32>;
...
struct jit_avx512_common_1x1_convolution_bwd_data_t : public cpu_primitive_t {
	struct pd_t : public cpu_convolution_bwd_data_pd_t {
		...
		status_t init() {
			bool ok = true && desc()->prop_kind == prop_kind::backward_data && set_default_alg_kind(alg_kind::convolution_direct) && expect_data_types(diff_src_type, wei_type, data_type::undef, diff_dst_type, data_type::undef) && !has_zero_dim_memory() && set_default_formats();
			if (!ok)
				return status::unimplemented;
	...
	jit_avx512_common_1x1_convolution_bwd_data_t(const pd_t *apd) : cpu_primitive_t(apd), kernel_(nullptr), rtus_driver_(nullptr) {
...
using jit_avx512_common_1x1_convolution_bwd_data_f32_t = jit_avx512_common_1x1_convolution_bwd_data_t<data_type::f32>;
