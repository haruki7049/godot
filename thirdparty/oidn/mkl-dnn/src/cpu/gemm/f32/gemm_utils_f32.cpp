...
void calc_nthr_nocopy_avx(int m, int n, int k,
	...
	if (mkldnn_thr_syncable()) {
		...
		while ((nthr_m * nthr_n * nthr_other < nthr) && (k / (nthr_other + 1) > BK_NOCOPY_AVX)) {
			nthr_other++;
...
void calc_nthr_nocopy_avx512_common(int m,
	...
	if ((nthr_m * nthr_n > nthr)) {
		if (nthr_m <= nthr_n) {
			...
			nthr_n = (int)sqrt((double)nthr);
			if (nthr_n > (n + BN_SMALL_NOCOPY_AVX512_COMMON - 1) / BN_SMALL_NOCOPY_AVX512_COMMON)
				nthr_n = (n + BN_SMALL_NOCOPY_AVX512_COMMON - 1) / BN_SMALL_NOCOPY_AVX512_COMMON;
			nthr_m = nthr / nthr_n;
...
template void sum_two_matrices<float>(int m, int n,
		float *__restrict p_src, dim_t ld_src,
		float *__restrict p_dst, dim_t ld_dst);

template void sum_two_matrices<double>(int m, int n,
