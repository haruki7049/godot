	...
	xbyak_gemm(char isTransA, char isTransB, float beta, bool hasBias = false,
			...
			if (!isTransA) {
				...
				if (isLoad1Unmasked) {
					...
					for (int i = 0; i < 2; i++) {
						...
						vmovups(ptr[AO1 + (unroll_m * (2 * i) + 1 * 4 - OFFSET) * SIZE],
								xmm0);
						vmovups(ptr[AO1 + (unroll_m * (2 * i + 1) + 1 * 4 - OFFSET) * SIZE],
								xmm1);
					}
					...
					auto el_cp = [&](int section, int ld_step) {
						...
						RegExp dst_addr = AO1 + (ld_step + section * 4 - OFFSET) * SIZE;
		...
		auto innerkernel8 = [&](int unroll_m, int unroll_n,
			...
			for (int i = 0; i < 8; i++) {
				...
				if (i == 1 || i == 2) {
					if (unroll_m >= 8) {
						if (!isDirect) {
							prefetcht0(ptr[AO1 + (PREFETCHSIZEA + (2 + 2 * i) * 8) * SIZE]);
						} else {
							prefetcht0(ptr[AO1 + LDA4]);
						}
					}
				...
				if (!isDirect) {
					if (isLoad1Unmasked) {
						vmovups(ymm0,
								ptr[AO1 + (unroll_m * (i + 1) + 0 * 8 - OFFSET) * SIZE]);
					} else {
						vmaskmovps(
								ymm0, VMASK,
								ptr[AO1 + (unroll_m * (i + 1) + 0 * 8 - OFFSET) * SIZE]);
					...
					if (unroll_m >= 16) {
						if (isLoad2Unmasked) {
							vmovups(ymm1, ptr[AO1 + (unroll_m * (i + 1) + 1 * 8 - OFFSET) * SIZE]);
						} else {
							vmaskmovps(ymm1, VMASK,
									ptr[AO1 + (unroll_m * (i + 1) + 1 * 8 - OFFSET) * SIZE]);
						}
					}
				}
			}
		...
		auto innerkernel2 = [&](int unroll_m, int unroll_n,
			...
			for (int i = 0; i < 2; i++) {
				...
				if (!isDirect) {
					if (isLoad1Unmasked) {
						vmovups(ymm0, ptr[AO1 + (unroll_m * 1 + 0 * 8 - OFFSET) * SIZE]);
					} else {
						vmaskmovps(ymm0, VMASK,
		...
		auto innerkernel1 = [&](int unroll_m, int unroll_n,
			...
			if (!isDirect) {
				...
				if (unroll_m >= 16) {
					if (isLoad2Unmasked) {
						vmovups(ymm1, ptr[AO1 + (unroll_m * 1 + 1 * 8 - OFFSET) * SIZE]);
					} else {
						vmaskmovps(ymm1, VMASK,
								ptr[AO1 + (unroll_m * 1 + 1 * 8 - OFFSET) * SIZE]);
					}
		...
		auto kernel = [&](int unroll_m, int unroll_n, bool isLoad1Unmasked,
			...
			if (!isDirect) {
				...
				if (unroll_m >= 16) {
					if (isLoad2Unmasked) {
						vmovups(ymm1, ptr[AO1 + (unroll_m * 0 + 1 * 8 - OFFSET) * SIZE]);
					} else {
						vmaskmovps(ymm1, VMASK,
			...
			for (int i = 0; i < unroll_n; i++) {
				...
				if (isLoad1Unmasked) {
					switch (i) {
						case 0:
							vmovups(ptr[CO1 + 0 * SIZE], Ymm(i + 4));
							break;
						case 1:
						...
						case 2:
							...
							break;
						case 3:
							vmovups(ptr[CO2 + 0 * SIZE], Ymm(i + 4));
							break;
						case 4:
