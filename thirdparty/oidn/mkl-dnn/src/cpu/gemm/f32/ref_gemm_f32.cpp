...
void kernel_mxn(int K, const data_t *A, const dim_t lda,
	...
	data_t c[unroll_factor<data_t>::m * unroll_factor<data_t>::n] = { static_cast<data_t>(0.) };
	for (int k = 0; k < K; k++) {
	...
	for (int j = 0; j < unroll_factor<data_t>::n; j++) {
		PRAGMA_OMP_SIMD()
		for (int i = 0; i < unroll_factor<data_t>::m; i++) {
			C[i + j * ldc] = (beta == static_cast<data_t>(0.)) ? alpha * c[i + unroll_factor<data_t>::m * j] : alpha * c[i + unroll_factor<data_t>::m * j] + beta * C[i + j * ldc];
		}
	}
}
...
void block_ker(const int M, const int N, const int K,
	...
	for (int i = 0; i < M; i++) {
		for (int j = Nu; j < N; j++) {
			data_t c = beta == static_cast<data_t>(0.) ? static_cast<data_t>(0.) : beta * C[i + j * ldc];
			for (int p = 0; p < K; p++) {
...
mkldnn_status_t ref_gemm(
	...
	if (nthr_k > 1) {
		c_buffers = (data_t *)malloc(nthr_m * nthr_n * (nthr_k - 1) * MB * NB * sizeof(data_t), PAGE_4K);
		if (!c_buffers) {
			nthr_k = 1;
			KB = K;
		}
	...
	const size_t ws_size_per_thr = rnd_up(ws_elems_per_thr * sizeof(data_t), PAGE_4K);
	if (do_copy) {
	...
	parallel_nd(nthr, [&](const int ithr) {
		...
		data_t *ws = do_copy ? ws_buffers + ithr * ws_size_per_thr / sizeof(data_t) : nullptr;
		...
		if (myM > 0 && myN > 0) {
			...
			const data_t *myA = isTransA ? &(A[k_from + m_from * lda]) : &(A[m_from + k_from * lda]);
			const data_t *myB = isTransB ? &(B[n_from + k_from * ldb]) : &(B[k_from + n_from * ldb]);
	...
	if (nthr_k > 1) {
		parallel_nd(nthr, [&](const int ithr) {
			...
			for (int ik = 1; ik < nthr_k; ++ik) {
				data_t *myC = c_buffers + MB * ((dim_t)NB * (cbase + ik - 1) + offset);
