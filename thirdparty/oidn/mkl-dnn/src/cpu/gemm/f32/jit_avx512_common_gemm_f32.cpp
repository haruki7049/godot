...
struct xbyak_gemm : public jit_generator {
	...
	xbyak_gemm(char isTransA, char isTransB, float beta, bool hasBias = false,
			...
			size_t code_size = 80 * Xbyak::DEFAULT_MAX_CODE_SIZE) : jit_generator(code_ptr, code_size) {
		using namespace Xbyak;

		enum { ver_avx512_core,
			ver_avx512_mic } ver =
		...
		auto do_pack = [&](int unroll_m) {
			...
			if (!isTransA) {
				for (int i = 0; i < 4; i++) {
					...
					vmovups(ptr[AO1 + (unroll_m * i + 0 * 16 - OFFSET) * SIZE] | k1,
							zmm0);
					if (unroll_m > 16)
						vmovups(ptr[AO1 + (unroll_m * i + 1 * 16 - OFFSET) * SIZE] | k2,
								zmm1);
					if (unroll_m > 32)
						vmovups(ptr[AO1 + (unroll_m * i + 2 * 16 - OFFSET) * SIZE] | k3,
								zmm2);
				}
			} else {
				for (int i = 0; i < 4; i++) {
					...
					vmovups(ptr[AO1 + (unroll_m * i + 0 * 16 - OFFSET) * SIZE],
							zmm0 | k1);
					if (unroll_m > 16)
						vmovups(ptr[AO1 + (unroll_m * i + 1 * 16 - OFFSET) * SIZE],
								zmm1 | k2);
					if (unroll_m > 32)
						vmovups(ptr[AO1 + (unroll_m * i + 2 * 16 - OFFSET) * SIZE],
								zmm2 | k3);
				}
				add(BO1, 4 * SIZE);
			}
					...
					if (!isTransB) {
						switch (i) {
							case 2:
								vbroadcastss(
										zmm3,
										ptr[BO1 + LDB * 2 + (iteration - OFFSET) * SIZE]);
								break;
							case 3:
								vbroadcastss(
										zmm3,
										ptr[BO1 + LDB3 + (iteration - OFFSET) * SIZE]);
								break;
							case 4:
								vbroadcastss(zmm3,
							...
							case 5:
								vbroadcastss(
										zmm3,
										ptr[BO2 + LDB * 1 + (iteration - OFFSET) * SIZE]);
								break;
							case 6:
								vbroadcastss(
										zmm3,
										ptr[BO2 + LDB * 2 + (iteration - OFFSET) * SIZE]);
								break;
							case 7:
								vbroadcastss(
										zmm3,
										ptr[BO2 + LDB3 + (iteration - OFFSET) * SIZE]);
								break;
						}
					} else {
						...
						switch (i) {
							case 2:
								vfmadd231ps(regs[i], zmm0,
										zword_b[BO1 + LDB * 2 + (iteration - OFFSET) * SIZE]);
								if (unroll_m >= 32)
							...
							case 3:
								vfmadd231ps(regs[i], zmm0,
										zword_b[BO1 + LDB3 + (iteration - OFFSET) * SIZE]);
								if (unroll_m >= 32)
									vfmadd231ps(regs[i + 8], zmm1,
											zword_b[BO1 + LDB3 + (iteration - OFFSET) * SIZE]);
								if (unroll_m >= 48)
							...
							case 5:
								vfmadd231ps(regs[i], zmm0,
										zword_b[BO2 + LDB * 1 + (iteration - OFFSET) * SIZE]);
								if (unroll_m >= 32)
									vfmadd231ps(regs[i + 8], zmm1,
											zword_b[BO2 + LDB * 1 + (iteration - OFFSET) * SIZE]);
								if (unroll_m >= 48)
							...
							case 6:
								vfmadd231ps(regs[i], zmm0,
										zword_b[BO2 + LDB * 2 + (iteration - OFFSET) * SIZE]);
								if (unroll_m >= 32)
									vfmadd231ps(regs[i + 8], zmm1,
											zword_b[BO2 + LDB * 2 + (iteration - OFFSET) * SIZE]);
								if (unroll_m >= 48)
							...
							case 7:
								vfmadd231ps(regs[i], zmm0,
										zword_b[BO2 + LDB3 + (iteration - OFFSET) * SIZE]);
								if (unroll_m >= 32)
									vfmadd231ps(regs[i + 8], zmm1,
											zword_b[BO2 + LDB3 + (iteration - OFFSET) * SIZE]);
								if (unroll_m >= 48)
		...
		auto innerkernel = [&](int unroll_m, int unroll_n, bool isDirect,
			...
			for (int i = 0; i < 8; i++) {
				if (!isDirect) {
					prefetcht0(ptr[AO1 + (PREFETCHSIZEA + i * unroll_m + 0 * 16 - OFFSET) * SIZE]);
					if (unroll_m >= 32)
						prefetcht0(ptr[AO1 + (PREFETCHSIZEA + i * unroll_m + 1 * 16 - OFFSET) * SIZE]);
					if (unroll_m >= 48)
						prefetcht0(ptr[AO1 + (PREFETCHSIZEA + i * unroll_m + 2 * 16 - OFFSET) * SIZE]);
				} else {
				...
				if (!isDirect) {
					if (i != 0) {
						if (isUnmasked || unroll_m > 16) {
							vmovups(zmm0,
									ptr[AO1 + (unroll_m * i + 0 * 16 - OFFSET) * SIZE]);
						} else {
							vmovups(zmm0 | k1 | T_z,
									ptr[AO1 + (unroll_m * i + 0 * 16 - OFFSET) * SIZE]);
						}
						if (unroll_m >= 32) {
							if (isUnmasked || unroll_m > 32) {
								vmovups(zmm1, ptr[AO1 + (unroll_m * i + 1 * 16 - OFFSET) * SIZE]);
							} else {
								vmovups(zmm1 | k2 | T_z,
										ptr[AO1 + (unroll_m * i + 1 * 16 - OFFSET) * SIZE]);
							}
						}
						if (unroll_m >= 48) {
							if (isUnmasked) {
								vmovups(zmm2, ptr[AO1 + (unroll_m * i + 2 * 16 - OFFSET) * SIZE]);
							} else {
								vmovups(zmm2 | k3 | T_z,
										ptr[AO1 + (unroll_m * i + 2 * 16 - OFFSET) * SIZE]);
							}
						}
					}
				} else {
					...
					if (!isTransB) {
						switch (i) {
							case 0:
								...
								break;
							case 1:
								prefetcht0(ptr[BO1 + LDB + (PREFETCHSIZEB - OFFSET) * SIZE]);
								break;
							case 2:
								prefetcht0(ptr[BO1 + LDB * 2 + (PREFETCHSIZEB - OFFSET) * SIZE]);
								break;
							case 3:
								prefetcht0(ptr[BO1 + LDB3 + (PREFETCHSIZEB - OFFSET) * SIZE]);
								break;
							case 4:
								prefetcht0(
								...
								break;
							case 5:
								prefetcht0(ptr[BO2 + LDB + (PREFETCHSIZEB - OFFSET) * SIZE]);
								break;
							case 6:
								prefetcht0(ptr[BO2 + LDB * 2 + (PREFETCHSIZEB - OFFSET) * SIZE]);
								break;
							case 7:
								prefetcht0(ptr[BO2 + LDB3 + (PREFETCHSIZEB - OFFSET) * SIZE]);
								break;
						}
					}
				}
				...
				if (isCopy) {
					if (isUnmasked || unroll_m > 16) {
						vmovups(ptr[LDA4 + (unroll_m * i + 0 * 16 - OFFSET) * SIZE],
								zmm0);
					} else {
						vmovups(ptr[LDA4 + (unroll_m * i + 0 * 16 - OFFSET) * SIZE],
								zmm0 | k1);
					...
					if (unroll_m >= 32) {
						if (isUnmasked || unroll_m > 32) {
							vmovups(ptr[LDA4 + (unroll_m * i + 1 * 16 - OFFSET) * SIZE],
									zmm1);
						} else {
							vmovups(ptr[LDA4 + (unroll_m * i + 1 * 16 - OFFSET) * SIZE],
									zmm1 | k2);
					...
					if (unroll_m >= 48) {
						if (isUnmasked) {
							vmovups(ptr[LDA4 + (unroll_m * i + 2 * 16 - OFFSET) * SIZE],
									zmm2);
						} else {
							vmovups(ptr[LDA4 + (unroll_m * i + 2 * 16 - OFFSET) * SIZE],
									zmm2 | k3);
			...
			if (!isDirect) {
				...
				if (unroll_m >= 32) {
					if (isUnmasked || unroll_m > 32) {
						vmovups(zmm1, ptr[AO1 + (unroll_m * 8 + 1 * 16 - OFFSET) * SIZE]);
					} else {
						vmovups(zmm1 | k2 | T_z,
								ptr[AO1 + (unroll_m * 8 + 1 * 16 - OFFSET) * SIZE]);
					}
				}
				if (unroll_m >= 48) {
					if (isUnmasked) {
						vmovups(zmm2, ptr[AO1 + (unroll_m * 8 + 2 * 16 - OFFSET) * SIZE]);
					} else {
						vmovups(zmm2 | k3 | T_z,
								ptr[AO1 + (unroll_m * 8 + 2 * 16 - OFFSET) * SIZE]);
					}
				}
		...
		auto kernel = [&](int unroll_m, int unroll_n, bool isDirect,
			...
			if (!isDirect) {
				...
				if (unroll_m >= 32) {
					if (isUnmasked || unroll_m > 32) {
						vmovups(zmm1, ptr[AO1 + (unroll_m * 0 + 1 * 16 - OFFSET) * SIZE]);
					} else {
						vmovups(zmm1 | k2 | T_z,
								ptr[AO1 + (unroll_m * 0 + 1 * 16 - OFFSET) * SIZE]);
					}
				}
				if (unroll_m >= 48) {
					if (isUnmasked) {
						vmovups(zmm2, ptr[AO1 + (unroll_m * 0 + 2 * 16 - OFFSET) * SIZE]);
					} else {
						vmovups(zmm2 | k3 | T_z,
								ptr[AO1 + (unroll_m * 0 + 2 * 16 - OFFSET) * SIZE]);
					}
				}
			}
			...
			if (isCopy) {
				...
				if (unroll_m >= 32) {
					if (isUnmasked || unroll_m > 32) {
						vmovups(ptr[LDA4 + (unroll_m * 0 + 1 * 16 - OFFSET) * SIZE],
								zmm1);
					} else {
						vmovups(ptr[LDA4 + (unroll_m * 0 + 1 * 16 - OFFSET) * SIZE],
								zmm1 | k2);
				...
				if (unroll_m >= 48) {
					if (isUnmasked) {
						vmovups(ptr[LDA4 + (unroll_m * 0 + 2 * 16 - OFFSET) * SIZE],
								zmm2);
					} else {
						vmovups(ptr[LDA4 + (unroll_m * 0 + 2 * 16 - OFFSET) * SIZE],
								zmm2 | k3);
			...
			if (!isDirect) {
				...
				if (unroll_m >= 32) {
					if (isUnmasked || unroll_m > 32) {
						vmovups(zmm1, ptr[AO1 + (unroll_m * 1 + 1 * 16 - OFFSET) * SIZE]);
					} else {
						vmovups(zmm1 | k2 | T_z,
								ptr[AO1 + (unroll_m * 1 + 1 * 16 - OFFSET) * SIZE]);
					}
				}
				if (unroll_m >= 48) {
					if (isUnmasked) {
						vmovups(zmm2, ptr[AO1 + (unroll_m * 1 + 2 * 16 - OFFSET) * SIZE]);
					} else {
						vmovups(zmm2 | k3 | T_z,
								ptr[AO1 + (unroll_m * 1 + 2 * 16 - OFFSET) * SIZE]);
					}
				}
...
mkldnn_status_t jit_avx512_common_gemm_f32(
	...
	if (nthr_k > 1) {
		...
		c_buffers = (float *)malloc(nthr_m * nthr_n * (nthr_k - 1) * MB * NB * sizeof(float), PAGE_4K);
	}
	...
	parallel_nd(nthr, [&](const int ithr) {
		...
		if (ithr < nthr_m * nthr_n * nthr_k) {
			...
			if (nthr_k > 1 && !sum_later) {
				...
				if (ithr_k > 0) {
					myC = c_buffers + (dim_t)MB * NB * (cbase + ithr_k - 1) + (dim_t)n1 * MB;
				...
				for (int ik = 1; ik < nthr_k; ++ik) {
					if (ik != ithr_k) {
						myC = c_buffers + (dim_t)MB * NB * (cbase + ik - 1) + (dim_t)n1 * MB;
