	...
	L_aligned(label_kernel_loop);
	{
		for (int h = 0; h < 4; h++) {
			for (int j = 0; j < unroll_n; j++) {
				...
				if (j == 1 && !(h & 1))
					prefetch_b(ptr[BO + isize * (prefetch_size_b + 2 * h * unroll_n - offset_b)]);
				else if (j % 3 == 0)
					prefetch_a(ptr[AO + isize * (prefetch_size_a + 32 * (j / 3) + 2 * h * unroll_m - offset_a)]);
...
void jit_avx512_core_gemm_s8u8s32_kern::remainder_kernel(int unroll_m,
	...
	if ((unroll_m > IGEMM_UNROLL_M) || (unroll_n > IGEMM_UNROLL_N) || (unroll_m < 0) || (unroll_n < 0))
		return;
	...
	for (int h = 0; h < unroll_k; h++) {
		for (int j = 0; j < unroll_n; j++) {
			Zmm b = b_regs[j & 1];
			auto b_src = ptr[BO + (-isize * offset_b + bwidth * (j + h * unroll_n))];
		...
		if (unroll_k > 1) {
			for (int i = 0; i < um_vecs; i++)
				vmovups(a_regs[i], ptr[AO + isize * (32 * i + (h + 1) * 2 * unroll_m - offset_a)]);
		}
	}
...
jit_avx512_core_gemm_s8u8s32_kern::jit_avx512_core_gemm_s8u8s32_kern(bool
																			 beta_zero_,
		bool enable_offset_c_, bool enable_offset_r_) :
		jit_generator(nullptr, 100000), arg_a(0), arg_b(0), arg_c(0), arg_ldc(0), arg_coffset_c(0), arg_coffset_r(0), coffset_cx(0), coffset_cy(0), coffset_rx(0), coffset_ry(0) {
	...
	stack_alloc_size = 32;
	auto args_offset = stack_alloc_size + get_size_of_abi_save_regs() + 8 + (is_windows ? 48 : 0);
