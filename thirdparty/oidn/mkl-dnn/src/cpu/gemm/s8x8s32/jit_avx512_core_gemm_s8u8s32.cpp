...
static inline dim_t ld_padd(const dim_t x) {
	return ((x + ((2048 / sizeof(int32_t)) - 1)) / (2048 / sizeof(int32_t))) * (2048 / sizeof(int32_t)) + (64 / sizeof(int32_t));
}
...
static int gemm_kernel_driver(const dim_t m, const dim_t n, const dim_t k,
	...
	size_t mem_size = a_buf_nelems * sizeof(*a) + PAGE_4K + b_buf_nelems * sizeof(*b) + PAGE_4K + a_row_sum_nelems * sizeof(*c) + PAGE_4K + b_col_sum_nelems * sizeof(*c) + PAGE_4K;
...
static int kernel_driver_parallel_acopiedbcopy(const dim_t m, const dim_t n,
	...
	size_t mem_size = b_buf_nelems * sizeof(*b) + PAGE_4K + b_col_sum_nelems * sizeof(*c) + PAGE_4K;
...
static inline void set_thread_opts_avx512(int *p_nthrs,
	...
	if (arg->ao != 0 || arg->bo != 0 || arg->co[0] != 0 || arg->offsetc != FIX_OFFSET) {
		condition_2D_bsrc = 0;
		condition_1D_copya = 1;
	}
...
static int parallel_a_copy(const int ithr, const int nthrs, const dim_t m,
	...
	if (ithr == 0) { // If thread master
		...
		size_t mem_size = (a_buf_nelems * sizeof(*a) + PAGE_4K) + a_row_sum_nelems * sizeof(*c) + PAGE_4K;
...
static void jit_init(blas_t *arg) {
	...
	std::call_once(initialized, [] {
		...
		gemv_s8u8s32_kern =
				gemv_s8u8s32_kernel->generate<jit_avx512_core_gemv_s8u8s32_kern::gemv_s8u8s32_kernel_t>(mayiuse(avx512_core_vnni));
		gemv_u8s8s32_kern =
				gemv_u8s8s32_kernel->generate<jit_avx512_core_gemv_s8u8s32_kern::gemv_u8s8s32_kernel_t>(mayiuse(avx512_core_vnni));
	});
