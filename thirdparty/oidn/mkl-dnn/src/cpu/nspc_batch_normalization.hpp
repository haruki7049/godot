...
struct nspc_batch_normalization_fwd_t : public cpu_primitive_t {
	struct pd_t : public cpu_batch_normalization_fwd_pd_t {
		pd_t(engine_t *engine, const batch_normalization_desc_t *adesc,
				...
				const batch_normalization_fwd_pd_t *hint_fwd_pd) : cpu_batch_normalization_fwd_pd_t(engine, adesc, attr, hint_fwd_pd) {}
		...
		status_t init() {
			...
			bool ok = true
					  ...
					  && mkldnn_thr_syncable() && is_fwd() && !has_zero_dim_memory() && src_md()->data_type == f32 && IMPLICATION(use_scaleshift(), weights_md()->data_type == f32) && memory_desc_matches_tag(*src_md(), format_tag::nhwc) && (attr()->has_default_values() || this->with_relu_post_op());
			if (!ok)
...
struct nspc_batch_normalization_bwd_t : public cpu_primitive_t {
	struct pd_t : public cpu_batch_normalization_bwd_pd_t {
		pd_t(engine_t *engine, const batch_normalization_desc_t *adesc,
				const primitive_attr_t *attr,
				const batch_normalization_fwd_pd_t *hint_fwd_pd) : cpu_batch_normalization_bwd_pd_t(engine, adesc, attr, hint_fwd_pd) {}
		...
		status_t init() {
			...
			bool ok = true
					  /* the algorithm requires barriers while switching
					   * between parallelization over N and C dimensions */
					  && mkldnn_thr_syncable() && is_bwd() && !has_zero_dim_memory() && utils::everyone_is(f32, src_md()->data_type, diff_src_md()->data_type) && IMPLICATION(use_scaleshift(), utils::everyone_is(f32, weights_md()->data_type, diff_weights_md()->data_type)) && memory_desc_matches_tag(*src_md(), format_tag::nhwc) && memory_desc_matches_tag(*diff_src_md(), format_tag::nhwc) && attr()->has_default_values();
			if (!ok)
				return status::unimplemented;
	...
	private:
		void init_scratchpad() {
			...
			scratchpad.book(key_bnorm_tmp_diff_ss, sizeof(data_t) * 2 * C() * (mkldnn_get_max_threads() + 1));
		}
	};
