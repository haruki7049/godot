...
inline void pick_loop_order(jit_conv_conf_t &jcp) {
	...
	if (jcp.prop_kind == backward_data) {
		jcp.loop_order = (w <= small_spatial && h <= small_spatial) ? loop_cgn : loop_gnc;
	} else {
		jcp.loop_order = (w <= small_spatial && h <= small_spatial) ? loop_cwgn : loop_gncw;
	}
}
...
void _jit_avx512_common_conv_fwd_kernel<Zmm>::compute_loop_4fma_1st(int ur_w,
	...
	size_t max_input_offset = (size_t)jcp.typesize_in * ((size_t)(kw + ur_w * stride_w - pad_l) + (size_t)ic_block * iw * ih * jcp.id);
	...
	for (int ki = 0; ki < kw; ki += 4) {
		for (int ic = 0; ic < ic_block; ic++) {
			for (int i = 0; i < 4; i++) {
				int aux_ker_offset = jcp.typesize_in * ((ki + i) * oc_block + ic * kw * jcp.kh * jcp.kd * oc_block);
				if (ki + i < kw)
					vmovups(vmm_ker(i),
			...
			for (int j = j_start, prf_count = 0; j < j_end; j++) {
				size_t aux_input_offset = (size_t)jcp.typesize_in * ((size_t)(ki + j * stride_w - pad_l) + (size_t)ic * iw * ih * jcp.id);
				v4fmaddps(vmm_out(j, 0), vmm_ker(0),
						EVEX_compress_addr_safe(aux_reg_inp, aux_input_offset,
								reg_long_offt));
				if (ki + prf_count < kw && prf_count < 4 && ((ki < 2 && j % 4) || j % 2)) {
					int aux_ker_offset = jcp.typesize_in * ((ki + prf_count) * oc_block + ic * kw * jcp.kh * jcp.kd * oc_block + kw * oc_block);
...
void _jit_avx512_common_conv_fwd_kernel<Zmm>::compute_loop_4fma(int ur_w,
	...
	auto prefetch_inp_next_kh = [&](int ki, int ki_start, int cnt0, int cnt1) {
		if (cnt1 >= ker_load_number && cnt0 >= ker_load_number && ki >= ki_start && oi_ipref_t0 < ow_end_ipref) {
			int aux_inp_offset = typesize * ((oi_ipref_t0 * stride_w - pad_l) * ic_block + (jcp.dilate_h + 1) * jcp.iw * ic_block);
		...
		for (int ki = 0; ki < kw; ki++)
			for (int ic = 0; ic < ic_block; ic += 4)
				for (int kk = 0; kk < jcp.nb_oc_blocking; kk++) {
					bool last_kernel_loads = (kk == jcp.nb_oc_blocking - 1 && ki == kw - 1 && (ic + 4) == ic_block);
					...
					for (int oi = get_ow_start(ki, pad_l), prf_count_t1 = 0,
						...
						int aux_input_offset = typesize * ((ki * (jcp.dilate_w + 1) + oi * stride_w - pad_l) * ic_block + ic);
						...
						if (oi % 2) {
							if (prf_count_t0 < 4) {
								...
								if (last_kernel_loads)
									...
									aux_kernel_prf = kernel_offset(kk, ic + 4 + prf_count_t0, ki);
								mic_prefetcht0(EVEX_compress_addr(aux_reg_ker,
										aux_kernel_prf));
								prf_count_t0++;
							} else if (prf_count_t1 < 4) {
								mic_prefetcht1(EVEX_compress_addr(
										aux_reg_ker_prf, kernel_offset(kk, ic + prf_count_t1, ki)));
								prf_count_t1++;
							}
						} else
	...
	if (jcp.ndims == 5) {
		...
		add(aux_reg_ker_d, typesize * jcp.kw * jcp.kh * jcp.oc_block * jcp.ic_block);
		add(aux_reg_inp_d_prf,
				typesize * (jcp.dilate_d + 1) * jcp.ih * jcp.iw * jcp.ic_block);
		add(aux_reg_ker_d_prf, typesize * jcp.kw * jcp.kh * jcp.oc_block * jcp.ic_block);
...
void _jit_avx512_common_conv_fwd_kernel<Vmm>::compute_loop_fma(int ur_w,
	...
	int prf_inst_spacing = (prf_ker || prf_inp) ? nstl::max(1, num_fmas / num_prfs) : 1;
		...
		for (int ki = 0; ki < kw; ki++) {
			for (int ic = 0; ic < ic_block; ic++) {
				...
				if (step == 0) {
					...
					int ker_load_reg_idx = (step + load_offset) % ker_pipeline_depth;
				...
				for (int j = j_start; j < j_end; j++) {
					...
					if (fma_idx % prf_inst_spacing == prf_inst_trigger) {
						if (prf_ker && !ker_prf_inserted && ker_prfs < num_ker_prfs) {
							int ker_prf_offset = jcp.typesize_in * ker_prfs * jcp.oc_block;
							...
							if (inp_prf_idx < num_inp_prfs) {
								...
								if (!jcp.is_1stconv) {
									inp_prf_offset = ic_block * jcp.typesize_in * ((inp_prf_idx / kw) * inp_prf_stride + (inp_prf_idx % kw));
								} else {
									size_t ic_prf_stride =
											(size_t)jcp.typesize_in * iw * ih * id;
									size_t iw_prf_stride = jcp.typesize_in * jcp.simd_w;
									inp_prf_offset = ((inp_prf_idx / ic_block) * iw_prf_stride + (inp_prf_idx % ic_block) * ic_prf_stride);
								}
...
void _jit_avx512_common_conv_fwd_kernel<Vmm>::compute_loop_fma_core(int ur_w,
	...
	int inp_mul = !jcp.is_1stconv ? ic_block : 1;
	int shift_input_ptr = jcp.typesize_in * (jcp.dilate_h + 1) * jcp.iw * inp_mul;

	auto input_offset = [=](int oi, int ic, int ki) {
		return (size_t)jcp.typesize_in * ((size_t)(ki * (jcp.dilate_w + 1) + oi * stride_w - pad_l) * inp_mul + (size_t)ic * (!jcp.is_1stconv ? 1 : (size_t)jcp.iw * jcp.ih * jcp.id));
	};
		...
		for (int ki = 0; ki < kw; ki++) {
			...
			for (int ic = 0; ic < ic_block; ic++) {
				...
				for (int ii = 0; ii < nb_oc_block; ii++) {
					int aux_kernel_offset = jcp.typesize_in * (ii * jcp.nb_ic * jcp.kh * jcp.kw * jcp.kd * ic_block * oc_block + ki * ic_block * oc_block + ic * oc_block);
...
void _jit_avx512_common_conv_fwd_kernel<Vmm>::compute_loop(int ur_w,
	...
	if (jcp.ndims == 5) {
		if ((jcp.dilate_d >= jcp.id) || (jcp.kd - 1) * (jcp.dilate_d + 1) < nstl::max(jcp.f_pad, jcp.back_pad)) {
			mov(reg_kj, ptr[param1 + GET_OFF(kd_padding)]);
			cmp(reg_kj, 0);
			je(skip_compute_loop, T_NEAR);
		}
	}
	if ((jcp.dilate_h >= jcp.ih) || (jcp.kh - 1) * (jcp.dilate_h + 1) < nstl::max(jcp.t_pad, jcp.b_pad)) {
		mov(reg_kj, ptr[param1 + GET_OFF(kh_padding)]);
		cmp(reg_kj, 0);
		je(skip_compute_loop, T_NEAR);
	}
	...
	if (jcp.ver == ver_4fma)
		...
		if ((jcp.is_1stconv && jcp.kernel_kind != expl_bcast) || mayiuse(avx512_mic))
...
void _jit_avx512_common_conv_fwd_kernel<Vmm>::generate() {
	...
	int n_oi = ow / ur_w;
	int r_pad1 = (ur_w * n_oi - 1) * stride_w + (kw - 1) * dilate_w - (iw + l_pad - 1);
...
status_t jit_avx512_common_conv_fwd_kernel::init_conf(
	...
	jcp.b_pad = (jcp.oh - 1) * jcp.stride_h + (jcp.kh - 1) * (jcp.dilate_h + 1) - (jcp.ih + jcp.t_pad - 1);
	jcp.back_pad = (jcp.od - 1) * jcp.stride_d + (jcp.kd - 1) * (jcp.dilate_d + 1) - (jcp.id + jcp.f_pad - 1);

	jcp.is_1stconv = is_1stconv(jcp);

	bool ok_to_pad_channels = true && jcp.ngroups == 1 && src_d.data_type() == data_type::f32;

	const int full_simd_w = cpu_isa_traits<avx512_common>::vlen / sizeof(float);
	jcp.simd_w = full_simd_w;
	bool ok_to_try_xmm = true && mayiuse(avx512_core) && src_d.data_type() == data_type::f32 && !jcp.is_1stconv && !ok_to_pad_channels && (jcp.ic % jcp.simd_w != 0 || jcp.oc % jcp.simd_w != 0) && (jcp.ic % 8 != 0 || jcp.oc % 8 != 0);
	if (ok_to_try_xmm)
		jcp.simd_w = 4;
	...
	bool args_ok = true && jcp.oc % jcp.oc_block == 0 && jcp.ic % jcp.ic_block == 0;
	if (!args_ok)
		return status::unimplemented;
	...
	auto src_tag = jcp.is_1stconv ? pick(ndims - 3, ncw, nchw, ncdhw) : ((jcp.simd_w == 4) ? pick(ndims - 3, nCw4c, nChw4c, nCdhw4c) : pick(ndims - 3, nCw16c, nChw16c, nCdhw16c));
	auto dst_tag = (jcp.simd_w == 4) ? pick(ndims - 3, nCw4c, nChw4c, nCdhw4c) : pick(ndims - 3, nCw16c, nChw16c, nCdhw16c);
	auto wei_tag = with_groups ? ((jcp.simd_w == 4) ? pick(ndims - 3, gOIw4i4o, gOIhw4i4o, gOIdhw4i4o) : pick(ndims - 3, gOIw16i16o, gOIhw16i16o, gOIdhw16i16o)) : ((jcp.simd_w == 4) ? pick(ndims - 3, OIw4i4o, OIhw4i4o, OIdhw4i4o) : pick(ndims - 3, OIw16i16o, OIhw16i16o, OIdhw16i16o));
	...
	if (mayiuse(avx512_common) &&
			src_d.data_type() == data_type::f32 && weights_d.data_type() == data_type::f32 && dst_d.data_type() == data_type::f32) {
		jcp.ver = ver_fma;
		...
		if (jcp.is_1stconv) {
			...
			bool not_for_4fma = IMPLICATION(everyone_is(0, jcp.l_pad, jcp.t_pad),
					nstl::max(jcp.kw, jcp.kh) < 7);
			bool is_dilated = !everyone_is(0, jcp.dilate_d, jcp.dilate_h, jcp.dilate_w);
			if (one_of(true, not_for_4fma, is_dilated))
				jcp.ver = ver_fma;
			if (jcp.ver == ver_4fma) {
				wei_tag = with_groups ? ((jcp.simd_w == 4) ? pick(ndims - 3, gOiw4o, gOihw4o, gOidhw4o) : pick(ndims - 3, gOiw16o, gOihw16o, gOidhw16o)) : ((jcp.simd_w == 4) ? pick(ndims - 3, Oiw4o, Oihw4o, Oidhw4o) : pick(ndims - 3, Oiw16o, Oihw16o, Oidhw16o));
			} else {
				wei_tag = with_groups ? ((jcp.simd_w == 4) ? pick(ndims - 3, gOwi4o, gOhwi4o, gOdhwi4o) : pick(ndims - 3, gOwi16o, gOhwi16o, gOdhwi16o)) : ((jcp.simd_w == 4) ? pick(ndims - 3, Owi4o, Ohwi4o, Odhwi4o) : pick(ndims - 3, Owi16o, Ohwi16o, Odhwi16o));
			}
		}
	} else {
		return status::unimplemented;
	...
	int r_pad = (jcp.ur_w * n_oi - 1) * jcp.stride_w + (jcp.kw - 1) * (jcp.dilate_w + 1) - (jcp.iw + jcp.l_pad - 1);
	if (jcp.l_pad > 0 && r_pad > 0)
		n_oi--;

	bool large_code_size = jcp.ur_w != jcp.ow && jcp.l_pad > 0 && r_pad > 0 && ((jcp.l_pad <= 0 && n_oi > 0) || (jcp.l_pad > 0 && n_oi > 1));
	if (large_code_size) {
	...
	jcp.nonblk_group_off = (jcp.ngroups > 1 && one_of(jcp.src_tag, ncw, nchw, ncdhw)) ?
								   jcp.ic :
								   1;
	...
	auto is_ow_threading_applicable = [=]() {
		return (true && !jcp.is_1stconv && one_of(jcp.ndims, 3, 4) && IMPLICATION(mayiuse(avx512_mic), jcp.ver == ver_4fma && IMPLICATION(jcp.mb != 1, jcp.ih == 1 && jcp.kh == 1)));
	};

	if (jcp.ver == ver_4fma && !jcp.is_1stconv) {
		if ((jcp.kw <= 5 && jcp.kh <= 5 && jcp.kw == jcp.kh && jcp.ow <= 8 && jcp.oh <= 8 && jcp.ow == jcp.oh) || (jcp.stride_h != 1 && jcp.ur_w < jcp.ow)) {
			if (jcp.nb_oc % 2 == 0) {
		...
		if (jcp.ver == ver_4fma && is_ow_threading_applicable()) {
			if (jcp.nb_oc % 2 == 0 && jcp.ur_w < jcp.ow && jcp.ow != 2 * jcp.ur_w) {
				jcp.nb_oc_blocking = 2;
				jcp.ur_w = nstl::min(jcp.ow, regs / jcp.nb_oc_blocking);
			}
		}
	...
	auto get_thr_eff = [=](int nb_oc_blocking, int ow_block) {
		...
		float thr_eff = disbalance * (float)work_amount / rnd_up(work_amount, nthreads);
		return thr_eff;
	};
	...
	auto get_ow_block = [=](int nb_oc_blocking, int ur_w, float &eff) {
		...
		int size_wei_chunk = jcp.oc_block * nb_oc_blocking * jcp.ic_block * jcp.kw * jcp.kh;
		int nurw_cache = (L2_part - 2 * size_wei_chunk) / (2 * size_dst_chunk + 2 * size_src_chunk);
		...
		for (int nb_ow = start_nb_ow; nb_ow <= max_nb_ow; nb_ow++) {
			int ow_block = nstl::min(rnd_up(div_up(jcp.ow, nb_ow), ur_w), jcp.ow);
	...
	if (jcp.ver == ver_fma && mayiuse(avx512_core)) {
		...
		unsigned int ker_inp_size = typesize * div_up(jcp.iw, jcp.stride_w) * jcp.ic_block * jcp.kh * jcp.kd;
		unsigned int ker_out_size = typesize * jcp.ow * jcp.oc_block * try_nb_oc_blocking;
		unsigned int ker_wei_size = typesize * jcp.kh * jcp.kw * jcp.ic_block * jcp.oc_block * try_nb_oc_blocking * jcp.kd;
		unsigned int ker_total_size = ker_inp_size + ker_out_size + ker_wei_size;

		bool embd_bcast_condition = true && (jcp.kw == 3 && jcp.ow <= 28 && ker_total_size < L1_cache_size) && !(jcp.kw == 3 && jcp.ow == 13 && jcp.ic >= 192) && !(jcp.kw == 3 && jcp.ow == 28 && jcp.ic >= 512);

		if (jcp.mb == 1) {
			unsigned int inp_size = jcp.mb * div_up(jcp.ih, jcp.stride_h) * div_up(jcp.iw, jcp.stride_w) * jcp.ic;
			...
			int ch_min = (job_size_min == 0) ? jcp.oh : rnd_up(jcp.oh, job_size_min);
			bool not_aligned_max = ch_max % jcp.oh != 0 && ch_max / jcp.oh < 2 && (jcp.oh != 8 || ch_max / jcp.oh > 1);
			bool not_aligned_min = ch_min % jcp.oh != 0 && ch_min / jcp.oh < 2 && (jcp.oh != 8 || ch_min / jcp.oh > 1);
			bool eligible_case = (jcp.stride_h == 1 && jcp.stride_w == 1) || nthreads > oc_chunks;
			if (jcp.loop_order == loop_cgn && oc_chunks > 1 && nthreads > 1 && wei_size / inp_size > 24 && (not_aligned_max || not_aligned_min) && eligible_case) {
		...
		if (jcp.kw > 3 || (jcp.stride_w == 1 && jcp.stride_h == 1 && embd_bcast_condition) || ((jcp.stride_w != 1 || jcp.stride_h != 1) && ((jcp.mb <= 16 && (jcp.oc <= 192 || jcp.oh <= 10) && embd_bcast_condition))) || (jcp.mb == 1 && (jcp.ur_w >= jcp.ow || jcp.is_1stconv || (jcp.ow <= 147 && jcp.oc <= 96)))) {
			jcp.kernel_kind = embd_bcast;
			jcp.ur_w = nstl::min(jcp.ow, regs);
			jcp.nb_ic_blocking = jcp.nb_oc_blocking = 1;
			if (ker_total_size < L1_cache_size && jcp.ow <= 8 && jcp.kh <= 3 && jcp.kw <= 3 && jcp.nb_oc % try_nb_oc_blocking == 0 && IMPLICATION(jcp.is_1stconv, jcp.mb == 1) && IMPLICATION(jcp.mb == 1, jcp.ur_w < jcp.ow)) {
	...
	args_ok = true && jcp.l_pad <= jcp.ur_w && jcp.ic <= src_d.padded_dims()[1] && jcp.oc <= dst_d.padded_dims()[1] && jcp.ic <= weights_d.padded_dims()[with_groups + 1] && jcp.oc <= weights_d.padded_dims()[with_groups + 0];
	if (!args_ok)
		return status::unimplemented;

	int r_pad_no_tail = nstl::max(0, (jcp.ow - jcp.ur_w_tail - 1) * jcp.stride_w + (jcp.kw - 1) * (jcp.dilate_w + 1) - (jcp.iw + jcp.l_pad - 1));
	...
	int h_L2 = int(((0.6f * L2_size) / jcp.simd_w - nstl::min(0, jcp.kh - jcp.stride_h) * jcp.iw) / (jcp.stride_h * jcp.iw + jcp.ow));
	...
	if (jcp.ver == ver_4fma) {
		if (!is_ow_threading_on(jcp)) {
			for (int divf = 2, temp_nb = jcp.nb_ic_L2; divf <= jcp.nb_ic;
					divf++) {
				size_t l2_src = (size_t)jcp.iw * jcp.ic_block * jcp.ih * temp_nb * jcp.id;
				size_t l2_dst = (size_t)jcp.ow * jcp.oc_block * jcp.nb_oc_blocking * jcp.oh * jcp.od;
				size_t l2_filt = (size_t)jcp.kw * jcp.oc_block * jcp.ic_block * jcp.kh * jcp.nb_oc_blocking * temp_nb * jcp.kd;
				if (4 * (l2_src + l2_dst + l2_filt) > KNx_L2_EFFECTIVE_CAPACITY) {
					if (jcp.kh == 3 && jcp.oh == 7) {
						jcp.nb_ic_L2 = 1;
						break;
					}
					temp_nb = (jcp.nb_ic_L2 % divf == 0 ? jcp.nb_ic_L2 / divf : jcp.nb_ic_L2);
				} else {
...
void jit_avx512_common_conv_bwd_data_kernel_f32::prepare_output(int ur_w) {
	for (int k = 0; k < jcp.nb_ic_blocking; k++) {
		for (int j = 0; j < ur_w; j++) {
			Zmm zmm = zmm_out(j, k);
			vpxord(zmm, zmm, zmm);
			size_t aux_src_offset = (size_t)typesize * ((size_t)k * jcp.ih * jcp.iw * jcp.id + j) * jcp.ic_block;
...
void jit_avx512_common_conv_bwd_data_kernel_f32::compute_loop_4fma(
	...
	auto prefetch_dst_next_kh = [&](int ki, int ki_start, int cnt0, int cnt1) {
		if (cnt1 >= ker_load_number && cnt0 >= ker_load_number && ki >= ki_start && ii_dpref_t0 < iw_end_ipref) {
			int aux_dst_offset = typesize * ((ii_dpref_t0 + jcp.l_pad) * oc_block + jcp.ow * oc_block);
		...
		for (int ki = 0; ki < kw; ki++)
			for (int oc = 0; oc < oc_block; oc += 4)
				for (int kk = 0; kk < jcp.nb_ic_blocking; kk++) {
					bool last_kernel_loads = (kk == jcp.nb_ic_blocking - 1 && ki == kw - 1 && (oc + 4) == oc_block);
					...
					for (int ii = get_iw_start(ki, l_overflow),
						...
						int aux_dst_offset = typesize * ((ii + jcp.l_pad - ki) * oc_block + oc);
						...
						if (ii % 2) {
							if (prf_count_t0 < 4) {
								int aux_kernel_prf;
								if (last_kernel_loads)
									aux_kernel_prf = kernel_offset(0, prf_count_t0 + oc + 4 - oc_block, 0) + typesize * kw * oc_block * ic_block;
								else
									aux_kernel_prf = kernel_offset(kk, oc + 4 + prf_count_t0, ki);
								mic_prefetcht0(EVEX_compress_addr(aux_reg_ker,
										aux_kernel_prf));
								prf_count_t0++;
							} else if (prf_count_t1 < 4) {
...
void jit_avx512_common_conv_bwd_data_kernel_f32::compute_loop_fma(
	...
	int num_inp_prfs = ur_w * nstl::min(kw, stride_w) + nstl::max(0, kw - stride_w);
	...
	L(kh_label);
	{
		...
		for (int ki = 0; ki < kw; ki++) {
			for (int oc = 0; oc < oc_block; oc++) {
				if (step == 0) {
					for (int i = 0; i < ker_pipeline_depth; i++) {
						int aux_kernel_offset = typesize * ((oc + i) * oc_block + ki * ic_block * oc_block);
					...
					int load_offset = ker_pipeline_depth - 1;
					int ker_load_reg_idx = (step + load_offset) % ker_pipeline_depth;
					int aux_kernel_offset = typesize * ((oc + load_offset) * oc_block + ki * ic_block * oc_block);
				...
				assert(stride_w != 1 || jj_start == nstl::max(0,
															l_overflow - (kw - 1 - ki) * dilate_w));
				assert(stride_w != 1 || jj_end == ur_w - nstl::max(0,
																 r_overflow - ki * dilate_w));

				for (int jj = jj_start; jj < jj_end; jj += stride_w) {
					assert((jj + l_pad - ki * dilate_w) % stride_w == 0);
					int aux_dst_offset = typesize *
										 (((jj + l_pad - ki * dilate_w) / stride_w) * jcp.oc_block + oc);
					...
					if (fma_idx % prf_inst_spacing == prf_inst_trigger) {
						if (!ker_prf_inserted && ker_prfs < num_ker_loads) {
							int ker_prf_offset = typesize * ker_prfs * jcp.oc_block;
							mic_prefetcht1(EVEX_compress_addr(
									aux_reg_ker_prf, ker_prf_offset));
							ker_prf_inserted = true;
							ker_prfs++;
							...
							int inp_prf_idx = prf_slot_idx - ker_prfs;
							if (inp_prf_idx < num_inp_prfs) {
								int inp_prf_offset = ic_block * typesize * ((inp_prf_idx / kw) * kw + (inp_prf_idx % kw));
								mic_prefetcht0(EVEX_compress_addr(
	...
	if (jcp.ndims == 5) {
		...
		add(aux_reg_ker_d, typesize * jcp.stride_d * jcp.kw * jcp.kh * oc_block * ic_block);
		sub(aux_reg_dst_d_prf,
				typesize * (jcp.dilate_d + 1) * jcp.oh * ow * ic_block);
		add(aux_reg_ker_d_prf, typesize * jcp.stride_d * jcp.kw * jcp.kh * oc_block * ic_block);
...
void jit_avx512_common_conv_bwd_data_kernel_f32::generate() {
	...
	int r_overflow = nstl::max(0, ((kw - 1) * dilate_w - nstl::max(0, jcp.r_pad)) / stride_w);
	int r_overflow1 = nstl::max(0, ((kw - 1) * dilate_w - nstl::max(0, jcp.r_pad) - ur_w_tail) / stride_w);
...
status_t jit_avx512_common_conv_bwd_data_kernel_f32::init_conf(
	...
	jcp.dilate_h = (ndims == 3) ? 0 : cd.dilates[ndims - 4];
	jcp.dilate_w = cd.dilates[ndims - 3];
	if ((jcp.dilate_w != 0 && jcp.stride_w != 1) || (jcp.dilate_d != 0 && jcp.stride_d != 1) || (jcp.dilate_h != 0 && jcp.stride_h != 1))
		return status::unimplemented;

	jcp.r_pad = (jcp.ow - 1) * jcp.stride_w + (jcp.kw - 1) * (jcp.dilate_w + 1) - (jcp.iw + jcp.l_pad - 1);
	jcp.b_pad = (jcp.oh - 1) * jcp.stride_h + (jcp.kh - 1) * (jcp.dilate_h + 1) - (jcp.ih + jcp.t_pad - 1);
	jcp.back_pad = (jcp.od - 1) * jcp.stride_d + (jcp.kd - 1) * (jcp.dilate_d + 1) - (jcp.id + jcp.f_pad - 1);
	...
	bool ok_to_pad_channels = true && jcp.ngroups == 1 && diff_src_d.data_type() == data_type::f32;
	...
	auto wei_tag = with_groups ? pick(ndims - 3, gOIw16o16i, gOIhw16o16i, gOIdhw16o16i) : pick(ndims - 3, OIw16o16i, OIhw16o16i, OIdhw16o16i);
	jcp.src_tag = diff_src_d.matches_one_of_tag(dat_tag);
	jcp.dst_tag = diff_dst_d.matches_one_of_tag(dat_tag);

	bool args_ok = true && jcp.oc % jcp.oc_block == 0 && jcp.ic % jcp.ic_block == 0 && jcp.src_tag == dat_tag && jcp.dst_tag == dat_tag;
	if (!args_ok)
		return status::unimplemented;
	...
	int l_overflow = nstl::max(0, ((jcp.kw - 1) * (jcp.dilate_w + 1) - jcp.l_pad) / jcp.stride_w);
	int r_overflow1 = nstl::max(0, ((jcp.kw - 1) * (jcp.dilate_w + 1) - nstl::max(0, jcp.r_pad) - jcp.iw % jcp.ur_w) / jcp.stride_w);
	int n_oi = jcp.iw / jcp.ur_w;
	if (r_overflow1 > 0)
		n_oi--;

	if (mayiuse(avx512_common) && diff_dst_d.data_type() == data_type::f32 && weights_d.data_type() == data_type::f32 && diff_src_d.data_type() == data_type::f32) {
		jcp.ver = ver_fma;
		jcp.typesize_in = sizeof(float);
		jcp.typesize_out = sizeof(float);
		if (mayiuse(avx512_mic_4ops) && jcp.stride_w == 1 && jcp.stride_h == 1 && jcp.stride_d == 1) {
			jcp.ver = ver_4fma;
		}
	} else {
		return status::unimplemented;
	...
	if (!utils::everyone_is(0, jcp.dilate_d, jcp.dilate_h, jcp.dilate_w) && jcp.ver != ver_fma)
		return status::unimplemented;
	...
	bool large_code_size = (jcp.ur_w != jcp.ow) && ((l_overflow <= 0 && n_oi > 0) || (l_overflow > 0 && n_oi > 1)) && (r_overflow1 > 0) && (l_overflow > 0);
	if (large_code_size) {
		const int max_code_size = 24 * 1024;
		...
		if (r_overflow1 > 0)
			mult += 1;
		for (int ur_w = jcp.ur_w; ur_w > regs / 2; --ur_w) {
			if ((ur_w / jcp.stride_w) * mult * num_ops_per_reg * 9.2 < max_code_size) {
				if (ur_w % jcp.stride_w == 0) {
					jcp.ur_w = ur_w;
					break;
				}
	...
	if (jcp.ver == ver_fma && mayiuse(avx512_core)) {
		...
		unsigned int ker_inp_size = typesize * jcp.iw * jcp.ic_block * try_nb_ic_blocking * jcp.kh;
		unsigned int ker_out_size = typesize * jcp.ow * jcp.oc_block;
		unsigned int ker_wei_size = typesize * jcp.kh * jcp.kw * jcp.ic_block * jcp.oc_block * try_nb_ic_blocking;
		unsigned int ker_total_size = ker_inp_size + ker_out_size + ker_wei_size;
		if (!(jcp.kw == 1 || (jcp.kw == 5 && jcp.iw < 8) || (jcp.kw < 5 && ((jcp.iw <= 5 || (jcp.iw > 8 && jcp.iw <= 13)) || ker_total_size > L1_cache_size))) || jcp.stride_h > 1 || jcp.stride_d > 1) {
			jcp.kernel_kind = embd_bcast;
			jcp.ur_w = nstl::min(jcp.iw, regs);
			jcp.nb_ic_blocking = jcp.nb_oc_blocking = 1;
			if (!(jcp.kw > 3 || (jcp.kw == 3 && ker_total_size < L1_cache_size && jcp.ow > 8)) && jcp.stride_h == 1)
	...
	if (l_overflow * jcp.stride_w > jcp.ur_w)
		return status::unimplemented;
	int r_overflow_no_tail = nstl::max(0, ((jcp.kw - 1) * (jcp.dilate_w + 1) - nstl::max(0, jcp.r_pad) - jcp.ur_w_tail) / jcp.stride_w);
	...
	if (jcp.ver == ver_4fma && (jcp.kh < 5 && jcp.kw < 5)) {
		for (int divf = 2, temp_nb = jcp.nb_oc_L2; divf <= jcp.nb_oc;
				divf++) {
			size_t l2_src = jcp.iw * jcp.ic_block * jcp.nb_ic_blocking * jcp.ih * jcp.id;
			size_t l2_dst = jcp.ow * jcp.oc_block * temp_nb * jcp.oh * jcp.od;
			size_t l2_filt = jcp.kw * jcp.oc_block * jcp.ic_block * jcp.kh * jcp.kd * jcp.nb_ic_blocking * temp_nb;
			if (4 * (l2_src + l2_dst + l2_filt) > KNx_L2_EFFECTIVE_CAPACITY) {
				if (jcp.kh == 3 && jcp.ih == 7) {
					jcp.nb_oc_L2 = 1;
					break;
				}
				temp_nb = (jcp.nb_oc_L2 % divf == 0 ? jcp.nb_oc_L2 / divf : jcp.nb_oc_L2);
			} else {
	...
	args_ok = true && jcp.ic <= diff_src_d.padded_dims()[1] && jcp.oc <= diff_dst_d.padded_dims()[1] && jcp.ic <= weights_d.padded_dims()[with_groups + 1] && jcp.oc <= weights_d.padded_dims()[with_groups + 0];
	if (!args_ok)
		return status::unimplemented;
...
void jit_avx512_common_conv_bwd_weights_kernel_f32::compute_ic_block_step_fma(
	...
	for (int i_kw = 0; i_kw < kw; i_kw++)
		for (int i_ic = 0; i_ic < ic_block_step; i_ic++)
			vmovups(Zmm(i_kw * ic_block_step + i_ic),
					EVEX_compress_addr(reg_kernel, typesize * (i_kw * ic_block + i_ic) * jcp.oc_block + kernel_offset));

	for (int i_ur = 0; i_ur < ur_w; i_ur++) {
		if (i_ur == 0) {
			vmovups(Zmm(kw * ic_block_step + (i_ur + 0) % 4),
					EVEX_compress_addr(reg_output, typesize * (i_ur + 0) * oc_block + output_offset));
			if (ur_w > 1)
				vmovups(Zmm(kw * ic_block_step + (i_ur + 1) % 4),
						EVEX_compress_addr(reg_output, typesize * (i_ur + 1) * oc_block + output_offset));
			if (ur_w > 2)
				vmovups(Zmm(kw * ic_block_step + (i_ur + 2) % 4),
						EVEX_compress_addr(reg_output, typesize * (i_ur + 2) * oc_block + output_offset));
			if (ur_w > 3)
				vmovups(Zmm(kw * ic_block_step + (i_ur + 3) % 4),
						EVEX_compress_addr(reg_output, typesize * (i_ur + 3) * oc_block + output_offset));
		} else if (i_ur + 3 < ur_w)
			vmovups(Zmm(kw * ic_block_step + (i_ur + 3) % 4),
					EVEX_compress_addr(reg_output, typesize * (i_ur + 3) * oc_block + output_offset));
		...
		for (int i_kw = 0; i_kw < kw; i_kw++) {
			...
			if (i_iw - pad_l < 0 || i_iw > (ur_w - 1) * jcp.stride_w +
				...
				continue;
			for (int i_ic = 0; i_ic < ic_block_step; i_ic++) {
				const size_t i_offset = (size_t)input_offset + (size_t)typesize * (jcp.ver == ver_4fma ? (i_iw - pad_l + i_ic * jcp.tr_iw) : (jcp.is_1stconv ? (i_iw - pad_l) + (size_t)i_ic * ((size_t)jcp.ih * jcp.iw * jcp.id) : (i_iw - pad_l) * ic_block + i_ic));
	...
	for (int i_kw = 0; i_kw < kw; i_kw++)
		for (int i_ic = 0; i_ic < ic_block_step; i_ic++)
			vmovups(EVEX_compress_addr(reg_kernel, typesize * (i_kw * ic_block + i_ic) * jcp.oc_block + kernel_offset),
...
void jit_avx512_common_conv_bwd_weights_kernel_f32::compute_ic_block_step_4fma(
	...
	auto ker_addr = [=](int i_kw, int i_ic) {
		size_t local_offset = jcp.typesize_out * (i_kw * ic_block + i_ic) * jcp.oc_block;
	...
	auto pf_callback = [=](int i_ur, int i_kw, int i_ic) {
		...
		const ptrdiff_t next_input_block_offset = jcp.typesize_in * ic_block_step * jcp.tr_iw;
		if (i_ur % 16 == 4 && i_kw == 0) {
			if (i_ur + 16 < ur_w)
...
void jit_avx512_common_conv_bwd_weights_kernel_f32 ::compute_oh_step_unroll_ow_icblock(
	...
	int r_pad = nstl::max(0, (ow - 1) * jcp.stride_w + (jcp.kw - 1) * (jcp.dilate_w + 1) - (jcp.iw + jcp.l_pad - 1));
	int l_pad = jcp.l_pad;
	...
	L(kh_label);
	{
		for (int i_b_ic = 0; i_b_ic < jcp.ic_block; i_b_ic += ic_block_step) {
			const int input_offset = jcp.typesize_in * (jcp.ver == ver_4fma ? i_b_ic * iw : i_b_ic);
	...
	if (jcp.ndims == 5) {
		...
		add(aux_reg_kernel, jcp.typesize_out * jcp.kh * jcp.kw * ic_block * oc_block);
		dec(ki);
		cmp(ki, 0);
		jg(kd_label, T_NEAR);
	}
...
void jit_avx512_common_conv_bwd_weights_kernel_f32 ::compute_oh_step_unroll_ow(
	...
	int r_pad = nstl::max(0,
			(ow - 1) * jcp.stride_w + (jcp.kw - 1) * (jcp.dilate_w + 1) - (jcp.iw + jcp.l_pad - 1));
	int l_pad = jcp.l_pad;
	...
	L(kh_label);
	{
		...
		L(ic_block_label);
		{
			compute_ic_block_step(ow, l_pad, r_pad, ic_block_step,
					0, 0, 0);
			size_t inp_icblk_stride = jcp.is_1stconv ? (size_t)jcp.ih * jcp.iw * jcp.id : (jcp.ver == ver_4fma ? jcp.tr_iw : 1);
			size_t input_offset = inp_icblk_stride * jcp.typesize_in * ic_block_step;
		...
		if (jcp.is_1stconv) {
			size_t input_offset = (size_t)jcp.typesize_in * jcp.id * jcp.ih * jcp.iw * ic_block;
			safe_sub(reg_input, input_offset, reg_long_offt);
			add(reg_input, jcp.typesize_in * (jcp.dilate_h + 1) * jcp.iw);
		} else if (jcp.ver != ver_4fma) {
			add(reg_input, jcp.typesize_in * ((jcp.dilate_h + 1) * jcp.iw - 1) * ic_block);
		}
	...
	if (jcp.ndims == 5) {
		add(aux_reg_input, jcp.typesize_in * (jcp.dilate_d + 1) * jcp.ih * jcp.iw * (jcp.is_1stconv ? 1 : ic_block));
		add(aux_reg_kernel, jcp.typesize_out * jcp.kh * jcp.kw * ic_block * oc_block);
		dec(ki);
		cmp(ki, 0);
		jg(kd_label, T_NEAR);
	...
	L(kh_label);
	{
		...
		L(ic_block_label);
		{
			if (l_pad != 0) {
				...
				add(reg_input, jcp.typesize_in * (ur_w * jcp.stride_w - l_pad) * inp_mult);
			...
			if (ur_w_trips > 0) {
				...
				L(ow_block_label);
				{
					...
					add(reg_input, jcp.typesize_in * ur_w * jcp.stride_w * inp_mult);
					add(reg_output, jcp.typesize_in * ur_w * oc_block);
			...
			int inp_icblk_stride = jcp.is_1stconv ? jcp.ih * jcp.iw * jcp.id : (jcp.ver == ver_4fma ? jcp.tr_iw : 1);
			size_t input_offset = inp_icblk_stride * jcp.typesize_in * ic_block_step;
			safe_add(reg_input, input_offset, reg_long_offt);
			add(reg_kernel, jcp.typesize_out * ic_block_step * oc_block);
...
void jit_avx512_common_conv_bwd_weights_kernel_f32 ::compute_oh_step_disp() {
	...
	if (jcp.is_1stconv) {
		...
		ic_block_step = (jcp.kw * jcp.ic_block <= 28 && !large_code) ? jcp.ic_block : 1;
	}

	bool too_large_to_unroll = (jcp.kw > 1 || jcp.kh > 1 || jcp.kd > 1) && (jcp.stride_w > 1 || jcp.stride_h > 1 || jcp.stride_d > 1);
...
void jit_avx512_common_conv_bwd_weights_kernel_f32::maybe_zero_kernel() {
	...
	L(zeroing_loop);
	{
		assert(jcp.oc_block * jcp.typesize_out == cpu_isa_traits<avx512_common>::vlen);
		for (int ic1 = 0; ic1 < jcp.ic_block; ic1++)
			vmovups(ptr[reg_kernel + reg_tmp + ic1 * jcp.oc_block * jcp.typesize_out], zero);
		add(reg_tmp, jcp.ic_block * jcp.oc_block * jcp.typesize_out);
		cmp(reg_tmp, jcp.ic_block * jcp.oc_block * jcp.kw * jcp.kh * jcp.kd * jcp.typesize_out);
		jnz(zeroing_loop);
	}
...
void jit_avx512_common_conv_bwd_weights_kernel_f32 ::compute_oh_loop_common() {
	...
	if (t_pad > 0) {
		...
		const int overflow = nstl::max(0, jcp.kh - div_up(t_pad + jcp.ih, dilate_h));
		...
		add(reg_kernel, jcp.typesize_out * underflow * jcp.kw * jcp.ic_block * jcp.oc_block);
		...
		if (kh_range < t_pad + jcp.ih) {
			...
			L(oh_tpad_label);
			{
				...
				sub(reg_kernel, jcp.typesize_out * stride_h * jcp.kw * jcp.ic_block * jcp.oc_block);
				add(reg_kh, stride_h);
				if (is_dilated) {
				...
				const int final_inp_ker_overlap = nstl::min(jcp.kh, div_up(jcp.ih, dilate_h));
		...
		if (t_pad <= jcp.oh * stride_h) {
			...
			if (t_pad % stride_h != 0) {
				...
				add(reg_kernel, jcp.typesize_out * inp_corr * jcp.kw * jcp.ic_block * jcp.oc_block);
				add(reg_input, jcp.typesize_in * inp_corr * iw * inp_mult);
			}
		} else {
			// kernel still overlaps padding (complete reset)
			assert(!is_dilated);
			sub(reg_kernel, jcp.typesize_out * (t_pad - jcp.oh * stride_h) * jcp.kw * jcp.ic_block * jcp.oc_block);
		}
	}
...
void jit_avx512_common_conv_bwd_weights_kernel_f32::compute_d_loop_common() {
	...
	int iw = jcp.ver == ver_4fma ? jcp.tr_iw : jcp.iw;
	int ow = jcp.ow;
	const int input_backpad_overlap = div_up(jcp.id + jcp.f_pad - (jcp.kd - 1), jcp.stride_d);

	const size_t filter_shift = jcp.typesize_out * jcp.kh * jcp.kw * ic_block * oc_block;
...
bool jit_avx512_common_conv_bwd_weights_kernel_f32::compute_full_spat_loop() {
	...
	bool ok = jcp.ver == ver_4fma && everyone_is(0, jcp.dilate_h, jcp.dilate_w) && everyone_is(1, jcp.stride_h, jcp.stride_w);
	if (!ok)
		return false;
	...
	if (h_block_size < nstl::max(1, jcp.t_pad) || jcp.b_pad > (jcp.oh % h_block_size == 0 ? h_block_size : jcp.oh % h_block_size))
		return false;
	...
	int num_inp_l2_pfs_per_fma_block = div_up(num_inp_l2_pfs, avg_h_loop_len * jcp.kw * jcp.kh);
	int num_out_l2_pfs = pad_ow * h_block_size;
	int num_out_l2_pfs_per_fma_block = div_up(num_out_l2_pfs, avg_h_loop_len * jcp.kw * jcp.kh);
	...
	auto out_addr = [&](int oi, int oj = 0) {
		assert(jcp.ver == ver_4fma);
		return ptr[reg_out + ((oi + oj * jcp.ow) * jcp.oc_block) * jcp.typesize_in];
	};
	...
	auto emit_block = [&](int h_block_size,
		...
		auto emit_step = [&](int ur_ow,
			...
			auto emit_out_pf = [&](int oi) {
				...
				if (oi + def_step_size < ur_ow || !block_wraparound)
					mic_prefetcht0(ptr[reg_out + ((def_step_size + oi) * jcp.oc_block * jcp.typesize_in)]);
				else {
					assert(block_wraparound);
					assert(oi + def_step_size >= ur_ow);
					mic_prefetcht0(ptr[reg_out_save + ((oi + def_step_size - ur_ow) * jcp.oc_block * jcp.typesize_in)]);
				}
#else
				...
				if (!block_wraparound)
					mic_prefetcht0(ptr[reg_out + (jcp.ow + oi) * jcp.oc_block * jcp.typesize_in]);
				else
					mic_prefetcht0(ptr[reg_out + reg_ohs - ((h_block_size - 1) * jcp.ow - oi) * jcp.oc_block * jcp.typesize_in]);
#endif
				if (oi < num_out_l2_pfs_per_fma_step)
					mic_prefetcht1(ptr[reg_out_pf_l2 + oi * jcp.oc_block * jcp.typesize_in]);
			};
			...
			auto emit_inp_pf = [&](int oi4, int ic1) {
				...
				int num_pfs = num_inp_l1_pfs_per_fma_step + num_inp_l2_pfs_per_fma_step;
				...
				if (pf_idx < num_inp_l2_pfs_per_fma_step)
					mic_prefetcht1(ptr[reg_inp_pf_l2 + pf_idx * jcp.ic_block * jcp.typesize_in]);
				else {
					pf_idx -= num_inp_l2_pfs_per_fma_step;
					// prefetch the 'tail' of the cache line because most of
					// the accesses are not aligned
					mic_prefetcht0(ptr[reg_inp_pf_l1 + pf_idx * jcp.ic_block * jcp.typesize_in + cache_line_size - jcp.typesize_in]);
				}
			};
		...
		int num_inp_l1_pfs = div_up(jcp.iw * jcp.typesize_in, cache_line_size) * jcp.ic_block;
		...
		if (full_w_unroll) {
			...
			int num_inp_l1_pfs_per_fma_step = div_up(num_inp_l1_pfs, num_w_iters_full);
			int num_inp_l2_pfs_per_fma_step = div_up(num_inp_l2_pfs_per_fma_block, num_w_iters_full);
			int num_out_l2_pfs_per_fma_step = div_up(num_out_l2_pfs_per_fma_block, num_w_iters_full);
	...
	auto emit_kh_kw_loop = [&](bool is_first_block, bool is_last_block,
		...
		int last_oh_block_size = jcp.oh - rnd_up(jcp.oh - h_block_size, h_block_size);
		int oh_block_size = (is_last_block) ? last_oh_block_size : h_block_size;
		// NB1: t_pad <= oh_block_size and b_pad <= last_oh_block_size
		int ih_block_size = oh_block_size - 1 + jcp.kh - is_first_block * jcp.t_pad - is_last_block * jcp.b_pad;
		...
		L(kh_loop);
		{
			...
			L(skip_ker_zeroing);
			add(reg_ker, jcp.oc_block * jcp.ic_block * jcp.kw * jcp.typesize_out);
			jmp(kh_loop_end, T_NEAR);
	...
	size_t out_block_step = pad_ow * jcp.oc_block * jcp.typesize_in * h_block_size;
	...
	if (!single_kh_kw_loop) {
		size_t ker_reset_offset = jcp.oc_block * jcp.ic_block * jcp.typesize_out * jcp.kw * jcp.kh;
...
bool jit_avx512_common_conv_bwd_weights_kernel_f32 ::flat_4ops_compute() {
	const auto &j = jcp;
	const bool ok = j.ver == ver_4fma && j.is_1stconv && everyone_is(0, j.dilate_h, j.dilate_w);
	if (!ok)
		return false;
	...
	auto addr_tr_src = [&](int kh, int iw) {
		return ptr[reg_ptr_tr_src + (kh * j.stride_w * j.tr_ld + iw) * jcp.typesize_in];
	};
	auto addr_dst = [&](int ow) {
		return ptr[reg_ptr_dst + ow * jcp.oc_block * jcp.typesize_in];
	};
	auto addr_wei = [&](int kh, int kw) {
		return ptr[reg_ptr_wei + (kh * j.kw + kw) * j.oc_block * jcp.typesize_out];
	};
	...
	auto emit_fma_block = [&](int kh_step) {
		...
		for (int ow = 0; ow < j.ow; ow += 4) {
			...
			for (int kh = 0; kh < kh_step; ++kh) {
				for (int kw = 0; kw < j.kw; ++kw) {
					const int iw = ow + (kw % j.stride_w) * j.tr_ld + (kw / j.stride_w);
					v4fmaddps(zmm_wei(kh, kw), zmm_dst(ow),
							addr_tr_src(kh, iw));
					if (1 && kh == 0 && kw < 4) {
						prefetcht1(ptr[reg_ptr_dst + (j.ow + ow + kw) * jcp.oc_block * jcp.typesize_in]);
					}
	...
	auto emit_kh_loop = [&]() {
		...
		L(l_kh_loop);
		{
			...
			add(reg_ptr_tr_src, j.kh_step * j.stride_w * j.tr_ld * jcp.typesize_in);
	...
	auto emit_oh_loop = [&]() {
		...
		L(l_oh_loop);
		{
			...
			add(reg_ptr_tr_src, j.stride_h * j.stride_w * j.tr_ld * jcp.typesize_in);
...
status_t jit_avx512_common_conv_bwd_weights_kernel_f32::init_conf(
	...
	bool ok = true
			  ...
			  && IMPLICATION(jcp.dilate_d != 0, jcp.stride_d == 1) && IMPLICATION(jcp.dilate_h != 0, jcp.stride_h == 1) && IMPLICATION(jcp.dilate_w != 0, jcp.stride_w == 1)
			  // special condition to simplify dilations in compute_oh_loop_common
			  && IMPLICATION(jcp.dilate_h != 0, kh_range <= jcp.ih);
	if (!ok)
		return status::unimplemented;

	jcp.r_pad = nstl::max(0, (jcp.ow - 1) * jcp.stride_w + (jcp.kw - 1) * (jcp.dilate_w + 1) - (jcp.iw + jcp.l_pad - 1));
	jcp.b_pad = nstl::max(0, (jcp.oh - 1) * jcp.stride_h + (jcp.kh - 1) * (jcp.dilate_h + 1) - (jcp.ih + jcp.t_pad - 1));
	jcp.back_pad = nstl::max(0, (jcp.od - 1) * jcp.stride_d + (jcp.kd - 1) * (jcp.dilate_d + 1) - (jcp.id + jcp.f_pad - 1));
	...
	auto wei_tag = with_groups ? pick(ndims - 3, gOIw16i16o, gOIhw16i16o, gOIdhw16i16o) : pick(ndims - 3, OIw16i16o, OIhw16i16o, OIdhw16i16o);
	...
	const bool boundaries_ok = true && jcp.t_pad <= max_pad && jcp.b_pad <= max_pad && IMPLICATION(jcp.f_pad > 0, jcp.kd < jcp.id + jcp.f_pad) && jcp.f_pad < jcp.kd;
	if (!boundaries_ok)
		return status::unimplemented;
	...
	if (jcp.is_1stconv) {
		...
		const bool src_ok = true && utils::everyone_is(data_type::f32, src_d.data_type(), diff_weights_d.data_type(), diff_dst_d.data_type()) && one_of(jcp.ic, 1, 2, 3) && jcp.ngroups == 1;
		if (!src_ok)
			return status::unimplemented;
		...
		const auto wei_4fma_tag = with_groups ? pick(ndims - 3, gOiw16o, gOihw16o, gOidhw16o) : pick(ndims - 3, Oiw16o, Oihw16o, Oidhw16o);
		...
		const bool use_4fma = true && one_of(ndims, 3, 4) && mayiuse(avx512_mic_4ops) && mkldnn_thr_syncable() && everyone_is(0, jcp.dilate_d, jcp.dilate_h, jcp.dilate_w) && everyone_is(0, jcp.l_pad, jcp.r_pad, jcp.t_pad, jcp.b_pad) && jcp.kw <= 28 - jcp.with_bias && jcp.stride_w == 4 && tr_ld / jcp.simd_w <= 4 /* [bwd_w:tr_src:r1] */
		...
		if (use_4fma) {
			...
			wei_tag = with_groups ? pick(ndims - 3, gOwi16o, gOhwi16o, gOdhwi16o) : pick(ndims - 3, Owi16o, Ohwi16o, Odhwi16o);
		...
		if (ok_to_pad_channels)
			jcp.ic = rnd_up(jcp.ic, jcp.ic_block);
		jcp.nb_ic = jcp.ic / jcp.ic_block;
		if ((mayiuse(avx512_mic) || mayiuse(avx512_core)) && utils::everyone_is(data_type::f32,
	...
	bool args_ok = true && jcp.ic % jcp.ic_block == 0 && jcp.oc % jcp.oc_block == 0 && jcp.ic <= src_d.padded_dims()[1] && jcp.oc <= diff_dst_d.padded_dims()[1] && jcp.ic <= diff_weights_d.padded_dims()[with_groups + 1] && jcp.oc <= diff_weights_d.padded_dims()[with_groups + 0];
	if (!args_ok)
		return status::unimplemented;
...
void jit_avx512_common_conv_bwd_weights_kernel_f32::init_scratchpad(
	...
	if (jcp.ver == ver_4fma) {
		if (jcp.is_1stconv) {
			...
			const size_t tr_src_size = max_nthr * min_tr_src_size_per_thr + jcp.tr_src_num_guard_elems;
			scratchpad.book(key_conv_tr_src, jcp.typesize_in * tr_src_size);
		}
...
void jit_avx512_common_conv_bwd_weights_kernel_f32::balance(
	...
	auto calc_mem_cost = [=](int nthr_mb, int nthr_oc_b, int nthr_ic_b) {
		...
		return 0 + src_coef * div_up(j.mb, nthr_mb) * div_up(j.ngroups, nthr_g_) * div_up(j.nb_ic, nthr_ic_b) * j.ic_block * j.ih * j.iw * j.id / j.stride_d / j.stride_h / j.stride_w /* (n1) */
			   + dst_coef * div_up(j.mb, nthr_mb) * div_up(j.ngroups, nthr_g_) * div_up(j.nb_oc, nthr_oc_b) * j.oc_block * j.oh * j.ow * j.od + wei_coef /* (n2) */
																																						* div_up(j.ngroups, nthr_g_) * div_up(j.nb_oc, nthr_oc_b) * div_up(j.nb_ic, nthr_ic_b) * j.kh * j.kw * j.kd * j.ic_block * j.oc_block;
	};
	...
	if (!mayiuse(avx512_mic)) {
		auto calc_comp_cost = [=](int nthr_mb, int nthr_oc_b, int nthr_ic_b) {
			return 1 * div_up(j.mb, nthr_mb) * div_up(j.ngroups, nthr_g_) * div_up(j.nb_oc, nthr_oc_b) * div_up(j.nb_ic, nthr_ic_b);
		};
		...
		for (int nthr_mb = 1; nthr_mb <= nthr_mb_max; ++nthr_mb) {
			...
			for (int nthr_oc_b = 1; nthr_oc_b <= nthr_oc_b_max; ++nthr_oc_b) {
				...
				const bool opt1 = comp_cost <= best_comp_cost && mem_cost < 1.1 * best_mem_cost;
