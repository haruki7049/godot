...
void _jit_uni_dw_convolution_fwd_t<isa>::execute_forward(
	...
	auto kernel_params = [&](int ur_w_step, int ow, int oh, int ih, int kh,
		...
		const int i_r_overflow = nstl::max(jcp.iw, (ow * str_w + (jcp.kw - 1) * dil_w - jcp.l_pad + 1)) - jcp.iw;

		const int iw = nstl::max((ow * str_w - jcp.l_pad + div_up(i_l_overflow, dil_w) * dil_w), 0);
		const int kw = div_up(i_l_overflow, dil_w);

		const int kw_padding = jcp.kw - div_up(i_l_overflow, dil_w) - div_up(i_r_overflow, dil_w);
		...
		if (bias)
			par_conv.bias = &bias[bias_d.blk_off(ch * jcp.ch_block)];
	...
	parallel_nd(jcp.mb, chb_work, jcp.oh,
			[&](int n, int chb, int oh) {
				...
				const int i_b_overflow = nstl::max(jcp.ih,
												 (int)(oh * str_h + (jcp.kh - 1) * dil_h - jcp.t_pad + 1)) -
										 jcp.ih;

				const int ih = nstl::max((int)(oh * str_h - jcp.t_pad + div_up(i_t_overflow, dil_h) * dil_h), 0);
				const int kh = div_up(i_t_overflow, dil_h);
				const int kh_padding = jcp.kh - div_up(i_t_overflow, dil_h) - div_up(i_b_overflow, dil_h);
...
void _jit_uni_dw_convolution_bwd_data_t<isa>::execute_backward_data(
	...
	auto kernel_params = [&](int ur_str_w, int iw, int oh, int ih,
		...
		par_conv.filt = &weights[weights_d.blk_off(ch, 0, 0, i_b_overflow + stride_off_h, i_r_overflow + stride_off_w)];
	...
	parallel_nd(jcp.mb, chb_work, jcp.ih,
			[&](int n, int chb, int ih) {
				...
				for (int i_str_w = 0; i_str_w < jcp.stride_w; i_str_w++) {
					...
					// main loop
					ur_str_w = nstl::min((jcp.iw - jcp.kw + jcp.r_pad - iw) / jcp.stride_w, jcp.iw);
					if (ur_str_w > 0) {
						jit_conv_call_s par_conv = kernel_params(ur_str_w, iw, oh,
								ih, i_t_overflow, i_b_overflow,
								stride_off_h, ch, ch_num, n);
...
_jit_uni_dw_convolution_bwd_weights_t<isa>::
		_jit_uni_dw_convolution_bwd_weights_t(const pd_t *apd) : cpu_primitive_t(apd),
																 kernel_(nullptr),
																 acc_ker_(nullptr) {
...
void _jit_uni_dw_convolution_bwd_weights_t<isa>::execute_backward_weights(
	...
	auto set_kernel_params = [&](jit_dw_conv_call_s *conv_params,
		...
		conv_params->filter_pad_off = filter_off * jcp.kw * ch_block * sizeof(float);
		conv_params->oh_index = oh_s;
		conv_params->oh_count = oh_e;

		size_t diff_dst_off = ((batch * (jcp.ngroups / ch_block) + group) * jcp.oh + oh_start) * jcp.ow;

		size_t src_off = ((batch * (jcp.ngroups / ch_block) + group) * jcp.ih + ih_s - tpad_underflow_off) * jcp.iw;
	...
	parallel(jcp.nthr, [&](const int ithr, const int nthr) {
		...
		auto diff_wei = ithr_mb == 0 ? diff_weights : diff_wei_reduction_buf + (ithr_mb - 1) * wei_size;
		auto diff_bia = ithr_mb == 0 ? diff_bias : diff_bia_reduction_buf + (ithr_mb - 1) * bias_size;
		...
		for (int g = g_start; g < g_end; ++g) {
			...
			for (int mb = mb_start; mb < mb_end; ++mb) {
				...
				while (oh < jcp.oh) {
					...
					auto kh_b_padding = (oh * jcp.stride_h + jcp.kh - 1 > jcp.ih) ?
												jcp.b_pad - (h_work - 1) :
												0;
		...
		if (do_parallel_reduction() && jcp.nthr_mb > 1) {
			...
			for (int thr_mb = 1; thr_mb < jcp.nthr_mb; ++thr_mb) {
				auto *src_data = diff_wei_reduction_buf + (thr_mb - 1) * wei_size + reduct_off;
		...
		for (int g = 0; g < jcp.nb_ch; ++g) {
			...
			if (jcp.with_bias) {
				...
				for (int g_block = 0; g_block < ch_block; ++g_block) {
					...
					diff_bias[bias_offset] += diff_bia_reduction_buf[b_accum_offset + bias_offset];
				}
			}
