...
const Zmm _jit_avx512_core_x8s8s32x_fwd_kernel<Zmm>::
	...
	return mask_flag ? (store ? zmm_in | ktail_mask : zmm_in | ktail_mask | T_z) : zmm_in;
}
...
void _jit_avx512_core_x8s8s32x_fwd_kernel<Vmm>::store_output(
	...
	if (p_sum_scale) { // post_op: sum
		for (int k = 0; k < nb_oc_block; k++) {
			const bool mask_flag = last_oc_block_flag && k == nb_oc_block - 1;
			for (int j = 0; j < ur_w; j++) {
				int aux_output_offset = jcp.typesize_out * (k * oc_block + j * jcp.oc_without_padding * jcp.ngroups);
...
void _jit_avx512_core_x8s8s32x_fwd_kernel<Zmm>::compute_ker_dw(
	...
	for (int ci = 0; ci < jcp.nb_ch_blocking; ci++) {
		const bool mask_flag = last_ic_block_flag != no_last_block && ci == jcp.nb_ch_blocking - 1;
		if (jcp.is_resrc_depthwise && !h_padded) {
			...
			for (int ii = ii_start; ii <= ii_end; ii++) {
				...
				const Zmm zmm_inp_msk = mask_flag ? zmm_inp_tmp | ktail_mask | T_z : zmm_inp_tmp;
				if (jcp.is_fast_depthwise) {
					assert(!mask_flag);
...
void _jit_avx512_core_x8s8s32x_fwd_kernel<Vmm>::compute_ker(int ur_w, int pad_l,
	...
	auto input_offset = [=](int oi, int ic, int ki) {
		return jcp.typesize_in * ((ki * (jcp.dilate_w + 1) + oi * stride_w - pad_l) * jcp.ic_without_padding * jcp.ngroups + 4 * ic);
	};
	auto kernel_offset = [=](int ii, int ic, int ki) {
		return jcp.typesize_in * ((ii * jcp.nb_ic * jcp.kh * jcp.kw + ki) * ch_block_all + 4 * ic * oc_block);
	};
	...
	for (int ki = 0; ki < kw; ki++) {
		...
		int icb = (last_ic_block_flag != no_last_block) ? div_up((jcp.ic_without_padding % ic_block), 4) : ic_block / 4;
		for (int ic = 0; ic < icb; ic++) {
			if (h_padded == true) {
				...
				for (int jj = _start; jj < _end; jj++) {
					...
					if (jj >= jj_start && jj < jj_end) {
						if (last_ic_block_flag == last_sp_block && tail_size != 0 && ic == icb - 1) {
			...
			for (int ii = 0; ii < nb_oc_block; ii++) {
				...
				vmovups(vmm_wei,
						EVEX_compress_addr(aux_reg_ker, aux_kernel_offset));
				for (int jj = _start; jj < _end; jj++) {
					Vmm inp = (h_padded == true) ? vmm_inp(0, nb_oc_block) : vmm_inp(jj, nb_oc_block);
...
void _jit_avx512_core_x8s8s32x_fwd_kernel<Vmm>::kh_loop(
	...
	int shift_input_ptr = jcp.typesize_in * (jcp.dilate_h + 1) * jcp.iw * jcp.ic_without_padding * jcp.ngroups;
...
void _jit_avx512_core_x8s8s32x_fwd_kernel<Vmm>::generate() {
	...
	int inp_shift_pad = jcp.typesize_in * (jcp.ur_w * jcp.stride_w - jcp.l_pad) * jcp.ic_without_padding * jcp.ngroups;
	int inp_shift_pad_second_block = -1 * jcp.typesize_in * jcp.l_pad * jcp.ic_without_padding * jcp.ngroups;
	...
	if (jcp.ngroups % jcp.ch_block != 0 || jcp.oc_without_padding != jcp.oc) {
		int tail_size = jcp.is_depthwise ? jcp.ngroups % jcp.ch_block : jcp.oc_without_padding % jcp.oc_block;
	...
	int r_pad = nstl::max(0, (jcp.ow - 1) * jcp.stride_w + (jcp.kw - 1) * (jcp.dilate_w + 1) - (jcp.iw + jcp.l_pad - 1));
	int n_oi = jcp.ow / jcp.ur_w;
	int r_pad1 = (jcp.ur_w * n_oi - 1) * jcp.stride_w + (jcp.kw - 1) * (jcp.dilate_w + 1) - (jcp.iw + jcp.l_pad - 1);
	...
	if (jcp.nb_ow == 1) {
		...
		int n_oi_last_ow_block = (jcp.ow - jcp.ow_block * (jcp.nb_ow - 1)) / jcp.ur_w;
		...
		bool last_ow_block_padded = (r_pad1 > 0 || jcp.ur_w_tail == 0) && n_oi_last_ow_block > 0;
	...
	if (jcp.is_fast_depthwise) {
		...
		const uint32_t _idx[] = { 0, 4, 8, 12, 1, 5, 9, 13, 2, 6, 10, 14, 3, 7, 11, 15 };
...
bool jit_avx512_core_x8s8s32x_fwd_kernel::post_ops_ok(
	...
	switch (p.len_) {
		case 0:
			return true;
		case 1:
			return is_eltwise(0) || p.contain(sum, 0);
		case 2:
...
status_t jit_avx512_core_x8s8s32x_fwd_kernel::init_conf(jit_conv_conf_t &jcp,
	...
	if (!(mayiuse(avx512_core) && one_of(src_d.data_type(), data_type::u8, data_type::s8) && weights_d.data_type() == data_type::s8 && one_of(dst_d.data_type(), data_type::f32, data_type::s32, data_type::s8, data_type::u8)))
	...
	jcp.is_fast_depthwise = true && jcp.is_depthwise && jcp.ver == ver_vnni && jcp.ngroups % jcp.ch_block == 0; // for groups not multiple of 16
																												// would require byte masking
																												// for load from src
	jcp.is_resrc_depthwise = jcp.is_depthwise && jcp.stride_w < jcp.kw && jcp.kw < 4 && jcp.dilate_w == 0;
	if (jcp.is_depthwise) {
		jcp.max_regs_ur = 31 - jcp.is_fast_depthwise - !jcp.is_resrc_depthwise - 2 * jcp.signed_input - (jcp.ver != ver_vnni);
	} else {
	...
	auto set_or_check_wei_format = [&]() {
		...
		if (jcp.ic_block == 16 || jcp.ch_block == 16) {
			if (is_1d) {
				wei_tag = with_groups ? jcp.is_depthwise ? Goiw16g : gOIw4i16o4i : OIw4i16o4i;
			} else {
				wei_tag = with_groups ? jcp.is_depthwise ? Goihw16g : gOIhw4i16o4i : OIhw4i16o4i;
			}
		...
		if (jcp.signed_input) {
			want_wei_md.extra.flags = 0 | memory_extra_flags::compensation_conv_s8s8 | memory_extra_flags::scale_adjust;
			want_wei_md.extra.compensation_mask = (1 << 0) + (with_groups && !jcp.is_depthwise ? (1 << 1) : 0);
	...
	auto is_oc_blocking_ok = [&](int block) {
		...
		return jcp.nb_oc % block == 0 && jcp.l_pad <= ur_w && jcp.ow % ur_w != 1;
	};
	...
	if (jcp.ver == ver_vnni && jcp.mb == 1 && jcp.kh == 3 && jcp.kw == 3 && jcp.stride_w == 1 && jcp.ic % 64 == 0)
	...
	if (jcp.mb == 1 && jcp.ow <= size_treshold_for_nb_oc_blocking_reduction && jcp.stride_w == 1 && !(jcp.kh == 1 && jcp.kw == 3) && !(jcp.kh >= 7 && jcp.oc % 64 == 0)) {
		...
		for (; jcp.nb_oc_blocking > 1; jcp.nb_oc_blocking--)
			if (jcp.nb_oc_blocking_thr_chunk % jcp.nb_oc_blocking == 0 && is_oc_blocking_ok(jcp.nb_oc_blocking))
				break;
	}

	if (jcp.is_resrc_depthwise)
		jcp.ur_w = (jcp.max_regs_ur - jcp.kw + jcp.stride_w) / (jcp.nb_ch_blocking + jcp.stride_w);
	else
		jcp.ur_w = jcp.max_regs_ur / (jcp.is_depthwise ? jcp.nb_ch_blocking : jcp.nb_oc_blocking + 1);
	if (jcp.ow < jcp.ur_w)
		jcp.ur_w = jcp.ow;
	jcp.ur_w_tail = jcp.ow % jcp.ur_w;

	jcp.ow_block = jcp.ow;
	int base_work_amount = jcp.mb * jcp.nb_ch * jcp.oh * (jcp.nb_oc / jcp.nb_oc_blocking_thr_chunk);
	float best_thr_eff = (float)base_work_amount / rnd_up(base_work_amount, nthreads);
	int max_nb_ow = div_up(jcp.ow, 2 * jcp.ur_w);
	for (int nb_ow = 1; nb_ow <= max_nb_ow; nb_ow++) {
		int ow_block = nstl::min(rnd_up(div_up(jcp.ow, nb_ow), jcp.ur_w), jcp.ow);
		if (ow_block < jcp.nb_oc_blocking_thr_chunk * jcp.oc_block && best_thr_eff > 0.8f)
			break;
	...
	bool args_ok = true && jcp.oc % jcp.oc_block == 0 && jcp.l_pad <= jcp.ur_w && IMPLICATION(!jcp.is_1stconv, jcp.ic % jcp.ic_block == 0);
	if (!args_ok)
		return status::unimplemented;
	...
	jcp.wei_adj_scale =
			(weights_d.extra().flags | memory_extra_flags::scale_adjust) ? weights_d.extra().scale_adjust : 1.f;
