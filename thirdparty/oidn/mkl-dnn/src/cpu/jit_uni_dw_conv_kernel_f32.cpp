	...
	for (int i = 0; i < repeats; i++) {
		for (int ch = 0; ch < ur_ch_blocks; ch++) {
			for (int ow = 0; ow < ur_w; ow++) {
				...
				int o_off = ch * jcp.oh * jcp.ow * jcp.ch_block + ow * jcp.ch_block + i * 4;
		...
		L(kw_label);
		{
			...
			for (int i = 0; i < repeats; i++) {
				for (int ch = 0; ch < ur_ch_blocks; ch++) {
					...
					for (int ow = 0; ow < ur_w; ow++) {
						int inp_off = ch * jcp.ih * jcp.iw * ch_blk + ow * stride_w * ch_blk + i * 4;
		...
		for (int i = 0; i < repeats; i++) {
			for (int ch = 0; ch < ur_ch_blocks; ch++) {
				for (int kw = 0; kw < jcp.kw; kw++) {
					...
					for (int ow = 0; ow < ur_w; ow++) {
						int inp_off = ch * jcp.ih * jcp.iw * ch_blk + ow * stride_w * ch_blk + kw * ch_blk * dilate_w + i * 4;

						Vmm vmm_src = get_src_reg(0);
						uni_vmovups(vmm_src, ptr[aux_reg_input + inp_off * sizeof(float)]);
...
status_t jit_uni_dw_conv_fwd_kernel_f32<isa>::init_conf(jit_conv_conf_t &jcp,
	...
	bool ok_to_pad_channels = true && jcp.oc == jcp.ngroups && jcp.ic == jcp.ngroups && one_of(isa, avx512_common, avx2);
	...
	bool args_ok = true && jcp.oc == jcp.ngroups && jcp.ic == jcp.ngroups && jcp.ngroups % simd_w == 0 && jcp.src_tag == dat_tag && jcp.wei_tag == wei_tag && jcp.dst_tag == dat_tag && jcp.ic <= src_d.padded_dims()[1] && jcp.oc <= dst_d.padded_dims()[1] && jcp.ngroups <= weights_d.padded_dims()[0];
	if (!args_ok)
		return status::unimplemented;

	jcp.ur_w = isa == avx512_common ? 6 : isa == avx2 ? 4 :
														3;

	jcp.ch_block = simd_w;
	jcp.nb_ch = jcp.oc / jcp.ch_block;
	jcp.nb_ch_blocking = isa == avx512_common ? 4 : isa == avx2 ? 3 :
																  2;
	...
	for (int i = 0; i < repeats; i++) {
		for (int ch = 0; ch < ur_ch_blocks; ch++) {
			for (int w = 0; w < ur_str_w; w++) {
				Vmm vmm_acc = get_acc_reg(i * ur_ch_blocks * ur_str_w + ch * ur_str_w + w);
				uni_vpxor(vmm_acc, vmm_acc, vmm_acc);
			}
		}
	}
			...
			for (int i = 0; i < repeats; i++) {
				for (int ch = 0; ch < ur_ch_blocks; ch++) {
					...
					for (int w = 0; w < ur_str_w; w++) {
						...
						Vmm vmm_src = get_src_reg(0);
						uni_vmovups(vmm_src, ptr[aux1_reg_ddst + ddst_off * sizeof(float)]);
...
status_t jit_uni_dw_conv_bwd_data_kernel_f32<isa>::init_conf(
	...
	bool args_ok = true && jcp.oc == jcp.ngroups && jcp.ic == jcp.ngroups && jcp.ngroups % simd_w == 0 && jcp.dilate_h == 0 && jcp.dilate_w == 0 && jcp.src_tag == dat_tag && jcp.wei_tag == wei_tag && jcp.dst_tag == dat_tag && jcp.oh == (jcp.ihp - jcp.kh) / jcp.stride_h + 1 && jcp.ow == (jcp.iwp - jcp.kw) / jcp.stride_w + 1 && jcp.ic <= diff_src_d.padded_dims()[1] && jcp.oc <= diff_dst_d.padded_dims()[1] && jcp.ngroups <= weights_d.padded_dims()[0];
	if (!args_ok)
		return status::unimplemented;
		...
		for (int i_ur = 0; i_ur < unroll_w; ++i_ur) {
			...
			if (i_ur == 0) {
				for (int c = 0; c < input_overlap; ++c) {
					int off_input = ((c - pad_offset) * reg_repeats + r) * simd_w;
					Vmm vmm_input = get_input_reg((c % jcp.kw) * reg_repeats + r);
				...
				for (int c = 0; c < cascade_input; ++c) {
					...
					int off_input = ((overlap + c - pad_offset) * reg_repeats + r) * simd_w;
			...
			for (int i_kw = 0; i_kw < jcp.kw; ++i_kw) {
				...
				if (io_overlap - l_pad < 0 || io_overlap - jcp.l_pad >= right_border)
					continue;
...
jit_uni_dw_conv_bwd_weights_kernel_f32<isa>::compute_ow_block_unroll() {
	...
	if (jcp.ow > max_unroll_w) {
		...
		if ((unroll_w_tail == 0 && r_pad != 0) || (r_pad > 0 && r_pad >= unroll_w_tail)) {
			if (unroll_w_trips > 1) {
...
status_t jit_uni_dw_conv_bwd_weights_kernel_f32<isa>::init_conf(
	...
	bool args_ok = true && jcp.src_tag == dat_tag && jcp.wei_tag == wei_tag && jcp.dst_tag == dat_tag && jcp.ngroups % jcp.ch_block == 0 && jcp.dilate_h == 0 && jcp.dilate_w == 0 && jcp.kw <= 3 && jcp.oh == (jcp.ihp - jcp.kh) / jcp.stride_h + 1 && jcp.ow == (jcp.iwp - jcp.kw) / jcp.stride_w + 1;
	if (!args_ok)
		return status::unimplemented;
	...
	const bool boundaries_ok = true && jcp.t_pad <= max_hpad && jcp.b_pad <= max_hpad && jcp.l_pad <= max_wpad && jcp.r_pad <= max_wpad;
	if (!boundaries_ok)
		return status::unimplemented;
