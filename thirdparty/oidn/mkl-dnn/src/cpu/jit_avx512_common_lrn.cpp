...
struct jit_avx512_common_lrn_fwd_t::jit_avx512_common_lrn_kernel_f32 : public jit_generator {
	int HW, W;
	bool is_first;
	bool is_last;
	bool is_single;
	...
	inline void compute_loop(int loop_size_param) {
		...
		IRB_LOOP(vmovups(zreg(irb, za), EVEX_compress_addr(t, irb * BUFFER_BLOCK + XMM_SIZE - 2 * sizeof(float))));
		IRB_LOOP(vmovups(zreg(irb, zb), EVEX_compress_addr(t, irb * BUFFER_BLOCK + XMM_SIZE - sizeof(float))));
		IRB_LOOP(vmovups(zreg(irb, zd), EVEX_compress_addr(t, irb * BUFFER_BLOCK + XMM_SIZE + sizeof(float))));
		IRB_LOOP(vmovups(zreg(irb, ze), EVEX_compress_addr(t, irb * BUFFER_BLOCK + XMM_SIZE + 2 * sizeof(float))));
	...
	jit_avx512_common_lrn_kernel_f32(
			...
			size_t code_size = 2 * Xbyak::DEFAULT_MAX_CODE_SIZE) : jit_generator(code_ptr, code_size),
																   pk(prop_kind),
...
status_t jit_avx512_common_lrn_fwd_t::pd_t::init() {
	...
	bool ok = true && mayiuse(avx512_common) && is_fwd() && !has_zero_dim_memory() && everyone_is(data_type::f32, data_d.data_type()) && data_d.ndims() == 4 && data_d.dims()[1] % vsize == 0 && attr()->has_default_values();
	if (!ok)
	...
	bool args_ok_across = true && desc()->alg_kind == lrn_across_channels && desc()->local_size == 5 && desc()->lrn_beta == 0.75 && data_d.matches_tag(format_tag::nChw16c);

	return args_ok_across ? success : unimplemented;
}

jit_avx512_common_lrn_fwd_t::jit_avx512_common_lrn_fwd_t(const pd_t *apd) : cpu_primitive_t(apd), use_h_parallelism(0), ker_(nullptr), ker_first_(nullptr), ker_last_(nullptr) {
	using namespace alg_kind;
	const int C = pd()->C();
	const int H = pd()->H();
	const int W = pd()->W();
	...
	parallel(0, [&](const int ithr, const int nthr) {
		...
		if (use_h_parallelism) {
			...
			for (size_t iwork = start; iwork < end; ++iwork) {
				auto offset = n * C * H * W + c16 * H * W * vsize + h * W * vsize;
				auto ws_offset0 = n * C * H * 2 * W + c16 * H * 2 * W * vsize + h * 2 * W * vsize;
...
struct jit_avx512_common_lrn_bwd_t::jit_avx512_common_lrn_kernel_f32 : public jit_generator {
	int HW, W;
	bool is_first;
	bool is_last;
	bool is_single;
	...
	inline void compute_loop(int loop_size_param, int prefetchL1,
		...
		if (!is_first && !is_single) {
			if (prefetchL1)
				IRB_LOOP(mic_prefetcht0(ptr[workspace1 + (irb + prf0_offt - 2 * HW) * vlen]));
			if (prefetchL1)
				IRB_LOOP(mic_prefetcht0(ptr[diffdst + (irb + prf0_offt - HW) * vlen]));
		}
		...
		if (!is_last && !is_single) {
			if (prefetchL1)
				IRB_LOOP(mic_prefetcht0(ptr[workspace1 + (irb + prf0_offt + 2 * HW) * vlen]));
			if (prefetchL2)
				IRB_LOOP(mic_prefetcht2(ptr[workspace1 + (irb + prf2_offt + 2 * HW) * vlen]));

			if (prefetchL1)
				IRB_LOOP(mic_prefetcht0(ptr[diffdst + (irb + prf0_offt + HW) * vlen]));
			if (prefetchL2)
				IRB_LOOP(mic_prefetcht2(ptr[diffdst + (irb + prf2_offt + HW) * vlen]));
		}
		if (prefetchL1)
		...
		if (!is_first && !is_single) {
			IRB_LOOP(vmovups(xreg(irb, xws1_prev), ptr[workspace1 + (irb - 2 * HW) * vlen + SRC_PREV_OFFSET]));
			IRB_LOOP(vmovups(xreg(irb, xdiffdst_prev), ptr[diffdst + (irb - HW) * vlen + SRC_PREV_OFFSET]));
		...
		if (!is_last && !is_single) {
			IRB_LOOP(vmovups(xreg(irb, xws1_next), ptr[workspace1 + (irb + 2 * HW) * vlen]));
			IRB_LOOP(vmovups(xreg(irb, xdiffdst_next), ptr[diffdst + (irb + HW) * vlen]));
		...
		IRB_LOOP(vmovups(zreg(irb, za), EVEX_compress_addr(t, irb * BUFFER_BLOCK + XMM_SIZE - 2 * sizeof(float))));
		IRB_LOOP(vmovups(zreg(irb, zb), EVEX_compress_addr(t, irb * BUFFER_BLOCK + XMM_SIZE - 1 * sizeof(float))));
		IRB_LOOP(vmovups(zreg(irb, zd), EVEX_compress_addr(t, irb * BUFFER_BLOCK + XMM_SIZE + 1 * sizeof(float))));
		IRB_LOOP(vmovups(zreg(irb, ze), EVEX_compress_addr(t, irb * BUFFER_BLOCK + XMM_SIZE + 2 * sizeof(float))));
	...
	jit_avx512_common_lrn_kernel_f32(
			...
			int use_h_parallel,
			void *code_ptr = nullptr,
			size_t code_size = 1 * Xbyak::DEFAULT_MAX_CODE_SIZE) : jit_generator(code_ptr, code_size),
																   nalphabeta(-2 * A * B),
																   use_h_parallelism(use_h_parallel) {
		this->preamble();
...
status_t jit_avx512_common_lrn_bwd_t::pd_t::init() {
	...
	const memory_desc_wrapper data_d(src_md());
	bool ok = true && mayiuse(avx512_common) && !is_fwd() && utils::everyone_is(data_type::f32, data_d.data_type()) && !has_zero_dim_memory() && data_d.ndims() == 4 && data_d.dims()[1] % vsize == 0 && attr()->has_default_values();
	if (!ok)
		return unimplemented;
...
jit_avx512_common_lrn_bwd_t::jit_avx512_common_lrn_bwd_t(const pd_t *apd) : cpu_primitive_t(apd), use_h_parallelism(0), ker_(nullptr), ker_first_(nullptr), ker_last_(nullptr) {
	const int C = pd()->C();
	const int H = pd()->H();
	const int W = pd()->W();
	const int ls = pd()->desc()->local_size;
