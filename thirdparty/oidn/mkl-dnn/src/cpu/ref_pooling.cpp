...
void ref_pooling_fwd_t<data_type, acc_type>::execute_forward(
	...
	auto set_ws = [=](int mb, int oc, int od, int oh, int ow, int value) {
		if (ws) {
			...
			size_t offset = is_3d ? ws_d.off(mb, oc, od, oh, ow) : ws_d.off(mb, oc, oh, ow);
			;
	...
	auto ker_avg = [=](data_t *d, int mb, int oc, int oh, int ow) {
		...
		auto num_summands = (alg == pooling_avg_include_padding) ? KW * KH : (ih_end - ih_start) * (iw_end - iw_start);
	...
	auto ker_avg_3d = [=](data_t *d, int mb, int oc, int od, int oh, int ow) {
		...
		auto num_summands = (alg == pooling_avg_include_padding) ? KW * KH * KD : (ih_end - ih_start) * (iw_end - iw_start) * (id_end - id_start);
	...
	if (alg == pooling_max) {
		parallel_nd(MB, OC, OD, OH, OW,
				[&](int mb, int oc, int od, int oh, int ow) {
					data_t *d = is_3d ? &dst[dst_d.off(mb, oc, od, oh, ow)] : &dst[dst_d.off(mb, oc, oh, ow)];
					d[0] = nstl::numeric_limits<data_t>::lowest();
					set_ws(mb, oc, od, oh, ow, 0);
					if (is_3d)
						ker_max_3d(d, mb, oc, od, oh, ow);
					else
...
void ref_pooling_bwd_t<data_type, acc_type>::execute_backward(
	...
	auto ker_max = [=](const data_t *d, int mb, int oc, int oh, int ow) {
		...
		const int index = ws_d.data_type() == data_type::u8 ? (int)ws[ws_off] : ((int *)ws)[ws_off];
			...
			for (int od = 0; od < OD; ++od) {
				for (int oh = 0; oh < OH; ++oh) {
					for (int ow = 0; ow < OW; ++ow) {
						const data_t *d = is_3d ? &diff_dst[diff_dst_d.off(mb, oc, od, oh, ow)] : &diff_dst[diff_dst_d.off(mb, oc, oh, ow)];
						if (is_3d)
							ker_max_3d(d, mb, oc, od, oh, ow);
						else
							ker_max(d, mb, oc, oh, ow);
