...
jit_uni_lrn_fwd_t<isa>::jit_uni_lrn_fwd_t(const pd_t *apd) : cpu_primitive_t(apd), ker_(nullptr), ker_first_(nullptr), ker_last_(nullptr) {
...
status_t jit_uni_lrn_fwd_t<isa>::pd_t::init() {
	...
	bool ok = true && mayiuse(isa) && is_fwd() && everyone_is(data_type::f32, data_d.data_type()) && !has_zero_dim_memory() && data_d.ndims() == 4 && data_d.dims()[1] % VECTOR_LENGTH == 0 && data_d.dims()[1] >= 2 * VECTOR_LENGTH && desc()->lrn_beta == 0.75 && attr()->has_default_values();
	if (!ok)
	...
	bool args_ok_across = true && desc()->alg_kind == lrn_across_channels && desc()->local_size == 5 && one_of(dat_tag_, nChw8c, nchw, nhwc);

	const int jit_max_local_size = 5; // bigger size triggers too big code size
	bool args_ok_within = true && desc()->alg_kind == lrn_within_channel && desc()->local_size <= (jit_max_local_size <= MAX_LOCAL_SIZE ? jit_max_local_size : MAX_LOCAL_SIZE) && data_d.dims()[2] >= desc()->local_size && data_d.dims()[3] >= desc()->local_size && one_of(dat_tag_, nChw8c);

	return args_ok_across || args_ok_within ? success : unimplemented;
}

template <cpu_isa_t isa>
jit_uni_lrn_bwd_t<isa>::jit_uni_lrn_bwd_t(const pd_t *apd) : cpu_primitive_t(apd), ker_(nullptr), ker_first_(nullptr), ker_last_(nullptr) {
	using namespace alg_kind;
	const int C = pd()->C();
	const int H = pd()->H();
	const int W = pd()->W();
...
void jit_uni_lrn_bwd_t<isa>::execute_backward(const exec_ctx_t &ctx) const {
	...
	if (use_h_parallelizm) {
		parallel_nd(N, C / VECTOR_LENGTH, H, [&](int n, int c8, int h) {
			auto offset = n * C * H * W + c8 * H * W * VECTOR_LENGTH + h * W * VECTOR_LENGTH;
			jit_args_bwd_t args;
			args.src = &src[offset];
...
status_t jit_uni_lrn_bwd_t<isa>::pd_t::init() {
	...
	const memory_desc_wrapper data_d(src_md());
	bool ok = true && mayiuse(isa) && !is_fwd() && utils::everyone_is(data_type::f32, data_d.data_type()) && !has_zero_dim_memory() && data_d.ndims() == 4 && data_d.dims()[1] % VECTOR_LENGTH == 0 && desc()->lrn_beta == 0.75 && attr()->has_default_values();
	if (!ok)
		return unimplemented;
	...
	bool args_ok_across = true && desc()->alg_kind == lrn_across_channels && desc()->local_size == 5 && utils::one_of(dat_tag_, nChw8c);
