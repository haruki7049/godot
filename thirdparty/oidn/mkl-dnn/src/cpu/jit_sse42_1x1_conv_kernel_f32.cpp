...
void jit_sse42_1x1_conv_kernel_f32::generate_reduce_loop(
	...
	auto bcast_ptr = [=](int u, int j) {
		...
		if (one_of(jcp.prop_kind,
			...
			assert(jcp.reduce_loop_unroll == (jcp.prop_kind == backward_data) ? jcp.oc_block : jcp.ic_block);
			auto height = (jcp.prop_kind == backward_data) ? jcp.os : jcp.is;
			offt = (u == jcp.reduce_loop_unroll) ? (height + j) * jcp.reduce_loop_unroll : j * jcp.reduce_loop_unroll + u;
		} else
	...
	auto load_ptr = [=](int u, int i, int n) {
		...
		return ptr[aux_reg_load_data + u1 * jcp.reduce_loop_load_step + sizeof(float) * offt + n * 4 * sizeof(float)];
	};
	...
	auto output_ptr = [=](int i, int j, int n) {
		switch (jcp.prop_kind) {
			...
			case backward_weights:
				return ptr[aux_reg_output_data + (i ? reg_output_stride * i : 0) // TODO: Xbyak should allow 0 scale
...
void jit_sse42_1x1_conv_kernel_f32::generate_diff_bias_loop(int load_loop_blk) {
	...
	auto load_ptr = [=](int u, int i, int n) {
		return ptr[aux_reg_load_data + (i * jcp.os + u) * jcp.oc_block * sizeof(float) + 4 * n * sizeof(float)];
	};
...
status_t jit_sse42_1x1_conv_kernel_f32::init_conf(jit_1x1_conv_conf_t &jcp,
	...
	format_tag_t wei_tag = with_groups ? utils::pick(2 * ndims - 6 + is_bwd_d, gOIw8i8o, gOIw8o8i, gOIhw8i8o,
												 gOIhw8o8i) :
	...
	bool args_ok = true && jcp.ngroups == 1 && jcp.src_tag == dat_tag && jcp.wei_tag == wei_tag && jcp.dst_tag == dat_tag;
	if (!args_ok)
		return status::unimplemented;
	...
	args_ok = true && jcp.oc % jcp.oc_block == 0 && jcp.ic % jcp.ic_block == 0 && jcp.t_pad == 0 && jcp.l_pad == 0 && jcp.stride_w == 1 && jcp.stride_h == 1 // TODO: support some strides
			  && jcp.kh == 1 && jcp.kw == 1;
	if (!args_ok)
		return status::unimplemented;
	...
	if (one_of(jcp.prop_kind, forward_training, forward_inference)) {
		...
		jcp.reduce_loop_bcast_step = jcp.reduce_loop_unroll * jcp.is * sizeof(float);
		jcp.reduce_loop_load_step = jcp.reduce_loop_unroll * jcp.oc_block * sizeof(float);
		...
		jcp.reduce_loop_unroll = jcp.reduce_block;
		jcp.reduce_loop_bcast_step = jcp.reduce_loop_unroll * jcp.os * sizeof(float);
		jcp.reduce_loop_load_step = jcp.reduce_loop_unroll * jcp.ic * sizeof(float);
		...
		jcp.reduce_loop_unroll = jcp.reduce_block;
		jcp.reduce_loop_bcast_step = jcp.reduce_loop_unroll * jcp.ic_block * sizeof(float);
		jcp.reduce_loop_load_step = jcp.reduce_loop_unroll * jcp.oc_block * sizeof(float);
