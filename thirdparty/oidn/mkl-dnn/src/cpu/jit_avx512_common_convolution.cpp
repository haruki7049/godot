...
#define wht_blk_off(d, g, ...) \
	(pd()->with_groups() ? (d).blk_off((g), __VA_ARGS__) : (d).blk_off(__VA_ARGS__))
		...
		for (int icb_l2 = 0; icb_l2 < jcp.nb_ic; icb_l2 += jcp.nb_ic_L2) {
			...
			while (start < end) {
				...
				for (int oh_b = oh_s; oh_b < oh_e; oh_b += jcp.h_blocking) {
					...
					auto src_w = src + src_d.blk_off(n, g_icb + icb_l2, ih_b, iw_s);
					auto wht_w = weights + wht_blk_off(weights_d, g, ocb, icb_l2);
					...
					for (int icb = icb_l2;
						...
						for (int oj = oh_b, ij = ih_b;
							...
							int i_b_overflow = div_up(max(0, ij - jcp.ih + (jcp.kh - 1) * dilate_h + 1), dilate_h);
...
void jit_avx512_common_convolution_fwd_t<src_type, wei_type, dst_type>::
	...
	parallel(0, [&](const int ithr, const int nthr) {
		int oc_chunks = jcp.nb_oc / jcp.nb_oc_blocking;
		int start{ 0 }, end{ 0 }, start_copy;
		int work_amount = jcp.mb * jcp.ngroups * oc_chunks * jcp.od * jcp.oh * jcp.nb_ow;
		balance211(work_amount, nthr, ithr, start, end);
		start_copy = start;
		...
		for (int icb_l2 = 0; icb_l2 < jcp.nb_ic; icb_l2 += jcp.nb_ic_L2) {
			...
			while (start < end) {
				...
				auto src_w = src + src_d.blk_off(n, g_icb + icb_l2, id_s, ih_s, iw_s) + d_t_overflow * dilate_d * src_d_stride;
				auto wht_w = weights + wht_blk_off(weights_d, g, ocb, icb_l2) + d_t_overflow * wht_d_stride;
	...
	parallel(0, [&](const int ithr, const int nthr) {
		...
		for (int ocb_l2 = 0; ocb_l2 < jcp.nb_oc; ocb_l2 += jcp.nb_oc_L2) {
			...
			while (start < end) {
				...
				auto diff_dst_w = diff_dst + diff_dst_d.blk_off(n, g_ocb + ocb_l2);
				...
				for (int ocb = ocb_l2;
					...
					for (int ij = ih_s; ij < ih_e; ++ij) {
						...
						if (is_fast_path) { // dilate == 0 && stride == 1
							...
							int i_t_overflow = div_up(max(0, (jcp.kh - 1) * dilate_h - ij - jcp.t_pad), dilate_h);
							int i_b_overflow = div_up(max(0, (jcp.kh - 1) * dilate_h + 1 - jcp.ih + ij - jcp.b_pad), dilate_h);
							k_len = jcp.kh - i_t_overflow - i_b_overflow;
							k_lo = i_b_overflow;
							oj = ij + jcp.t_pad - i_b_overflow * dilate_h;
						} else { // dilate == 0
							int i_t_overflow = max(0, (jcp.kh - 1 - ij - jcp.t_pad) / jcp.stride_h);
							int i_b_overflow = max(0, (jcp.kh - jcp.ih + ij - jcp.b_pad) / jcp.stride_h);
							int overflow_kh_hi = jcp.kh - 1 - abs((jcp.ih - 1 + jcp.b_pad - ij) % jcp.stride_h);
							int overflow_kh_lo = (ij + jcp.t_pad) % jcp.stride_h;

							k_len = (overflow_kh_hi - overflow_kh_lo) / jcp.stride_h + 1 - i_t_overflow - i_b_overflow;
		...
		for (int ocb_l2 = 0; ocb_l2 < jcp.nb_oc; ocb_l2 += jcp.nb_oc_L2) {
			...
			while (start < end) {
				...
				if (is_fast_path_d) { // dilate == 0 && stride == 1
					...
					int d_b_overflow = max(0, jcp.kd - jcp.id + id_s - jcp.back_pad);
					...
					int dilate_d = jcp.dilate_d + 1;
					// Note: use div_up to account for "holes" in filter
					int d_t_overflow = div_up(max(0, (jcp.kd - 1) * dilate_d - id_s - jcp.f_pad), dilate_d);
					int d_b_overflow = div_up(max(0, (jcp.kd - 1) * dilate_d + 1 - jcp.id + id_s - jcp.back_pad), dilate_d);
					d_len = jcp.kd - d_t_overflow - d_b_overflow;
					d_lo = d_b_overflow;
					d_oj = id_s + jcp.f_pad - d_b_overflow * dilate_d;
				} else { // dilate == 0
					int d_t_overflow = max(0, (jcp.kd - 1 - id_s - jcp.f_pad) / jcp.stride_d);
					int d_b_overflow = max(0, (jcp.kd - jcp.id + id_s - jcp.back_pad) / jcp.stride_d);
					int overflow_kd_hi = jcp.kd - 1 - abs((jcp.id - 1 + jcp.back_pad - id_s) % jcp.stride_d);
					int overflow_kd_lo = (id_s + jcp.f_pad) % jcp.stride_d;

					d_len = (overflow_kd_hi - overflow_kd_lo) / jcp.stride_d + 1 - d_t_overflow - d_b_overflow;
					d_lo = overflow_kd_lo + d_b_overflow * jcp.stride_d;
					d_oj = (id_s + jcp.f_pad - d_lo) / jcp.stride_d;
				}
				assert(d_len >= 0);

				auto diff_src_w = diff_src + diff_src_d.blk_off(n, g_icb) + id_s * diff_src_d_stride;
				auto diff_dst_w = diff_dst + diff_dst_d.blk_off(n, g_ocb + ocb_l2) + d_oj * diff_dst_d_stride;
				auto wht_w = weights + wht_blk_off(weights_d, g, ocb_l2, icb) + d_lo * wht_d_stride;
...
jit_avx512_common_convolution_bwd_weights_t<src_type, diff_dst_type,
		diff_weights_type>::
		jit_avx512_common_convolution_bwd_weights_t(const pd_t *apd) : cpu_primitive_t(apd),
																	   kernel_(nullptr),
...
struct jit_avx512_common_convolution_bwd_weights_t<src_type, diff_dst_type,
	...
	thread_info_t(const jit_avx512_common_convolution_bwd_weights_t *self,
			const exec_ctx_t &ctx, int ithr) : scratchpad(self->scratchpad(ctx)),
											   ithr(ithr) {
		...
		diff_bias = self->pd()->wants_padded_bias() ? scratchpad.template get<diff_weights_data_t>(
															  key_conv_padded_bias) :
		...
		ithr_but_oc = (ithr_mb * self->nthr_g_ + ithr_g) * self->nthr_ic_b_ + ithr_ic_b;

		ithr_but_ic = (ithr_mb * self->nthr_g_ + ithr_g) * self->nthr_oc_b_ + ithr_oc_b;
...
void jit_avx512_common_convolution_bwd_weights_t<src_type, diff_dst_type,
	...
	diff_weights_data_t *diff_wei = ti->ithr_mb == 0 ? (diff_weights_data_t *)ti->diff_weights : ti->wei_bia_reduction + (ti->ithr_mb - 1) * wei_size;
	diff_weights_data_t *diff_bia = ti->ithr_mb == 0 ? (diff_weights_data_t *)ti->diff_bias : ti->wei_bia_reduction + (nthr_mb_ - 1) * wei_size + (ti->ithr_mb - 1) * jcp.ngroups * jcp.oc;
...
#pragma unroll
	...
	if (jcp.is_1stconv && jcp.ver == ver_4fma) {
		...
		tr_ctx.tr_src = ti->tr_src + ti->ithr_but_oc * jcp.ih * jcp.stride_w * jcp.tr_ld;
		...
		for (int img = ti->img_start; img < ti->img_end; ++img) {
				...
				for (int oc_b = ti->oc_b_start; oc_b < ti->oc_b_end; ++oc_b) {
					for (int ic_b = ti->ic_b_start; ic_b < ti->ic_b_end; ++ic_b) {
						...
						jit_conv_ker_pipeline(kernel_->jit_ker, p,
								jcp.ver == ver_4fma ? &ti->tr_src[tr_src_off(ti->ithr_mb, _ic, 0)] : &ti->src[src_d.blk_off(img, _ic)],
			...
			jit_conv_ker_pipeline(kernel_->jit_ker, p,
					jcp.ver == ver_4fma ? &ti->tr_src[tr_src_off(ti->ithr_mb, _ic, 0)] : &ti->src[src_d.blk_off(img + 1, _ic)],
...
void jit_avx512_common_convolution_bwd_weights_t<src_type, diff_dst_type,
	...
	while (img_start < img_end) {
		...
		const int kd_back_pad = nstl::max(0, id_s - jcp.f_pad - jcp.id + jcp.kd);
		int kd_pad_off = nstl::min(jcp.kd - 1, kd_front_pad) * jcp.kh * jcp.kw * jcp.ic_block * jcp.oc_block * jcp.typesize_out;
		...
		for (int g = ti->g_start; g < ti->g_end; ++g) {
			for (int oc_b = ti->oc_b_start; oc_b < ti->oc_b_end; ++oc_b) {
				for (int ic_b = ti->ic_b_start; ic_b < ti->ic_b_end; ++ic_b) {
					...
					auto src = &ti->src[src_d.blk_off(img, _ic) + ik_overlap * input_step];
					auto dst = &ti->diff_dst[diff_dst_d.blk_off(img, _oc) + od_s * output_step];
...
void jit_avx512_common_convolution_bwd_weights_t<src_type, diff_dst_type,
	...
	const diff_weights_data_t *diff_bias_ws = ti->wei_bia_reduction + (nthr_mb_ - 1) * wei_size;
	...
	for (int thr_mb = 1; thr_mb < nthr_mb_; ++thr_mb) {
		...
		while (w < end) {
			...
			const int acc_size = nstl::min(end - w, ic_b_kh_work - sub_ic_b_kh_start) * jcp.kw * jcp.ic_block * jcp.oc_block;

			const size_t off = wht_blk_off(diff_weights_d, g, oc_b, ic_b, kh);

			diff_weights_data_t *d = (diff_weights_data_t *)ti->diff_weights + off;
			diff_weights_data_t *s = ti->wei_bia_reduction + (thr_mb - 1) * wei_size + off;
...
void jit_avx512_common_convolution_bwd_weights_t<src_type, diff_dst_type,
	...
	for (int thr_mb = 1; thr_mb < nthr_mb_; ++thr_mb) {
		...
		while (w < end) {
			...
			const int acc_size = nstl::min(end - w, ic_b_kh_work - sub_ic_b_kh_start) * jcp.kw * jcp.ic_block * jcp.oc_block * jcp.kh;
...
void jit_avx512_common_convolution_bwd_weights_t<src_type, diff_dst_type,
	...
	for (int img = img_start; img < img_end; ++img) {
		...
		for (int b_job_loc = 0; b_job_loc < b_njobs; ++b_job_loc) {
			...
			const diff_dst_data_t *d_dst = &ti->diff_dst[diff_dst_d.blk_off(img, _oc)];
...
void jit_avx512_common_convolution_bwd_weights_t<src_type, diff_dst_type,
	...
	const size_t wei_size = (size_t)jcp.ngroups * jcp.oc * jcp.ic * jcp.kh * jcp.kw * jcp.kd;
	const int bia_size = jcp.ngroups * jcp.oc;
	const diff_weights_data_t *diff_bias_ws = ti->wei_bia_reduction + (size_t)(nthr_mb_ - 1) * wei_size;
...
void jit_avx512_common_convolution_bwd_weights_t<src_type, diff_dst_type,
	...
	parallel(nthr_, [&](const int ithr, const int nthr) {
		...
		if (utils::one_of(pd()->ndims(), 3, 4)) {
			...
			if (pd()->with_bias())
				compute_diff_bias_3d(&thread_info);
		} else {
			assert(false);
		}
	});
