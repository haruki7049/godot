...
#pragma omp parallel for collapse(3) schedule(static)
			...
			for (int cb = 0; cb < C; cb += blksize)
				for (int sp = 0; sp < SP; ++sp) {
					...
					for (int cc = 0; cc < nstl::min(blksize, C - cb); ++cc) {
						...
						const size_t input_off = off + input_c / blksize * SP * blksize + input_c % blksize;
						output[output_off + cc] = input[input_off];
					}
				}
#else
		parallel_nd(MB, utils::div_up(C, blksize), SP, [&](int mb, int c, int sp) {
			const size_t off = mb * stride_mb + sp * blksize;
		...
		parallel_nd(outer_size, axis_size, inner_size, [&](size_t ou, int a, size_t in) {
