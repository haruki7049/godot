...
struct jit_uni_i8i8_pooling_fwd_ker_t : public jit_generator {
	...
	enum : int {
		s32_to_i8_ratio = sizeof(typename prec_traits<avg_proc_dt>::type) / sizeof(typename prec_traits<data_type::u8>::type),
		...
		case pooling_max:
			switch (jpp.src_dt) {
				...
				case u8:
					...
					break;
				default:
					assert(!"unsupported src data_type");
			}
...
jit_uni_i8i8_pooling_fwd_t<isa>::
		jit_uni_i8i8_pooling_fwd_t(const pd_t *apd) : cpu_primitive_t(apd),
													  ker_(nullptr) {
...
void jit_uni_i8i8_pooling_fwd_t<isa>::execute_forward(
	...
	parallel_nd(jpp.mb, jpp.oh, jpp.ow,
			[&](int n, int oh, int ow) {
				...
				p.src_i8 = &src_i8[src_d.blk_off(n, 0, ih, iw) * src_d.data_type_size()];
				p.dst_i8 = &dst_i8[dst_d.blk_off(n, 0, oh, ow) * dst_d.data_type_size()];
