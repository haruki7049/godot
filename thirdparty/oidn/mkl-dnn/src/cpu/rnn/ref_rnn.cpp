...
void _ref_rnn_common_t<aprop, src_type, weights_type>::gates_reduction(
	...
	auto body = [&](int i, int k) {
		for (int j = 0; j < rnn.mb; j++)
			diff_bias_[i * rnn.dic + k] += ws_gates_[j * rnn.gates_ws_ld + i * rnn.dic + k];
	};

	// @todo block k on simd-width
#if MKLDNN_THR == MKLDNN_THR_OMP && _OPENMP >= 201307 /* icc 17.0 has a problem with simd collapse */ \
...
void ref_rnn_bwd_f32_t::copy_init_layer(const rnn_conf_t &rnn,
	...
	switch (rnn.exec_dir) {
		case bi_concat:
			parallel_nd(rnn.n_iter, rnn.mb, [&](int it, int b) {
				auto diff_dst_layer_x = diff_dst_layer_ + diff_dst_layer_d.blk_off(it, b);
				for (int s = 0; s < rnn.dic; s++) {
					ws_diff_states(rnn.n_layer, 0, rnn.n_states, it, b, s) = diff_dst_layer_x[s];
					ws_diff_states(
							rnn.n_layer, 1, rnn.n_states, rnn.n_iter - it - 1, b, s) = diff_dst_layer_x[rnn.dic + s];
				}
			});
			break;
		case bi_sum:
			parallel_nd(rnn.n_iter, rnn.mb, [&](int it, int b) {
				...
				for (int s = 0; s < rnn.dic; s++) {
					ws_diff_states(rnn.n_layer, 0, rnn.n_states, it, b, s) = diff_dst_layer_x[s];
					ws_diff_states(
							rnn.n_layer, 1, rnn.n_states, rnn.n_iter - it - 1, b, s) = diff_dst_layer_x[s];
				}
			});
			break;
		case l2r:
		...
		case r2l:
			parallel_nd(rnn.n_iter, rnn.mb, [&](int it, int b) {
				auto diff_dst_layer_x = diff_dst_layer_ + diff_dst_layer_d.blk_off(rnn.n_iter - it - 1, b);
				for (int s = 0; s < rnn.dic; s++) {
					ws_diff_states(rnn.n_layer, 0, rnn.n_states, it, b, s) = diff_dst_layer_x[s];
				}
			});
...
void _ref_rnn_common_t<aprop, src_type, weights_type>::copy_init_iter(
	...
	const bool quantize = pd()->with_src_iter() && pd()->src_md(1)->data_type == data_type::f32 && rnn.dt_conf != all_f32;
	...
	const bool dequantize = pd()->with_src_iter() && pd()->src_md(1)->data_type == data_type::u8;
		...
		parallel_nd(rnn.n_layer, rnn.n_dir, rnn.n_states, rnn.mb,
				[&](int lay, int dir, int state, int b) {
					array_copy(&(ws_diff_states(
							...
							diff_dst_iter_ + diff_dst_iter_d.blk_off(
													 lay, dir, state, b),
							rnn.dic);
				});
	} else {
		parallel_nd(rnn.n_layer, rnn.n_dir, rnn.n_states, rnn.mb,
				[&](int lay, int dir, int state, int i) {
					for (int j = 0; j < rnn.dic; j++)
						ws_diff_states(lay, dir, state, rnn.n_iter, i, j) = 0.0f;
				});
	}
}
...
void _ref_rnn_common_t<aprop, src_type, weights_type>::copy_res_layer(
	...
	const bool dequantize = pd()->dst_md(0)->data_type == data_type::f32 && rnn.dt_conf != all_f32;
	...
	parallel_nd(rnn.n_iter, rnn.mb, [&](int it, int b) {
		...
		if (rnn.exec_dir != r2l) {
			for (int s = 0; s < rnn.dic; s++) {
				dst_layer_[dst_layer_d.blk_off(it, b, dir * rnn.dic + s)] = maybe_deq(ws_states(rnn.n_layer, dir, it + 1, b, s));
			}
			dir = 1;
		}
		if (rnn.exec_dir != l2r) {
			for (int s = 0; s < rnn.dic; s++)
				switch (rnn.exec_dir) {
					case bi_sum:
						dst_layer_[dst_layer_d.blk_off(it, b, s)] += maybe_deq(ws_states(
								rnn.n_layer, dir, rnn.n_iter - it, b, s));
						break;
					default:
						dst_layer_[dst_layer_d.blk_off(it, b, dir * rnn.dic + s)] = maybe_deq(ws_states(
								rnn.n_layer, dir, rnn.n_iter - it, b, s));
				}
		}
	});
...
void ref_rnn_bwd_f32_t::copy_res_layer(
	...
	parallel_nd(rnn.n_iter, rnn.mb, [&](int it, int b) {
		...
		for (int s = 0; s < rnn.slc; s++) {
			float *dst_addr = diff_src_layer_ + diff_src_layer_d.blk_off(
...
void _ref_rnn_common_t<aprop, src_type, weights_type>::copy_res_iter(
	...
	const bool quantize = pd()->with_dst_iter() && pd()->dst_md(1)->data_type == data_type::u8 && rnn.dt_conf != all_f32;
	...
	const bool dequantize = pd()->with_dst_iter() && pd()->dst_md(1)->data_type == data_type::f32 && rnn.dt_conf != all_f32;
	auto maybe_deq = [&](src_data_t s) {
		if (dequantize)
	...
	if (dst_iter_) {
		parallel_nd(rnn.n_layer, rnn.n_dir, rnn.mb,
				[&](int lay, int dir, int b) {
					for (int s = 0; s < rnn.dic; s++) {
						dst_iter_[dst_iter_d.blk_off(lay, dir, 0, b, s)] = maybe_deq(ws_states(lay + 1, dir, rnn.n_iter, b, s));
					}
					if (pd()->cell_kind() == alg_kind::vanilla_lstm)
						for (int s = 0; s < rnn.dic; s++) {
							dst_iter_[dst_iter_d.blk_off(lay, dir, 1, b, s)] = maybe_q(ws_c_states(
				...
				[&](int lay, int dir, int state, int b) {
					for (int s = 0; s < rnn.sic; s++) {
						diff_src_iter_[diff_src_iter_d.blk_off(
								lay, dir, state, b, s)] = ws_diff_states(lay, dir, state, 0, b, s);
					}
				});
	}
}
	...
	for (int i = 0; i < rnn.n_layer; i++) {
		for (int d = 0; d < rnn.n_dir; d++) {
			...
			for (int p = 0; p < rnn.n_parts_bias; p++) {
				bias(i, d, p) = rnn.copy_bias ? (float *)&scratch_bias(i, d, offset_bias) : (float *)&b(i, d, offset_bias);
	...
	if (rnn.dt_conf != all_f32) {
		...
		for (int i = 0; i < rnn.n_layer * rnn.n_dir; i++)
			for (int j = 0; j < rnn.n_bias * rnn.dic; j++) {
				...
				float weights_scale = scale_per_oc ? weights_scales[j] : weights_scales[0];
				scratch_bias_[off] -= (w_iter_comp[off] + w_layer_comp[off]) * data_shift / (weights_scale * data_scale);
			}
	}
}
	...
	for (int l = 0; l < rnn.n_layer; l++)
		for (int d = 0; d < rnn.n_dir; d++) {
			for (int p = 0; p < packed_desc.n_parts; p++) {
				...
				offset_packed += packed_desc.part_pack_size[p] / sizeof(weights_data_t);
			}
		}
}
...
void _ref_rnn_common_t<aprop, src_type, weights_type>::execute_(
	...
	auto dst_last_layer = rnn.is_fwd ? CTX_OUT_MEM(char *, MKLDNN_ARG_DST_LAYER) : const_cast<char *>(CTX_IN_MEM(const char *, MKLDNN_ARG_DST_LAYER));
	auto dst_last_iter = rnn.is_fwd ? CTX_OUT_MEM(char *, MKLDNN_ARG_DST_ITER) : const_cast<char *>(CTX_IN_MEM(const char *, MKLDNN_ARG_DST_ITER));
	...
	auto ptr_wei_layer = scratchpad.template get<weights_data_t *>(key_rnn_ptrs_wei_layer);
	auto ptr_wei_iter = scratchpad.template get<weights_data_t *>(key_rnn_ptrs_wei_iter);
	auto ptr_bias =
	...
	if (rnn.use_workspace)
		ws_ptr = rnn.is_fwd ? CTX_OUT_MEM(char *, MKLDNN_ARG_WORKSPACE) : const_cast<char *>(CTX_IN_MEM(const char *, MKLDNN_ARG_WORKSPACE));
	...
	if (rnn.dt_conf == f32u8f32u8 || rnn.dt_conf == f32u8f32f32 || rnn.dt_conf == all_f32)
	...
	if (rnn.dt_conf == u8u8u8f32 || rnn.dt_conf == f32u8f32f32 || rnn.dt_conf == all_f32)
