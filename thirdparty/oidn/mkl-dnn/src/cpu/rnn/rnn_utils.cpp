...
bool rnn_utils::is_ldigo(const memory_desc_wrapper &md) {
	...
	auto dims = md.dims();
	return md.ndims() == 5 && blk.inner_nblks == 0 && str[4] == 1 && str[3] == dims[4] && str[1] == str[2] * dims[2] && str[0] == str[1] * dims[1];
};
...
bool rnn_utils::is_ldgoi(const memory_desc_wrapper &md) {
	...
	auto str = blk.strides;
	auto dims = md.dims();
	return md.ndims() == 5 && blk.inner_nblks == 0 && str[2] == 1 && str[3] == dims[4] * str[4] && str[1] == str[3] * dims[3] && str[0] == str[1] * dims[1];
};
...
void rnn_utils::init_conf(rnn_conf_t &rnn, const rnn_desc_t &rd,
	...
	rnn.merge_gemm_layer = ((rnn.is_fwd && rnn.mb < 128) || !rnn.is_fwd) || is_int8;
	...
	rnn.use_jit_gemm = !mayiuse(avx512_mic) && ((is_inference && (rnn.n_layer > 1 || rnn.mb < 100)) || (rnn.is_training && rnn.dic < 500));

	/* Decide to copy bias */
	rnn.copy_bias = rnn.dt_conf != all_f32;

#if USE_MKL_PACKED_GEMM
	rnn.use_layer_packed_gemm = (weights_layer_d.format_kind() == format_kind::any && rnn.slc > 760 && rnn.dic > 760 && is_inference) || is_int8; // packed gemm is the only supported option for int8
	rnn.use_iter_packed_gemm = (weights_iter_d.format_kind() == format_kind::any && rnn.sic > 760 && rnn.dic > 760 && is_inference) || is_int8;
#else
	...
	if (rnn.use_layer_packed_gemm) {
		...
		for (int p = 0; p < rnn.n_parts_weights_layer; p++) {
			int m_p = rnn.is_fwd ? (rnn.parts_weights_layer[p] * rnn.dic) : rnn.slc;
			int k_p = rnn.is_fwd ? rnn.slc : (rnn.parts_weights_layer[p] * rnn.dic);
			...
			if (rnn.dt_conf == all_f32)
				...
				rnn.part_weights_layer_pack_size[p] = cblas_gemm_s8u8s32_pack_get_size(
						CblasAMatrix, m_p, n_p, k_p);
#else
			UNUSED(m_p);
			UNUSED(k_p);
...
#endif
			rnn.weights_layer_pack_size += rnn.n_layer * rnn.n_dir * rnn.part_weights_layer_pack_size[p];
		}
		rnn.weights_layer_comp_offset = rnn.weights_layer_pack_size;
		rnn.weights_layer_pack_size += rnn.dt_conf == all_f32 ? 0 : rnn.n_layer * rnn.n_dir * rnn.n_gates * rnn.dlc * sizeof(float);
	}
	...
	if (rnn.use_iter_packed_gemm) {
		...
		for (int p = 0; p < rnn.n_parts_weights_iter; p++) {
			...
			if (rnn.dt_conf == all_f32)
				...
				rnn.part_weights_iter_pack_size[p] = cblas_gemm_s8u8s32_pack_get_size(
						CblasAMatrix, m_p, n_p, k_p);
#else
			UNUSED(m_p);
			UNUSED(k_p);
...
#endif
			rnn.weights_iter_pack_size += rnn.n_layer * rnn.n_dir * rnn.part_weights_iter_pack_size[p];
		}
		rnn.weights_iter_comp_offset = rnn.weights_iter_pack_size;
		rnn.weights_iter_pack_size += rnn.dt_conf == all_f32 ? 0 : rnn.n_layer * rnn.n_dir * rnn.n_gates * rnn.dic * sizeof(float);
	}
}
...
void rnn_utils::set_conf(rnn_conf_t &rnn, const rnn_desc_t &rd,
	 ...
	 */
	rnn.weights_layer_is_packed = weights_layer_d.format_kind() == format_kind::rnn_packed;
	rnn.weights_iter_is_packed = weights_iter_d.format_kind() == format_kind::rnn_packed;
	...
	int sizeof_states_dt = rnn.dt_conf == all_f32 ? sizeof(float) : sizeof(uint8_t);
	rnn.states_ws_ld = get_good_ld(nstl::max(rnn.slc, nstl::max(rnn.sic, rnn.dic)),
			sizeof_states_dt);
	...
	rnn.ws_states_size = (size_t)(rnn.n_layer + 1) * rnn.n_dir * (rnn.n_iter + 1) * rnn.mb * rnn.states_ws_ld * sizeof_states_dt;
	bool is_lstm = rd.cell_desc.cell_kind == mkldnn_vanilla_lstm;
	rnn.ws_c_states_size = is_lstm ? (size_t)(rnn.n_layer + 1) * rnn.n_dir * (rnn.n_iter + 1) * rnn.mb * rnn.states_ws_ld * sizeof(float) : 0;
	rnn.ws_diff_states_size = rnn.is_training ? (size_t)(rnn.n_layer + 1) * rnn.n_dir * (rnn.n_iter + 1) * (rnn.n_states + 1) * rnn.mb * rnn.states_ws_ld * sizeof(float) : (size_t)0;
	rnn.ws_gates_size = (size_t)rnn.n_layer * rnn.n_dir * rnn.n_iter * rnn.mb * rnn.gates_ws_ld * sizeof(float);

	/* set other sizes */
	rnn.ws_per_cell = (size_t)rnn.is_lbr * rnn.mb * rnn.dic * sizeof(float);
	rnn.ws_cell_comp_size = rnn.is_lbr || rnn.dt_conf != all_f32 ? (size_t)rnn.gates_nld * rnn.gates_ws_ld * sizeof(float) : 0;
	rnn.ws_grid_comp_size = (size_t)rnn.is_lbr * rnn.is_training * rnn.n_layer * rnn.n_dir * rnn.n_iter * rnn.ws_per_cell * sizeof(float);
	rnn.ws_bias_size = (size_t)rnn.n_layer * rnn.n_dir * rnn.n_bias * rnn.dic * sizeof(float);
}
