...
protected:
	...
	status_t check_layout_consistency() {
		...
		bool ok = true;
		ok = ok && is_blocked(src_layer_md_, 3) && is_blocked(dst_layer_md_, 3);
		ok = ok && IMPLICATION(!is_zero_md(&src_iter_md_), is_blocked(src_iter_md_, 5)) && IMPLICATION(!is_zero_md(&dst_iter_md_), is_blocked(dst_iter_md_, 5));

		if (weights_layer_md_.format_kind == format_kind::rnn_packed)
			ok = ok && (weights_layer_md_.format_desc.rnn_packed_desc.format == mkldnn_ldigo_p);
		else
			ok = ok && rnn_utils::is_ldigo(&weights_layer_md_);

		if (weights_iter_md_.format_kind == format_kind::rnn_packed)
			ok = ok && (weights_iter_md_.format_desc.rnn_packed_desc.format == mkldnn_ldigo_p);
		else
		...
		ok = ok && IMPLICATION(
						   weights_iter_dt == s8, weights_iter_md_.format_kind == format_kind::rnn_packed);
		ok = ok && IMPLICATION(
						   weights_layer_dt == s8, weights_layer_md_.format_kind == format_kind::rnn_packed);
...
protected:
	...
	status_t check_layout_consistency() {
		...
		if (weights_layer_md_.format_kind == format_kind::rnn_packed)
			ok = ok && (weights_layer_md_.format_desc.rnn_packed_desc.format == mkldnn_ldgoi_p);
		else
			ok = ok && rnn_utils::is_ldgoi(&weights_layer_md_);

		if (weights_iter_md_.format_kind == format_kind::rnn_packed)
			ok = ok && (weights_iter_md_.format_desc.rnn_packed_desc.format == mkldnn_ldgoi_p);
		else
		...
		ok = ok && is_blocked(diff_src_layer_md_, 3) && is_blocked(diff_dst_layer_md_, 3);
		ok = ok && IMPLICATION(!is_zero_md(&diff_src_iter_md_), is_blocked(diff_src_iter_md_, 5)) && IMPLICATION(!is_zero_md(&diff_dst_iter_md_), is_blocked(diff_dst_iter_md_, 5));

		ok = ok && rnn_utils::is_ldigo(&diff_weights_layer_md_) && rnn_utils::is_ldigo(&diff_weights_iter_md_);
