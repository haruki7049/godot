...
struct rnn_data_reorder_t : public cpu_primitive_t {
	struct pd_t : public cpu_reorder_pd_t {
		...
		static status_t create(reorder_pd_t **reorder_pd,
			...
			bool args_ok = true && id.data_type() == type_i && od.data_type() == type_o && id.matches_one_of_tag(format_tag::tnc, format_tag::ldsnc) && od == id;
			if (!args_ok)
...
struct rnn_weights_reorder_t : public cpu_primitive_t {
...
#endif
			const memory_desc_wrapper id(src_md), od(dst_md);
			bool args_ok = true && id.data_type() == type_i && od.data_type() == type_o && od.format_kind() == format_kind::rnn_packed && od.rnn_packed_desc().format == mkldnn_ldigo_p && od.rnn_packed_desc().n_parts == 1 && attr != nullptr;
			if (!args_ok)
				return status::invalid_arguments;

			format_tag_t itag = id.matches_one_of_tag(
					format_tag::ldigo, format_tag::ldgoi);
			if (itag == format_tag::undef)
				return status::invalid_arguments;

			const int mask = attr->rnn_weights_qparams_.mask_;
			if (!utils::one_of(mask, 0, 3))
				return status::unimplemented;
	...
	private:
		void init_scratchpad() {
			...
			size_t reduction_size = itag_ == ldigo ? sizeof(int32_t) * mkldnn_get_max_threads() * dims[0] * dims[1] * dims[3] * dims[4] : 0;
			scratchpad.book(
...
private:
	...
	virtual status_t execute(const exec_ctx_t &ctx) const override {
				...
				for (int ld = LD_s; ld < LD_e; ld++) {
					...
					for (int i = I_s; i < I_e; i++) {
						...
						for (int go = 0; go < G * O; go++) {
							...
							quantized[ld * I * G * O + i * G * O + go] = (int32_t)q;
		...
		const size_t *size_packed_cell = output_d.rnn_packed_desc().part_pack_size;
...
#endif
			const memory_desc_wrapper id(src_md), od(dst_md);
			bool args_ok = true && id.data_type() == data_type::f32 && od.data_type() == data_type::f32 && od.format_kind() == format_kind::rnn_packed && utils::one_of(od.rnn_packed_desc().format, mkldnn_ldigo_p, mkldnn_ldgoi_p) && attr->has_default_values();
			if (!args_ok)
				return status::invalid_arguments;
...
private:
	...
	virtual status_t execute(const exec_ctx_t &ctx) const override {
		...
		/* Pack */
		bool cross_case = false || (pd()->itag_ == format_tag::ldigo && rnn_pdata.format == mkldnn_ldgoi_p) || (pd()->itag_ == format_tag::ldgoi && rnn_pdata.format == mkldnn_ldigo_p);
			...
			for (int d = 0; d < D; d++) {
				for (int p = 0; p < n_parts; p++) {
					...
					cblas_sgemm_pack(CblasColMajor, CblasAMatrix, trans, m_p, n,
							k_p, 1.0f, &input[is_igo ? off_igo(l, d, 0, g, 0) : off_goi(l, d, 0, g, 0)],
							ld, output);
