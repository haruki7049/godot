...
rnn_cell_execution_sig(ref_rnn_fwd_f32_t::cell_execution_gru) {
	...
	parallel_nd(rnn.mb, [&](int i) {
		...
		for (int j = 0; j < rnn.dic; j++) {
			...
			states_t_l(i, j) = states_tm1_l(i, j) * ws_gates(i, 0, j) + (1.0f - ws_gates(i, 0, j)) * ws_gates(i, 2, j);
		}
	});
}
...
rnn_cell_execution_sig(ref_rnn_bwd_f32_t::cell_execution_gru) {
	...
	parallel_nd(rnn.mb, [&](int i) {
		...
		for (int j = 0; j < rnn.dic; j++) {
			...
			float dG2 = (1.0f - ws_gates(i, 0, j)) * dHt * one_m_square(ws_gates(i, 2, j));
			float dG0 = (h - ws_gates(i, 2, j)) * dHt * x_m_square(ws_gates(i, 0, j));
