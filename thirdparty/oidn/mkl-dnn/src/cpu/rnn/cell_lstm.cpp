...
rnn_elemwise_sig(ref_rnn_fwd_f32_t::lstm_elemwise) {
	...
	parallel_nd(rnn.mb, [&](int i) {
		...
		for (int j = 0; j < rnn.dic; j++) {
			...
			float tmp = ws_gates(i, 1, j) * c_states_tm1_l(i, j) + ws_gates(i, 0, j) * ws_gates(i, 2, j);
...
rnn_elemwise_sig(ref_rnn_fwd_u8s8_t::lstm_elemwise) {
	...
	auto deq_w = [&](acc_data_t s, int gate, int j) {
		return pd()->attr()->rnn_weights_qparams_.mask_ == 0 ?
					   ...
					   saturate<float>(s) * (1.f / (weights_scales[gate * rnn.dic + j] * data_scale));
	};
...
rnn_elemwise_sig(ref_rnn_bwd_f32_t::lstm_elemwise) {
	...
	parallel_nd(rnn.mb, [&](int i) {
		...
		for (int j = 0; j < rnn.dic; j++) {
			...
			float dHt = diff_states_tp1_l(0, i, j) + diff_states_t_lp1(rnn.n_states, i, j);
			float dCt = diff_states_tp1_l(1, i, j) + one_m_square(tanhCt) * ws_gates(i, 3, j) * dHt;

			float dG1 = c_states_tm1_l(i, j) * dCt * x_m_square(ws_gates(i, 1, j));
			float dG0 = ws_gates(i, 2, j) * dCt * x_m_square(ws_gates(i, 0, j));
			float dG3 = tanhCt * dHt * x_m_square(ws_gates(i, 3, j));
			float dG2 = ws_gates(i, 0, j) * dCt * one_m_square(ws_gates(i, 2, j));
