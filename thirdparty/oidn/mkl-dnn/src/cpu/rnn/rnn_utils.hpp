...
template <typename T>
struct ws_gates_aoc {
	ws_gates_aoc(const rnn_conf_t &rnn, T *data) : gates_(data, rnn.gates_nld, rnn.gates_ws_ld), DIC_(rnn.dic) {}
...
struct bias_aoc_t {
	bias_aoc_t(const rnn_conf_t &rnn, const float *data) : bias_(data, rnn.n_bias, rnn.dic) {}
...
struct ws_states_aoc {
	ws_states_aoc(const rnn_conf_t &rnn, T *data) : state_(data, rnn.states_nld, rnn.states_ws_ld) {}
...
struct ws_diff_states_aoc_t {
	ws_diff_states_aoc_t(const rnn_conf_t &rnn, float *data) : diff_states_(data, rnn.n_states + 1, rnn.n_iter + 1, rnn.states_nld,
																	   rnn.states_ws_ld) {}
...
struct ws_diff_w_iter_aoc_t {
	ws_diff_w_iter_aoc_t(const rnn_conf_t &rnn, float *data) : diff_weights_iter_(
