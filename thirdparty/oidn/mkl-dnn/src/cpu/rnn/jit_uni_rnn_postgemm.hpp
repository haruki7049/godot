...
struct jit_uni_lstm_postgemm_kernel_fwd : public jit_uni_rnn_postgemm_kernel {
	...
	jit_uni_lstm_postgemm_kernel_fwd(const rnn_utils::rnn_conf_t &rnn, const primitive_attr_t *attr) : jit_uni_rnn_postgemm_kernel(rnn, attr) {}
	...
	void generate() {
		...
		L(vector_loop_start_label);
		{
			...
			if (vlen_dst == vlen)
				...
				switch (vlen_dst) {
					case 16:
						uni_vmovups(ptr[addr_states_t_l_reg], Xmm(tmp1_vmm.getIdx()));
						break;
					case 8:
						uni_vmovsd(ptr[addr_states_t_l_reg], Xmm(tmp1_vmm.getIdx()));
						break;
					case 4:
						uni_vmovss(ptr[addr_states_t_l_reg], Xmm(tmp1_vmm.getIdx()));
						break;
					default:
		...
		L(rem_loop_start_label);
		{
			...
			if (vlen_dst == vlen)
				...
				switch (vlen_dst) {
					case 16:
						uni_vmovups(ptr[addr_states_t_l_reg], Xmm(tmp1s_vmm.getIdx()));
						break;
					case 8:
						uni_vmovsd(ptr[addr_states_t_l_reg], Xmm(tmp1s_vmm.getIdx()));
						break;
					case 4:
						uni_vmovss(ptr[addr_states_t_l_reg], Xmm(tmp1s_vmm.getIdx()));
						break;
					default:
						assert(!"Unsuported vector length for quantization");
				}
