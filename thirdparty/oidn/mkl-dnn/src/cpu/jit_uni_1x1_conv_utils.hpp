...
inline void rtus_prepare(conv_pd_t *self, const convolution_desc_t *&conv_d,
	...
	const bool is_bwd_data = self->desc()->prop_kind == prop_kind::backward_data;

	const int ndims = src_d->ndims;
	const auto dat_tag = ndims == 3 ? memory_desc_wrapper(dst_d).matches_one_of_tag(
											  format_tag::nCw8c, format_tag::nCw16c) :
									  memory_desc_wrapper(dst_d).matches_one_of_tag(
											  format_tag::nChw8c, format_tag::nChw16c);

	bool rtus_applicable = true && utils::pick(ndims - 3, (conv_d->strides[0] != 1 && !one_of(conv_d->src_desc.data_type, data_type::s32)), (conv_d->strides[0] != 1 || conv_d->strides[1] != 1)) && dat_tag != format_tag::undef;
	for (int d = 2; d < ndims; ++d) {
		/* TODO: relax these conditions (by improving reducer) */
		rtus_applicable = rtus_applicable && conv_d->padding[0][d - 2] == 0 && dst_d->dims[d] * conv_d->strides[d - 2] == src_d->dims[d];
	}
...
struct rtus_driver_t : public jit_generator {
	...
	rtus_driver_t(int iw, int stride_w, int src_step_h,
			int src_step_icb, int ws_step_icb, bool src_to_ws, size_t typesize) : iw_(iw),
																				  stride_w_(stride_w),
	...
	void generate() {
		using namespace Xbyak;
		assert(isa == avx2 || isa == avx512_common || isa == avx512_core || isa == avx512_mic);
...
inline int best_divider(int value, int min_divider, int max_divider,
	...
	auto loss_ratio = [](int total, int chunk) { return float(rnd_up(total, chunk) - total) / rnd_up(total, chunk); };
