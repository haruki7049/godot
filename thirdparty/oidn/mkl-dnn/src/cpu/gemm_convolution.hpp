...
struct gemm_convolution_fwd_t : public cpu_primitive_t {
	struct pd_t : public cpu_convolution_fwd_pd_t {
		...
		status_t init() {
			bool ok = true && is_fwd() && set_default_alg_kind(alg_kind::convolution_direct) && expect_data_types(data_type::f32, data_type::f32, data_type::f32, data_type::f32, data_type::f32) && !has_zero_dim_memory() && set_default_formats_common(dat_tag(), wei_tag(), dat_tag()) && post_ops_ok() && memory_desc_matches_tag(*src_md(), dat_tag()) && memory_desc_matches_tag(*dst_md(), dat_tag()) && memory_desc_matches_tag(*weights_md(), wei_tag());
			if (!ok)
	...
	protected:
		...
		format_tag_t wei_tag() const {
			using namespace format_tag;
			return with_groups() ? utils::pick(ndims() - 3, goiw, goihw, goidhw) : utils::pick(ndims() - 3, oiw, oihw, oidhw);
		}
	...
	gemm_convolution_fwd_t(const pd_t *apd) : cpu_primitive_t(apd, true), eltwise_(nullptr) {
...
struct gemm_convolution_bwd_data_t : public cpu_primitive_t {
	...
	gemm_convolution_bwd_data_t(const pd_t *apd) : cpu_primitive_t(apd, true) {}
...
struct gemm_convolution_bwd_weights_t : public cpu_primitive_t {
	...
	gemm_convolution_bwd_weights_t(const pd_t *apd) : cpu_primitive_t(apd, true) {}
