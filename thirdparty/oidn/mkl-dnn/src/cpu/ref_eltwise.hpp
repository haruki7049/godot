...
struct ref_eltwise_fwd_t : public cpu_primitive_t {
	struct pd_t : public cpu_eltwise_fwd_pd_t {
		...
		status_t init() {
			...
			use_dense_ = false || src_d.is_dense() || (src_d.is_dense(true) && is_zero_preserved());

			use_nCspBc_padded_ = !use_dense_ && src_d.blocking_desc().inner_nblks == 1 && one_of(src_d.blocking_desc().inner_blks[0], 8, 16) && src_d.blocking_desc().inner_idxs[0] == 1 && src_d.only_padded_dim(1) && src_d.is_dense(true);
			...
			bool ok = true && is_fwd() && everyone_is(data_type, desc()->data_desc.data_type) && IMPLICATION(use_generic, one_of(src_d.ndims(), 4, 5)) && attr()->has_default_values();
			if (!ok)
...
struct ref_eltwise_bwd_t : public cpu_primitive_t {
	struct pd_t : public cpu_eltwise_bwd_pd_t {
		...
		status_t init() {
			...
			bool ok = true && !is_fwd() && everyone_is(data_type, desc()->data_desc.data_type, desc()->diff_data_desc.data_type) && attr()->has_default_values();
			if (!ok)
				return status::unimplemented;
			...
			use_dense_ = true && same_fmt_ && diff_dst_d.is_dense(true) && is_zero_preserved() && !has_zero_dim_memory();
