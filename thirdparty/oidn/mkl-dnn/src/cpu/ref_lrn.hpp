...
struct ref_lrn_fwd_t : public cpu_primitive_t {
	struct pd_t : public cpu_lrn_fwd_pd_t {
		...
		status_t init() {
			...
			bool ok = true && is_fwd() && src_md()->data_type == data_type && attr()->has_default_values();
			if (!ok)
	...
	virtual status_t execute(const exec_ctx_t &ctx) const override {
		...
		switch (pd()->dat_tag_) {
			case nChw16c:
				execute_forward<nChw16c>(ctx);
				break;
			case nChw8c:
				execute_forward<nChw8c>(ctx);
				break;
			case nchw:
				execute_forward<nchw>(ctx);
				break;
			case nhwc:
				execute_forward<nhwc>(ctx);
				break;
			default:
				execute_forward<any>(ctx);
		}
		return status::success;
	}
...
struct ref_lrn_bwd_t : public cpu_primitive_t {
	struct pd_t : public cpu_lrn_bwd_pd_t {
		...
		status_t init() {
			...
			bool ok = true && !is_fwd() && utils::one_of(desc()->alg_kind, lrn_across_channels
	...
	virtual status_t execute(const exec_ctx_t &ctx) const override {
		...
		switch (pd()->dat_tag_) {
			case nChw16c:
				execute_backward<nChw16c>(ctx);
				break;
			case nChw8c:
				execute_backward<nChw8c>(ctx);
				break;
			case nchw:
				execute_backward<nchw>(ctx);
				break;
			case nhwc:
				execute_backward<nhwc>(ctx);
				break;
			default:
				execute_backward<any>(ctx);
		}
		return status::success;
	}
