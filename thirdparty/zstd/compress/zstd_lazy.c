...
ZSTD_insertDUBT1(ZSTD_matchState_t *ms,
	...
	while (nbCompares-- && (matchIndex > windowLow)) {
		...
		if ((dictMode != ZSTD_extDict) || (matchIndex + matchLength >= dictLimit) /* both in current segment*/
			...
			const BYTE *const mBase = ((dictMode != ZSTD_extDict) || (matchIndex + matchLength >= dictLimit)) ?
											  base :
											  dictBase;
...
ZSTD_DUBT_findBetterDictMatch(
	...
	if (bestLength >= MINMATCH) {
		U32 const mIndex = curr - ((U32)*offsetPtr - ZSTD_REP_MOVE);
		(void)mIndex;
...
static size_t ZSTD_HcFindBestMatch_dedicatedDictSearch_selectMLS(
	...
	switch (ms->cParams.minMatch) {
		default: /* includes case 3 */
		case 4:
			return ZSTD_HcFindBestMatch_generic(ms, ip, iLimit, offsetPtr, 4, ZSTD_dedicatedDictSearch);
		case 5:
			return ZSTD_HcFindBestMatch_generic(ms, ip, iLimit, offsetPtr, 5, ZSTD_dedicatedDictSearch);
		case 7:
		case 6:
			return ZSTD_HcFindBestMatch_generic(ms, ip, iLimit, offsetPtr, 6, ZSTD_dedicatedDictSearch);
	}
}
 ...
 *********************************/
typedef enum { search_hashChain,
	search_binaryTree } searchMethod_e;
...
ZSTD_compressBlock_lazy_generic(
	...
	while (ip < ilimit) {
		...
		if (offset) {
			if (dictMode == ZSTD_noDict) {
				while (((start > anchor) & (start - (offset - ZSTD_REP_MOVE) > prefixLowest)) && (start[-1] == (start - (offset - ZSTD_REP_MOVE))[-1])) /* only search for offset within prefix */
				{
					start--;
					matchLength++;
				}
