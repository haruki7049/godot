...
static void oc_enc_rc_reset(oc_enc_ctx *_enc) {
	...
	inter_delay = (_enc->rc.twopass ?
								  OC_MAXI(_enc->keyframe_frequency_force, 12) :
								  _enc->rc.buf_delay) >>
				  1;
...
void oc_enc_rc_resize(oc_enc_ctx *_enc) {
	...
	if (_enc->rc.twopass == 2) {
		...
		reset_window = _enc->rc.frame_metrics == NULL && (_enc->rc.frames_total[0] == 0 ||
																 buf_delay < _enc->rc.frames_total[0] + _enc->rc.frames_total[1] + _enc->rc.frames_total[2]);
...
static int oc_rc_scale_drop(oc_rc_state *_rc, int _nframes) {
	if (_rc->prev_drop_count > 0 || _rc->log_drop_scale > OC_Q57(0)) {
		...
		dup_scale = oc_bexp64((_rc->log_drop_scale + oc_blog64(_rc->prev_drop_count + 1) >> 1) + OC_Q57(8));
...
int oc_enc_select_qi(oc_enc_ctx *_enc, int _qti, int _clamp) {
	...
	switch (_enc->rc.twopass) {
		default: {
			...
			next_key_frame = _qti ? _enc->keyframe_frequency_force - (_enc->state.curframe_num - _enc->state.keyframe_num) : 0;
			nframes[0] = (_enc->rc.buf_delay - OC_MINI(next_key_frame, _enc->rc.buf_delay) + _enc->keyframe_frequency_force - 1) / _enc->keyframe_frequency_force;
			if (nframes[0] + _qti > 1) {
				nframes[0]--;
		...
		case 2: {
			...
			buf_pad = OC_MINI(_enc->rc.buf_delay, _enc->state.keyframe_num + _enc->keyframe_frequency_force - _enc->rc.scale_window0);
			...
			if (qti != _qti) {
				...
				_enc->rc.log_scale[_qti] = oc_blog64(scale) - oc_blog64(nframes[qti]) - OC_Q57(24);
			} else
			...
			if (buf_pad > 0) {
				...
				/*And blend in the low-pass filtered scale according to how many frames
				   we added.*/
				scale =
						oc_bexp64(_enc->rc.log_scale[1] + OC_Q57(24)) * (ogg_int64_t)nframes[1] + oc_bexp_q24(_enc->rc.scalefilter[1].y[0]) * (ogg_int64_t)nextra_frames;
	...
	rate_total = _enc->rc.fullness - _enc->rc.target + rate_bias + buf_delay * _enc->rc.bits_per_frame;
	...
	if (_bits > 0) {
		if (_trial) {
			...
			_enc->rc.log_scale[_qti] = oc_iir_filter_update(
											   _enc->rc.scalefilter + _qti, oc_q57_to_q24(log_scale))
									   << 33;
...
int oc_enc_rc_2pass_in(oc_enc_ctx *_enc, unsigned char *_buf, size_t _bytes) {
	...
	if (_enc->rc.frames_total[0] == 0) {
		if (!_buf) {
			...
			return OC_RC_2PASS_HDR_SZ + frames_needed * OC_RC_2PASS_PACKET_SZ - _enc->rc.twopass_buffer_fill;
		}
	...
	if (_enc->rc.frames_total[0] != 0) {
		...
		nframes_total = _enc->rc.frames_total[0] + _enc->rc.frames_total[1] + _enc->rc.frames_total[2];
		if (curframe_num >= nframes_total) {
			...
			if (_enc->rc.frame_metrics == NULL) {
				...
				frames_needed = OC_CLAMPI(0, _enc->rc.buf_delay - (_enc->rc.scale_window_end - _enc->rc.scale_window0),
						_enc->rc.frames_left[0] + _enc->rc.frames_left[1] - _enc->rc.nframes[0] - _enc->rc.nframes[1]);
				while (frames_needed > 0) {
					if (!_buf) {
						return OC_RC_2PASS_PACKET_SZ * frames_needed - _enc->rc.twopass_buffer_fill;
					}
					consumed = oc_rc_buffer_fill(&_enc->rc,
							_buf, _bytes, consumed, OC_RC_2PASS_PACKET_SZ);
					if (_enc->rc.twopass_buffer_fill >= OC_RC_2PASS_PACKET_SZ) {
