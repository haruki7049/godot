...
#define OC_SAD2_LOOP __asm { /*We want to compute (mm0+mm1>>1) on unsigned bytes without overflow, but    \
...
#define OC_HADAMARD_AB_8x4 __asm { /*Stage A:                                                                                                                                                                                   \
...
#define OC_HADAMARD_C_8x4 __asm { /*Stage C:*/ \
  __asm  paddw mm0,mm1 \
  __asm  paddw mm2,mm3 \
  __asm  paddw mm4,mm5 \
  __asm  paddw mm6,mm7 \
...
/*Performs the first part of the final stage of the Hadamard transform and
  ...
  At the end of this part, mm1 will contain the DC coefficient of the
   transform.*/
#define OC_HADAMARD_C_ABS_ACCUM_A_8x4(_r6, _r7) __asm { /*We use the fact that                                                                       \
