...
static int oc_enc_pipeline_set_stripe(oc_enc_ctx *_enc,
	...
	for (pli = 0; pli < 3; pli++) {
		...
		_pipe->froffset[pli] = fplane->froffset + _pipe->fragy0[pli] * (ptrdiff_t)fplane->nhfrags;
		if (notdone) {
...
static int oc_enc_block_transform_quantize(oc_enc_ctx *_enc,
	...
	ref = _enc->state.ref_frame_data[_enc->state.ref_frame_idx[OC_FRAME_FOR_MODE(mb_mode)]] + frag_offs;
	dst = _enc->state.ref_frame_data[_enc->state.ref_frame_idx[OC_FRAME_SELF]] + frag_offs;
	/*Motion compensation:*/
	switch (mb_mode) {
		case OC_MODE_INTRA: {
			nmv_offs = 0;
	...
	*&qs_checkpoint = *(_pipe->qs + 0);
	...
	if (_enc->state.frame_type != OC_INTRA_FRAME) {
		if (ncoded > 0 && !mo.dc_flag) {
			...
			cost = mo.coded_ac_ssd + _enc->lambda * (mo.ac_bits + oc_fr_cost4(&fr_checkpoint, _pipe->fr + 0) + _mode_overhead);
...
#define OC_MODE_RD_COST(_ssd, _rate, _lambda) \
	((_ssd) >> OC_BIT_SCALE) + ((_rate) >> OC_BIT_SCALE) * (_lambda) + (((_ssd) & (1 << OC_BIT_SCALE) - 1) + ((_rate) & (1 << OC_BIT_SCALE) - 1) * (_lambda) + ((1 << OC_BIT_SCALE) >> 1) >> OC_BIT_SCALE)
...
static unsigned oc_analyze_intra_mb_luma(oc_enc_ctx *_enc,
	...
	for (qii = 0; qii < nqis; qii++) {
		...
		rate[0][qii] = oc_dct_cost2(ssd[0] + qii, _enc->state.qis[qii], 0, 0, satd) + (qs[0][qii].bits - _qs->bits << OC_BIT_SCALE);
	...
	for (bi = 1; bi < 4; bi++) {
		...
		for (qii = 0; qii < nqis; qii++) {
			...
			best_rate = rate[bi - 1][0] + cur_rate + (qt[0].bits - qs[bi - 1][0].bits << OC_BIT_SCALE);
			...
			for (qij = 1; qij < nqis; qij++) {
				...
				chain_rate = rate[bi - 1][qij] + cur_rate + (qt[qij].bits - qs[bi - 1][qij].bits << OC_BIT_SCALE);
...
static unsigned oc_analyze_intra_chroma_block(oc_enc_ctx *_enc,
	...
	for (qii = 0; qii < nqis; qii++) {
		...
		cur_rate = oc_dct_cost2(&cur_ssd, _enc->state.qis[qii], _pli, 0, satd) + (qt[qii].bits - _qs->bits << OC_BIT_SCALE);
	...
	*&qs = *_qs;
	...
	for (bi = 0; bi < 4; bi++) {
		...
		*(ft + 0) = *&fr;
		...
		best_rate = oc_dct_cost2(&best_ssd, _enc->state.qis[0], 0, _qti, satd) + (qt[0].bits - qs.bits << OC_BIT_SCALE);
		best_cost = OC_MODE_RD_COST(ssd + best_ssd, rate + best_rate + best_overhead, lambda);
		best_fri = 0;
		best_qii = 0;
		for (qii = 1; qii < nqis; qii++) {
			oc_qii_state_advance(qt + qii, &qs, qii);
			cur_rate = oc_dct_cost2(&cur_ssd, _enc->state.qis[qii], 0, _qti, satd) + (qt[qii].bits - qs.bits << OC_BIT_SCALE);
...
static void oc_analyze_mb_mode_chroma(oc_enc_ctx *_enc,
	...
	for (pli = 1; pli < 3; pli++) {
		for (; bi < nblocks; bi++) {
			unsigned best_cost;
			satd = _frag_satd[bi];
			best_rate = oc_dct_cost2(&best_ssd, _enc->state.qis[0], pli, _qti, satd) + OC_CHROMA_QII_RATE;
			best_cost = OC_MODE_RD_COST(ssd + best_ssd, rate + best_rate, lambda);
			best_qii = 0;
			for (qii = 1; qii < nqis; qii++) {
				cur_rate = oc_dct_cost2(&cur_ssd, _enc->state.qis[qii], 0, _qti, satd) + OC_CHROMA_QII_RATE;
...
static void oc_cost_inter(oc_enc_ctx *_enc, oc_mode_choice *_modec,
	...
	ptrdiff_t frag_offs;
	src = _enc->state.ref_frame_data[OC_FRAME_IO];
	ref = _enc->state.ref_frame_data[_enc->state.ref_frame_idx[OC_FRAME_FOR_MODE(_mb_mode)]];
	ystride = _enc->state.ref_ystride[0];
	frag_buf_offs = _enc->state.frag_buf_offs;
	sb_map = _enc->state.sb_maps[_mbi >> 2][_mbi & 3];
	dx = _mv[0];
...
static int oc_cost_inter1mv(oc_enc_ctx *_enc, oc_mode_choice *_modec,
	...
	_modec->overhead += OC_MINI(_enc->mv_bits[0] + bits0, _enc->mv_bits[1] + 12) - OC_MINI(_enc->mv_bits[0], _enc->mv_bits[1]) << OC_BIT_SCALE;
	oc_mode_set_cost(_modec, _enc->lambda);
	return bits0;
}
...
static void oc_cost_inter4mv(oc_enc_ctx *_enc, oc_mode_choice *_modec,
	...
	oc_analyze_mb_mode_chroma(_enc, _modec, _fr, _qs, frag_satd, _skip_ssd, 1);
	_modec->overhead +=
			oc_mode_scheme_chooser_cost(&_enc->chooser, OC_MODE_INTER_MV_FOUR) + OC_MINI(_enc->mv_bits[0] + bits0, _enc->mv_bits[1] + bits1) - OC_MINI(_enc->mv_bits[0], _enc->mv_bits[1]) << OC_BIT_SCALE;
	oc_mode_set_cost(_modec, _enc->lambda);
}
							...
							case OC_MODE_INTER_MV_FOUR: {
								...
								for (bi = 0; bi < 4; bi++) {
									...
									if (frags[fragi].coded) {
										...
										_enc->mv_bits[0] += OC_MV_BITS[0][frag_mvs[fragi][0] + 31] + OC_MV_BITS[0][frag_mvs[fragi][1] + 31];
										_enc->mv_bits[1] += 12;
									}
	...
	for (pli = 0; pli < 3; pli++) {
		...
		for (; fragii < ncoded_fragis; fragii++) {
			...
			for (zzi = 0; zzi < 64;) {
				...
				huffi = _enc->huff_idxs[qti][zzi > 0][pli + 1 >> 1] + OC_ZZI_HUFF_OFFSET[zzi];
				if (eob_token[zzi] < OC_NDCT_EOB_TOKEN_MAX) {
					...
					frag_bits += _enc->huff_codes[huffi][eob_token[zzi]].nbits + OC_DCT_TOKEN_EXTRA_BITS[eob_token[zzi]];
					eob_token[zzi] = OC_NDCT_EOB_TOKEN_MAX;
				}
				...
				if (skip < 0) {
					...
					frag_bits += _enc->huff_codes[huffi][token].nbits + OC_DCT_TOKEN_EXTRA_BITS[token];
					zzi += skip;
				}
			}
