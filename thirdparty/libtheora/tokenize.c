...
static int oc_token_bits(oc_enc_ctx *_enc, int _huffi, int _zzi, int _token) {
	return _enc->huff_codes[_huffi + OC_ZZI_HUFF_OFFSET[_zzi]][_token].nbits + OC_DCT_TOKEN_EXTRA_BITS[_token];
}
		...
		if (qc <= 1) {
			...
			for (;;) {
				...
				if (zzj == 0) {
					...
					if (eob < 4095) {
						bits = oc_token_bits(_enc, huffi, zzi, oc_make_eob_token(eob + 1)) - flush_bits;
					} else
...
void oc_enc_tokenize_finish(oc_enc_ctx *_enc) {
	...
	for (zzi = 0; zzi < 64; zzi++)
		for (pli = 0; pli < 3; pli++) {
			...
			run_count = oc_decode_eob_token(old_tok1, old_eb1) + oc_decode_eob_token(old_tok2, old_eb2);
