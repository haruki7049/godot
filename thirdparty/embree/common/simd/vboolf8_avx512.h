...
struct vboolf<8> {
	...
	__forceinline vboolf8 &operator=(const vboolf8 &f) {
		v = f.v;
		return *this;
	}
	...
	__forceinline vboolf(bool a, bool b, bool c, bool d, bool e, bool f, bool g, bool h) : v((__mmask8)((int(h) << 7) | (int(g) << 6) | (int(f) << 5) | (int(e) << 4) | (int(d) << 3) | (int(c) << 2) | (int(b) << 1) | int(a))) {}
...
__forceinline vboolf8 &operator&=(vboolf8 &a, const vboolf8 &b) {
	return a = a & b;
}
__forceinline vboolf8 &operator|=(vboolf8 &a, const vboolf8 &b) {
	return a = a | b;
...
__forceinline int all(const vboolf8 &a) {
	return a.v == 0xff;
}
__forceinline int any(const vboolf8 &a) {
	return _mm512_kortestz(a, a) == 0;
}
__forceinline int none(const vboolf8 &a) {
	return _mm512_kortestz(a, a) != 0;
}

__forceinline int all(const vboolf8 &valid, const vboolf8 &b) {
	return all((!valid) | b);
}
__forceinline int any(const vboolf8 &valid, const vboolf8 &b) {
	return any(valid & b);
}
__forceinline int none(const vboolf8 &valid, const vboolf8 &b) {
	return none(valid & b);
}

__forceinline size_t movemask(const vboolf8 &a) {
	return _mm512_kmov(a);
}
__forceinline size_t popcnt(const vboolf8 &a) {
	return popcnt(a.v);
}
...
__forceinline unsigned int toInt(const vboolf8 &a) {
...
__forceinline bool get(const vboolf8 &a, size_t index) {
	assert(index < 8);
	return (toInt(a) >> index) & 1;
}
__forceinline void set(vboolf8 &a, size_t index) {
	assert(index < 8);
	a |= 1 << index;
}
__forceinline void clear(vboolf8 &a, size_t index) {
	assert(index < 8);
	a = andn(a, 1 << index);
}
