...
struct TaskScheduler : public RefCount {
	...
	struct __aligned(64) Task {
		...
		__forceinline Task() : state(DONE) {}

		/*! construction of new task */
		__forceinline Task(TaskFunction *closure, Task *parent, size_t stackPtr, size_t N) : dependencies(1), stealable(true), closure(closure), parent(parent), stackPtr(stackPtr), N(N) {
			if (parent)
		...
		__forceinline Task(TaskFunction *closure, Task *parent) : dependencies(1), stealable(false), closure(closure), parent(parent), stackPtr(-1), N(1) {
			switch_state(DONE, INITIALIZED);
		}
	...
	struct TaskQueue {
		TaskQueue() : left(0), right(0), stackPtr(0) {}
	...
	struct Thread {
		...
		Thread(size_t threadIndex, const Ref<TaskScheduler> &scheduler) : threadIndex(threadIndex), task(nullptr), scheduler(scheduler) {}
	...
	static __forceinline void spawn(size_t size, const Closure &closure) {
		...
		if (likely(thread != nullptr))
			...
			instance()->spawn_root(closure, size);
	}
