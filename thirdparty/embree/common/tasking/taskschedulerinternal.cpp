...
void TaskScheduler::Task::run_internal(Thread &thread) // FIXME: avoid as many dll_exports as possible
{
	...
	steal_loop(thread, [&]() { return dependencies > 0; }, [&]() { while (thread.tasks.execute_local_internal(thread,this)); });
...
TaskScheduler::ThreadPool::ThreadPool(bool set_affinity) : numThreads(0), numThreadsRunning(0), set_affinity(set_affinity), running(false) {}
...
void TaskScheduler::ThreadPool::thread_loop(size_t globalThreadIndex) {
	while (globalThreadIndex < numThreadsRunning) {
		...
		ssize_t threadIndex = -1;
		{
			...
			if (globalThreadIndex >= numThreadsRunning)
				break;
...
TaskScheduler::TaskScheduler() : threadCounter(0), anyTasksRunning(0), hasRootTask(false) {
...
void TaskScheduler::create(size_t numThreads, bool set_affinity, bool start_threads) {
	if (!threadPool)
		threadPool = new TaskScheduler::ThreadPool(set_affinity);
...
// -- GODOT end --
{
	...
	while (anyTasksRunning) {
		steal_loop(thread, [&]() { return anyTasksRunning > 0; }, [&]() {
                   anyTasksRunning++;
