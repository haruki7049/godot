...
public:
	...
	__forceinline T &at(size_t i) {
		assert(i < N);
		return items[i];
	}
	__forceinline const T &at(size_t i) const {
		assert(i < N);
		return items[i];
	}

	__forceinline T &front() const {
		assert(N > 0);
		return items[0];
	};
	__forceinline T &back() const {
		assert(N > 0);
		return items[N - 1];
	};
...
struct __aligned(64) StackArray {
	__forceinline StackArray(const size_t N) : N(N) {
	...
	__forceinline Ty &operator[](const int i) {
		assert(i >= 0 && i < N);
		return data[i];
	}
	__forceinline const Ty &operator[](const int i) const {
		assert(i >= 0 && i < N);
		return data[i];
	}

	__forceinline Ty &operator[](const unsigned i) {
		assert(i < N);
		return data[i];
	}
	__forceinline const Ty &operator[](const unsigned i) const {
		assert(i < N);
		return data[i];
	}
	...
	__forceinline const Ty &operator[](const size_t i) const {
		assert(i < N);
		return data[i];
	}
#endif
...
struct __aligned(64) DynamicStackArray {
	__forceinline DynamicStackArray() : data(&arr[0]) {}
	...
	__forceinline void resize(size_t M) {
		...
		if (likely(M <= max_stack_elements))
			return;
		if (likely(!isStackAllocated()))
			return;
	...
	__forceinline Ty &operator[](const int i) {
		assert(i >= 0 && i < max_total_elements);
		resize(i + 1);
		return data[i];
	}
	__forceinline Ty &operator[](const unsigned i) {
		assert(i < max_total_elements);
		resize(i + 1);
		return data[i];
	}
	...
	__forceinline DynamicStackArray(const DynamicStackArray &other) : data(&arr[0]) {
