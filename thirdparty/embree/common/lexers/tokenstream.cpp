...
bool TokenStream::tryString(Token &token, const ParseLocation &loc) {
	...
	while (cin->peek() != '\"') {
		const int c = cin->get();
		if (!isStringChar(c))
			THROW_RUNTIME_ERROR("invalid string character " + std::string(1, c) + " at " + loc.str());
		str += (char)c;
	}
	cin->drop();
...
Token TokenStream::next() {
	...
	if (trySymbols(token, loc))
		return token; /**< try to parse a symbol */
	if (tryFloat(token, loc))
		return token; /**< try to parse float */
	if (tryInt(token, loc))
		return token; /**< try to parse integer */
	if (tryString(token, loc))
		return token; /**< try to parse string */
	if (tryIdentifier(token, loc))
		return token; /**< try to parse identifier */
	if (cin->peek() == EOF)
		return Token(loc); /**< return EOF token */
