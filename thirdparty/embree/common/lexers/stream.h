...
public:
	...
	ParseLocation(std::shared_ptr<std::string> fileName, ssize_t lineNumber, ssize_t colNumber, ssize_t /*charNumber*/) : fileName(fileName), lineNumber(lineNumber), colNumber(colNumber) {}
	...
	std::string str() const {
		...
		if (fileName)
			str = *fileName;
		if (lineNumber >= 0)
			str += " line " + toString(lineNumber);
		if (lineNumber >= 0 && colNumber >= 0)
			str += " character " + toString(colNumber);
		return str;
	}
...
private:
	...
	__forceinline void pop_front() {
		if (past == 0)
			THROW_RUNTIME_ERROR("stream buffer empty");
...
public:
	...
	const T &unget(size_t n = 1) {
		if (past < n)
			THROW_RUNTIME_ERROR("cannot unget that many items");
		past -= n;
		future += n;
		return peek();
	}
...
public:
	FileStream(FILE *file, const std::string &name = "file") : file(file), lineNumber(1), colNumber(0), charNumber(0), name(std::shared_ptr<std::string>(new std::string(name))) {}

	FileStream(const FileName &fileName) : lineNumber(1), colNumber(0), charNumber(0), name(std::shared_ptr<std::string>(new std::string(fileName.str()))) {
...
public:
	StrStream(const char *str) : str(str), lineNumber(1), colNumber(0), charNumber(0) {}
...
public:
	CommandLineStream(int argc, char **argv, const std::string &name = "command line") : i(0), j(0), charNumber(0), name(std::shared_ptr<std::string>(new std::string(name))) {
		if (argc > 0) {
			for (size_t i = 0; argv[0][i] && i < 1024; i++)
				charNumber++;
			charNumber++;
		}
