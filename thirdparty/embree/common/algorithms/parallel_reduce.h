...
__forceinline Value parallel_reduce(const Index first, const Index last, const Index minStepSize, const Value &identity, const Func &func, const Reduction &reduction) {
	...
	const Value v = tbb::parallel_reduce(tbb::blocked_range<Index>(first, last, minStepSize), identity, [&](const tbb::blocked_range<Index> &r, const Value &start) { return reduction(start, func(range<Index>(r.begin(), r.end()))); }, reduction, context);
	// -- GODOT start --
	...
	return v;
#else
	const Value v = tbb::parallel_reduce(tbb::blocked_range<Index>(first, last, minStepSize), identity, [&](const tbb::blocked_range<Index> &r, const Value &start) { return reduction(start, func(range<Index>(r.begin(), r.end()))); }, reduction);
	// -- GODOT start --
	...
	struct AlignedValue {
		...
		AlignedValue(const AlignedValue &&v) { new (getValuePtr()) Value(*v.getValuePtr()); };
		AlignedValue &operator=(const AlignedValue &v) {
			*getValuePtr() = *v.getValuePtr();
			return *this;
		};
		AlignedValue &operator=(const AlignedValue &&v) {
			*getValuePtr() = *v.getValuePtr();
