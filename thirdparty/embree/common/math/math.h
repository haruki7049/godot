...
#if defined(__WIN32__)
__forceinline bool finite(const float x) {
	return _finite(x) != 0;
}
#endif
...
__forceinline float nextafter(float x, float y) {
	if ((x < y) == (x > 0))
		return x * (1.1f + float(ulp));
	else
		return x * (0.9f - float(ulp));
}
__forceinline double nextafter(double x, double y) {
	return _nextafter(x, y);
}
__forceinline int roundf(float f) {
	return (int)(f + 0.5f);
}
#else
__forceinline float nextafter(float x, float y) {
...
template <typename T>
__forceinline T min(const T &a, const T &b, const T &c, const T &d, const T &e) {
	return min(min(min(a, b), min(c, d)), e);
}

template <typename T>
__forceinline T mini(const T &a, const T &b, const T &c) {
...
template <typename T>
__forceinline T mini(const T &a, const T &b, const T &c, const T &d, const T &e) {
	return mini(mini(mini(a, b), mini(c, d)), e);
}
...
template <typename T>
__forceinline T max(const T &a, const T &b, const T &c, const T &d, const T &e) {
	return max(max(max(a, b), max(c, d)), e);
}

template <typename T>
__forceinline T maxi(const T &a, const T &b, const T &c) {
...
template <typename T>
__forceinline T maxi(const T &a, const T &b, const T &c, const T &d, const T &e) {
	return maxi(maxi(maxi(a, b), maxi(c, d)), e);
}

#if defined(__MACOSX__)
__forceinline ssize_t min(ssize_t a, ssize_t b) {
	return a < b ? a : b;
}
__forceinline ssize_t max(ssize_t a, ssize_t b) {
	return a < b ? b : a;
}
#endif
...
template <typename T>
__forceinline T clamp(const T &x, const T &lower = T(zero), const T &upper = T(one)) {
	return max(min(x, upper), lower);
}
template <typename T>
__forceinline T clampz(const T &x, const T &upper) {
	return max(T(zero), min(x, upper));
}
...
template <typename T>
__forceinline T rad2deg(const T &x) {
	return x * T(5.72957795130823208768e1f);
}
template <typename T>
__forceinline T sin2cos(const T &x) {
	return sqrt(max(T(zero), T(one) - x * x));
}
template <typename T>
__forceinline T cos2sin(const T &x) {
	return sin2cos(x);
}
...
#if defined(_WIN32)
template <>
__forceinline int random() {
	return int(rand()) ^ (int(rand()) << 8) ^ (int(rand()) << 16);
}
template <>
__forceinline uint32_t random() {
	return uint32_t(rand()) ^ (uint32_t(rand()) << 8) ^ (uint32_t(rand()) << 16);
}
#else
template <>
__forceinline int random() {
...
/*! exchange */
template <typename T>
__forceinline void xchg(T &a, T &b) {
	const T tmp = a;
	a = b;
	b = tmp;
}
...
struct mem<float> {
	...
	static __forceinline void store(bool mask, void *ptr, const float v) {
		if (mask)
			*(float *)ptr = v;
	}
	static __forceinline void storeu(bool mask, void *ptr, const float v) {
		if (mask)
			*(float *)ptr = v;
	}
};
