...
struct LinearSpace3 {
	...
	__forceinline LinearSpace3(const LinearSpace3 &other) {
		vx = other.vx;
		vy = other.vy;
		vz = other.vz;
	}
	__forceinline LinearSpace3 &operator=(const LinearSpace3 &other) {
		vx = other.vx;
		vy = other.vy;
		vz = other.vz;
		return *this;
	...
	__forceinline LinearSpace3(const LinearSpace3<L1> &s) : vx(s.vx), vy(s.vy), vz(s.vz) {}

	/*! matrix construction from column vectors */
	__forceinline LinearSpace3(const Vector &vx, const Vector &vy, const Vector &vz) : vx(vx), vy(vy), vz(vz) {}

	/*! construction from quaternion */
	__forceinline LinearSpace3(const QuaternionT<Scalar> &q) : vx((q.r * q.r + q.i * q.i - q.j * q.j - q.k * q.k), 2.0f * (q.i * q.j + q.r * q.k), 2.0f * (q.i * q.k - q.r * q.j)), vy(2.0f * (q.i * q.j - q.r * q.k), (q.r * q.r - q.i * q.i + q.j * q.j - q.k * q.k), 2.0f * (q.j * q.k + q.r * q.i)), vz(2.0f * (q.i * q.k + q.r * q.j), 2.0f * (q.j * q.k - q.r * q.i), (q.r * q.r - q.i * q.i - q.j * q.j + q.k * q.k)) {}
	...
	__forceinline LinearSpace3(const Scalar &m00, const Scalar &m01, const Scalar &m02,
			...
			const Scalar &m20, const Scalar &m21, const Scalar &m22) : vx(m00, m10, m20),
																	   vy(m01, m11, m21),
																	   vz(m02, m12, m22) {}
...
/*! compute transposed matrix */
template <>
__forceinline const LinearSpace3<Vec3fa> LinearSpace3<Vec3fa>::transposed() const {
	vfloat4 rx, ry, rz;
...
template <typename T>
__forceinline LinearSpace3<T> operator-(const LinearSpace3<T> &a) {
	return LinearSpace3<T>(-a.vx, -a.vy, -a.vz);
}
template <typename T>
__forceinline LinearSpace3<T> operator+(const LinearSpace3<T> &a) {
	return LinearSpace3<T>(+a.vx, +a.vy, +a.vz);
...
template <typename T>
__forceinline LinearSpace3<T> rcp(const LinearSpace3<T> &a) {
	return a.inverse();
}

/* constructs a coordinate frame form a normalized normal */
template <typename T>
__forceinline LinearSpace3<T> frame(const T &N) {
...
template <typename T>
__forceinline LinearSpace3<T> frame(const T &N, const T &dxi) {
...
template <typename T>
__forceinline LinearSpace3<T> clamp(const LinearSpace3<T> &space) {
...
template <typename T>
__forceinline LinearSpace3<T> operator+(const LinearSpace3<T> &a, const LinearSpace3<T> &b) {
...
template <typename T>
__forceinline LinearSpace3<T> operator*(const typename T::Scalar &a, const LinearSpace3<T> &b) {
	return LinearSpace3<T>(a * b.vx, a * b.vy, a * b.vz);
}
template <typename T>
__forceinline T operator*(const LinearSpace3<T> &a, const T &b) {
	return madd(T(b.x), a.vx, madd(T(b.y), a.vy, T(b.z) * a.vz));
}
template <typename T>
__forceinline LinearSpace3<T> operator*(const LinearSpace3<T> &a, const LinearSpace3<T> &b) {
	return LinearSpace3<T>(a * b.vx, a * b.vy, a * b.vz);
...
template <typename T>
__forceinline LinearSpace3<T> operator/(const LinearSpace3<T> &a, const typename T::Scalar &b) {
...
template <typename T>
__forceinline LinearSpace3<T> &operator*=(LinearSpace3<T> &a, const LinearSpace3<T> &b) {
	return a = a * b;
}
template <typename T>
__forceinline LinearSpace3<T> &operator/=(LinearSpace3<T> &a, const LinearSpace3<T> &b) {
...
template <typename T>
__forceinline T xfmPoint(const LinearSpace3<T> &s, const T &a) {
	return madd(T(a.x), s.vx, madd(T(a.y), s.vy, T(a.z) * s.vz));
}
template <typename T>
__forceinline T xfmVector(const LinearSpace3<T> &s, const T &a) {
	return madd(T(a.x), s.vx, madd(T(a.y), s.vy, T(a.z) * s.vz));
}
template <typename T>
__forceinline T xfmNormal(const LinearSpace3<T> &s, const T &a) {
...
template <typename T>
__forceinline bool operator==(const LinearSpace3<T> &a, const LinearSpace3<T> &b) {
...
template <typename T>
__forceinline LinearSpace3<T> select(const typename T::Scalar::Bool &s, const LinearSpace3<T> &t, const LinearSpace3<T> &f) {
...
template <typename T>
static embree_ostream operator<<(embree_ostream cout, const LinearSpace3<T> &m) {
...
template <int N>
using LinearSpace3vf = LinearSpace3<Vec3<vfloat<N>>>;
