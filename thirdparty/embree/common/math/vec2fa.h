...
struct __aligned(16) Vec2fa {
	...
	__forceinline Vec2fa(const Vec2<float> &other) {
		x = other.x;
		y = other.y;
	}
	__forceinline Vec2fa &operator=(const Vec2<float> &other) {
		x = other.x;
		y = other.y;
		return *this;
	}
	...
	__forceinline Vec2fa &operator=(const Vec2fa &other) {
		m128 = other.m128;
		return *this;
	}
...
__forceinline Vec2fa floor(const Vec2fa &a) {
	return Vec2fa(floorf(a.x), floorf(a.y));
}
__forceinline Vec2fa ceil(const Vec2fa &a) {
	return Vec2fa(ceilf(a.x), ceilf(a.y));
}
#endif
