...
#define DECLARE_SYMBOL2(type, name)                                                               \
	...
	void name##_error2() {                                                                        \
		throw_RTCError(RTC_ERROR_UNKNOWN, "internal error in ISA selection for " TOSTRING(name)); \
	}                                                                                             \
	type name##_error() {                                                                         \
		return type(name##_error2);                                                               \
	}                                                                                             \
	type name##_zero() {                                                                          \
		return type(nullptr);                                                                     \
	}
...
#define DECLARE_ISA_FUNCTION(type, symbol, args)                                                              \
	...
	inline type symbol##_error(args) {                                                                        \
		throw_RTCError(RTC_ERROR_UNSUPPORTED_CPU, "function " TOSTRING(symbol) " not supported by your CPU"); \
	}                                                                                                         \
...
#define SELECT_SYMBOL_SSE42(features, intersector) \
	if ((features & SSE42) == SSE42)               \
		intersector = sse42::intersector;
#else
#define SELECT_SYMBOL_SSE42(features, intersector)
#endif
...
#define SELECT_SYMBOL_AVX2(features, intersector) \
	if ((features & AVX2) == AVX2)                \
		intersector = avx2::intersector;
#else
#define SELECT_SYMBOL_AVX2(features, intersector)
#endif
...
#define SELECT_SYMBOL_AVX512(features, intersector) \
	if ((features & AVX512) == AVX512)              \
		intersector = avx512::intersector;
#else
#define SELECT_SYMBOL_AVX512(features, intersector)
#endif
