...
public:
	...
	void interpolate_impl(const RTCInterpolateArguments *const args) {
		...
		for (unsigned int i = 0; i < valueCount; i += N) {
			...
			if (P)
				mem<vfloat<N>>::storeu(valid, P + i, lerp(p0, p1, u));
			if (dPdu)
				mem<vfloat<N>>::storeu(valid, dPdu + i, p1 - p0);
			if (ddPdudu)
				mem<vfloat<N>>::storeu(valid, dPdu + i, vfloat<N>(zero));
		}
	}
...
public:
	...
	__forceinline bool valid(size_t i, const range<size_t> &itime_range) const {
		...
		for (size_t itime = itime_range.begin(); itime <= itime_range.end(); itime++) {
			...
			if (unlikely(!isvalid4(v0)))
				return false;
			const Vec3ff v1 = vertex(index + 1, itime);
			if (unlikely(!isvalid4(v1)))
				return false;
			if (min(v0.w, v1.w) < 0.0f)
				return false;
		}
		return true;
	}
	...
	__forceinline bool linearBounds(size_t i, const BBox1f &time_range, LBBox3fa &bbox) const {
		if (!valid(i, timeSegmentRange(time_range)))
			return false;
...
struct LineSegmentsISA : public LineSegments {
	LineSegmentsISA(Device *device, Geometry::GType gtype) : LineSegments(device, gtype) {}
