...
void Stat::print(std::ostream &cout) {
	...
	for (size_t i = 0; i < SIZE_HISTOGRAM; i++)
		cout << "[" << i << "] " << 100.0f * data.normal.trav_hit_boxes[i] * i / weighted_box_hits << " ";
	cout << std::endl;
	...
	if (data.shadow.travs) {
		...
		cout << "    #hit_boxes    = ";
		for (size_t i = 0; i < SIZE_HISTOGRAM; i++)
			cout << "[" << i << "] " << 100.0f * data.shadow.trav_hit_boxes[i] / shadow_box_hits << " ";
		cout << std::endl;
		for (size_t i = 0; i < SIZE_HISTOGRAM; i++)
			cout << "[" << i << "] " << 100.0f * data.shadow.trav_hit_boxes[i] * i / weighted_shadow_box_hits << " ";
		cout << std::endl;
	}
	cout << std::endl;
