...
public:
	...
	struct KeyHalfEdge {
		...
		KeyHalfEdge(uint64_t key, HalfEdge *edge) : key(key), edge(edge) {}
...
public:
	...
	__forceinline float getEdgeLevel(const size_t i) const {
		if (levels)
			return clamp(levels[i], 1.0f, 4096.0f); // FIXME: do we want to limit edge level?
		else
			return clamp(tessellationRate, 1.0f, 4096.0f); // FIXME: do we want to limit edge level?
	}
	...
	public:
		...
		__forceinline bool valid(size_t i) const {
			if (unlikely(subdiv_mode == RTC_SUBDIVISION_MODE_NO_BOUNDARY)) {
				if (getHalfEdge(i)->faceHasBorder())
					return false;
			}
			return true;
		}
...
struct SubdivMeshISA : public SubdivMesh {
	SubdivMeshISA(Device *device) : SubdivMesh(device) {}
