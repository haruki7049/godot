...
public:
	...
	__forceinline void gather(Vec3ff &p0, Vec3ff &p1, Vec3ff &p2, Vec3ff &p3, Vec3fa &n0, Vec3fa &n1, Vec3fa &n2, Vec3fa &n3, size_t i, float time) const {
		...
		const float t1 = ftime;
		Vec3ff a0, a1, a2, a3;
		Vec3fa an0, an1, an2, an3;
		gather(a0, a1, a2, a3, an0, an1, an2, an3, i, itime);
		Vec3ff b0, b1, b2, b3;
		Vec3fa bn0, bn1, bn2, bn3;
...
struct CurveGeometryInterface : public CurveGeometry {
	...
	CurveGeometryInterface(Device *device, Geometry::GType gtype) : CurveGeometry(device, gtype) {}
	...
	void interpolate_impl(const RTCInterpolateArguments *const args) {
		...
		for (unsigned int i = 0; i < valueCount; i += N) {
			...
			if (P)
				mem<vfloat<N>>::storeu(valid, P + i, curve.eval(u));
			if (dPdu)
				mem<vfloat<N>>::storeu(valid, dPdu + i, curve.eval_du(u));
			if (ddPdudu)
				mem<vfloat<N>>::storeu(valid, ddPdudu + i, curve.eval_dudu(u));
		}
	}
...
struct HermiteCurveGeometryInterface : public CurveGeometry {
	...
	HermiteCurveGeometryInterface(Device *device, Geometry::GType gtype) : CurveGeometry(device, gtype) {}
	...
	void interpolate_impl(const RTCInterpolateArguments *const args) {
		...
		if (bufferType == RTC_BUFFER_TYPE_VERTEX_ATTRIBUTE) {
			...
			for (unsigned int i = 0; i < valueCount; i += N) {
				...
				if (P)
					mem<vfloat<N>>::storeu(valid, P + i, madd(1.0f - u, p0, u * p1));
				if (dPdu)
