...
struct BVH : public RefCount {
	BVH(Device *device) : device(device), allocator(device, true), morton_src(device, 0), morton_tmp(device, 0) {
		device->refInc();
	}
...
void *rtcBuildBVHBinnedSAH(const RTCBuildArguments *arguments) {
	...
	void *root = BVHBuilderBinnedSAH::build<void *>(
			...
			[&](BVHBuilderBinnedSAH::BuildRecord *children, const size_t N, const FastAllocator::CachedAllocator &alloc) -> void * {
				...
				for (size_t i = 0; i < N; i++)
					cbounds[i] = (const RTCBounds *)&children[i].prims.geomBounds;
				setNodeBounds(node, cbounds, (unsigned int)N, userPtr);
				return node;
			},
...
void *rtcBuildBVHSpatialSAH(const RTCBuildArguments *arguments) {
	...
	struct Splitter {
		Splitter(RTCSplitPrimitiveFunction splitPrimitive, unsigned geomID, unsigned primID, void *userPtr) : splitPrimitive(splitPrimitive), geomID(geomID), primID(primID), userPtr(userPtr) {}
		...
		__forceinline void operator()(PrimRef &prim, const size_t dim, const float pos, PrimRef &left_o, PrimRef &right_o) const {
			...
			left_o.geomIDref() = geomID;
			left_o.primIDref() = primID;
			right_o.geomIDref() = geomID;
			right_o.primIDref() = primID;
		}
