...
struct GeometryCounts {
	__forceinline GeometryCounts() : numFilterFunctions(0),
									 numTriangles(0),
									 numMBTriangles(0),
									 numQuads(0),
									 numMBQuads(0),
									 ...
									 numMBLineSegments(0),
									 numSubdivPatches(0),
									 numMBSubdivPatches(0),
									 numUserGeometries(0),
									 numMBUserGeometries(0),
									 numInstancesCheap(0),
									 numMBInstancesCheap(0),
									 numInstancesExpensive(0),
									 numMBInstancesExpensive(0),
									 numGrids(0),
									 numMBGrids(0),
									 numPoints(0),
									 numMBPoints(0) {}
	...
	__forceinline unsigned int enabledGeometryTypesMask() const {
		...
		if (numQuads)
			mask |= 1 << 1;
		if (numBezierCurves + numLineSegments)
			mask |= 1 << 2;
		if (numSubdivPatches)
			mask |= 1 << 3;
		if (numUserGeometries)
			mask |= 1 << 4;
		if (numInstancesCheap)
			mask |= 1 << 5;
		if (numInstancesExpensive)
			mask |= 1 << 6;
		if (numGrids)
			mask |= 1 << 7;
		if (numPoints)
		...
		if (numMBQuads)
			maskMB |= 1 << 1;
		if (numMBBezierCurves + numMBLineSegments)
			maskMB |= 1 << 2;
		if (numMBSubdivPatches)
			maskMB |= 1 << 3;
		if (numMBUserGeometries)
			maskMB |= 1 << 4;
		if (numMBInstancesCheap)
			maskMB |= 1 << 5;
		if (numMBInstancesExpensive)
			maskMB |= 1 << 6;
		if (numMBGrids)
			maskMB |= 1 << 7;
		if (numMBPoints)
