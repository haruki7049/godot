...
Geometry::Geometry(Device *device, GType gtype, unsigned int numPrimitives, unsigned int numTimeSteps) : device(device), userPtr(nullptr), numPrimitives(numPrimitives), numTimeSteps(unsigned(numTimeSteps)), fnumTimeSegments(float(numTimeSteps - 1)), time_range(0.0f, 1.0f), mask(-1), gtype(gtype), gsubtype(GTY_SUBTYPE_DEFAULT), quality(RTC_BUILD_QUALITY_MEDIUM), state((unsigned)State::MODIFIED), enabled(true), intersectionFilterN(nullptr), occlusionFilterN(nullptr), pointQueryFunc(nullptr) {
	device->refInc();
}
...
void Geometry::interpolateN(const RTCInterpolateNArguments *const args) {
	...
	if (valueCount > 256)
		throw_RTCError(RTC_ERROR_INVALID_OPERATION, "maximally 256 floating point values can be interpolated per vertex");
	...
	if (ddPdudu) {
		ddPdudut = ddPdudu_tmp;
		ddPdvdvt = ddPdvdv_tmp;
		ddPdudvt = ddPdudv_tmp;
	}
...
bool Geometry::pointQuery(PointQuery *query, PointQueryContext *context) {
	...
	bool update = false;
	if (context->func)
		update |= context->func(&args);
	if (pointQueryFunc)
		update |= pointQueryFunc(&args);
