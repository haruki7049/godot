...
State::ErrorHandler::ErrorHandler() : thread_error(createTls()) {}
...
State::State() : enabled_cpu_features(getCPUFeatures()),
...
void State::parse(Ref<TokenStream> cin) {
	...
	while (cin->peek() != Token::Eof()) {
		...
		if (tok == Token::Id("threads") && cin->trySymbol("="))
			...
			else if (freq == "simd256")
				frequency_level = FREQUENCY_SIMD256;
			else if (freq == "simd512")
			...
			if (cin->trySymbol("=")) {
				Token flag = cin->get();
				if (flag == Token::Id("low"))
					quality_flags = RTC_BUILD_QUALITY_LOW;
				else if (flag == Token::Id("medium"))
					quality_flags = RTC_BUILD_QUALITY_MEDIUM;
				else if (flag == Token::Id("high"))
					quality_flags = RTC_BUILD_QUALITY_HIGH;
			}
		}
			...
			if (cin->trySymbol("=")) {
				do {
					Token flag = cin->get();
					if (flag == Token::Id("dynamic"))
						scene_flags |= RTC_SCENE_FLAG_DYNAMIC;
					else if (flag == Token::Id("compact"))
						scene_flags |= RTC_SCENE_FLAG_COMPACT;
					else if (flag == Token::Id("robust"))
...
void State::print() {
	...
	switch (frequency_level) {
		case FREQUENCY_SIMD128:
			std::cout << "simd128" << std::endl;
			break;
		case FREQUENCY_SIMD256:
			std::cout << "simd256" << std::endl;
			break;
		case FREQUENCY_SIMD512:
			std::cout << "simd512" << std::endl;
			break;
		default:
