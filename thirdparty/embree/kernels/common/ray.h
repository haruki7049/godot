...
struct RayK {
	...
	__forceinline RayK(const Vec3vf<K> &org, const Vec3vf<K> &dir,
			...
			const vfloat<K> &time = zero, const vint<K> &mask = -1, const vint<K> &id = 0, const vint<K> &flags = 0) : org(org),
																													   dir(dir),
																													   _tnear(tnear),
																													   tfar(tfar),
																													   _time(time),
...
struct RayHitK : RayK<K> {
	...
	__forceinline RayHitK(const Vec3vf<K> &org, const Vec3vf<K> &dir,
			const vfloat<K> &tnear = zero, const vfloat<K> &tfar = inf,
			const vfloat<K> &time = zero, const vint<K> &mask = -1, const vint<K> &id = 0, const vint<K> &flags = 0) : RayK<K>(org, dir, tnear, tfar, time, mask, id, flags),
																													   geomID(RTC_INVALID_GEOMETRY_ID) {
		for (unsigned l = 0; l < RTC_MAX_INSTANCE_LEVEL_COUNT; ++l)
			instID[l] = RTC_INVALID_GEOMETRY_ID;
	}

	__forceinline RayHitK(const RayK<K> &ray) : RayK<K>(ray),
												geomID(RTC_INVALID_GEOMETRY_ID) {
		for (unsigned l = 0; l < RTC_MAX_INSTANCE_LEVEL_COUNT; ++l)
			instID[l] = RTC_INVALID_GEOMETRY_ID;
	}
	...
	__forceinline void verifyHit(const vbool<K> &valid0) const {
		...
		if (any(valid & !vt))
			throw_RTCError(RTC_ERROR_UNKNOWN, "invalid t");
		if (any(valid & !vu))
			throw_RTCError(RTC_ERROR_UNKNOWN, "invalid u");
		if (any(valid & !vv))
			throw_RTCError(RTC_ERROR_UNKNOWN, "invalid v");
		if (any(valid & !vnx))
			throw_RTCError(RTC_ERROR_UNKNOWN, "invalid Ng.x");
		if (any(valid & !vny))
			throw_RTCError(RTC_ERROR_UNKNOWN, "invalid Ng.y");
		if (any(valid & !vnz))
			throw_RTCError(RTC_ERROR_UNKNOWN, "invalid Ng.z");
	}
...
struct RayK<1> {
	...
	/* Constructs a ray from origin, direction, and ray segment. Near
	 *  has to be smaller than far */
	__forceinline RayK(const Vec3fa &org, const Vec3fa &dir, float tnear = zero, float tfar = inf, float time = zero, int mask = -1, int id = 0, int flags = 0) : org(org, tnear), dir(dir, time), tfar(tfar), mask(mask), id(id), flags(flags) {}
...
struct RayHitK<1> : RayK<1> {
	...
	/* Constructs a ray from origin, direction, and ray segment. Near
	 *  has to be smaller than far */
	__forceinline RayHitK(const Vec3fa &org, const Vec3fa &dir, float tnear = zero, float tfar = inf, float time = zero, int mask = -1, int id = 0, int flags = 0) : RayK<1>(org, dir, tnear, tfar, time, mask, id, flags),
																																									 geomID(RTC_INVALID_GEOMETRY_ID) {}

	__forceinline RayHitK(const RayK<1> &ray) : RayK<1>(ray),
												geomID(RTC_INVALID_GEOMETRY_ID) {}
...
__forceinline void RayK<K>::copy(size_t dest, size_t source) {
	org.x[dest] = org.x[source];
	org.y[dest] = org.y[source];
	org.z[dest] = org.z[source];
	tnear()[dest] = tnear()[source];
	dir.x[dest] = dir.x[source];
	dir.y[dest] = dir.y[source];
	dir.z[dest] = dir.z[source];
	time()[dest] = time()[source];
	tfar[dest] = tfar[source];
	mask[dest] = mask[source];
	id[dest] = id[source];
	flags[dest] = flags[source];
}
...
__forceinline void RayHitK<K>::copy(size_t dest, size_t source) {
	...
	id[dest] = id[source];
	flags[dest] = flags[source];
	Ng.x[dest] = Ng.x[source];
	Ng.y[dest] = Ng.y[source];
	Ng.z[dest] = Ng.z[source];
	u[dest] = u[source];
	v[dest] = v[source];
	primID[dest] = primID[source];
	geomID[dest] = geomID[source];
...
struct RayStreamSOA {
	__forceinline RayStreamSOA(void *rays, size_t N) : ptr((char *)rays), N(N) {}
...
struct StackRayStreamSOA : public RayStreamSOA {
	__forceinline StackRayStreamSOA(size_t K) : RayStreamSOA(data, K) { assert(K <= MAX_K); }
...
struct RayStreamSOP {
	...
	__forceinline void setHitByOffset(size_t offset, const RayHit &ray) {
		if (ray.geomID != RTC_INVALID_GEOMETRY_ID) {
			*(float *__restrict__)((char *)tfar + offset) = ray.tfar;
			...
			if (likely(Ng_x))
				*(float *__restrict__)((char *)Ng_x + offset) = ray.Ng.x;
			if (likely(Ng_y))
				*(float *__restrict__)((char *)Ng_y + offset) = ray.Ng.y;
			if (likely(Ng_z))
				*(float *__restrict__)((char *)Ng_z + offset) = ray.Ng.z;
	...
	__forceinline void setHitByOffset(const vbool<K> &valid_i, size_t offset, const RayHitK<K> &ray) {
		...
		if (likely(any(valid))) {
			...
			if (likely(Ng_x))
				vfloat<K>::storeu(valid, (float *__restrict__)((char *)Ng_x + offset), ray.Ng.x);
			if (likely(Ng_y))
				vfloat<K>::storeu(valid, (float *__restrict__)((char *)Ng_y + offset), ray.Ng.y);
			if (likely(Ng_z))
				vfloat<K>::storeu(valid, (float *__restrict__)((char *)Ng_z + offset), ray.Ng.z);
		...
		if (likely(any(valid))) {
			...
			while (valid_bits != 0) {
				...
				*(float *__restrict__)((char *)tfar + ofs) = ray.tfar[k];
				...
				if (likely(Ng_x))
					*(float *__restrict__)((char *)Ng_x + ofs) = ray.Ng.x[k];
				if (likely(Ng_y))
					*(float *__restrict__)((char *)Ng_y + ofs) = ray.Ng.y[k];
				if (likely(Ng_z))
					*(float *__restrict__)((char *)Ng_z + ofs) = ray.Ng.z[k];
...
struct RayStreamAOS {
	__forceinline RayStreamAOS(void *rays) : ptr((Ray *)rays) {}
...
struct RayStreamAOP {
	__forceinline RayStreamAOP(void *rays) : ptr((Ray **)rays) {}
