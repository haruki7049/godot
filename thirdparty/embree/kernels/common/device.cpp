...
Device::Device(const char *cfg) {
	...
	switch (getCPUModel()) {
		...
		case CPU::XEON_PHI_KNIGHTS_MILL:
			frequency_level = FREQUENCY_SIMD512;
			break;
...
void Device::process_error(Device *device, RTCError error, const char *str) {
	...
	if (device->verbosity(1)) {
		switch (error) {
			case RTC_ERROR_NONE:
				std::cerr << "Embree: No error";
				break;
			case RTC_ERROR_UNKNOWN:
				std::cerr << "Embree: Unknown error";
				break;
			case RTC_ERROR_INVALID_ARGUMENT:
				std::cerr << "Embree: Invalid argument";
				break;
			case RTC_ERROR_INVALID_OPERATION:
				std::cerr << "Embree: Invalid operation";
				break;
			case RTC_ERROR_OUT_OF_MEMORY:
				std::cerr << "Embree: Out of memory";
				break;
			case RTC_ERROR_UNSUPPORTED_CPU:
				std::cerr << "Embree: Unsupported CPU";
				break;
			default:
				std::cerr << "Embree: Invalid error code";
				break;
		};
		if (str)
			std::cerr << ", (" << str << ")";
...
void Device::setCacheSize(size_t bytes) {
	...
	if (bytes == 0)
		g_cache_size_map.erase(this);
	else
		g_cache_size_map[this] = bytes;
...
ssize_t Device::getProperty(const RTCDeviceProperty prop) {
	...
	if (iprop >= 2000000 && iprop < 3000000) {
		...
		if (test)
			return (ssize_t)test->name.c_str();
		else
			return 0;
	}
	...
	switch (prop) {
		...
		case RTC_DEVICE_PROPERTY_NATIVE_RAY4_SUPPORTED:
			return hasISA(SSE2);
#else
		case RTC_DEVICE_PROPERTY_NATIVE_RAY4_SUPPORTED:
			return 0;
		...
		case RTC_DEVICE_PROPERTY_NATIVE_RAY8_SUPPORTED:
			return hasISA(AVX);
#else
		case RTC_DEVICE_PROPERTY_NATIVE_RAY8_SUPPORTED:
			return 0;
		...
		case RTC_DEVICE_PROPERTY_NATIVE_RAY16_SUPPORTED:
			return hasISA(AVX512);
#else
		case RTC_DEVICE_PROPERTY_NATIVE_RAY16_SUPPORTED:
			return 0;
#endif
...
#if defined(EMBREE_RAY_PACKETS)
		case RTC_DEVICE_PROPERTY_RAY_STREAM_SUPPORTED:
			return 1;
#else
		case RTC_DEVICE_PROPERTY_RAY_STREAM_SUPPORTED:
			return 0;
...
#if defined(TASKING_INTERNAL)
		case RTC_DEVICE_PROPERTY_TASKING_SYSTEM:
			return 0;
#endif
...
#if defined(TASKING_PPL)
		case RTC_DEVICE_PROPERTY_JOIN_COMMIT_SUPPORTED:
			return 0;
		...
		case RTC_DEVICE_PROPERTY_PARALLEL_COMMIT_SUPPORTED:
			return 1;
#else
		case RTC_DEVICE_PROPERTY_PARALLEL_COMMIT_SUPPORTED:
			return 0;
		...
		default:
			throw_RTCError(RTC_ERROR_INVALID_ARGUMENT, "unknown readable property");
			break;
	};
}
} // namespace embree
