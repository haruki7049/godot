...
public:
	...
	public:
		...
		Iterator2(Scene *scene, Geometry::GTypeMask typemask, bool mblur) : scene(scene), typemask(typemask), mblur(mblur) {}
...
public:
	...
	__forceinline Mesh *getSafe(size_t i) {
		...
		if (geometries[i] == null)
			return nullptr;
		if (!(geometries[i]->getTypeMask() & Mesh::geom_type))
			return nullptr;
		else
			return (Mesh *)geometries[i].ptr;
	}
...
public:
	struct BuildProgressMonitorInterface : public BuildProgressMonitor {
		BuildProgressMonitorInterface(Scene *scene) : scene(scene) {}
