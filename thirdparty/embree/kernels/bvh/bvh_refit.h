...
public:
	...
	static const size_t MAX_SUB_TREE_EXTRACTION_DEPTH = (N == 4) ? 4 : (N == 8) ? 3 :
																				  3;
	static const size_t MAX_NUM_SUB_TREES = (N == 4) ? 256 : (N == 8) ? 512 :
																		N * N * N; // N ^ MAX_SUB_TREE_EXTRACTION_DEPTH
	size_t numSubTrees;
...
public:
	...
	virtual const BBox3fa leafBounds(NodeRef &ref) const {
		size_t num;
		char *prim = ref.leaf(num);
		if (unlikely(ref == BVH::emptyNode))
			return empty;
