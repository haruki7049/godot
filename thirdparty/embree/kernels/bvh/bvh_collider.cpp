...
struct Collision {
	...
	__forceinline Collision(unsigned geomID0, unsigned primID0, unsigned geomID1, unsigned primID1) : geomID0(geomID0), primID0(primID0), geomID1(geomID1), primID1(primID1) {}
...
bool intersect_triangle_triangle(Scene *scene0, unsigned geomID0, unsigned primID0, Scene *scene1, unsigned geomID1, unsigned primID1) {
	...
	if (scene0 == scene1 && geomID0 == geomID1) {
		...
		if (any(vint4(tri1.v[0]) == t0))
			return false;
		if (any(vint4(tri1.v[1]) == t0))
			return false;
		if (any(vint4(tri1.v[2]) == t0))
			return false;
	}
...
__forceinline void BVHNColliderUserGeom<N>::processLeaf(NodeRef node0, NodeRef node1) {
	...
	size_t N0;
	Object *leaf0 = (Object *)node0.leaf(N0);
	size_t N1;
	Object *leaf1 = (Object *)node1.leaf(N1);
	for (size_t i = 0; i < N0; i++) {
		for (size_t j = 0; j < N1; j++) {
			...
			if (this->scene0 == this->scene1 && geomID0 == geomID1 && primID0 == primID1)
				continue;
...
void BVHNColliderUserGeom<N>::collide(BVH *__restrict__ bvh0, BVH *__restrict__ bvh1, RTCCollideFunc callback, void *userPtr) {
	BVHNColliderUserGeom<N>(bvh0->scene, bvh1->scene, callback, userPtr).collide_recurse_entry(bvh0->root, bvh0->bounds.bounds(), bvh1->root, bvh1->bounds.bounds());
}
