...
struct TravRayBase<N, false> {
	...
	__forceinline TravRayBase(const Vec3fa &ray_org, const Vec3fa &ray_dir) : org_xyz(ray_org), dir_xyz(ray_dir) {
...
struct TravRay : TravRayBase<N, robust> {
	...
	__forceinline TravRay(const Vec3fa &ray_org, const Vec3fa &ray_dir, float ray_tnear, float ray_tfar) : TravRayBase<N, robust>(ray_org, ray_dir),
																										   tnear(ray_tnear),
																										   tfar(ray_tfar) {}
...
struct BVHNNodePointQuerySphere1<N, BVH_AN1_UN1> {
	static __forceinline bool pointQuery(const typename BVHN<N>::NodeRef &node, const TravPointQuery<N> &query, float time, vfloat<N> &dist, size_t &mask) {
		if (likely(node.isAABBNode()))
			mask = pointQueryNodeSphere(node.getAABBNode(), query, dist);
		else if (unlikely(node.isOBBNode()))
			mask = pointQueryNodeSphere(node.ungetAABBNode(), query, dist);
		else
			return false;
		return true;
	}
...
struct BVHNNodePointQuerySphere1<N, BVH_AN2_UN2> {
	static __forceinline bool pointQuery(const typename BVHN<N>::NodeRef &node, const TravPointQuery<N> &query, float time, vfloat<N> &dist, size_t &mask) {
		if (likely(node.isAABBNodeMB()))
			mask = pointQueryNodeSphere(node.getAABBNodeMB(), query, time, dist);
		else if (unlikely(node.isOBBNodeMB()))
			mask = pointQueryNodeSphere(node.ungetAABBNodeMB(), query, time, dist);
		else
			return false;
		return true;
	}
...
struct BVHNNodePointQuerySphere1<N, BVH_AN2_AN4D_UN2> {
	static __forceinline bool pointQuery(const typename BVHN<N>::NodeRef &node, const TravPointQuery<N> &query, float time, vfloat<N> &dist, size_t &mask) {
		if (unlikely(node.isLeaf()))
			return false;
		if (unlikely(node.isOBBNodeMB()))
			mask = pointQueryNodeSphere(node.ungetAABBNodeMB(), query, time, dist);
		else
			mask = pointQueryNodeSphereMB4D(node, query, time, dist);
		return true;
	}
};
...
struct BVHNNodePointQueryAABB1<N, BVH_AN1_UN1> {
	static __forceinline bool pointQuery(const typename BVHN<N>::NodeRef &node, const TravPointQuery<N> &query, float time, vfloat<N> &dist, size_t &mask) {
		if (likely(node.isAABBNode()))
			...
			mask = pointQueryNodeAABB(node.ungetAABBNode(), query, dist);
		else
			return false;
		return true;
	}
...
struct BVHNNodePointQueryAABB1<N, BVH_AN2_UN2> {
	static __forceinline bool pointQuery(const typename BVHN<N>::NodeRef &node, const TravPointQuery<N> &query, float time, vfloat<N> &dist, size_t &mask) {
		if (likely(node.isAABBNodeMB()))
			...
			mask = pointQueryNodeAABB(node.ungetAABBNodeMB(), query, time, dist);
		else
			return false;
		return true;
	}
...
struct BVHNNodePointQueryAABB1<N, BVH_AN2_AN4D_UN2> {
	static __forceinline bool pointQuery(const typename BVHN<N>::NodeRef &node, const TravPointQuery<N> &query, float time, vfloat<N> &dist, size_t &mask) {
		...
		if (unlikely(node.isOBBNodeMB()))
			...
			mask = pointQueryNodeAABBMB4D(node, query, time, dist);
		return true;
	}
};
...
struct BVHNNodeIntersector1<N, BVH_AN1_UN1, false> {
	static __forceinline bool intersect(const typename BVHN<N>::NodeRef &node, const TravRay<N, false> &ray, float time, vfloat<N> &dist, size_t &mask) {
		if (likely(node.isAABBNode()))
			...
			mask = intersectNode(node.ungetAABBNode(), ray, dist);
		else
			return false;
		return true;
	}
...
struct BVHNNodeIntersector1<N, BVH_AN2_UN2, false> {
	static __forceinline bool intersect(const typename BVHN<N>::NodeRef &node, const TravRay<N, false> &ray, float time, vfloat<N> &dist, size_t &mask) {
		if (likely(node.isAABBNodeMB()))
			...
			mask = intersectNode(node.ungetAABBNodeMB(), ray, time, dist);
		else
			return false;
		return true;
	}
...
struct BVHNNodeIntersector1<N, BVH_AN2_AN4D_UN2, false> {
	static __forceinline bool intersect(const typename BVHN<N>::NodeRef &node, const TravRay<N, false> &ray, float time, vfloat<N> &dist, size_t &mask) {
		...
		if (unlikely(node.isOBBNodeMB()))
			...
			mask = intersectNodeMB4D(node, ray, time, dist);
		return true;
	}
};
...
struct BVHNNodeIntersector1<N, BVH_AN2_AN4D_UN2, true> {
	static __forceinline bool intersect(const typename BVHN<N>::NodeRef &node, const TravRay<N, true> &ray, float time, vfloat<N> &dist, size_t &mask) {
		...
		if (unlikely(node.isOBBNodeMB()))
			...
			mask = intersectNodeMB4DRobust(node, ray, time, dist);
		return true;
	}
};
