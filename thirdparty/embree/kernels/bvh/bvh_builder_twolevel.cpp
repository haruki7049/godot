...
BVHNBuilderTwoLevel<N, Mesh, Primitive>::BVHNBuilderTwoLevel(BVH *bvh, Scene *scene, Geometry::GTypeMask gtype, bool useMortonBuilder, const size_t singleThreadThreshold) : bvh(bvh), scene(scene), refs(scene->device, 0), prims(scene->device, 0), singleThreadThreshold(singleThreadThreshold), gtype(gtype), useMortonBuilder_(useMortonBuilder) {}
...
void BVHNBuilderTwoLevel<N, Mesh, Primitive>::build() {
	...
	if (num < bvh->objects.size()) {
		parallel_for(num, bvh->objects.size(), [&](const range<size_t> &r) {
			for (size_t i = r.begin(); i < r.end(); i++) {
				builders[i].reset();
				delete bvh->objects[i];
				bvh->objects[i] = nullptr;
			}
		});
	}
...
#endif
	{
		...
		if (bvh->objects.size() < num)
			bvh->objects.resize(num);
		if (builders.size() < num)
			builders.resize(num);
		resizeRefsList();
		nextRef.store(0);
		...
		if (nextRef == 1) {
...

			{
              ...
              return pinfo; }, [](const PrimInfo &a, const PrimInfo &b) { return PrimInfo::merge(a, b); });
...
void BVHNBuilderTwoLevel<N, Mesh, Primitive>::deleteGeometry(size_t geomID) {
	if (geomID >= bvh->objects.size())
		return;
	if (builders[geomID])
		builders[geomID].reset();
	delete bvh->objects[geomID];
	bvh->objects[geomID] = nullptr;
}
