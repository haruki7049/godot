...
public:
	...
	public:
		...
		__forceinline BuildRef(const BBox3fa &bounds, NodeRef node) : PrimRef(bounds, (size_t)node), node(node) {
			if (node.isLeaf())
				bounds_area = 0.0f;
			else
		...
		__forceinline BuildRef(const BBox3fa &bounds, NodeRef node, const unsigned int geomID, const unsigned int numPrimitives) : PrimRef(bounds, geomID, numPrimitives), node(node) {
...
private:
	...
	public:
		RefBuilderSmall(size_t objectID) : objectID_(objectID) {}
	...
	public:
		RefBuilderLarge(size_t objectID, const Ref<Builder> &builder, RTCBuildQuality quality) : objectID_(objectID), builder_(builder), quality_(quality) {}

		void attachBuildRefs(BVHNBuilderTwoLevel *topBuilder) {
			BVH *object = topBuilder->getBVH(objectID_);
			assert(object);
	...
	void resizeRefsList() {
		size_t num = parallel_reduce(size_t(0), scene->size(), size_t(0), [this](const range<size_t> &r) -> size_t {
            size_t c = 0;
