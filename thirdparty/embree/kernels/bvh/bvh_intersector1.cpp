...
struct PointQueryDispatch {
	...
	static __forceinline bool pointQuery(const Accel::Intersectors *This, PointQuery *query, PointQueryContext *context) {
		...
		bool changed = false;
		float cull_radius = context->query_type == POINT_QUERY_TYPE_SPHERE ? query->radius * query->radius : dot(context->query_radius, context->query_radius);
		...
		pop: {
			...
			if (PrimitiveIntersector1::pointQuery(This, query, context, prim, num, tquery, lazy_node)) {
				...
				cull_radius = context->query_type == POINT_QUERY_TYPE_SPHERE ? query->radius * query->radius : dot(context->query_radius, context->query_radius);
			}
