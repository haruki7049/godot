...
public:
	FeatureAdaptiveEval(const HalfEdge *edge, const char *vertices, size_t stride, const float u, const float v,
			Vertex *P, Vertex *dPdu, Vertex *dPdv, Vertex *ddPdudu, Vertex *ddPdvdv, Vertex *ddPdudv) : P(P),
																										dPdu(dPdu),
																										dPdv(dPdv),
																										ddPdudu(ddPdudu),
																										ddPdvdv(ddPdvdv),
		...
		switch (edge->patch_type) {
			case HalfEdge::BILINEAR_PATCH:
				BilinearPatch(edge, vertices, stride).eval(u, v, P, dPdu, dPdv, ddPdudu, ddPdvdv, ddPdudv, 1.0f);
				break;
			case HalfEdge::REGULAR_QUAD_PATCH:
				RegularPatchT(edge, vertices, stride).eval(u, v, P, dPdu, dPdv, ddPdudu, ddPdvdv, ddPdudv, 1.0f);
				break;
#if PATCH_USE_GREGORY == 2
			case HalfEdge::IRREGULAR_QUAD_PATCH:
				GregoryPatch(edge, vertices, stride).eval(u, v, P, dPdu, dPdv, ddPdudu, ddPdvdv, ddPdudv, 1.0f);
				break;
#endif
			default: {
	...
	void eval_general_quad(const GeneralCatmullClarkPatch &patch, array_t<CatmullClarkPatch, GeneralCatmullClarkPatch::SIZE> &patches, const Vec2f &uv, size_t depth) {
		...
		if (v < 0.5f) {
			if (u < 0.5f) {
				...
				patch.getLimitBorder(borders, 0);
...
#if PATCH_USE_GREGORY == 2
				BezierCurve borders[2];
				patch.getLimitBorder(borders, 1);
				BezierCurve border0l, border0r;
				borders[0].subdivide(border0l, border0r);
				BezierCurve border2l, border2r;
				borders[1].subdivide(border2l, border2r);
...
#if PATCH_USE_GREGORY == 2
				BezierCurve borders[2];
				patch.getLimitBorder(borders, 2);
				BezierCurve border0l, border0r;
				borders[0].subdivide(border0l, border0r);
				BezierCurve border2l, border2r;
				borders[1].subdivide(border2l, border2r);
...
#if PATCH_USE_GREGORY == 2
				BezierCurve borders[2];
				patch.getLimitBorder(borders, 3);
				BezierCurve border0l, border0r;
				borders[0].subdivide(border0l, border0r);
				BezierCurve border2l, border2r;
				borders[1].subdivide(border2l, border2r);
		...
		while (true) {
			...
			if (unlikely(final(patch, ty, depth))) {
				...
				if (v < 0.5f) {
					if (u < 0.5f) {
						patch = patches[0];
						uv = Vec2f(2.0f * u, 2.0f * v);
						dscale *= 2.0f;
					} else {
						patch = patches[1];
						uv = Vec2f(2.0f * u - 1.0f, 2.0f * v);
						dscale *= 2.0f;
					}
				} else {
					if (u > 0.5f) {
						patch = patches[2];
						uv = Vec2f(2.0f * u - 1.0f, 2.0f * v - 1.0f);
						dscale *= 2.0f;
					} else {
						patch = patches[3];
						uv = Vec2f(2.0f * u, 2.0f * v - 1.0f);
						dscale *= 2.0f;
					}
				}
				depth++;
	...
	void eval(const GeneralCatmullClarkPatch &patch, const Vec2f &uv, const size_t depth) {
		...
		/* parametrization for arbitrary polygons */
			...
			const unsigned l = (unsigned)floor(0.5f * uv.x);
			const float u = 2.0f * frac(0.5f * uv.x) - 0.5f;
			const unsigned h = (unsigned)floor(0.5f * uv.y);
			const float v = 2.0f * frac(0.5f * uv.y) - 0.5f;
