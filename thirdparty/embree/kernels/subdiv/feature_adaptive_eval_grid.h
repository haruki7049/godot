...
public:
	FeatureAdaptiveEvalGrid(const GeneralCatmullClarkPatch3fa &patch, unsigned subPatch,
			...
			const unsigned dwidth, const unsigned dheight) : x0(x0),
															 x1(x1),
															 y0(y0),
															 y1(y1),
															 swidth(swidth),
															 sheight(sheight),
															 rcp_swidth(1.0f / (swidth - 1.0f)),
															 rcp_sheight(1.0f / (sheight - 1.0f)),
															 Px(Px),
															 Py(Py),
															 Pz(Pz),
															 U(U),
		...
		if (N == 4) {
			...
			BezierCurve3fa borders[GeneralCatmullClarkPatch3fa::SIZE];
			patch.getLimitBorder(borders);
			BezierCurve3fa border0l, border0r;
			borders[0].subdivide(border0l, border0r);
			BezierCurve3fa border1l, border1r;
			borders[1].subdivide(border1l, border1r);
			BezierCurve3fa border2l, border2r;
			borders[2].subdivide(border2l, border2r);
			BezierCurve3fa border3l, border3r;
...
#if PATCH_USE_GREGORY == 2
			BezierCurve3fa borders[2];
			patch.getLimitBorder(borders, subPatch);
			BezierCurve3fa border0l, border0r;
			borders[0].subdivide(border0l, border0r);
			BezierCurve3fa border2l, border2r;
	...
	void evalLocalGrid(const Patch &patch, const BBox2f &srange, const int lx0, const int lx1, const int ly0, const int ly1) {
		...
		foreach2(lx0, lx1, ly0, ly1, [&](const vboolx &valid, const vintx &ix, const vintx &iy) {
			...
			if (unlikely(Nx != nullptr))
				n = normalize_safe(patch.normal(lu, lv));
