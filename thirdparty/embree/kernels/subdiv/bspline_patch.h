...
public:
	...
	__forceinline Vertex convex_corner(const float vertex_crease_weight,
			...
			const Vertex &v10, const Vertex &v11, const Vertex &v12,
			const Vertex &v20, const Vertex &v21, const Vertex &v22) {
		if (std::isinf(vertex_crease_weight))
			return hard_corner(v01, v02, v10, v11, v12, v20, v21, v22);
		else
			return soft_convex_corner(v01, v02, v10, v11, v12, v20, v21, v22);
	}
	...
	void init(const HalfEdge *edge0, const char *vertices, size_t stride) {
		...
		/* fill inner vertices */
		const Vertex v11 = v[1][1] = load(edge0, vertices, stride);
		const HalfEdge *edge1 = edge0->next();
		const Vertex v12 = v[1][2] = load(edge1, vertices, stride);
		const HalfEdge *edge2 = edge1->next();
		const Vertex v22 = v[2][2] = load(edge2, vertices, stride);
		const HalfEdge *edge3 = edge2->next();
		const Vertex v21 = v[2][1] = load(edge3, vertices, stride);
		assert(edge0 == edge3->next());
	...
	void eval(const float u, const float v,
		...
		if (dPdu) {
			assert(dPdu);
			*dPdu = eval_du(u, v) * dscale;
			assert(dPdv);
			*dPdv = eval_dv(u, v) * dscale;
		}
		if (ddPdudu) {
			assert(ddPdudu);
			*ddPdudu = eval_dudu(u, v) * sqr(dscale);
			assert(ddPdvdv);
			*ddPdvdv = eval_dvdv(u, v) * sqr(dscale);
			assert(ddPdudv);
			*ddPdudv = eval_dudv(u, v) * sqr(dscale);
		}
	}
	...
	void eval(const vbool &valid, const vfloat &uu, const vfloat &vv,
		...
		if (P) {
			...
			for (size_t i = 0; i < N; i++)
				vfloat::store(valid, P + i * dstride, eval(i, uu, vv, u_n, v_n));
		}
		if (dPdu) {
			{
				assert(dPdu);
				...
				const Vec4<vfloat> v_n = BSplineBasis::eval(vv);
				for (size_t i = 0; i < N; i++)
					vfloat::store(valid, dPdu + i * dstride, eval(i, uu, vv, u_n, v_n) * dscale);
			}
			{
				assert(dPdv);
				const Vec4<vfloat> u_n = BSplineBasis::eval(uu);
				...
				for (size_t i = 0; i < N; i++)
					vfloat::store(valid, dPdv + i * dstride, eval(i, uu, vv, u_n, v_n) * dscale);
			}
		}
		if (ddPdudu) {
			{
				...
				const Vec4<vfloat> v_n = BSplineBasis::eval(vv);
				for (size_t i = 0; i < N; i++)
					vfloat::store(valid, ddPdudu + i * dstride, eval(i, uu, vv, u_n, v_n) * sqr(dscale));
			}
			{
				assert(ddPdvdv);
				const Vec4<vfloat> u_n = BSplineBasis::eval(uu);
				...
				for (size_t i = 0; i < N; i++)
					vfloat::store(valid, ddPdvdv + i * dstride, eval(i, uu, vv, u_n, v_n) * sqr(dscale));
			}
			{
				assert(ddPdudv);
				const Vec4<vfloat> u_n = BSplineBasis::derivative(uu);
				const Vec4<vfloat> v_n = BSplineBasis::derivative(vv);
				for (size_t i = 0; i < N; i++)
					vfloat::store(valid, ddPdudv + i * dstride, eval(i, uu, vv, u_n, v_n) * sqr(dscale));
			}
		}
	}
