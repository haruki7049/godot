...
public:
	CatmullClark1RingT() : eval_start_index(0), eval_unique_identifier(0) {} // FIXME: default constructor should be empty
	...
	__forceinline void serialize(char *ptr, size_t &ofs) const {
		...
		Vertex_t::storeu(&ptr[ofs], vtx);
		ofs += sizeof(Vertex);
		for (size_t i = 0; i < edge_valence; i++) {
			Vertex_t::storeu(&ptr[ofs], ring[i]);
			ofs += sizeof(Vertex);
		}
	}
	...
	__forceinline void init(const HalfEdge *const h, const char *vertices, size_t stride) {
		...
		do {
			...
			if (index0 < min_vertex_index) {
				min_vertex_index = index0;
				min_vertex_index_face = i >> 1;
			}
			p = p->next();
	...
	__forceinline void subdivide(CatmullClark1RingT &dest) const {
		...
		for (size_t i = 0; i < face_valence; i++) {
			...
			if (face_index >= face_valence)
				face_index -= face_valence;
			assert(face_index < face_valence);
			size_t index0 = 2 * face_index + 0;
			if (index0 >= edge_valence)
				index0 -= edge_valence;
			assert(index0 < edge_valence);
			size_t index1 = 2 * face_index + 1;
			if (index1 >= edge_valence)
				index1 -= edge_valence;
			assert(index1 < edge_valence);
			size_t index2 = 2 * face_index + 2;
			if (index2 >= edge_valence)
				index2 -= edge_valence;
	...
	__forceinline Type type() const {
		...
		if (noInnerCreases)
			crease_mask = (Type)(crease_mask | TYPE_REGULAR_CREASES | TYPE_GREGORY_CREASES);
		if (numCreases != 0)
			crease_mask = (Type)(crease_mask | TYPE_CREASES);

		/* calculate if this vertex is regular */
		bool hasBorder = border_index != -1;
		if (face_valence == 2 && hasBorder) {
			if (vertex_crease_weight == 0.0f)
				return (Type)(crease_mask & (TYPE_REGULAR | TYPE_REGULAR_CREASES | TYPE_GREGORY | TYPE_GREGORY_CREASES | TYPE_CREASES));
			else if (vertex_crease_weight == float(inf))
				return (Type)(crease_mask & (TYPE_REGULAR | TYPE_REGULAR_CREASES | TYPE_GREGORY | TYPE_GREGORY_CREASES | TYPE_CREASES));
			else
				return TYPE_CREASES;
		} else if (vertex_crease_weight != 0.0f)
			return TYPE_CREASES;
		else if (face_valence == 3 && hasBorder)
			return (Type)(crease_mask & (TYPE_REGULAR | TYPE_REGULAR_CREASES | TYPE_GREGORY | TYPE_GREGORY_CREASES | TYPE_CREASES));
		else if (face_valence == 4 && !hasBorder)
			return (Type)(crease_mask & (TYPE_REGULAR | TYPE_REGULAR_CREASES | TYPE_GREGORY | TYPE_GREGORY_CREASES | TYPE_CREASES));
		else
			return (Type)(crease_mask & (TYPE_GREGORY | TYPE_GREGORY_CREASES | TYPE_CREASES));
	}
	...
	friend __forceinline embree_ostream operator<<(embree_ostream o, const CatmullClark1RingT &c) {
		o << "vtx " << c.vtx << " size = " << c.edge_valence << ", " << "hard_edge = " << c.border_index << ", face_valence " << c.face_valence << ", edge_level = " << c.edge_level << ", vertex_level = " << c.vertex_level << ", eval_start_index: " << c.eval_start_index << ", ring: " << embree_endl;

		for (unsigned int i = 0; i < min(c.edge_valence, (unsigned int)MAX_RING_FACE_VALENCE); i++) {
			o << i << " -> " << c.ring[i];
			if (i % 2 == 0)
				o << " crease = " << c.crease_weight[i / 2];
			o << embree_endl;
		}
		return o;
	}
...
struct __aligned(64) GeneralCatmullClark1RingT {
	...
	struct Face {
		__forceinline Face() {}
		__forceinline Face(int size, float crease_weight) : size(size), crease_weight(crease_weight) {}
...
public:
	GeneralCatmullClark1RingT() : eval_start_face_index(0), eval_start_vertex_index(0), eval_unique_identifier(0) {}

	__forceinline bool isRegular() const {
		if (border_face == -1 && face_valence == 4)
			return true;
		return false;
	}
	...
	__forceinline void init(const HalfEdge *const h, const char *vertices, size_t stride) {
		...
		do {
			...
			if (vertex_index < min_vertex_index) {
				...
				min_vertex_index_face = f;
				min_vertex_index_vertex = e;
			}
	...
	__forceinline void subdivide(CatmullClark1Ring &dest) const {
		...
		for (size_t face = 0, v = eval_start_vertex_index; face < face_valence; face++) {
			...
			for (size_t k = v; k <= v + faces[f].size; k++)
				F += ring[k % edge_valence]; // FIXME: optimize
		...
		for (size_t face = 0, j = eval_start_vertex_index; face < face_valence; face++) {
			...
			if (i == 0)
				f += dest.ring[dest.edge_valence - 1];
			else
				f += dest.ring[2 * i - 1];
			S += ring[j];
	...
	friend __forceinline embree_ostream operator<<(embree_ostream o, const GeneralCatmullClark1RingT &c) {
		o << "vtx " << c.vtx << " size = " << c.edge_valence << ", border_face = " << c.border_face << ", " << " face_valence = " << c.face_valence << ", edge_level = " << c.edge_level << ", vertex_level = " << c.vertex_level << ", ring: " << embree_endl;
		for (size_t v = 0, f = 0; f < c.face_valence; v += c.faces[f++].size) {
			for (size_t i = v; i < v + c.faces[f].size; i++) {
				o << i << " -> " << c.ring[i];
				if (i == v)
					o << " crease = " << c.faces[f].crease_weight;
				o << embree_endl;
			}
		}
		return o;
