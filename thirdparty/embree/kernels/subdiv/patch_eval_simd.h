...
public:
	...
	PatchEvalSimd(SharedLazyTessellationCache::CacheEntry &entry, size_t commitCounter,
		...
		Ref patch = SharedLazyTessellationCache::lookup(entry, commitCounter, [&]() {
              ...
              return Patch::create(alloc,edge,vertices,stride); }, true);
	...
	vbool eval_quad(const vbool &valid, const typename Patch::SubdividedQuadPatch *This, const vfloat &u, const vfloat &v, const float dscale, const size_t depth) {
		...
		if (any(u0v0_mask))
			ret |= eval(u0v0_mask, This->child[0], 2.0f * u, 2.0f * v, 2.0f * dscale, depth + 1);
		if (any(u1v0_mask))
			ret |= eval(u1v0_mask, This->child[1], 2.0f * u - 1.0f, 2.0f * v, 2.0f * dscale, depth + 1);
		if (any(u1v1_mask))
			ret |= eval(u1v1_mask, This->child[2], 2.0f * u - 1.0f, 2.0f * v - 1.0f, 2.0f * dscale, depth + 1);
		if (any(u0v1_mask))
			ret |= eval(u0v1_mask, This->child[3], 2.0f * u, 2.0f * v - 1.0f, 2.0f * dscale, depth + 1);
		return ret;
	}
	...
	vbool eval_general(const vbool &valid, const typename Patch::SubdividedGeneralPatch *patch, const vfloat &U, const vfloat &V, const size_t depth) {
		vbool ret = false;
		const vint l = (vint)floor(0.5f * U);
		const vfloat u = 2.0f * frac(0.5f * U) - 0.5f;
		const vint h = (vint)floor(0.5f * V);
		const vfloat v = 2.0f * frac(0.5f * V) - 0.5f;
		const vint i = (h << 2) + l;
		assert(all(valid, i < patch->N));
