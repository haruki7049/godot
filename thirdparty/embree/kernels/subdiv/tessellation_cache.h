...
struct __aligned(64) ThreadWorkState {
	...
	__forceinline ThreadWorkState(bool allocated = false) : counter(0), next(nullptr), allocated(allocated) {
...
public:
	...
	__forceinline size_t unlockThread(ThreadWorkState *const t_state, const ssize_t plus = -1) {
		assert(isLocked(t_state));
		return t_state->counter.fetch_add(plus);
	}
	...
	static __forceinline auto lookup(CacheEntry &entry, size_t globalTime, const Constructor constructor, const bool before = false) -> decltype(constructor()) {
		...
		while (true) {
			...
			if (patch)
				return (decltype(constructor()))patch;
	...
	__forceinline size_t alloc(const size_t blocks) {
		...
		if (unlikely(index + blocks >= switch_block_threshold))
			return (size_t)-1;
		return index;
	}
