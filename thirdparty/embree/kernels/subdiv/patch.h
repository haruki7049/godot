...
#define PATCH_DEBUG_SUBDIVISION(ptr, x, y, z)                                                                      \
	{                                                                                                              \
		...
		if (P)                                                                                                     \
			*P = Vertex(0.5f + 0.5f * x, 0.5f + 0.5f * y, 0.5f + 0.5f * z, 0.0f);                                  \
	}
#endif
...
public:
	...
	struct Ref {
		__forceinline Ref(void *p = nullptr) : ptr((size_t)p) {}

		__forceinline operator bool() const { return ptr != 0; }
		__forceinline operator size_t() const { return ptr; }

		__forceinline Ref(Type ty, void *in) : ptr(((size_t)in) + ty) { assert((((size_t)in) & 0xF) == 0); }
	...
	struct BilinearPatch {
		...
		__forceinline BilinearPatch(const CatmullClarkPatch &patch) : patch(patch) {}
		...
		__forceinline BilinearPatch(const HalfEdge *edge, const char *vertices, size_t stride) : patch(edge, vertices, stride) {}
	...
	struct BSplinePatch {
		...
		__forceinline BSplinePatch(const HalfEdge *edge, const char *vertices, size_t stride) : patch(edge, vertices, stride) {}
		...
		__forceinline BSplinePatch(const CatmullClarkPatch &patch, const BezierCurve *border0, const BezierCurve *border1, const BezierCurve *border2, const BezierCurve *border3) : patch(patch, border0, border1, border2, border3) {}
	...
	struct BezierPatch {
		...
		__forceinline BezierPatch(const HalfEdge *edge, const char *vertices, size_t stride) : patch(edge, vertices, stride) {}
		...
		__forceinline BezierPatch(const CatmullClarkPatch &patch, const BezierCurve *border0, const BezierCurve *border1, const BezierCurve *border2, const BezierCurve *border3) : patch(patch, border0, border1, border2, border3) {}
	...
	struct GregoryPatch {
		...
		__forceinline GregoryPatch(const HalfEdge *edge, const char *vertices, size_t stride) : patch(CatmullClarkPatch(edge, vertices, stride)) {}
		...
		__forceinline GregoryPatch(const CatmullClarkPatch &patch, const BezierCurve *border0, const BezierCurve *border1, const BezierCurve *border2, const BezierCurve *border3) : patch(patch, border0, border1, border2, border3) {}
	...
	__noinline static Ref create(const Allocator &alloc, const HalfEdge *edge, const char *vertices, size_t stride) {
		...
		switch (edge->patch_type) {
			case HalfEdge::BILINEAR_PATCH:
				child = BilinearPatch::create(alloc, edge, vertices, stride);
				break;
			case HalfEdge::REGULAR_QUAD_PATCH:
				child = RegularPatch::create(alloc, edge, vertices, stride);
				break;
#if PATCH_USE_GREGORY == 2
			case HalfEdge::IRREGULAR_QUAD_PATCH:
				child = GregoryPatch::create(alloc, edge, vertices, stride);
				break;
#endif
			default: {
	...
	__noinline static Ref create(const Allocator &alloc, GeneralCatmullClarkPatch &patch, const HalfEdge *edge, const char *vertices, size_t stride, size_t depth) {
		...
		if (N == 4) {
			...
			BezierCurve border0l, border0r;
			borders[0].subdivide(border0l, border0r);
			BezierCurve border1l, border1r;
			borders[1].subdivide(border1l, border1r);
			BezierCurve border2l, border2r;
			borders[2].subdivide(border2l, border2r);
			BezierCurve border3l, border3r;
			borders[3].subdivide(border3l, border3r);
			...
			for (size_t i0 = 0; i0 < N; i0++) {
				...
				BezierCurve border0l, border0r;
				borders[i0].subdivide(border0l, border0r);
				BezierCurve border2l, border2r;
				borders[i2].subdivide(border2l, border2r);
	...
	__noinline static Ref create(const Allocator &alloc, CatmullClarkPatch &patch, const HalfEdge *edge, const char *vertices, size_t stride, size_t depth,
		...
		if (unlikely(final(patch, ty, depth))) {
			if (ty & CatmullClarkRing::TYPE_REGULAR)
				return RegularPatch::create(alloc, patch, border0, border1, border2, border3);
			else
				return IrregularFillPatch::create(alloc, patch, border0, border1, border2, border3);
			...
			assert(depth > 0);
			return GregoryPatch::create(alloc, patch, border0, border1, border2, border3);
		}
#endif
