...
public:
	...
	static __forceinline void init_regular(const Vertex_t &center, const Vertex_t center_ring[8], const unsigned int offset, CatmullClark1RingT<Vertex, Vertex_t> &dest) {
		...
		if (dest.eval_start_index >= dest.face_valence)
...
public:
	...
	__forceinline GeneralCatmullClarkPatchT() : N(0) {}
	...
	__noinline void subdivide(array_t<CatmullClarkPatch, SIZE> &patch, unsigned &N_o) const {
		...
		for (unsigned i = 0; i < N; i++) {
			...
			if (likely(regular))
				init_regular(patch[i].ring[0], patch[ip1].ring[0], patch[i].ring[1], patch[ip1].ring[3]);
			else
				init_border(patch[i].ring[0], patch[ip1].ring[0], patch[i].ring[1], patch[ip1].ring[3]);
