...
struct HeuristicArrayOpenMergeSAH {
	...
	__forceinline HeuristicArrayOpenMergeSAH() : prims0(nullptr) {}

	/*! remember prim array */
	__forceinline HeuristicArrayOpenMergeSAH(const NodeOpenerFunc &nodeOpenerFunc, PrimRef *prims0, size_t max_open_size) : prims0(prims0), nodeOpenerFunc(nodeOpenerFunc), max_open_size(max_open_size) {
	...
	__noinline void openNodesBasedOnExtend(PrimInfoExtRange &set) {
		...
		if (false && set.size() < PARALLEL_THRESHOLD) {
			...
			PrimInfo info = parallel_reduce(set.begin(), set.end(), CREATE_SPLITS_STEP_SIZE, PrimInfo(empty), [&](const range<size_t> &r) -> PrimInfo {
                ...
                return info; }, [](const PrimInfo &a, const PrimInfo &b) { return PrimInfo::merge(a, b); });
	...
	__noinline void openNodesBasedOnExtendLoop(PrimInfoExtRange &set, const size_t est_new_elements) {
		...
		while (next_iteration_extra_elements <= set.ext_range_size()) {
			...
			if (unlikely(next_iteration_extra_elements == 0))
				break;
		}
	}
	...
	__noinline const Split find(PrimInfoExtRange &set, const size_t logBlockSize) {
		...
		if (unlikely(set.has_ext_range() && set.size() <= D)) {
			...
			if (disjoint)
				set.set_ext_range(set.end()); /* disables opening */
		}
	...
	__forceinline const Split object_find(const PrimInfoExtRange &set, const size_t logBlockSize) {
		if (set.size() < PARALLEL_THRESHOLD)
			return sequential_object_find(set, logBlockSize);
		else
			return parallel_object_find(set, logBlockSize);
	}
	...
	std::pair<size_t, size_t> sequential_object_split(const Split &split, const PrimInfoExtRange &set, PrimInfoExtRange &lset, PrimInfoExtRange &rset) {
		...
		size_t center = serial_partitioning(prims0, begin, end, local_left, local_right, [&](const PrimRef &ref) { return split.mapping.bin_unsafe(ref, vSplitPos, vSplitMask); }, [](PrimInfo &pinfo, const PrimRef &ref) { pinfo.add_center2(ref); });
