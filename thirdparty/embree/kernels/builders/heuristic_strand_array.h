...
struct HeuristicStrandSplit {
	...
	struct Split {
		...
		__forceinline Split() : sah(inf), axis0(zero), axis1(zero) {}

		/*! constructs specified split */
		__forceinline Split(const float sah, const Vec3fa &axis0, const Vec3fa &axis1) : sah(sah), axis0(axis0), axis1(axis1) {}
	...
	/*! remember prim array */
	__forceinline HeuristicStrandSplit(Scene *scene, PrimRef *prims) : scene(scene), prims(prims) {}
	...
	const Split find(const range<size_t> &set, size_t logBlockSize) {
		...
		for (size_t i = set.begin(); i < set.end(); i++) {
			...
			if (cos0 > cos1) {
				lnum++;
				lbounds.extend(bounds(space0, prim));
			} else {
				rnum++;
				rbounds.extend(bounds(space1, prim));
			}
		}
