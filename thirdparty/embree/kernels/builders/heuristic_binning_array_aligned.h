...
struct PrimInfoRange : public CentGeomBBox3fa, public range<size_t> {
	...
	__forceinline PrimInfoRange(const PrimInfo &pinfo) : CentGeomBBox3fa(pinfo), range<size_t>(pinfo.begin, pinfo.end) {}

	__forceinline PrimInfoRange(EmptyTy) : CentGeomBBox3fa(EmptyTy()), range<size_t>(0, 0) {}

	__forceinline PrimInfoRange(size_t begin, size_t end, const CentGeomBBox3fa &centGeomBounds) : CentGeomBBox3fa(centGeomBounds), range<size_t>(begin, end) {}
...
struct HeuristicArrayBinningSAH {
	...
	__forceinline HeuristicArrayBinningSAH() : prims(nullptr) {}

	/*! remember prim array */
	__forceinline HeuristicArrayBinningSAH(PrimRef *prims) : prims(prims) {}
	...
	void splitByGeometry(const range<size_t> &range, PrimInfoRange &linfo, PrimInfoRange &rinfo) {
		...
		size_t center = serial_partitioning(prims, range.begin(), range.end(), left, right, [&](const PrimRef &prim) { return prim.geomID() == geomID; }, [](CentGeomBBox3fa &a, const PrimRef &ref) { a.extend_center2(ref); });
