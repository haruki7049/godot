...
struct SharedVector {
	...
	__forceinline SharedVector(T *ptr, size_t refCount = 1) : prims(ptr), refCount(refCount) {}
...
struct LocalChildListT {
	...
	__forceinline LocalChildListT(const BuildRecord &record) : numChildren(1), numSharedPrimVecs(1) {
...
struct RecalculatePrimRef {
	...
	__forceinline RecalculatePrimRef(Scene *scene) : scene(scene) {}
...
struct VirtualRecalculatePrimRef {
	...
	__forceinline VirtualRecalculatePrimRef(Scene *scene) : scene(scene) {}
...
struct BVHBuilderMSMBlur {
	...
	struct Settings {
		/*! default settings */
		Settings() : branchingFactor(2), maxDepth(32), logBlockSize(0), minLeafSize(1), maxLeafSize(8), travCost(1.0f), intCost(1.0f), singleLeafTimeSegment(false), singleThreadThreshold(1024) {}

		Settings(size_t sahBlockSize, size_t minLeafSize, size_t maxLeafSize, float travCost, float intCost, size_t singleThreadThreshold) : branchingFactor(2), maxDepth(32), logBlockSize(bsr(sahBlockSize)), minLeafSize(minLeafSize), maxLeafSize(maxLeafSize), travCost(travCost), intCost(intCost), singleThreadThreshold(singleThreadThreshold) {
	...
	public:
		...
		__forceinline BuildRecord(size_t depth) : depth(depth) {}

		__forceinline BuildRecord(const SetMB &prims, size_t depth) : depth(depth), prims(prims) {}
	...
	struct BuildRecordSplit : public BuildRecord {
		...
		__forceinline BuildRecordSplit(size_t depth) : BuildRecord(depth) {}

		__forceinline BuildRecordSplit(const BuildRecord &record, const BinSplit<MBLUR_NUM_OBJECT_BINS> &split) : BuildRecord(record), split(split) {}
	...
	public:
		BuilderT(MemoryMonitorInterface *device,
				...
				const Settings &settings) : cfg(settings),
		...
		void splitByGeometry(const SetMB &set, SetMB &lset, SetMB &rset) {
			...
			size_t center = serial_partitioning(prims.data(), begin, end, left, right, [&](const PrimRefMB &prim) { return prim.geomID() == geomID; }, [](PrimInfoMB &dst, const PrimRefMB &prim) { dst.add_primref(prim); });
			...
			while (children.size() < cfg.branchingFactor) {
				...
				for (size_t i = 0; i < children.size(); i++) {
					...
					if (expectedApproxHalfArea(children[i].prims.geomBounds) > bestArea) {
						bestChild = i;
						bestArea = expectedApproxHalfArea(children[i].prims.geomBounds);
					}
				}
				if (bestChild == -1)
					break;
