...
struct UnalignedHeuristicArrayBinningSAH {
	...
	/*! remember prim array */
	__forceinline UnalignedHeuristicArrayBinningSAH(Scene *scene, PrimRef *prims) : scene(scene), prims(prims) {}
	...
	struct BinBoundsAndCenter {
		__forceinline BinBoundsAndCenter(Scene *scene, const LinearSpace3fa &space) : scene(scene), space(space) {}
	...
	__forceinline void split_template(const Split &split, const LinearSpace3fa &space, const Set &set, PrimInfoRange &lset, PrimInfoRange &rset) {
		...
		size_t center = 0;
		if (likely(set.size() < 10000))
			center = serial_partitioning(prims, begin, end, local_left, local_right, [&](const PrimRef &ref) { return split.mapping.bin_unsafe(ref, binBoundsAndCenter)[splitDim] < splitPos; }, [](CentGeomBBox3fa &pinfo, const PrimRef &ref) { pinfo.extend_center2(ref); });
		else
			center = parallel_partitioning(prims, begin, end, EmptyTy(), local_left, local_right, [&](const PrimRef &ref) { return split.mapping.bin_unsafe(ref, binBoundsAndCenter)[splitDim] < splitPos; }, [](CentGeomBBox3fa &pinfo, const PrimRef &ref) { pinfo.extend_center2(ref); }, [](CentGeomBBox3fa &pinfo0, const CentGeomBBox3fa &pinfo1) { pinfo0.merge(pinfo1); }, 128);
...
struct UnalignedHeuristicArrayBinningMB {
	...
	UnalignedHeuristicArrayBinningMB(Scene *scene) : scene(scene) {}
	...
	struct BinBoundsAndCenter {
		__forceinline BinBoundsAndCenter(Scene *scene, BBox1f time_range, const LinearSpace3fa &space) : scene(scene), time_range(time_range), space(space) {}
	...
	const Split find(const SetMB &set, const size_t logBlockSize, const LinearSpace3fa &space) {
		...
		if (!osplit.valid())
			osplit.data = Split::SPLIT_FALLBACK; // use fallback split
		return osplit;
	}
