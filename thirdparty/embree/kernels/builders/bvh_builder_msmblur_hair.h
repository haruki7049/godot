...
struct BVHBuilderHairMSMBlur {
	...
	struct Settings {
		/*! default settings */
		Settings() : branchingFactor(2), maxDepth(32), logBlockSize(0), minLeafSize(1), maxLeafSize(8) {}
	...
	public:
		BuilderT(Scene *scene,
				...
				: cfg(settings),
				  ...
				  createAABBNodeMB(createAABBNodeMB),
				  setAABBNodeMB(setAABBNodeMB),
				  createOBBNodeMB(createOBBNodeMB),
				  setOBBNodeMB(setOBBNodeMB),
	...
	private:
		...
		void splitByGeometry(const SetMB &set, SetMB &lset, SetMB &rset) {
			...
			size_t center = serial_partitioning(set.prims->data(), begin, end, linfo, rinfo, [&](const PrimRefMB &prim) { return prim.geomID() == geomID; }, [](PrimInfoMB &a, const PrimRefMB &ref) { a.add_primref(ref); });
