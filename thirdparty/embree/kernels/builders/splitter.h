...
__forceinline void splitPolygon(const BBox3fa &bounds,
	...
	for (size_t i = 0; i < N; i++) {
		...
		if (v0d <= pos)
			left.extend(v0); // this point is on left side
		if (v0d >= pos)
			right.extend(v0); // this point is on right side
...
struct TriangleSplitterFactory {
	__forceinline TriangleSplitterFactory(const Scene *scene) : scene(scene) {}
...
struct QuadSplitterFactory {
	__forceinline QuadSplitterFactory(const Scene *scene) : scene(scene) {}
...
struct DummySplitterFactory {
	__forceinline DummySplitterFactory(const Scene *scene) : scene(scene) {}
