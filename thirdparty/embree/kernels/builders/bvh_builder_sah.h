...
struct GeneralBVHBuilder {
	...
	struct Settings {
		/*! default settings */
		Settings() : branchingFactor(2), maxDepth(32), logBlockSize(0), minLeafSize(1), maxLeafSize(7), travCost(1.0f), intCost(1.0f), singleThreadThreshold(1024), primrefarrayalloc(inf) {}

		/*! initialize settings from API settings */
		Settings(const RTCBuildArguments &settings) : branchingFactor(2), maxDepth(32), logBlockSize(0), minLeafSize(1), maxLeafSize(7), travCost(1.0f), intCost(1.0f), singleThreadThreshold(1024), primrefarrayalloc(inf) {
	...
	public:
		...
		__forceinline BuildRecordT(size_t depth) : depth(depth), alloc_barrier(false), prims(empty) {}

		__forceinline BuildRecordT(size_t depth, const Set &prims) : depth(depth), alloc_barrier(false), prims(prims) {}
	...
	class BuilderT {
		...
		const ReductionTy recurse(BuildRecord &current, Allocator alloc, bool toplevel) {
			...
			while (numChildren < cfg.branchingFactor) {
				...
				for (size_t i = 0; i < numChildren; i++) {
					/* ignore leaves as they cannot get split */
					if (children[i].prims.size() <= cfg.minLeafSize)
						continue;
...
struct BVHBuilderBinnedFastSpatialSAH {
	...
	struct CreateLeafExt {
		__forceinline CreateLeafExt(const UserCreateLeaf userCreateLeaf) : userCreateLeaf(userCreateLeaf) {}
