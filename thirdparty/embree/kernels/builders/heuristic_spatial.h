...
struct SpatialBinSplit {
	...
	__forceinline SpatialBinSplit() : sah(inf), dim(-1), pos(0), left(-1), right(-1), factor(1.0f) {}

	/*! constructs specified split */
	__forceinline SpatialBinSplit(float sah, int dim, int pos, const SpatialBinMapping<BINS> &mapping) : sah(sah), dim(dim), pos(pos), left(-1), right(-1), factor(1.0f), mapping(mapping) {}

	/*! constructs specified split */
	__forceinline SpatialBinSplit(float sah, int dim, int pos, int left, int right, float factor, const SpatialBinMapping<BINS> &mapping) : sah(sah), dim(dim), pos(pos), left(left), right(right), factor(factor), mapping(mapping) {}
...
struct __aligned(64) SpatialBinInfo {
			...
			if (unlikely(splits == 1)) {
				...
				for (size_t dim = 0; dim < 3; dim++) {
					...
					for (bin = (size_t)bin0[dim]; bin < (size_t)bin1[dim]; bin++) {
						...
						if (unlikely(left.bounds().empty()))
							l++;
						bounds[bin][dim].extend(left.bounds());
						rest = right;
					}
					if (unlikely(rest.bounds().empty()))
						r--;
					numBegin[l][dim]++;
					numEnd[r][dim]++;
	...
	SpatialBinSplit<BINS> best(const SpatialBinMapping<BINS> &mapping, const size_t blocks_shift) const {
		...
		for (size_t i = BINS - 1; i > 0; i--) {
			...
			rAreas[i][0] = halfArea(bx);
			by.extend(bounds[i][1]);
			rAreas[i][1] = halfArea(by);
			bz.extend(bounds[i][2]);
			rAreas[i][2] = halfArea(bz);
			rAreas[i][3] = 0.0f;
		}
		...
		vuint4 vbestPos = 0;
		vuint4 vbestlCount = 0;
		vuint4 vbestrCount = 0;
		count = 0;
		bx = empty;
		by = empty;
		bz = empty;
		for (size_t i = 1; i < BINS; i++, ii += 1) {
			...
			float Ax = halfArea(bx);
			by.extend(bounds[i - 1][1]);
			float Ay = halfArea(by);
			bz.extend(bounds[i - 1][2]);
			float Az = halfArea(bz);
