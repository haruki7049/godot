...
struct PlueckerHitM {
	...
	__forceinline PlueckerHitM(const vbool<M> &valid, const vfloat<M> &U, const vfloat<M> &V, const vfloat<M> &UVW, const vfloat<M> &t, const Vec3vf<M> &Ng, const UVMapper &mapUV) : U(U), V(V), UVW(UVW), mapUV(mapUV), valid(valid), vt(t), vNg(Ng) {}
...
#endif
		if (unlikely(early_out && none(valid)))
			return false;
...
struct PlueckerHitK {
	...
	__forceinline PlueckerHitK(const vfloat<K> &U, const vfloat<K> &V, const vfloat<K> &UVW, const vfloat<K> &t, const Vec3vf<K> &Ng, const UVMapper &mapUV) : U(U), V(V), UVW(UVW), t(t), Ng(Ng), mapUV(mapUV) {}
