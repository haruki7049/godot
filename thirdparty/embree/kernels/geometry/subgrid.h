...
public:
	...
	__forceinline SubGrid(const unsigned int x,
			...
			const unsigned int primID) : _x(x),
										 _y(y),
										 _geomID(geomID),
										 _primID(primID) {
	}
...
public:
	...
	__forceinline unsigned int primID(const size_t i) const {
		assert(i < N);
		return subgridIDs[i].primID;
	}
	__forceinline unsigned int x(const size_t i) const {
		assert(i < N);
		return subgridIDs[i].x;
	}
	__forceinline unsigned int y(const size_t i) const {
		assert(i < N);
		return subgridIDs[i].y;
	}
