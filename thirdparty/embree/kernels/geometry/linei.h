...
public:
	...
	__forceinline LineMi(const vuint<M> &v0, unsigned short leftExists, unsigned short rightExists, const vuint<M> &geomIDs, const vuint<M> &primIDs, Geometry::GType gtype) : gtype((unsigned char)gtype), m((unsigned char)popcnt(vuint<M>(primIDs) != vuint<M>(-1))), sharedGeomID(geomIDs[0]), leftExists(leftExists), rightExists(rightExists), v0(v0), primIDs(primIDs) {
