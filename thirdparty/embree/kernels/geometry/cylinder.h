...
struct Cylinder {
	...
	__forceinline Cylinder(const Vec3fa &p0, const Vec3fa &p1, const float r) : p0(p0), p1(p1), rr(sqr(r)) {}

	__forceinline Cylinder(const Vec3fa &p0, const Vec3fa &p1, const float rr, bool) : p0(p0), p1(p1), rr(rr) {}
	...
	static bool verify(const size_t id, const Cylinder &cylinder, const RayHit &ray, bool shouldhit, const float t0, const float t1) {
		...
		if (shouldhit)
			failed |= std::isinf(t0) ? t0 != t.lower : abs(t0 - t.lower) > eps;
		if (shouldhit)
			failed |= std::isinf(t1) ? t1 != t.upper : abs(t1 - t.upper) > eps;
		if (!failed)
			return true;
...
struct CylinderN {
	...
	__forceinline CylinderN(const Vec3vf<N> &p0, const Vec3vf<N> &p1, const vfloat<N> &r) : p0(p0), p1(p1), rr(sqr(r)) {}

	__forceinline CylinderN(const Vec3vf<N> &p0, const Vec3vf<N> &p1, const vfloat<N> &rr, bool) : p0(p0), p1(p1), rr(rr) {}
