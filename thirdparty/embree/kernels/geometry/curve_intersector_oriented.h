...
struct TensorLinearCubicBezierSurfaceIntersector {
	...
	__forceinline TensorLinearCubicBezierSurfaceIntersector(const LinearSpace3fa &ray_space, Ray &ray, const TensorLinearCubicBezierSurface3fa &curve3d, const Epilog &epilog) : ray_space(ray_space), ray(ray), curve3d(curve3d), epilog(epilog), isHit(false) {
	...
	__forceinline Interval1f solve_linear(const float u0, const float u1, const float &p0, const float &p1) {
		if (p1 == p0) {
			if (p0 == 0.0f)
				return Interval1f(u0, u1);
			else
				return Interval1f(empty);
		}
	...
	__forceinline void solve_linear(const float u0, const float u1, const Interval1f &p0, const Interval1f &p1, Interval1f &u) {
		if (sign(p0.lower) != sign(p0.upper))
			u.extend(u0);
		if (sign(p0.lower) != sign(p1.lower))
			u.extend(solve_linear(u0, u1, p0.lower, p1.lower));
		if (sign(p0.upper) != sign(p1.upper))
			u.extend(solve_linear(u0, u1, p0.upper, p1.upper));
		if (sign(p1.lower) != sign(p1.upper))
			u.extend(u1);
	}
	...
	__forceinline void solve_bezier_clipping(BBox1f cu, BBox1f cv, const TensorLinearCubicBezierSurface2fa &curve2) {
		...
		if (bounds.upper.x < 0.0f)
			return;
		if (bounds.upper.y < 0.0f)
			return;
		if (bounds.lower.x > 0.0f)
			return;
		if (bounds.lower.y > 0.0f)
			return;
	...
	__forceinline bool solve_krawczyk(bool very_small, BBox1f &cu, BBox1f &cv) {
		...
		const TensorLinearCubicBezierSurface1f curve1v = curve2.xfm(dv);
		LinearBezierCurve<Interval1f> curve0v = curve1v.reduce_u();
		if (unlikely(!curve0v.hasRoot()))
			return true;
		Interval1f v = bezier_clipping(curve0v);
		if (unlikely(isEmpty(v)))
			return true;
		v = intersect(v + Interval1f(-0.1f, +0.1f), Interval1f(0.0f, 1.0f));
		...
		Vec2fa f, dfdu, dfdv;
		curve2d.eval(c.x, c.y, f, dfdu, dfdv);
		const LinearSpace2fa rcp_J = rcp(LinearSpace2fa(dfdu, dfdv));
	...
	__forceinline void solve_newton_raphson_recursion(BBox1f cu, BBox1f cv) {
		...
		while (sptr) {
			/* pop from stack */
			{
				...
				if (mask)
					sptr++; // there are still items on the stack
...
struct OrientedCurve1IntersectorK {
	...
	struct Ray1 {
		__forceinline Ray1(RayK<K> &ray, size_t k) : org(ray.org.x[k], ray.org.y[k], ray.org.z[k]), dir(ray.dir.x[k], ray.dir.y[k], ray.dir.z[k]), _tnear(ray.tnear()[k]), tfar(ray.tfar[k]) {}
