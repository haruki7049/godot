...
struct Cone {
	...
	__forceinline Cone(const Vec3fa &p0, const float r0, const Vec3fa &p1, const float r1) : p0(p0), p1(p1), r0(r0), r1(r1) {}
	...
	__forceinline bool intersect(const Vec3fa &org, const Vec3fa &dir,
		...
		if (unlikely(abs(A) < eps)) {
			...
			/* cone case */
				...
				if (dOz * dr > 0.0f)
					t_o = BBox1f(t, pos_inf);
				else
					t_o = BBox1f(neg_inf, t);
			}
		}
	...
	static bool verify(const size_t id, const Cone &cone, const Ray &ray, bool shouldhit, const float t0, const float t1) {
		...
		if (shouldhit)
			failed |= std::isinf(t0) ? t0 != t.lower : (t0 == -1E6) ? t.lower > -1E6f :
																	  abs(t0 - t.lower) > eps;
		if (shouldhit)
			failed |= std::isinf(t1) ? t1 != t.upper : (t1 == +1E6) ? t.upper < +1E6f :
																	  abs(t1 - t.upper) > eps;
		if (!failed)
			return true;
...
struct ConeN {
	...
	__forceinline ConeN(const Vec3vfN &p0, const vfloat<N> &r0, const Vec3vfN &p1, const vfloat<N> &r1) : p0(p0), p1(p1), r0(r0), r1(r1) {}
