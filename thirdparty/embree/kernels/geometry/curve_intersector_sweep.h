...
struct BezierCurveHit {
	...
	__forceinline BezierCurveHit(const float t, const float u, const Vec3fa &Ng) : t(t), u(u), v(0.0f), Ng(Ng) {}

	__forceinline BezierCurveHit(const float t, const float u, const float v, const Vec3fa &Ng) : t(t), u(u), v(v), Ng(Ng) {}
...
__forceinline bool intersect_bezier_iterative_debug(const Ray &ray, const float dt, const NativeCurve3ff &curve, size_t i,
		...
		const Epilog &epilog) {
	if (tp.lower[i] + dt > ray.tfar)
		return false;
	Vec3fa Ng_o = Vec3fa(Ng.x[i], Ng.y[i], Ng.z[i]);
	if (h0.lower[i] == tp.lower[i])
		Ng_o = -Vec3fa(dP0du.x[i], dP0du.y[i], dP0du.z[i]);
	if (h1.lower[i] == tp.lower[i])
		Ng_o = +Vec3fa(dP3du.x[i], dP3du.y[i], dP3du.z[i]);
...
bool intersect_bezier_recursive_jacobian(const Ray &ray, const float dt, const NativeCurve3ff &curve,
	...
	while (sptr) {
		/* pop from stack */
		{
			...
			const size_t i = select_min(valid, tlower);
			clear(valid, i);
	...
	entry:
		...
		const vfloatx vu0 = lerp(u0, u1, vfloatx(step) * (1.0f / (vfloatx::size - 1)));
		Vec4vfx P0, dP0du;
		curve.template veval<VSIZEX_>(vu0, P0, dP0du);
		dP0du = dP0du * Vec4vfx(dscale);
		...
		vboolx valid = true;
		clear(valid, vfloatx::size - 1);

		/* intersect with outer cylinder */
		BBox<vfloatx> tc_outer;
		vfloatx u_outer0;
		Vec3vfx Ng_outer0;
		vfloatx u_outer1;
		Vec3vfx Ng_outer1;
		...
		vfloatx u_inner0 = zero;
		Vec3vfx Ng_inner0 = zero;
		vfloatx u_inner1 = zero;
		Vec3vfx Ng_inner1 = zero;
