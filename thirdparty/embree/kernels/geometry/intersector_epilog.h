...
struct Intersect1Epilog1 {
	...
	__forceinline Intersect1Epilog1(RayHit &ray,
			IntersectContext *context,
			const unsigned int geomID,
			const unsigned int primID) : ray(ray),
										 context(context),
										 geomID(geomID),
										 primID(primID) {}
...
struct OccludedKEpilogMU {
	...
	__forceinline OccludedKEpilogMU(vbool<K> &valid0,
			...
			IntersectContext *context,
			const unsigned int geomID,
			const unsigned int primID) : valid0(valid0),
										 ray(ray),
										 context(context),
										 geomID(geomID),
										 primID(primID) {}
		...
		entry:
			...
			if (filter) {
				if (unlikely(context->hasContextFilter() || geometry->hasOcclusionFilter())) {
					...
					ray.tfar[k] = hit.t(i);
					HitK<K> h(context->user, geomID, primIDs[i], uv.x, uv.y, hit.Ng(i));
					if (any(runOcclusionFilter(vbool<K>(1 << k), geometry, ray, context, h)))
						return true;
					ray.tfar[k] = old_t;
					m = btc(m, i);
					continue;
