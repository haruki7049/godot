...
struct MoellerTrumboreHitM {
	...
	__forceinline MoellerTrumboreHitM(const vbool<M> &valid, const vfloat<M> &U, const vfloat<M> &V, const vfloat<M> &T, const vfloat<M> &absDen, const Vec3vf<M> &Ng, const UVMapper &mapUV) : U(U), V(V), T(T), absDen(absDen), mapUV(mapUV), valid(valid), vNg(Ng) {}
...
struct MoellerTrumboreIntersector1 {
	...
	__forceinline bool intersect(const vbool<M> &valid0,
		...
		valid &= (den != vfloat<M>(zero)) & (U >= 0.0f) & (V >= 0.0f) & (U + V <= absDen);
#endif
		if (likely(early_out && none(valid)))
			return false;
	...
	__forceinline bool intersectEdge(Ray &ray,
		...
		if (likely(intersectEdge(ray, v0, e1, e2, mapUV, hit)))
	...
	__forceinline bool intersect(Ray &ray,
			...
			const Epilog &epilog) const {
		MoellerTrumboreHitM<M, UVMapper> hit(mapUV);
		if (likely(intersect(ray, v0, v1, v2, mapUV, hit)))
			return epilog(hit.valid, hit);
		return false;
	}
	...
	__forceinline bool intersect(const vbool<M> &valid,
			...
			const Epilog &epilog) const {
		MoellerTrumboreHitM<M, UVMapper> hit(mapUV);
		if (likely(intersect(valid, ray, v0, v1, v2, mapUV, hit)))
			return epilog(hit.valid, hit);
		return false;
	}
};
...
struct MoellerTrumboreHitK {
	...
	__forceinline MoellerTrumboreHitK(const vfloat<K> &U, const vfloat<K> &V, const vfloat<K> &T, const vfloat<K> &absDen, const Vec3vf<K> &Ng, const UVMapper &mapUV) : U(U), V(V), T(T), absDen(absDen), Ng(Ng), mapUV(mapUV) {}
...
struct MoellerTrumboreIntersectorK {
	...
	__forceinline bool intersectEdge(RayK<K> &ray,
			...
			const Epilog &epilog) const {
		MoellerTrumboreHitM<M, UVMapper> hit(mapUV);
		if (likely(intersectEdge(ray, k, tri_v0, tri_e1, tri_e2, mapUV, hit)))
			return epilog(hit.valid, hit);
		return false;
	}
	...
	__forceinline bool intersectEdge(RayK<K> &ray,
			...
			const Epilog &epilog) const {
		MoellerTrumboreHitM<M, UVMapper> hit(mapUV);
		if (likely(intersectEdge(ray, k, time_range, tri_v0, tri_e1, tri_e2, mapUV, hit)))
			return epilog(hit.valid, hit);
		return false;
	}
