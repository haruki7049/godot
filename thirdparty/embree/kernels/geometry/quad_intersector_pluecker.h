...
struct QuadMIntersector1Pluecker {
	...
	__forceinline bool occluded(Ray &ray, IntersectContext *context,
		...
		if (PlueckerIntersectorTriangle1::intersect<M>(ray, v0, v1, v3, vbool<M>(false), epilog))
			return true;
		if (PlueckerIntersectorTriangle1::intersect<M>(ray, v2, v3, v1, vbool<M>(true), epilog))
			return true;
		return false;
	}
};
...
struct QuadMIntersectorKPluecker : public QuadMIntersectorKPlueckerBase<M, K, filter> {
	__forceinline QuadMIntersectorKPluecker(const vbool<K> &valid, const RayK<K> &ray) : QuadMIntersectorKPlueckerBase<M, K, filter>(valid, ray) {}
	...
	__forceinline bool occluded1(RayK<K> &ray, size_t k, IntersectContext *context,
		...
		if (PlueckerIntersector1KTriangleM::intersect1<M, K>(ray, k, v0, v1, v3, vbool<M>(false), epilog))
			return true;
		if (PlueckerIntersector1KTriangleM::intersect1<M, K>(ray, k, v2, v3, v1, vbool<M>(true), epilog))
			return true;
		return false;
	}
};
...
struct QuadMIntersectorKPluecker<4, K, filter> : public QuadMIntersectorKPlueckerBase<4, K, filter> {
	__forceinline QuadMIntersectorKPluecker(const vbool<K> &valid, const RayK<K> &ray) : QuadMIntersectorKPlueckerBase<4, K, filter>(valid, ray) {}
