      ...
      for(i=0;i<vi->channels;i++){
        ...
        _analysis_output(buf,seq,work,n/2,1,0,0);
			...
			for (i = 0; i < info->submaps; i++) {
				...
				_residue_P[ci->residue_type[resnum]]->forward(opb, vb, b->residue[resnum],
...
static int mapping0_inverse(vorbis_block *vb, vorbis_info_mapping *l) {
	...
	for (i = 0; i < info->submaps; i++) {
		...
		_residue_P[ci->residue_type[info->residuesubmap[i]]]->inverse(vb, b->residue[info->residuesubmap[i]],
	...
	for (i = 0; i < vi->channels; i++) {
		float *pcm = vb->pcm[i];
		int submap = info->chmuxlist[i];
		_floor_P[ci->floor_type[info->floorsubmap[submap]]]->inverse2(vb, b->flr[info->floorsubmap[submap]],
				floormemo[i], pcm);
	}
