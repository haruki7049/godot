...
typedef struct PS_Parser_FuncsRec_ {
	...
	FT_Fixed (*to_fixed)(PS_Parser parser,
			FT_Int power_ten);
	...
	FT_Int (*to_coord_array)(PS_Parser parser,
			FT_Int max_coords,
			FT_Short *coords);
...
typedef struct PS_Builder_FuncsRec_ {
	void (*init)(PS_Builder *ps_builder,
			void *builder,
			FT_Bool is_t1);
...
typedef void (*CFF_Decoder_Free_Glyph_Callback)(TT_Face face,
		FT_Byte **pointer,
		FT_ULong length);
...
typedef struct T1_Decoder_FuncsRec_ {
	...
	FT_Error (*parse_charstrings_old)(T1_Decoder decoder,
			FT_Byte *base,
			FT_UInt len);
#else
	FT_Error (*parse_metrics)(T1_Decoder decoder,
			FT_Byte *base,
			FT_UInt len);
#endif

	FT_Error (*parse_charstrings)(PS_Decoder *decoder,
...
typedef void (*CFF_Builder_Add_Point_Func)(CFF_Builder *builder,
		FT_Pos x,
		FT_Pos y,
		FT_Byte flag);
typedef FT_Error (*CFF_Builder_Add_Point1_Func)(CFF_Builder *builder,
		FT_Pos x,
		FT_Pos y);
typedef FT_Error (*CFF_Builder_Start_Point_Func)(CFF_Builder *builder,
		FT_Pos x,
		FT_Pos y);
...
typedef struct CFF_Decoder_FuncsRec_ {
	...
	FT_Error (*prepare)(CFF_Decoder *decoder,
			CFF_Size size,
			FT_UInt glyph_index);
...
typedef struct AFM_Parser_FuncsRec_ {
	FT_Error (*init)(AFM_Parser parser,
			FT_Memory memory,
			FT_Byte *base,
			FT_Byte *limit);
...
typedef struct AFM_ParserRec_ {
	...
	FT_Int (*get_index)(const char *name,
			FT_Offset len,
			void *user_data);
...
typedef struct PSAux_ServiceRec_ {
	...
	void (*ps_decoder_init)(PS_Decoder *ps_decoder,
			void *decoder,
			FT_Bool is_t1);

	void (*t1_make_subfont)(FT_Face face,
			PS_Private priv,
			CFF_SubFont subfont);
