...
static REFERENCE_MODE read_frame_reference_mode(const VP9_COMMON *cm,
	...
	if (is_compound_reference_allowed(cm)) {
		return vpx_read_bit(r) ? (vpx_read_bit(r) ? REFERENCE_MODE_SELECT : COMPOUND_REFERENCE) : SINGLE_REFERENCE;
	} else {
...
static void predict_and_reconstruct_intra_block(MACROBLOCKD *const xd,
	...
	if (!mi->skip) {
		const TX_TYPE tx_type = (plane || xd->lossless) ?
										DCT_DCT :
										intra_mode_to_tx_type_lookup[mode];
...
static void dec_build_inter_predictors(VPxWorker *const worker, MACROBLOCKD *xd,
	...
	if (plane == 0) {
		...
		ref_frame = plane == 1 ? ref_frame_buf->buf.u_buffer : ref_frame_buf->buf.v_buffer;
	}
...
static void setup_segmentation(struct segmentation *seg,
	...
	if (seg->update_map) {
		for (i = 0; i < SEG_TREE_PROBS; i++)
			seg->tree_probs[i] = vpx_rb_read_bit(rb) ? vpx_rb_read_literal(rb, 8) : MAX_PROB;

		seg->temporal_update = vpx_rb_read_bit(rb);
		if (seg->temporal_update) {
			for (i = 0; i < PREDICTION_PROBS; i++)
				seg->pred_probs[i] = vpx_rb_read_bit(rb) ? vpx_rb_read_literal(rb, 8) : MAX_PROB;
		} else {
			for (i = 0; i < PREDICTION_PROBS; i++)
...
static INTERP_FILTER read_interp_filter(struct vpx_read_bit_buffer *rb) {
	const INTERP_FILTER literal_to_filter[] = { EIGHTTAP_SMOOTH,
		...
		EIGHTTAP_SHARP,
		BILINEAR };
	return vpx_rb_read_bit(rb) ? SWITCHABLE : literal_to_filter[vpx_rb_read_literal(rb, 2)];
}
