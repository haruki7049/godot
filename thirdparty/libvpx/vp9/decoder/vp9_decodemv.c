...
static int dec_get_pred_context_switchable_interp(const MACROBLOCKD *xd) {
	...
	const int above_type = above_mi ? above_mi->interp_filter : SWITCHABLE_FILTERS;
...
static void read_inter_block_mode_info(VP9Decoder *const pbi,
	...
	mi->interp_filter = (cm->interp_filter == SWITCHABLE) ? read_switchable_interp_filter(cm, xd, r) : cm->interp_filter;
