	...
	if (above_in_image && left_in_image) { // both edges available
		...
		if (above_intra && left_intra) { // intra/intra (2)
			...
			if (!has_second_ref(edge_mi)) // single pred (1/3)
				...
				pred_context = 1 + 2 * (edge_mi->ref_frame[var_ref_idx] != cm->comp_var_ref[1]);
		} else { // inter/inter
			const int l_sg = !has_second_ref(left_mi);
			const int a_sg = !has_second_ref(above_mi);
			const MV_REFERENCE_FRAME vrfa = a_sg ? above_mi->ref_frame[0] : above_mi->ref_frame[var_ref_idx];
			const MV_REFERENCE_FRAME vrfl = l_sg ? left_mi->ref_frame[0] : left_mi->ref_frame[var_ref_idx];
		...
		if (!is_inter_block(edge_mi)) {
			...
			if (has_second_ref(edge_mi))
				pred_context = 4 * (edge_mi->ref_frame[var_ref_idx] != cm->comp_var_ref[1]);
			else
