	...
	if (!(*iter) && ctx->yv12_frame_buffers.pbi[0]) {
		...
		if (ctx->base.init_flags & VPX_CODEC_USE_POSTPROC) {
			flags.post_proc_flag = ctx->postproc_cfg.post_proc_flag
								   ...
								   | ((ctx->dbg_color_ref_frame_flag != 0) ? VP8D_DEBUG_CLR_FRM_REF_BLKS : 0) | ((ctx->dbg_color_mb_modes_flag != 0) ? VP8D_DEBUG_CLR_BLK_MODES : 0) | ((ctx->dbg_color_b_modes_flag != 0) ? VP8D_DEBUG_CLR_BLK_MODES : 0) | ((ctx->dbg_display_mv_flag != 0) ? VP8D_DEBUG_DRAW_MV : 0)
#endif
					;
...
static vpx_codec_err_t vp8_get_last_ref_updates(vpx_codec_alg_priv_t *ctx,
	...
	if (update_info && !ctx->yv12_frame_buffers.use_frame_threads) {
		...
		*update_info = pbi->common.refresh_alt_ref_frame * (int)VP8_ALTR_FRAME + pbi->common.refresh_golden_frame * (int)VP8_GOLD_FRAME + pbi->common.refresh_last_frame * (int)VP8_LAST_FRAME;
