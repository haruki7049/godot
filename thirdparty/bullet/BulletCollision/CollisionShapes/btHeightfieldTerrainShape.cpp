...
btHeightfieldTerrainShape::btHeightfieldTerrainShape(
		...
		PHY_ScalarType hdt, bool flipQuadEdges) : m_userIndex2(-1),
												  m_userValue3(0),
												  m_triangleInfoMap(0) {
...
btHeightfieldTerrainShape::btHeightfieldTerrainShape(int heightStickWidth, int heightStickLength, const void *heightfieldData, btScalar maxHeight, int upAxis, bool useFloatData, bool flipQuadEdges) : m_userIndex2(-1),
																																																		m_userValue3(0),
																																																		m_triangleInfoMap(0) {
...
void gridRaycast(Action_T &quadAction, const btVector3 &beginPos, const btVector3 &endPos, int indices[3]) {
	...
	const int xiStep = rayDirectionFlatX > 0 ? 1 : rayDirectionFlatX < 0 ? -1 :
																		   0;
	const int ziStep = rayDirectionFlatZ > 0 ? 1 : rayDirectionFlatZ < 0 ? -1 :
																		   0;
...
struct ProcessVBoundsAction {
	...
	ProcessVBoundsAction(const btAlignedObjectArray<btHeightfieldTerrainShape::Range> &bnd, int *indices) : vbounds(bnd),
																											m_indices(indices) {
	}
