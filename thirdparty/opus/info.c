...
static int op_is_jpeg(const unsigned char *_buf, size_t _buf_sz) {
	return _buf_sz >= 11 && memcmp(_buf, "\xFF\xD8\xFF\xE0", 4) == 0 && (_buf[4] << 8 | _buf[5]) >= 16 && memcmp(_buf + 6, "JFIF", 5) == 0;
}
...
static int opus_picture_tag_parse_impl(OpusPictureTag *_pic, const char *_tag,
	...
	if (mime_type_length == 3 && strcmp(mime_type, "-->") == 0) {
		...
		} else if (mime_type_length == 0 || (mime_type_length == 6 && op_strncasecmp(mime_type, "image/", mime_type_length) == 0)) {
			if (op_is_jpeg(_buf + i, data_length))
				format = OP_PIC_FORMAT_JPEG;
