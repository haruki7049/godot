...
static int op_seek_helper(OggOpusFile *_of, opus_int64 _offset) {
	if (_offset == _of->offset)
		return 0;
	if (_of->callbacks.seek == NULL || (*_of->callbacks.seek)(_of->source, _offset, SEEK_SET)) {
		return OP_EREAD;
	}
	_of->offset = _offset;
	...
	for (;;) {
		switch (ogg_stream_packetout(&_of->os, &op)) {
			...
			default: {
				...
				if (OP_UNLIKELY(ret != 0) || OP_UNLIKELY(_og->header[_og->header_len - 1] == 255)) {
...
static int op_find_final_pcm_offset(OggOpusFile *_of,
	...
	/*This implementation requires that the difference between the first and last
	   ...
	   pre-skip requires.*/
	if (OP_UNLIKELY(op_granpos_diff(&duration, _end_gp, _link->pcm_start) < 0) || OP_UNLIKELY(duration < _link->head.pre_skip)) {
		return OP_EBADTIMESTAMP;
	}
...
static int op_bisect_forward_serialno(OggOpusFile *_of,
	...
	for (;;) {
		...
		if (nlinks > 1) {
			...
			if (OP_LIKELY(last_offset > _searched - avg_link_size) && OP_LIKELY(last_offset < upper_limit)) {
...
opus_int64 op_raw_total(const OggOpusFile *_of, int _li) {
	if (OP_UNLIKELY(_of->ready_state < OP_OPENED) || OP_UNLIKELY(!_of->seekable) || OP_UNLIKELY(_li >= _of->nlinks)) {
		return OP_EINVAL;
	}
	if (_li < 0)
		return _of->end - _of->links[0].offset;
	return (_li + 1 >= _of->nlinks ? _of->end : _of->links[_li + 1].offset) - _of->links[_li].offset;
}
...
ogg_int64_t op_pcm_total(const OggOpusFile *_of, int _li) {
	...
	int nlinks;
	nlinks = _of->nlinks;
	if (OP_UNLIKELY(_of->ready_state < OP_OPENED) || OP_UNLIKELY(!_of->seekable) || OP_UNLIKELY(_li >= nlinks)) {
		return OP_EINVAL;
	}
	links = _of->links;
...
static int op_fetch_and_process_page(OggOpusFile *_of,
	...
	for (;;) {
		...
		if (OP_LIKELY(_of->ready_state >= OP_STREAMSET) && cur_serialno != (ogg_uint32_t)ogg_page_serialno(&og)) {
			/*Two possibilities:
		...
		if (OP_LIKELY(_of->ready_state >= OP_INITSET)) {
			...
			if (op_count > 0) {
				...
				if (OP_UNLIKELY(cur_page_eos) && OP_LIKELY(!op_granpos_diff(&diff, cur_page_gp, prev_packet_gp)) && OP_LIKELY(diff < total_duration)) {
...
static int op_pcm_seek_page(OggOpusFile *_of,
	...
	if (OP_UNLIKELY(op_granpos_add(&_target_gp, _target_gp, -80 * 48) < 0) || OP_UNLIKELY(op_granpos_cmp(_target_gp, pcm_start) < 0)) {
	...
	if (op_granpos_cmp(_target_gp, pcm_pre_skip) < 0)
		...
		if (_li == _of->cur_link && _of->ready_state >= OP_INITSET) {
			...
			if (op_count > 0 && OP_LIKELY(offset <= end)) {
				...
				if (OP_LIKELY(gp != -1) && OP_LIKELY(op_granpos_cmp(pcm_start, gp) < 0) && OP_LIKELY(op_granpos_cmp(pcm_end, gp) > 0)) {
		...
		while (begin < end) {
			...
			if (page_offset < 0) {
				...
				if (op_granpos_cmp(gp, _target_gp) < 0) {
					...
					begin = _of->offset;
					if (OP_UNLIKELY(op_granpos_cmp(pcm_start, gp) > 0) || OP_UNLIKELY(op_granpos_cmp(pcm_end, gp) < 0)) {
					...
					if (bisect <= begin + 1)
						...
						/*Don't let pcm_end get out of range!
						  That could happen with an invalid timestamp.*/
						if (OP_LIKELY(op_granpos_cmp(pcm_end, gp) > 0) && OP_LIKELY(op_granpos_cmp(pcm_start, gp) <= 0)) {
							pcm_end = gp;
						}
						break;
					}
...
int op_pcm_seek(OggOpusFile *_of, ogg_int64_t _pcm_offset) {
	...
	for (;;) {
		...
		for (op_pos = _of->op_pos; op_pos < op_count; op_pos++) {
			...
			if (OP_LIKELY(!op_granpos_diff(&diff, cur_packet_gp, pcm_start)) && diff > skip) {
				break;
			}
			...
			if (OP_LIKELY(op_pos < _of->op_count)) {
				...
				if (OP_UNLIKELY(duration * nchannels > _buf_size)) {
					...
					if (OP_LIKELY(trimmed_duration > 0)) {
						...
						if (OP_LIKELY(trimmed_duration > 0) && OP_UNLIKELY(od_buffer_pos > 0)) {
