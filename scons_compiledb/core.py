...
class _EntryCounter:
    ...
    def __str__(self):
        ...
        updated = [k for k, (org, new) in self._updated.items() if org and org != new]
...
def enable(env, config):
    ...
    def create_db_entry_emitter(cxx, shared):
        def emitter(target, source, env):
            ...
            def add_db_entry():
                ...
                if entry:
                    key = "{}:{}".format(entry["file"], str(target[0]) if config.multi else "")
            ...
            entry = env._AddDbEntry(entry_node, [], _COMPILE_DB_ENTRY_FUNC=add_db_entry)
    ...
    for (cxx, suffix), shared in itertools.product(
        [(True, s) for s in config.cxx_suffixes] + [(False, s) for s in config.cc_suffixes], (True, False)
    ):
    ...
    # Add builders
    #
    env["BUILDERS"]["_AddDbEntry"] = Builder(action=Action(add_db_entry_action, None))

    env["BUILDERS"]["_UpdateInternalDb"] = Builder(
        action=Action(update_internal_db_action, "Check compilation DB : $TARGET ... " "$_COMPILE_DB_COUNTER")
    )

    env["BUILDERS"]["_UpdateDb"] = Builder(action=Action(update_db_action, "Update compilation DB : $TARGET"))
    ...
    def compile_db(env, target=config.db):
        ...
        if (not config.reset) and internal_db.exists():
            compile_commands.update(json.loads(internal_db.get_text_contents()))
